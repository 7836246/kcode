...
function getDefaultServerDataRoot(): string {
  const configured = process.env.KCODE_DATA_BASE_DIR?.trim();
  return join(configured || homedir(), ".kcode", "server");
}
  ...
  return {
    ...
    controlEndpoint:
      process.platform === "win32"
        ? `\\\\.\\pipe\\kcode-server-${stablePathId(root)}`
...
function inferDataBaseDir(serverRoot: string): string {
  ...
  if (basename(serverRoot) === "server" && basename(parent) === ".kcode") {
