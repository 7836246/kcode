...
import { IKCodeAgentService } from "@kcode/services";
...
export async function runServerCore(generation: number): Promise<void> {
  ...
  const explicitKCodeBuiltinProviderConfigFilePath =
    process.env[KCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV]?.trim();
  const kcodeBuiltinProviderConfigFilePath = explicitKCodeBuiltinProviderConfigFilePath
    ? explicitKCodeBuiltinProviderConfigFilePath
    : typeof __KCODE_BUILTIN_PROVIDER_CONFIG_JSON__ === "string"
      ? await materializeKCodeBuiltinProviderConfig({
          environmentConfigRoot: getAppConfigDir(),
          content: __KCODE_BUILTIN_PROVIDER_CONFIG_JSON__,
        })
      : undefined;
  if (!kcodeBuiltinProviderConfigFilePath) {
    throw new Error(
      `当前构建未嵌入 KCode Built-in Provider Config，且未设置 ${KCODE_BUILTIN_PROVIDER_CONFIG_FILE_ENV}`,
    );
  }
  ...
  const taskActivityTracker = createTaskActivityTracker(services.getOptional(IKCodeAgentService));
