...
import type { ProviderConfigLayerSnapshot, ProviderSource } from "@kcode/provider";
import { atomicWritePrivateTextFile, withFileLock } from "@kcode/shared/node";
import {
  decodeKCodeBuiltinRelease,
  encodeKCodeBuiltinRelease,
  serializeKCodeBuiltinRelease,
  type KCodeBuiltinRelease,
} from "./kcode-builtin-release.js";

export interface NodeKCodeBuiltinProviderConfigSourceOptions {
  readonly bundledFilePath: string;
  readonly activeFilePath?: string;
  readonly watch?: boolean;
}

export type ApplyKCodeBuiltinReleaseResult = "updated" | "unchanged" | "stale";

/** Bundled、Active/LKG 与 Remote 共用同一 Release，并最终发布为现有 Config Snapshot。 */
export class NodeKCodeBuiltinProviderConfigSource implements ProviderSource<ProviderConfigLayerSnapshot> {
  readonly #bundledFilePath: string;
  readonly #activeFilePath: string;
  ...
  constructor(options: NodeKCodeBuiltinProviderConfigSourceOptions) {
    const bundledFilePath = options.bundledFilePath.trim();
    if (!bundledFilePath) throw new Error("KCode Built-in bundledFilePath 不能为空");
  ...
  async read(): Promise<ProviderConfigLayerSnapshot> {
    ...
    let release: KCodeBuiltinRelease;
    try {
  ...
  async applyRemoteRelease(release: KCodeBuiltinRelease): Promise<ApplyKCodeBuiltinReleaseResult> {
    this.#assertNotDisposed();
    await this.#ensureWatcher();
    const result = await withFileLock(this.#activeFilePath, async () => {
      ...
      if (release.revision === current.revision) {
        if (serializeKCodeBuiltinRelease(release) === serializeKCodeBuiltinRelease(current)) {
          return "unchanged" as const;
        }
        throw new Error(`KCode Built-in 相同 revision ${release.revision} 对应不同内容`);
      }
  ...
  async #readAndMaterializeLocked(): Promise<KCodeBuiltinRelease> {
  ...
  async #writeActiveLocked(release: KCodeBuiltinRelease): Promise<void> {
    this.#assertNotDisposed();
    await atomicWritePrivateTextFile(
      this.#activeFilePath,
      JSON.stringify(encodeKCodeBuiltinRelease(release), null, 2),
    );
  }
  ...
  #assertNotDisposed(): void {
    if (this.#disposed) throw new Error("NodeKCodeBuiltinProviderConfigSource 已 dispose");
  }
}

export function createNodeKCodeBuiltinProviderConfigSource(
  options: NodeKCodeBuiltinProviderConfigSourceOptions,
): NodeKCodeBuiltinProviderConfigSource {
  return new NodeKCodeBuiltinProviderConfigSource(options);
}

interface ReleaseCandidate {
  readonly release?: KCodeBuiltinRelease;
  readonly error?: unknown;
}

async function readReleaseCandidate(filePath: string): Promise<ReleaseCandidate | null> {
  try {
    return { release: decodeKCodeBuiltinRelease(JSON.parse(await readFile(filePath, "utf8"))) };
  } catch (error) {
...
function selectReleaseCandidate(
  ...
  if (
    ...
    serializeKCodeBuiltinRelease(bundled.release) !== serializeKCodeBuiltinRelease(active.release)
  ) {
  ...
  const valid = [bundled?.release, active?.release].filter(
    (candidate): candidate is KCodeBuiltinRelease => candidate !== undefined,
  );
  if (valid.length === 0) {
    throw new AggregateError(
      [bundled?.error, active?.error].filter((error) => error !== undefined),
      "Bundled 与 Active KCode Built-in Release 均不可用",
    );
  }
...
function snapshotFromRelease(
  release: KCodeBuiltinRelease,
  sourceKey: string,
): ProviderConfigLayerSnapshot {
  return Object.freeze({
    revision: `kcode-builtin:${release.revision}:${sourceKey}`,
...
function signatureOf(release: KCodeBuiltinRelease): string {
  return `${release.revision}:${serializeKCodeBuiltinRelease(release)}`;
}
