import { z } from "zod";
import { decodeKCodeBuiltinRelease, type KCodeBuiltinRelease } from "./kcode-builtin-release.js";
...
export interface KCodeBuiltinDownloadOptions {
...
export async function downloadKCodeBuiltinRelease(
  options: KCodeBuiltinDownloadOptions,
  ...
  try {
    ...
    stage = "cdn";
    return decodeKCodeBuiltinRelease(await readJson(new URL(downloadUrl)));
  } catch (error) {
    ...
    const reason = signal.aborted
      ...
      : error instanceof DownloadBoundaryError
        ...
        : error instanceof z.ZodError
          ...
          : "invalid response";
    throw new Error(`KCode Built-in ${stage}: ${reason}`);
  } finally {
    clearTimeout(timer);
  }
