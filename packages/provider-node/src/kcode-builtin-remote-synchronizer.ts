...
import type { NodeKCodeBuiltinProviderConfigSource } from "./kcode-builtin-provider-config-source.js";
import type { KCodeBuiltinRelease } from "./kcode-builtin-release.js";

const HOUR_MS = 60 * 60 * 1_000;

export type KCodeBuiltinRefreshResult =
  | "updated"
  | "unchanged"
  | "stale"
  | "missing"
...
export interface KCodeBuiltinRemoteSynchronizerOptions {
  ...
  readonly onRefreshResult?: (event: KCodeBuiltinRefreshEvent) => void;
  readonly now?: () => number;
...
export interface KCodeBuiltinRefreshEvent {
  readonly result: KCodeBuiltinRefreshResult;
...
export class KCodeBuiltinRemoteSynchronizer {
  readonly #options: KCodeBuiltinRemoteSynchronizerOptions;
  readonly #now: () => number;
  #inFlight: Promise<KCodeBuiltinRefreshResult> | null = null;
  #disposed = false;
  #abortController: AbortController | null = null;

  constructor(options: KCodeBuiltinRemoteSynchronizerOptions) {
    this.#options = options;
    this.#now = options.now ?? Date.now;
  }

  refresh(options: { readonly force?: boolean } = {}): Promise<KCodeBuiltinRefreshResult> {
  ...
  async #refresh(force: boolean, signal: AbortSignal): Promise<KCodeBuiltinRefreshResult> {
    const endpointKey = (await this.#options.resolveEndpointKey()).trim();
    if (this.#disposed) return "disposed";
    if (!endpointKey) throw new Error("KCode Built-in 远端 Endpoint 不能为空");
  ...
  #report(event: KCodeBuiltinRefreshEvent): void {
