...
import { resolveKCodeEndpointOrigin, pickProductEndpointEnv } from "@kcode/shared/kcodeEndpoint";
...
function resolveKCodeEnv(value: string | undefined): "test" | "production" {
...
function createE2EUIRendererCoveragePlugin(repoRoot: string): Plugin {
  ...
  return {
    name: "kcode:e2e-ui-source-coverage",
    enforce: "pre",
...
export default defineConfig(({ mode }) => {
  // `.env*` 只提供链接常量；当前产品环境由启动脚本或 CI 注入 KCODE_ENV。
  const env = { ...loadEnv(mode, "../..", ""), ...process.env };
  const repoRoot = resolve(__dirname, "../..");
  const kcodeEnv = resolveKCodeEnv(env.KCODE_ENV);
  // 安装包身份与后端环境分轴；renderer 用它决定是否展示更新入口。
  const kcodeProductFlavor = resolveDesktopProductFlavor({
    ...process.env,
    ...env,
    KCODE_ENV: kcodeEnv,
  });
  const e2eCoverageEnabled =
    env.KCODE_E2E_COVERAGE === "1" || process.env.KCODE_E2E_COVERAGE === "1";
  const e2eStoreBridgeEnabled =
    env.VITE_KCODE_E2E_STORE_BRIDGE === "1" || process.env.VITE_KCODE_E2E_STORE_BRIDGE === "1";
  const kcodeEndpointOrigin = resolveKCodeEndpointOrigin({
    env: kcodeEnv,
  ...
  return {
    ...
    define: {
      ...
      __KCODE_LOCAL_DEVELOPMENT_RUNTIME__: JSON.stringify(mode !== "production"),
      ...
      // E2E store bridge 只能由 WDIO 专用变量打开，避免把 KCODE_ENV=test 产品环境误当成测试运行态。
      "import.meta.env.VITE_KCODE_E2E_STORE_BRIDGE": JSON.stringify(
