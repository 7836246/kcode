...
type PatchedIpcRenderer = IpcRenderer & { __kcodeArmsIpcPatched?: boolean };
...
type ArmsEventBridgeLike = {
  ...
  __kcodeArmsBridgeForwardPatched?: boolean;
};
...
function patchArmsEventBridgeSend(bridge: ArmsEventBridgeLike): void {
  if (bridge.__kcodeArmsBridgeForwardPatched) {
    return;
  }
  ...
  bridge.__kcodeArmsBridgeForwardPatched = true;
}
...
export function installArmsRumBridgeIpcForward(ipc: IpcRenderer): void {
  ...
  if (patched.__kcodeArmsIpcPatched) {
    return;
  }
  ...
  patched.__kcodeArmsIpcPatched = true;
}
