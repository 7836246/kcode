...
import { RendererUserActionTelemetry, setUserActionTelemetry } from "@kcode/ui";
...
export function initializeDesktopUserActionTrace(options: {
  ...
  const telemetry = new RendererUserActionTelemetry({
    ...
    resource: {
      ...
      serviceVersion: KCODE_VERSION || "unknown",
      deploymentEnvironment: options.isLocalDevelopmentRuntime ? "development" : KCODE_ENV,
      rendererInstanceId,
    },
