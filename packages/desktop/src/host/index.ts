...
import {
  ...
  type KCodeAutomationRunOutcome,
  type ModelSelection,
} from "@kcode/shared";
import {
...
import type {
  ...
  RemoteConnection,
} from "@kcode/server/remote";
import type { RemoteTarget } from "@kcode/shared";
...
import { createRemoteConnectionProgressContext } from "@kcode/server/remote/remoteConnectionProgressContext.js";
...
const remoteMediaRangePreviewEnabled =
  process.env["KCODE_REMOTE_MEDIA_RANGE_PREVIEW_ENABLED"] !== "0";
...
// 这里根据 main 传入的窗口 label 补一层稳定的 kcode-* title，方便系统进程列表过滤。
process.title = formatKCodeHostProcessName(process.env["KCODE_PROCESS_LABEL"]);
...
function writeHostLog(level: HostLogLevel, ...args: unknown[]): void {
  const prefix = formatLogPrefix("kcode-host", process.pid);
  const consoleFn =
...
function createFullFeedbackLogArchiveViaMain(
  ...
  return new Promise((resolve, reject) => {
    ...
    // 收集范围和“导出日志”不一致，缺少 kcode-cli 日志、rollout/debug 以及导出链路脱敏。
...
async function resolveOffPeakFilesChanged(params: {
  ...
  try {
    const snapshot = await params.kcodeTaskService.getTaskSnapshot({
...
async function finalizeOffPeakRun(params: {
  ...
  workspacePath: string;
  workspaceIdentity?: string;
  outcome: KCodeAutomationRunOutcome;
  error?: string;
}): Promise<void> {
  ...
  void params.kcodeTaskService.setTaskUnread({
    taskId: params.taskId,
    workspacePath: params.workspacePath,
    ...(params.workspaceIdentity ? { workspaceIdentity: params.workspaceIdentity } : {}),
    unread: true,
...
function trackOffPeakRunOutcome(params: {
  ...
  const disposable = params.kcodeTaskService.onDynamicTaskTerminalOutcome(params.taskId)(
    (result) => {
      if (result.inputId !== params.traceId) return;
      disposeOffPeakRunSubscription(key);
      void finalizeOffPeakRun({
        kcodeTaskService: params.kcodeTaskService,
...
async function dispatchOffPeakRun(request: OffPeakRunDispatchRequest): Promise<{
  ...
  const kcodeTaskService = activeServices?.getOptional(IKCodeTaskService);
  if (!kcodeTaskService) {
    throw new Error("KCode task service is not initialized.");
  }
  const runtime = await ensureOffPeakRuntime();
  if (!runtime) {
  ...
  try {
    ...
    if (dispatchKind === "bound-first-run") {
      ...
      const [deletedIds, tasks] = await Promise.all([
        kcodeTaskService.listDeletedTaskIds(workspaceScope),
        kcodeTaskService.listTasks(workspaceScope),
      ]);
      assertBoundSessionDispatchable({
        sessionId: taskId,
      ...
      await kcodeTaskService.setConfigOption({
        taskId,
        traceId,
        configId: "mode",
      ...
      const task = await kcodeTaskService.createTask({
        ...
        mode: request.permissionMode as KCodeTaskMode,
    ...
    await kcodeTaskService.sendPrompt({
      taskId,
      traceId,
...
async function dispatchCronRun(request: CronRunDispatchRequest): Promise<{
  ...
  const kcodeTaskService = targetServices.getOptional(IKCodeTaskService);
  if (!kcodeTaskService) {
    throw new Error("KCode task service is not initialized.");
  }
  ...
  try {
    const task = request.targetTaskId
      ...
      : await kcodeTaskService.createTask({
          workspacePath: request.workspacePath,
          workspaceIdentity: request.workspaceIdentity,
...
function isKCodeTaskMeta(value: unknown): value is KCodeTaskMeta {
  return (
    typeof value === "object" &&
    value !== null &&
    "taskId" in value &&
...
function createReportingRemoteKCodeTaskService<T extends object>(
  service: T,
  options?: {
    ...
    }) => Promise<{ content: string; attachments?: KCodePromptAttachment[] }>;
  },
): T {
  ...
  function subscribeSessionMessageRequests(target: T, meta: KCodeTaskMeta): void {
  ...
  async function mirrorRemotePrompt(
    ...
    // kcodeSessionStore 的整条消费链词表都是 KCodeStreamEvent。两个方案的评估结论：
    // a) relay 直接转发 v4 帧、手机端消费 v4 store（正解）：需要重做 relay stream-op
    //    协议 + 手机端 store；
    // b) 帧→KCodeStreamEvent 薄映射：等价复刻 adapter mapSessionEvent，
    //    否决。
    // 结论：本镜像保持 legacy 源不动。
    ...
    try {
      ...
      // 远端 kcode-server 没有 desktop realtime port；由窗口 Host 内的
  ...
  function finishWorkspaceTask(taskId: string, meta: KCodeTaskMeta): void {
    workspaceProxyState.disposeTaskReadySubscription(taskId);
    workspaceTaskTracker.finish(taskId, meta);
  }

  function beginWorkspaceTask(target: T, taskId: string, meta: KCodeTaskMeta): boolean {
    ...
    if (typeof onDynamicTaskReady !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote KCode task service does not expose onDynamicTaskReady");
    }
    const subscribe = onDynamicTaskReady.call(target, taskId);
    if (typeof subscribe !== "function") {
      workspaceTaskTracker.finish(taskId, meta);
      throw new Error("remote KCode task ready event is not subscribable");
    }
  ...
  // remote workspace 的 KCode Agent manager 跑在远端 server，desktop main 不能直接看到
  ...
  return new Proxy(service, {
    get(target, property, receiver) {
      ...
      return async (...args: unknown[]) => {
        let trackedTask: { taskId: string; meta: KCodeTaskMeta; started: boolean } | undefined;
...
function warmUpKCodeAgent(
  ...
  const kcodeSessionService = services.getOptional(IKCodeSessionService);
  if (!kcodeSessionService) {
    return;
  }
    ...
    .then((result) => {
      if (!result.available) {
        if (result.reasonCode === "provider_not_ready") {
          logger.info(
            `KCode agent warmup waiting for provider/model (${reason}) workspace=${workspacePath}`,
          );
          return;
        }
        logger.warn(
          `KCode agent warmup unavailable (${reason}) workspace=${workspacePath} reason=${result.reason ?? "unknown"}`,
        );
        return;
      }
      ...
      logger.info(
        `KCode agent warmup ready (${reason}) workspace=${workspacePath} transport=${result.transportKind ?? "unknown"}`,
      );
    })
    .catch((error) => {
      logger.warn(`KCode agent warmup failed (${reason}) workspace=${workspacePath}:`, error);
    });
}
...
const hostResourceUsageResponder = createHostResourceUsageResponder({
  getAgentService: () => activeServices?.getOptional(IKCodeAgentService),
...
async function createWindowRemoteConnectionHandle(params: {
  ...
  const services = createRemoteWorkspaceServiceCollection({
    ...
    createReportingRemoteKCodeTaskService: (service) =>
      createReportingRemoteKCodeTaskService(service, {
  ...
  let disposed = false;
  // 远端 workspace 的 CLI 与 MCP 样本走与本地同一条路径：远端 kcode-server → 本地 Host → main。
...
>({
  ...
  releaseWorkspace: async (services, context) => {
    await services.get(IKCodeTaskService).releaseWorkspacePreparation({
  ...
  resolveSource: (scope) => {
    ...
    if (remoteSession?.workspacePath && remoteSession.workspaceIdentity) {
      ...
      return {
        ...
        taskService: services.get(IKCodeTaskService),
        agentService: services.getOptional(IKCodeAgentService),
    ...
    const taskService = activeServices?.getOptional(IKCodeTaskService);
    if (!taskService) {
      return null;
    }
    return {
      ...
      taskService,
      agentService: activeServices?.getOptional(IKCodeAgentService),
      sourceAvailability: "online" as const,
    };
  },
});
  ...
  return new Proxy(base, {
    get(target, property, receiver) {
      if (property === "setTaskPinned") {
        return async (params: Parameters<IKCodeTaskService["setTaskPinned"]>[0]) => {
      ...
      if (property === "archiveTask" || property === "unarchiveTask") {
        return async (
          params:
            | Parameters<IKCodeTaskService["archiveTask"]>[0]
            | Parameters<IKCodeTaskService["unarchiveTask"]>[0],
        ) => {
      ...
      if (property === "deleteTask") {
        return async (params: Parameters<IKCodeTaskService["deleteTask"]>[0]) => {
          await route(params, { kind: "delete" });
        };
      }
      if (property === "deleteArchivedTasks") {
        return async (params: Parameters<IKCodeTaskService["deleteArchivedTasks"]>[0]) => {
      ...
      if (property === "deleteArchivedTask") {
        return async (params: Parameters<IKCodeTaskService["deleteArchivedTask"]>[0]) =>
      ...
      if (property === "setTaskUnread") {
        return async (params: Parameters<IKCodeTaskService["setTaskUnread"]>[0]) => {
...
function exposeServicesOnMessagePort(
  ...
  services: ServiceCollection,
  deferInit: boolean,
  clientMode: KCodeAgentV4ClientMode = "desktop-continuous",
  ...
  const agentService = services.getOptional(IKCodeAgentService);
  const connectionScope = agentService
    ? createKCodeAgentConnectionScope(agentService, {
  ...
  const taskService = services.getOptional(IKCodeTaskService);
  if (taskService) {
    overrides.set(
      IKCodeTaskService.channelName,
      createControllerRoutedTaskService(taskService, attachmentScope),
    );
  }
  if (connectionScope) {
    overrides.set(IKCodeAgentService.channelName, connectionScope.service);
  }
...
parentPort.on("message", async (e: Electron.MessageEvent) => {
    ...
    void (async () => {
      try {
        const dispatchResult = await dispatchCronRun({
          ...msg,
          mode: msg.mode as KCodeTaskMode | undefined,
        });
        parentPort.postMessage({
          type: HostResponseTypes.CronRunResult,
          runId: msg.runId,
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliver) {
    ...
    if (!kcodeTaskService) {
      parentPort.postMessage({
        ...
        result: {
          error: "KCode task service is not initialized.",
  ...
  if (msg.type === HostMessageTypes.SessionMessageDeliveryResult) {
    const kcodeTaskService = activeServices?.getOptional(IKCodeTaskService);
    if (!kcodeTaskService) {
      logger.warn("session message delivery result received before KCode task service initialized");
      return;
    }
    void kcodeTaskService.sendSessionMessageDeliveryResult(msg.result).catch((error) => {
  ...
  if (msg.type === HostMessageTypes.InitLocal) {
    ...
    databaseStartup = createHostDatabaseStartup({
      ...
      initializeServices: async () => {
        ...
        const services = await initializeHostApiNetworkTransportOwner({
          ...
          establishOwner: () => {
            const initializedServices = createLocalServices({
              ...
              kcodeAgentSpawnFallbackCwd: msg.agentSpawnFallbackCwd,
              kcodeBuiltinProviderConfigFilePath: msg.kcodeBuiltinProviderConfigFilePath,
              ...
              // browser-use：agent 的 interaction/browserExecute 经 kcodeAgentService 转到这个 executor，
        ...
        const kcodeTaskService = services.getOptional(IKCodeTaskService);
        if (kcodeTaskService) {
          const reportingKCodeTaskService = createReportingRemoteKCodeTaskService(
            kcodeTaskService,
            {
              reportRunningPromptCount: false,
            },
          );
          services.register(IKCodeTaskService, reportingKCodeTaskService);
        }
...
async function setupRemoteConnection(
  ...
  const { createRemoteBackend, connectRemote, pickRemoteRuntimeEnv } =
    await import("@kcode/server/remote");
  ...
  const connection = await connectRemote(backend, {
    ...
    // 这里显式透传编译期版本，避免漏导入后生成裸 KCODE_VERSION 引用导致 SSH 初始化直接 ReferenceError。
    appVersion: KCODE_VERSION,
    // 远端 kcode-server/agent 是独立进程，不能继承 host 里的测试/生产 endpoint 选择。
