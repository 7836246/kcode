import type { IRemoteBackend, RemoteUploadOptions } from "@kcode/server/remote";
import { quotePosixPathArg } from "@kcode/server/remote/posixShell.js";
import type { TraceId, KCodePromptAttachment } from "@kcode/shared";
import { randomUUID } from "node:crypto";

const REMOTE_PROMPT_ATTACHMENT_ROOT = "~/.kcode/tmp/prompt-attachments";
const REMOTE_PROMPT_ATTACHMENT_RELATIVE_ROOT = ".kcode/tmp/prompt-attachments";
...
export async function materializeRemotePromptAttachments(
  ...
  const nextAttachments: KCodePromptAttachment[] = [];
...
function getAttachmentLocalPath(attachment: KCodePromptAttachment): string | undefined {
      ...
      return async (...args: unknown[]) => {
        ...
        const prepared = await options.materializePromptAttachments({
          ...
          attachments: Array.isArray((params as { attachments?: unknown }).attachments)
            ? (params as { attachments?: KCodePromptAttachment[] }).attachments
            : undefined,
        });
