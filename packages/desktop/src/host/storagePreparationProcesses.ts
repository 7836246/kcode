...
import {
  ...
  kcodeStoragePreparationFrameSchema,
  type DatabaseStartupState,
} from "@kcode/shared";
import { resolveDefaultKCodeAgentCommand } from "@kcode/services/storage-startup";
...
export async function prepareSessionStorage(options: {
  ...
  const command = resolveDefaultKCodeAgentCommand({
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    lines.on("line", (line) => {
      try {
        ...
        const frame = kcodeStoragePreparationFrameSchema.parse(JSON.parse(line));
        clearTimeout(firstStateTimer);
