import type { KCodeAutomationRunOutcome, KCodeAutomationTrigger } from "@kcode/shared";
...
interface CronRunLifecycleRepo {
  ...
  markRunOutcome(runId: string, outcome: KCodeAutomationRunOutcome, error?: string): Promise<void>;
...
export async function settleCronRunTerminalOutcome(
  params: CronRunLifecycleIdentity & {
    repo: CronRunLifecycleRepo;
    outcome: Exclude<KCodeAutomationRunOutcome, "running">;
    error?: string;
    logWarn: LogWarn;
  },
): Promise<void> {
