...
function resolveScheduledAt(automation: KCodeAutomation, now: number): number {
...
async function handleClaimed(automation: KCodeAutomation, now: number): Promise<void> {
...
function postDispatchRequest(
  ...
  runId: string,
  fixedSelection?: KCodeAutomationRun["modelSelection"],
): void {
 ...
 */
async function handleOffPeakClaimed(task: KCodeOffPeakTask, now: number): Promise<void> {
...
async function settleDispatchResult(
  ...
  const trigger: KCodeAutomationTrigger =
