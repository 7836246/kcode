...
import { PlatformChannels, type RendererActionTraceConfigV1 } from "@kcode/shared";
...
function resolveRuntimeConfig(
  ...
  if (isTruthy(env.KCODE_LOCAL_TTFT_ENABLED)) config = { ...config, localTtftEnabled: true };
  if (!isTruthy(env.KCODE_RENDERER_ACTION_TRACE_ENABLED)) return config;
  return {
    ...config,
    enabled: true,
    sampleRatio: 1,
