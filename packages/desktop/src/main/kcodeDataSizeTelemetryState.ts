...
export interface KCodeDataSizeTelemetryState {
  lastReportedAt?: number;
  reportReservedAt?: number;
}

export async function readKCodeDataSizeTelemetryState(
  stateFile: string,
): Promise<KCodeDataSizeTelemetryState | null> {
  try {
    const parsed: unknown = JSON.parse(await readFile(stateFile, "utf8"));
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) {
      throw new TypeError("Invalid KCode data size telemetry state");
    }
    const record = parsed as Record<string, unknown>;
    const state: KCodeDataSizeTelemetryState = {};
    if (record.lastReportedAt !== undefined) {
      if (typeof record.lastReportedAt !== "number" || !Number.isFinite(record.lastReportedAt)) {
        throw new TypeError("Invalid lastReportedAt in KCode data size telemetry state");
      }
    ...
    if (record.reportReservedAt !== undefined) {
      if (
        ...
        throw new TypeError("Invalid reportReservedAt in KCode data size telemetry state");
      }
...
export async function writeKCodeDataSizeTelemetryState(
  stateFile: string,
  state: KCodeDataSizeTelemetryState,
): Promise<void> {
