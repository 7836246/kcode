...
import { LOCAL_TTFT_BUCKETS_MS, localTtftBatchSchema, type LocalTtftBatch } from "@kcode/shared";
import {
...
export function createLocalTtftExporter(options: {
  ...
  const resource = resourceFromAttributes({
    "service.name": "kcode-local-ttft",
    "service.version": options.version,
    "os.type": process.platform,
    "kcode.telemetry.schema_version": 1,
  });
  const meterProvider = new MeterProvider({
    ...
    views: [
      {
        instrumentName: "kcode.local_ttft.*",
        aggregation: {
  ...
  const meter = meterProvider.getMeter("@kcode/local-ttft", "1");
  const duration = meter.createHistogram("kcode.local_ttft.duration", { unit: "ms" });
  const stageDuration = meter.createHistogram("kcode.local_ttft.stage.duration", { unit: "ms" });
  const stageObservation = meter.createHistogram("kcode.local_ttft.stage.observation.duration", {
    unit: "ms",
  });
  const preparationObservation = meter.createHistogram(
    "kcode.local_ttft.preparation.observation.duration",
    { unit: "ms" },
  );
  const systemDuration = meter.createHistogram("kcode.local_ttft.system.duration", { unit: "ms" });
  const preparationDuration = meter.createHistogram("kcode.local_ttft.preparation.duration", {
    unit: "ms",
  });
  const attemptDuration = meter.createHistogram("kcode.local_ttft.attempt.duration", {
    unit: "ms",
  });
  const executionDuration = meter.createHistogram("kcode.local_ttft.execution.duration", {
    unit: "ms",
  });
  const failureWait = meter.createHistogram("kcode.local_ttft.no_output.wait", { unit: "ms" });
  const textDuration = meter.createHistogram("kcode.local_ttft.first_text.duration", {
    unit: "ms",
  });
  const outcomes = meter.createCounter("kcode.local_ttft.records");
  const drops = meter.createCounter("kcode.local_ttft.dropped");
