...
import { readKCodeStdioTapDevState } from "@kcode/services/node";
...
import {
  HELP_TOGGLE_KCODE_STDIO_TAP_MENU_ID,
  HELP_TOGGLE_DEV_TOOLS_MENU_ID,
} from "./desktopCommandHandlers.js";

const HELP_KCODE_ENDPOINT_PRODUCTION_MENU_ID = "help.kcode-endpoint.production";
...
export function updateKCodeStdioTapDevMenuState() {
  const menu = Menu.getApplicationMenu();
  const item = menu?.getMenuItemById(HELP_TOGGLE_KCODE_STDIO_TAP_MENU_ID);
  if (!item) {
    return;
  }
  const state = readKCodeStdioTapDevState();
...
function buildApplicationMenuTemplate(options: {
  ...
  kcodeEndpointSelection?: "production" | "test" | "custom";
  ...
  const stdioTapState = readKCodeStdioTapDevState();
      ...
      ? [
          {
            ...
            submenu: [
              ...
              ...(KCODE_PRODUCT_FLAVOR === "production"
                ? [
                    {
    ...
    },
    {
      ...
      submenu: [
        ...
        ...(isLocalDevelopmentRuntime && stdioTapState.visible
          ? [
              {
                id: HELP_TOGGLE_KCODE_STDIO_TAP_MENU_ID,
                label: getLabel(desktopMenuMessageIds.helpToggleKCodeStdioTap),
                type: "checkbox" as const,
                checked: stdioTapState.enabled,
                click: () =>
                  void options.executeDesktopCommand(DesktopCommandIds.ToggleKCodeStdioTapDevProxy),
              },
              { type: "separator" as const },
            ]
          : []),
        ...(KCODE_ENV === "test"
          ? [
              {
                label: getLabel(desktopMenuMessageIds.helpKCodeEndpoint),
                submenu: [
                  {
                    id: HELP_KCODE_ENDPOINT_PRODUCTION_MENU_ID,
                    label: getLabel(desktopMenuMessageIds.helpKCodeEndpointProduction),
                    type: "radio" as const,
                    checked: (options.kcodeEndpointSelection ?? "production") === "production",
                    click: () =>
                      void options.executeDesktopCommand(
                        DesktopCommandIds.SetKCodeEndpointProduction,
                      ),
                  },
                  { type: "separator" as const },
                  {
                    label: getLabel(desktopMenuMessageIds.helpKCodeEndpointCustom),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.SetKCodeEndpointCustom),
                  },
                  {
                    label: getLabel(desktopMenuMessageIds.helpKCodeEndpointReset),
                    click: () =>
                      void options.executeDesktopCommand(DesktopCommandIds.ResetKCodeEndpoint),
                  },
                ],
              },
              { type: "separator" as const },
  ...
  Menu.setApplicationMenu(
    Menu.buildFromTemplate(
      buildApplicationMenuTemplate({
        ...
        kcodeEndpointSelection: options.kcodeEndpointSelection,
