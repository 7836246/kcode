...
import { formatKCodeRendererProcessName } from "@kcode/shared";
...
export function buildAuxiliaryRendererName(contents: WebContents): string {
  ...
  if (url?.startsWith("devtools://")) {
    return formatKCodeRendererProcessName(
      `devtools-${contentsId}-${hostTitle ?? ownerLabel ?? "unknown"}`,
    );
  }

  if (type === "window") {
    return formatKCodeRendererProcessName(title ?? `window-${contentsId}`);
  }

  return formatKCodeRendererProcessName(`${type}-${contentsId}-${pageLabel}`);
}
