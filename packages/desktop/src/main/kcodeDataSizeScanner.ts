...
export type KCodeDataSizePartialReason = "file_limit" | "io_error" | "time_limit";

export type KCodeDataSizeScanResult = {
  bytes: number;
  ...
  | { status: "partial"; partialReason: KCodeDataSizePartialReason }
);

export interface KCodeDataSizeScanRequest {
  rootPath: string;
  maxDurationMs: number;
  maxFiles: number;
}

interface KCodeDataSizeScanOptions extends KCodeDataSizeScanRequest {
  signal?: AbortSignal;
...
function createAbortError(): DOMException {
  return new DOMException("KCode data size scan aborted", "AbortError");
}
 ...
 */
export async function scanKCodeDataDirectory(
  options: KCodeDataSizeScanOptions,
): Promise<KCodeDataSizeScanResult> {
  ...
  let terminalPartialReason: Exclude<KCodeDataSizePartialReason, "io_error"> | null = null;
...
export function isKCodeDataSizeScanResult(value: unknown): value is KCodeDataSizeScanResult {
  if (typeof value !== "object" || value === null) {
    return false;
  }
  const candidate = value as Partial<KCodeDataSizeScanResult>;
  const finiteNumbers = [
    candidate.bytes,
