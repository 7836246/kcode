...
import { KCODE_AGENT_LIFECYCLE_LOG_MARKER } from "@kcode/shared/process-diagnostic";
import {
...
function hasProductExecutable(
  ...
  // Linux Preview 使用 kcode-preview。两者都必须精确匹配，不能放宽成前缀以免混入 helper dump。
...
const armsRumEnv = mapKCodeEnvToArmsRumEnv(desktopRuntimeEnv);
...
function startArmsRum(): Promise<void> {
  return armsRum
    .init({
      ...
      endpoint: KCODE_ARMS_RUM_ENDPOINT,
      env: armsRumEnv,
          ...
          (payload?.events ?? []).filter(
            (event) =>
              !(
                ...
                event.message.includes(KCODE_AGENT_LIFECYCLE_LOG_MARKER)
              ),
          ),
    ...
    .then(() => {
      logger.info(`[arms] electron initialized env=${armsRumEnv} version=${KCODE_VERSION}`);
    })
    .catch((error) => {
...
export const armsInitPromise: Promise<void> =
  KCODE_TELEMETRY_ENABLED && KCODE_ARMS_RUM_ENDPOINT ? startArmsRum() : Promise.resolve();
