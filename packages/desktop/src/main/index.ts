...
import {
  ...
  DEFAULT_LOCALE,
  KCODE_VERSION,
  KCODE_TELEMETRY_ENABLED,
  KCODE_ARMS_RUM_ENDPOINT,
...
import {
  ...
  updateKCodeStdioTapDevMenuState,
...
import { resolveKCodeBuiltinProviderConfigFilePath } from "./desktopProviderConfig.js";
import {
  getCredentialsDir,
  ...
  resolveKCodeEndpointEnvBaseOrigin,
  desktopRuntimeEnv,
...
import {
  registerDesktopKCodeDataSizeTelemetry,
  stopDesktopKCodeDataSizeTelemetry,
} from "./desktopKCodeDataSizeTelemetry.js";
...
import { mapKCodeEnvToArmsRumEnv } from "@kcode/shared";
import {
...
// 会和开发态已打开的 KCode Dev 抢端口，导致 WebDriver session 创建前白屏超时。
// 仅本地开发运行默认开启远程调试端口，并允许 e2e 通过环境变量交给 Chromedriver 接管。
if (!app.isPackaged && process.env.KCODE_DISABLE_FIXED_REMOTE_DEBUGGING_PORT !== "1") {
...
const settingsFile = join(homedir(), ".kcode", "v2", "setting.json");
...
async function resolveCurrentKCodeEndpointOrigin() {
  return resolveKCodeEndpointOrigin({
    env: KCODE_ENV,
    envBaseOrigin: resolveKCodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    overrideOrigin: (await mainSettingService.get()).kcodeEndpointOrigin,
  });
}
...
app.on("browser-window-focus", (_event, win) => {
  ...
  rebuildMenu();
  // 设置/更新等无 Host 的 KCode 窗口也算前台：router 会先把旧 workspace Host 清成 null，
...
const readHelpConfig = createDesktopHelpConfigReader({
  appVersion: KCODE_VERSION || app.getVersion(),
  deviceMid,
...
const localTtftExporter = createLocalTtftExporter({
  ...
  version: KCODE_VERSION || app.getVersion(),
  logger,
});
...
const armsUserIdentitySync = createArmsUserIdentitySync({
  ...
  setUser:
    KCODE_TELEMETRY_ENABLED && KCODE_ARMS_RUM_ENDPOINT
...
async function prepareAppQuit(reason: string, kind: AppShutdownKind = "normal"): Promise<void> {
  ...
  appQuitPreparationInFlight = Promise.all([
    ...
    .finally(() => {
      ...
      // host 还没等到 agent 进程树的 SIGTERM/SIGKILL 兜底完成就被带走，kcode-cli 会被 init 接管成残留进程。
...
function exitPreparedApp(reason: string): never | void {
  ...
  if (process.env.KCODE_E2E_RUN_ID?.trim()) {
...
function shouldConfirmAppQuit() {
  ...
  return KCODE_ENV === "production" && getRunningAgentSessionCount() > 0;
}

function confirmAppQuit(originWindow?: BrowserWindow | null) {
  if (!shouldConfirmAppQuit()) {
    logger.info(`[app-quit] quit confirmation skipped in ${KCODE_ENV}`);
    return true;
  }
...
async function executeDesktopCommandForApp(
  ...
  return executeDesktopCommand({
    ...
    onKCodeEndpointChanged: handleKCodeEndpointChanged,
    kcodeEndpointEnvBaseOrigin: resolveKCodeEndpointEnvBaseOrigin(hostProcessLocalEnv),
    onRelaunchApp: async () => {
...
async function resolveKCodeEndpointSelection(): Promise<"production" | "test" | "custom"> {
  if (KCODE_ENV === "production") {
    return "production";
  }
  const origin = await resolveCurrentKCodeEndpointOrigin();
  if (origin === DEFAULT_KCODE_ENDPOINT_ORIGIN) {
    return "production";
  }
  return "custom";
}

async function handleKCodeEndpointChanged() {
  rebuildMenu();
}
...
function rebuildMenu() {
  void Promise.all([resolveKCodeEndpointSelection(), mainSettingService.get()]).then(
    ([kcodeEndpointSelection, settings]) => {
      rebuildApplicationMenu({
        currentApplicationLocale,
        kcodeEndpointSelection,
  ...
  const win = createWindow({
    ...
    spawnHostProcess: (win, label, initMessage) =>
      spawnHostProcess(
        ...
        {
          ...initMessage,
          kcodeBuiltinProviderConfigFilePath: resolveKCodeBuiltinProviderConfigFilePath({
            env: { ...hostProcessLocalEnv, ...process.env },
          }),
        },
        {
...
app.whenReady().then(async () => {
  ...
  if (process.platform === "win32") {
    ...
    app.setAppUserModelId(
      resolveWindowsAppUserModelIdForFlavor(KCODE_PRODUCT_FLAVOR, { isPackaged: app.isPackaged }),
    );
  }
  ...
  // Preview 身份无论连接哪个后端都不自动更新：stable feed 上只分发正式 KCode 安装包，
  // 不向 Preview 渠道提供更新。
  void initAutoUpdater({
    enabled: KCODE_PRODUCT_FLAVOR === "production",
  ...
  registerRemoteIpcHandlers({
    ...
    armsCustomContext: {
      ...
      armsEnv: mapKCodeEnvToArmsRumEnv(desktopRuntimeEnv),
    },
  ...
  if (KCODE_TELEMETRY_ENABLED && KCODE_ARMS_RUM_ENDPOINT) {
    configureDesktopStabilityTelemetry({
      deviceMid,
      platform: process.platform,
      appVersion: KCODE_VERSION,
  ...
  registerDesktopKCodeDataSizeTelemetry({
    ...
    isKCodeBusy: () => getRunningAgentSessionCount() > 0,
    logger,
    rootPath: getKCodeDataRootDir(),
    stateFile: join(app.getPath("userData"), "kcode-data-size-telemetry.json"),
  });
  registerDesktopNetworkTelemetry(logger);

  // 本地未打包 dev 构建（app.isPackaged === false）必须跳过远端强制升级 gate。
  // 原因：force-update gate 只看 KCODE_ENV === "production"，但 dev 构建（如 dev:desktop:cua
  ...
  const forceUpdateGuardResult =
    KCODE_PRODUCT_FLAVOR === "production" && !skipForceUpdateForLocalDevRuntime
