...
import type { ArmsRumEnv, FinalArmsCustomEventPayload } from "@kcode/shared";

import type { KCodeDataSizeScanResult } from "./kcodeDataSizeScanner.js";
import {
  readKCodeDataSizeTelemetryState,
  writeKCodeDataSizeTelemetryState,
  type KCodeDataSizeTelemetryState,
} from "./kcodeDataSizeTelemetryState.js";
import { scanKCodeDataDirectoryInWorker } from "./kcodeDataSizeWorkerClient.js";

export type { KCodeDataSizeTelemetryState } from "./kcodeDataSizeTelemetryState.js";

const KCODE_DATA_SIZE_SCAN_MAX_DURATION_MS = 30_000;
const KCODE_DATA_SIZE_SCAN_MAX_FILES = 200_000;
const KCODE_DATA_SIZE_DAILY_INTERVAL_MS = 24 * 60 * 60 * 1000;
const KCODE_DATA_SIZE_IDLE_POLL_MS = 5 * 60 * 1000;
const KCODE_DATA_SIZE_IDLE_WAIT_FALLBACK_MS = 6 * 60 * 60 * 1000;
const KCODE_DATA_SIZE_MINIMUM_IDLE_SECONDS = 5 * 60;
const KCODE_DATA_SIZE_STARTUP_MIN_DELAY_MS = 2 * 60 * 1000;
const KCODE_DATA_SIZE_STARTUP_JITTER_MAX_MS = 8 * 60 * 1000;
const KCODE_DATA_SIZE_DAILY_JITTER_MAX_MS = 60 * 60 * 1000;
const KCODE_DATA_SIZE_ABORTED_RETRY_MS = 30 * 60 * 1000;
const KCODE_DATA_SIZE_FAILURE_RETRY_MS = 6 * 60 * 60 * 1000;
const KCODE_DATA_SIZE_ACTIVITY_POLL_MS = 1_000;

interface KCodeDataSizeTelemetryTiming {
...
const DEFAULT_TIMING: KCodeDataSizeTelemetryTiming = {
  abortedRetryMs: KCODE_DATA_SIZE_ABORTED_RETRY_MS,
  activityPollMs: KCODE_DATA_SIZE_ACTIVITY_POLL_MS,
  dailyIntervalMs: KCODE_DATA_SIZE_DAILY_INTERVAL_MS,
  dailyJitterMaxMs: KCODE_DATA_SIZE_DAILY_JITTER_MAX_MS,
  failureRetryMs: KCODE_DATA_SIZE_FAILURE_RETRY_MS,
  idlePollMs: KCODE_DATA_SIZE_IDLE_POLL_MS,
  idleWaitFallbackMs: KCODE_DATA_SIZE_IDLE_WAIT_FALLBACK_MS,
  minimumIdleSeconds: KCODE_DATA_SIZE_MINIMUM_IDLE_SECONDS,
  startupJitterMaxMs: KCODE_DATA_SIZE_STARTUP_JITTER_MAX_MS,
  startupMinDelayMs: KCODE_DATA_SIZE_STARTUP_MIN_DELAY_MS,
};

interface KCodeDataSizeTelemetryLogger {
  info: (...args: unknown[]) => void;
  warn: (...args: unknown[]) => void;
}

interface KCodeDataSizeTelemetrySchedulerDependencies {
  deviceMid: string;
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isKCodeBusy: () => boolean;
  logger: KCodeDataSizeTelemetryLogger;
  readState: () => Promise<KCodeDataSizeTelemetryState | null>;
  report: (result: KCodeDataSizeScanResult) => Promise<void> | void;
  scan: (options: { signal: AbortSignal }) => Promise<KCodeDataSizeScanResult>;
  timing?: Partial<KCodeDataSizeTelemetryTiming>;
  writeState: (state: KCodeDataSizeTelemetryState) => Promise<void>;
}
...
function buildKCodeDataSizeArmsPayload(params: {
  context: {
    appVersion: string;
    armsEnv: ArmsRumEnv;
  ...
  result: KCodeDataSizeScanResult;
}): FinalArmsCustomEventPayload {
  const eventName = "perf_resource_kcode_data_size";
  return {
    group: "resource",
    name: eventName,
    properties: stringifyProperties({
      ...
      schema_version: 1,
      kcode_data_bytes: params.result.bytes,
    }),
    type: "custom",
...
function stableJitterMs(deviceMid: string, bucketAt: number, maxMs: number): number {
  ...
  const key = `${deviceMid}:${Math.floor(bucketAt / KCODE_DATA_SIZE_DAILY_INTERVAL_MS)}`;
...
function createKCodeDataSizeTelemetryScheduler(
  dependencies: KCodeDataSizeTelemetrySchedulerDependencies,
  ...
  let persistedState: KCodeDataSizeTelemetryState | null = null;
  let stopped = true;
  ...
  function collectionEligibilityLost(relaxedIdle: boolean): boolean {
    try {
      return dependencies.isKCodeBusy() || (!relaxedIdle && !isPreferredIdleWindow());
    } catch {
      return true;
    }
  }
  ...
  async function persistSuccess(reportedAt: number): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[kcode-data-size] failed to persist report state", error);
    }
  }

  async function reserveReport(reservedAt: number): Promise<KCodeDataSizeTelemetryState | null> {
  ...
  async function rollbackReportReservation(
    previousState: KCodeDataSizeTelemetryState | null,
  ): Promise<void> {
    try {
      ...
      dependencies.logger.warn("[kcode-data-size] failed to roll back report reservation", error);
    }
  }
  ...
  async function collectAndReport(relaxedIdle: boolean): Promise<void> {
    ...
    try {
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        ...
        abortController.abort();
        throw new DOMException("KCode data size scan eligibility lost", "AbortError");
      }
      ...
      if (abortController.signal.aborted || collectionEligibilityLost(relaxedIdle)) {
        await rollbackReportReservation(previousState);
        abortController.abort();
        throw new DOMException("KCode data size report eligibility lost", "AbortError");
      }
      try {
      ...
      dependencies.logger.info(
        `[kcode-data-size] reported status=${result.status} bytes=${result.bytes}`,
      );
      schedule(
      ...
      dependencies.logger.warn(
        `[kcode-data-size] ${aborted ? "scan aborted" : "scan/report failed"}`,
        error,
      );
  ...
  async function initializeFromPersistedState(): Promise<void> {
    let state: KCodeDataSizeTelemetryState | null;
    try {
      state = await dependencies.readState();
    } catch (error) {
      // Bug 根因：状态读取失败不等于没有历史状态；按首次启动继续采集会绕过持久化限流。
      // 无法确认配额时保持 fail-closed，并且只重试状态读取，不进入采集调度。
      dependencies.logger.warn("[kcode-data-size] failed to read report state", error);
...
let desktopScheduler: ReturnType<typeof createKCodeDataSizeTelemetryScheduler> | null = null;

export function registerDesktopKCodeDataSizeTelemetry(options: {
  context: Parameters<typeof buildKCodeDataSizeArmsPayload>[0]["context"];
  getSystemIdleTimeSeconds: () => number;
  isAppBackground: () => boolean;
  isKCodeBusy: () => boolean;
  logger: KCodeDataSizeTelemetryLogger;
  ...
  stopDesktopKCodeDataSizeTelemetry();
  desktopScheduler = createKCodeDataSizeTelemetryScheduler({
    ...
    isKCodeBusy: options.isKCodeBusy,
    logger: options.logger,
    readState: () => readKCodeDataSizeTelemetryState(options.stateFile),
    report: (result) => {
      const payload = buildKCodeDataSizeArmsPayload({ context: options.context, result });
      armsRum.sendCustom({
        group: payload.group,
        name: payload.name,
    ...
    scan: ({ signal }) =>
      scanKCodeDataDirectoryInWorker(
        {
          maxDurationMs: KCODE_DATA_SIZE_SCAN_MAX_DURATION_MS,
          maxFiles: KCODE_DATA_SIZE_SCAN_MAX_FILES,
          rootPath: options.rootPath,
        },
        signal,
      ),
    writeState: (state) => writeKCodeDataSizeTelemetryState(options.stateFile, state),
  });
  void desktopScheduler.start().catch((error) => {
    options.logger.warn("[kcode-data-size] scheduler start failed", error);
  });
}

export function stopDesktopKCodeDataSizeTelemetry(): void {
