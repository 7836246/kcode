...
import type { SaveFileRequest, SaveFileResult } from "@kcode/shared";
import { PlatformChannels } from "@kcode/shared";
...
async function downloadRemoteFile(sourceUrl: URL, destinationPath: string): Promise<void> {
  ...
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "kcode-save-file-"));
