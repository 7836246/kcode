...
contextBridge.exposeInMainWorld("__KCODE_DEVICE_ID__", parseDeviceIdFromArgs());
...
import {
  InternalChannels,
  PlatformChannels,
  formatKCodeRendererProcessName,
  shouldEnableE2ETestBridge,
} from "@kcode/shared";
if (shouldEnableE2ETestBridge(process.env)) {
  contextBridge.exposeInMainWorld("__kcodeFinalArmsCustomEventsE2E", {
...
function updateRendererProcessTitle(): void {
  process.title = formatKCodeRendererProcessName(document.title);
}
...
// 这里在 preload 阶段补上 kcode-* title，便于按窗口角色筛选。
...
contextBridge.exposeInMainWorld("kcode", {
  connectRemote: (
    ...
    context?: {
      ...
      connectTrigger?: import("@kcode/shared").RemoteWorkspaceConnectTrigger;
    },
  ) =>
  ...
  reportLocalTtftBatch: (batch: import("@kcode/shared").LocalTtftBatch): void =>
  ...
  /** 导出日志：打包 ~/.kcode/v2 及外部 agent 日志为 zip 并在 Finder 中显示 */
  exportLogs: (): Promise<{
    success: boolean;
    path?: string;
    error?: string;
  ...
  importChromeBrowserData: (options?: import("@kcode/shared").ChromeBrowserDataImportOptions) =>
  ...
  getKCodeStdioTapDevState: (): Promise<KCodeStdioTapDevState> =>
    ipcRenderer.invoke(PlatformChannels.GetKCodeStdioTapDevState),
