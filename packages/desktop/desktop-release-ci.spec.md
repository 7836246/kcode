...
| Linux x64 / ARM64 | `.deb`、`.rpm`、`.AppImage` | `.blockmap`、合并后的 `latest-linux.yml` |
...
- 各 arch 的 `latest-*.yml` 在 publish job 合并；同名安装包不得互相覆盖
...
- 推送匹配的 `v*` tag 或在 Actions 里选该 tag 重跑，Release 上出现上表安装包、zip/blockmap 与合并后的 `latest*.yml`
