...
import {
  ...
  KCODE_RPC_HOST_CAPABILITY_HEADER,
  KCODE_VERSION,
  type BotProvider,
...
const log = (...args: unknown[]) =>
  console.log(formatLogPrefix("kcode-server:http", process.pid), ...args);
...
function resolveServerId(options: HttpServerOptions): string {
  return (
    options.serverId?.trim() || readTrimmedEnv("KCODE_SERVER_ID") || hostname() || "kcode-server"
  );
}
...
function resolveServerWorkspaces(options: HttpServerOptions): ServerRemoteWorkspaceInfo[] {
  ...
  const workspacePath = readTrimmedEnv("KCODE_SERVER_WORKSPACE") || process.cwd();
  return [
    {
      path: workspacePath,
...
function createServerInfo(options: HttpServerOptions): ServerRemoteInfo {
  return {
    ...
    ...(options.name?.trim() || readTrimmedEnv("KCODE_SERVER_NAME")
      ? { name: options.name?.trim() || readTrimmedEnv("KCODE_SERVER_NAME") }
      : {}),
    version: KCODE_VERSION,
    protocolVersion: SERVER_REMOTE_PROTOCOL_VERSION,
    authRequired: options.authRequired ?? Boolean(readTrimmedEnv("KCODE_SERVER_TOKEN")),
...
const kcodeLiteTokenCookieName = "kcode_lite_token";
...
function hasValidLiteToken(c: Context, token: string): boolean {
  ...
  if (url.searchParams.get("token") === token) {
    c.header(
      "Set-Cookie",
      `${kcodeLiteTokenCookieName}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax`,
    );
    return true;
  }
  return parseCookieHeader(c.req.header("cookie")).get(kcodeLiteTokenCookieName) === token;
}
...
export function createHttpServer(
  ...
  app.use("/ws/host", async (c, next) => {
    const capability = c.req.header(KCODE_RPC_HOST_CAPABILITY_HEADER);
  ...
  const handleBotCallback = async (c: Context) => {
    ...
    const webhookSecret = c.req.header("x-kcode-bot-secret");
