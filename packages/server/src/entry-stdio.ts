import { disposeServiceResourcesAndWait, getAppConfigDir } from "@kcode/services/node";
import {
  KCODE_VERSION,
...
const log = (...args: unknown[]) =>
  console.error(formatLogPrefix("kcode-server:stdio", process.pid), ...args);
...
if (process.argv.includes("--version")) {
  process.stdout.write(KCODE_VERSION + "\n");
  process.exit(0);
}
...
async function main() {
  ...
  // 远端主机没有 Desktop main，没人写 telemetry-state.json，services 发往 KCode endpoint
