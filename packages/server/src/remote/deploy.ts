...
import { deployKCodeAgentRuntime } from "./kcodeAgentDeploy.js";
import {
  deployNodePtyPrebuilds,
  deployNodeRuntime,
  createRemoteComponentVersionResolver,
  logDeployRequired,
} from "@kcode/server/remote/remoteAssetDeployDecision.js";
import {
  REMOTE_BASE,
  fileExists,
  formatOptionalValue,
...
import { checkServerBundleRequiredMarkers } from "@kcode/server/remote/serverBundleDeployCheck.js";
import { deployRuntimeTools } from "@kcode/server/remote/runtimeToolDeploy.js";
import { REMOTE_AGENT_OFFICIAL_PLUGIN_REQUIRED_RELATIVE_PATHS } from "@kcode/server/remote/kcodeAgentOfficialPluginAssets.js";
import {
  ensureRemoteReleaseDirFromCdn,
  selectRemoteAssetManifestComponents,
  type RemoteAssetManifestRef,
} from "@kcode/server/remote/remoteAssetCache.js";
import {
...
} from "@kcode/server/remote/remoteAssetInstaller.js";
import {
...
import { detectRemoteAssetTools } from "@kcode/server/remote/remoteAssetPreflight.js";
import { assertSupportedRemoteEnvironment } from "@kcode/server/remote/remotePlatformSupport.js";
import { acquireRemoteDeployLock } from "@kcode/server/remote/remoteDeployLock.js";
...
/**
 * Deploy the kcode server to the remote machine.
...
export async function deployServer(
  ...
  const deployWithDecision = async (
    ...
    if (!serverDeployDecision.shouldDeploy) {
      ...
      // 主 server 版本相同只证明 node/kcode-server.cjs 可启动，不代表随包工具仍存在。
    ...
    await installer.installFile({
      componentId: SERVER_BUNDLE_COMPONENT_ID,
      sourceRelativePath: "server/kcode-server.cjs",
      remotePath: `${REMOTE_BASE}/kcode-server.cjs`,
    ...
    // 部署 KCode Agent runtime 到远程，历史资源包选择已在入口统一忽略。
    await deployKCodeAgentRuntime(
      backend,
      env,
      {
...
async function checkServerDeployDecision(
  ...
  try {
    ...
    const serverPath = `${REMOTE_BASE}/kcode-server.cjs`;
    ...
    log("remote version:", JSON.stringify(version), "local:", KCODE_VERSION);
    if (version !== KCODE_VERSION) {
      return {
        shouldDeploy: true,
        reason: `remote server version mismatch remote=${version} expected=${KCODE_VERSION}`,
...
function resolveMockCdnReleaseDir(mockCdnDir?: string): string | null {
  ...
  return join(mockCdnDir, "releases", KCODE_VERSION);
}
  ...
  for (const componentId of ids) {
    switch (componentId) {
      case SERVER_BUNDLE_COMPONENT_ID:
        requiredPaths.add("server/kcode-server.cjs");
        break;
      case "node-runtime":
      ...
      case "node-pty":
        ...
        break;
      case "glm":
        requiredPaths.add(`glm/${platformArch}/kcode.cjs`);
