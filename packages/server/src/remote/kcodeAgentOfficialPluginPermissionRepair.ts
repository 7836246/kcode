import { KCODE_AGENT_PROVIDER } from "@kcode/shared";
import type { IRemoteBackend } from "@kcode/server/remote/backend.js";
import { type DeployLoggers, waitForClose } from "@kcode/server/remote/deployShared.js";
...
export async function repairLegacyRemoteOfficialPluginDirectoryPermissions(params: {
  ...
  params.loggers.logWarn(
    `[kcode-agent-deploy] ${KCODE_AGENT_PROVIDER}: 检查并修复旧 builtin plugin 目录权限 ${params.remoteOfficialPluginDir}`,
  );
  ...
  try {
    ...
    params.loggers.logWarn(
      `[kcode-agent-deploy] ${KCODE_AGENT_PROVIDER}: 修复旧 builtin plugin 目录权限失败，将继续尝试替换 packages: ${
