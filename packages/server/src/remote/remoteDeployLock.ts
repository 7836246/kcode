...
import { REMOTE_BASE } from "@kcode/server/remote/deployShared.js";
...
function buildRemoteDeployLockScript(lockDir: string, ownerToken: string): string {
  const acquiredMarker = `kcode-deploy-lock-acquired:${ownerToken}`;
  const releaseMarker = `kcode-deploy-lock-release:${ownerToken}`;
  return [
    "set -eu",
