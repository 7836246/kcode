import type { HelloMessage, HelloAckMessage } from "@kcode/shared";
import { KCODE_VERSION, formatZodError, helloMessageSchema } from "@kcode/shared";
 ...
 * 1. Read lines from stdout until we find a kcode-hello JSON
 *    (skip SSH banner/motd lines)
 * 2. Send a kcode-hello-ack to stdin
...
export function performHandshake(
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      rejectWithDiagnostics("Handshake timeout: no kcode-hello received within timeout");
    }, timeoutMs);
        ...
        if (line.startsWith("{")) {
          try {
            ...
            if (
              ...
              "type" in rawValue &&
              (rawValue as { type?: unknown }).type === "kcode-hello"
            ) {
              rejectWithDiagnostics(`Invalid kcode-hello: ${formatZodError(result.error)}`);
              return;
            }
          } catch {
