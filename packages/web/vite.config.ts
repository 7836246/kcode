...
// Vite 配置在 Node 加载期执行，不能导入 @kcode/shared 根入口。
...
} from "@kcode/shared/kcodeEndpoint";
...
export default defineConfig(({ mode }) => {
  ...
  const kcodeEndpointOrigin = resolveRuntimeKCodeEndpointOrigin(endpointEnv);
  ...
  return {
    ...
    define: {
      ...
      __KCODE_VERSION__: JSON.stringify(version),
      __KCODE_COMMIT__: JSON.stringify(env.KCODE_COMMIT || "unknown"),
      __KCODE_ENV__: JSON.stringify(kcodeEnv),
      "import.meta.env.VITE_KCODE_BASE_URL": JSON.stringify(kcodeEndpointOrigin),
      // 兼容旧 Web runtime 读取名；新代码统一读 VITE_KCODE_BASE_URL。
