...
export class BrowserOAuthCredentialRepo {
  ...
  saveTokenSet(tokenSet: WebZaiTokenSet | OAuthTokenSet, provider: WebOAuthProviderId): void {
    ...
    const kcodeJwtToken = tokenSet.kcodeJwtToken;

    this.localStorage.setItem(providerKeys(provider).accessToken, accessToken);
    if (kcodeJwtToken) {
      this.localStorage.setItem(KCODE_JWT_TOKEN_KEY, kcodeJwtToken);
    } else {
      this.localStorage.removeItem(KCODE_JWT_TOKEN_KEY);
    }
  }
  ...
  loadCachedSessionState(): OAuthCachedSessionRestoreResult {
    ...
    const kcodeJwtToken = this.localStorage.getItem(KCODE_JWT_TOKEN_KEY);
    ...
    if (!keys || !hasText(kcodeJwtToken) || !hasText(accessToken) || !hasText(rawUserInfo)) {
    ...
    if (resolveJwtExpiration(kcodeJwtToken, this.now()).kind === "expired") {
  ...
  loadKCodeJwtToken(): string | null {
    const session = this.loadCachedSessionState();
    if (session.status !== "authenticated") return null;
    return this.localStorage.getItem(KCODE_JWT_TOKEN_KEY)?.trim() || null;
  }

  private hasAnyStoredCredential(): boolean {
    return Boolean(
      this.localStorage.getItem(ACTIVE_PROVIDER_KEY) ||
      this.localStorage.getItem(KCODE_JWT_TOKEN_KEY) ||
