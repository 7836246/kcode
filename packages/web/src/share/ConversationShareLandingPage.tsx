...
import type { ConversationSharePreview } from "@kcode/shared";
import { ConversationShareReadonlyTimeline } from "@kcode/ui/conversation-share-readonly";
import { applyTheme, resolveTheme, type Theme } from "@kcode/ui/useTheme";
...
interface Copy {
  ...
  retry: string;
  continueInKCode: string;
  switchToDarkTheme: string;
  switchToLightTheme: string;
  continueHelp: string;
  downloadKCode: string;
  retryOpen: string;
...
const KCODE_DOWNLOAD_URL = "https://zcode.z.ai";
...
const COPY: Record<ConversationShareLandingLocale, Copy> = {
  "zh-CN": {
    ...
    loginTitle: "无法在网页中登录查看",
    loginDescription: "KCode 不再提供网页官方账号登录。请在桌面端打开此分享，或使用公开链接。",
    login: "登录",
    expiredTitle: "分享已过期",
    expiredDescription: "这个分享链接已经过期，请让分享者重新生成链接。",
    notFoundTitle: "找不到分享内容",
    notFoundDescription: "链接可能无效、分享已被移除，或当前访问者无法查看。",
    notFoundAccountHint: "如果这是私有分享，请让分享者重新生成链接，或在 KCode 客户端中打开。",
    backToHome: "回到首页",
    networkTitle: "暂时无法加载分享",
    networkDescription: "请检查网络后重试。",
    invalidTitle: "分享格式无效",
    invalidDescription: "服务返回的分享内容无法通过安全校验。",
    outdatedTitle: "需要更新 KCode",
    outdatedDescription: "这个分享由更新版本的 KCode 创建，请升级后再查看。",
    unavailableTitle: "分享不可访问",
    unavailableDescription: "当前账号没有权限，或者分享内容已不存在。",
    retry: "重试",
    continueInKCode: "去 KCode 继续",
    switchToDarkTheme: "切换到深色主题",
    switchToLightTheme: "切换到浅色主题",
    continueHelp: "如果没有自动打开 KCode，请先下载客户端，或再次尝试打开。",
    downloadKCode: "下载 KCode",
    artifactCountOne: "{count} 个结果物",
    artifactCountOther: "{count} 个结果物",
    retryOpen: "再次打开",
  },
  "en-US": {
    brand: "KCode Conversation Share",
    loading: "Loading shared conversation",
    loadingDescription: "Please wait while we verify this share link.",
    loginTitle: "Sign-in is not available in the browser",
    loginDescription:
      "KCode no longer offers official account sign-in on the web. Open this share in the desktop app, or use a public link.",
    login: "Sign in",
    ...
    notFoundDescription:
      "The link may be invalid, the share may have been removed, or this visitor may not have access.",
    notFoundAccountHint:
      "If this is a private share, ask the author to create a new link or open it in the KCode app.",
    ...
    outdatedTitle: "Update KCode to continue",
    outdatedDescription:
      "This share was created by a newer version of KCode. Please update to view it.",
    unavailableTitle: "Share unavailable",
    unavailableDescription:
      "This account is not allowed to view the share, or it no longer exists.",
    retry: "Try again",
    continueInKCode: "Continue in KCode",
    switchToDarkTheme: "Switch to dark theme",
    switchToLightTheme: "Switch to light theme",
    continueHelp: "If KCode did not open, download the app or try opening it again.",
    downloadKCode: "Download KCode",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {importLink && headerView.continueVisible ? (
                      <a
                        ...
                        href={importLink}
                        aria-label={copy.continueInKCode}
                        title={copy.continueInKCode}
                        onClick={handleContinue}
                      >
                        <span className={headerView.continueCompact ? "sr-only" : "truncate"}>
                          {copy.continueInKCode}
                        </span>
                        <ArrowUpRightIcon
                          className={
                            headerView.continueCompact
                  ...
                  >
                    ...
                    {importLink ? (
                      <span className={SHARE_CONTINUE_MEASURE_CLASS}>
                        <span>{copy.continueInKCode}</span>
                        <ArrowUpRightIcon
        ...
        {importLink && showContinueHelp ? (
          ...
          <ShareContentInset>
            ...
            >
              ...
              <a className="text-brand underline underline-offset-2" href={KCODE_DOWNLOAD_URL}>
                {copy.downloadKCode}
              </a>
            </div>
          </ShareContentInset>
          ...
          <ShareContentInset>
            ...
            >
              <a
                ...
                onClick={handleContinue}
              >
                <span className="truncate">{copy.continueInKCode}</span>
...
export function ConversationShareLandingLoader({
  ...
  const handleThemeChange = useCallback((nextTheme: Theme) => {
    localStorage.setItem("kcode-theme", nextTheme);
