...
import { KCODE_MODE_OPTION_DESCRIPTION_IDS, KCODE_MODE_OPTION_LABEL_IDS } from "./display-help.js";
...
function isConfigSelectValueInOptions(option: KCodeConfigOption, value: string): boolean {
  return option.options?.some((entry) => String(entry.value) === value) ?? false;
}

function getConfigSelectTriggerTestId(option: KCodeConfigOption): string | undefined {
...
export function ChatApiRetryStatus({
  ...
  const retryLabel = useMemo(() => {
    ...
    // 当前 KCode Agent 只会推送某一刻的 retryDelayMs 快照，不会每秒递减。
...
export function getModeOptionDisplayLabel(
  ...
  entry: Pick<KCodeConfigSelectValue, "name" | "value">,
): string {
...
function getModeOptionLabelMessageId(
  provider: KCodeProvider | undefined,
  entry: Pick<KCodeConfigSelectValue, "value">,
): string | null {
  if (!provider) {
    return null;
  }

  return KCODE_MODE_OPTION_LABEL_IDS[provider]?.[entry.value] ?? null;
}
...
export function getModeOptionDescriptionMessageId(
  ...
  return KCODE_MODE_OPTION_DESCRIPTION_IDS[provider]?.[entry.value] ?? null;
}
...
export function resolveModeOptionIcon(value: unknown): LucideIcon {
  ...
  // build / edit / yolo 对齐 ChatGPT 批准菜单：虚线圆、火花、解锁。
  if (typeof value === "string" && value.toLocaleLowerCase() === "build") return CircleDashedIcon;
