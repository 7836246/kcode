...
import type { IBroadcastService } from "@kcode/services";
import { createKCodeStore, type KCodeStore, type KCodeState } from "./index.js";

// 导出 Context 供测试直接注入已构造的 store 实例（如跨窗口广播抑制用例）。
const StoreContext = createContext<KCodeStore | null>(null);
 ...
 */
export function useKCodeStore<T>(selector: (state: KCodeState) => T): T {
  const store = useContext(StoreContext);
  if (!store) {
    throw new Error("useKCodeStore 必须在 StoreProvider 内使用");
  }
  return useStore(store, selector);
}

/**
 * 带默认值的容错版 useKCodeStore（store 耦合剥离配套）。
 *
 ...
 */
export function useKCodeStoreWithDefault<T>(
  selector: (state: KCodeState) => T,
  defaultValue: T,
): T {
  const store = useContext(StoreContext);
