...
export function getSubagentsContextKey(
  ...
  return `${workspaceIdentity?.trim() || workspacePath}::${normalizeAgentProviderToKCodeAgent(provider)}`;
}
...
async function loadContext(
  ...
  const provider = normalizeAgentProviderToKCodeAgent(params.provider);
