...
import { getDefaultWorkspaceState } from "@/store/kcodeSessionStoreTypes.js";
...
function sortTasksByUpdatedAt(tasks: readonly KCodeTaskMeta[]): KCodeTaskMeta[] {
...
function arePermissionOptionsEqual(
  left: KCodePermissionRequest["options"],
  right: KCodePermissionRequest["options"],
): boolean {
...
function arePermissionRequestsEquivalent(
  left: KCodePermissionRequest,
  right: KCodePermissionRequest,
): boolean {
  if (left === right) {
    return true;
  }
          ...
          (current) => ({
            ...
            taskUiByTaskId: {
              ...
              [taskId]: {
                ...
                // 完整错误对象之前只放在 ChatView/useKCodeChat 的本地 state。
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => {
            ...
            return {
              ...current,
              selectedProvider: normalizeAgentProviderToKCodeAgent(params.provider),
