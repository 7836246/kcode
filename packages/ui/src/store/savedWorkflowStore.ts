...
import {
  ...
  type KCodeSavedWorkflowInvalidEntry,
...
export interface SavedWorkflowWorkspaceState {
  entries: KCodeSavedWorkflowEntry[];
  invalid: KCodeSavedWorkflowInvalidEntry[];
  runs: KCodeSavedWorkflowRun[];
 ...
 */
function savedWorkflowStoreKey(target: KCodeAgentSavedWorkflowTarget): string {
...
async function fetchWorkspace(
  target: KCodeAgentSavedWorkflowTarget,
  agentService: IKCodeAgentService,
): Promise<{ list: KCodeWorkflowsListResult; runs: KCodeSavedWorkflowRun[] }> {
  ...
  const [list, runs] = await Promise.all([
    ...
    agentService
      ...
      .catch((error: unknown) => {
        ...
        return [] as KCodeSavedWorkflowRun[];
      }),
  ]);
  return { list, runs };
}
...
export function selectSavedWorkflowState(
  ...
  target: KCodeAgentSavedWorkflowTarget | null,
