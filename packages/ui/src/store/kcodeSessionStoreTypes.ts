/* oxlint-disable eslint(max-lines) -- KCode Agent store 类型和默认状态集中导出，避免切片间重复定义共享结构。 */
/**
 * KCode Session Store 类型定义、接口、常量与默认值工厂
 *
 * 从 kcodeSessionStore.ts 拆分出来，供 store 本体和 selectors / navigation 等子模块共享。
 */
import {
  ...
  type KCodeWorkspaceInitStatus,
  type KCodeTaskRuntimeStatus,
  type KCodeProvider,
  type KCodeTaskMeta,
  type KCodeContextCacheUsage,
  ...
  type KCodeSessionActiveTurnKind,
  type KCodeContextUsageBreakdownItem,
  type InputId,
  type SessionCreateSource,
} from "@kcode/shared";
import type { KCodeUiError } from "@/lib/kcodeUiError.js";
import type {
...
export interface WorkspaceInitState {
  status: KCodeWorkspaceInitStatus;
  error: string | null;
  attempts: number;
}
...
export interface TaskRuntimeState {
  status: KCodeTaskRuntimeStatus;
  error: string | null;
  /** 该 task 当前运行态绑定的 KCode Agent 进程 provider，用于 workspace 级进程重建 busy lock。 */
  ...
  backgroundTaskControls: KCodeBackgroundTaskControlItem[];
...
export interface TaskUiState {
  permissionRequest: KCodePermissionRequest | null;
  pendingPermissionRequests: KCodePermissionRequest[];
  elicitationRequest: KCodeElicitationRequest | null;
  pendingElicitationRequests: KCodeElicitationRequest[];
  elicitationFormDraftsByRequestId: Record<string, ElicitationFormDraft>;
  error: KCodeUiError | null;
}
...
export interface WorkspaceKCodeUIState {
  /** 当前 workspace 激活中的 task */
  activeTaskId: string | null;
  /**
   * KCode Agent 的 workspace 初始化状态。
   *
   * 单 KCode Agent 迁移后继续按 provider 分桶会保留多份已经不会再被真实运行时更新的旧状态，
  ...
  taskConfigOptionsByTaskId: Record<string, KCodeConfigOption[]>;
  ...
  optimisticTaskListByTaskId: Record<string, KCodeTaskMeta>;
  ...
  /** 当前 workspace 选中的 KCode Agent provider */
  ...
  /** KCode Agent configOptions（模型、模式、思考级别等） */
  configOptions: KCodeConfigOption[] | null;
  /** configOptions 加载状态 */
  ...
  taskListCache: KCodeTaskMeta[] | null;
  /** 新建草稿时递增，驱动输入框在切到草稿态后主动聚焦 */
  draftFocusVersion: number;
}

export interface KCodeSessionStoreState {
  /** 按 workspace 维护聊天相关 UI 状态 */
  workspaces: Record<string, WorkspaceKCodeUIState>;
  getWorkspaceState: (workspacePath: string, workspaceIdentity?: string) => WorkspaceKCodeUIState;
  ...
  setTaskPermissionRequest: (
    workspacePath: string,
    taskId: string,
    request: KCodePermissionRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  setTaskElicitationRequest: (
    workspacePath: string,
    taskId: string,
    request: KCodeElicitationRequest | null,
    workspaceIdentity?: string,
  ) => void;
  ...
  /** 任务列表变更时调用（创建/删除任务），驱动 kcodeTaskMetaMerge 重新拉取列表 */
...
const FALLBACK_PROVIDER: KCodeProvider = KCODE_AGENT_PROVIDER;
...
export function getDefaultWorkspaceState(): WorkspaceKCodeUIState {
  // 单 KCode Agent 迁移后默认 provider 必须收敛到 glm。
