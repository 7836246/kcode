...
import type { ISubagentsService } from "@kcode/services";
...
interface SubagentsStoreState {
  ...
  loadedProvider: KCodeProvider | null;
  agents: AgentSummary[];
  ...
  initialize: (
    ...
    providerOrSubagentsService: KCodeProvider | ISubagentsService,
...
export const useSubagentsStore = create<SubagentsStoreState>((set, get) => ({
  ...
  async initialize(
    ...
    const provider = normalizeAgentProviderToKCodeAgent(
      hasProvider ? providerOrSubagentsService : KCODE_AGENT_PROVIDER,
    );
  ...
  async refresh(subagentsService: ISubagentsService, workspaceIdentity?: string) {
    ...
    const provider = normalizeAgentProviderToKCodeAgent(get().provider);
  ...
  async createAgent(
    ...
    try {
      const result = await subagentsService.createAgent({
        config,
        provider: normalizeAgentProviderToKCodeAgent(provider),
      });
      await get().refresh(subagentsService, workspaceIdentityFromState);
      return result.agent;
    } catch (error) {
...
export async function refreshLoadedSubagentsStoreForWorkspace(params: {
  ...
  const contextKey = getSubagentsContextKey(workspacePath, KCODE_AGENT_PROVIDER, workspaceIdentity);
