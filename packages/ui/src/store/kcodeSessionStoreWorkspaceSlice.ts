...
} from "@/store/kcodeSessionStoreTypes.js";
import {
  getTaskMeta,
  getWorkspaceState,
  getWorkspaceInitState,
  updateWorkspaceState,
} from "@/store/kcodeSessionStoreSelectors.js";

type SetFn = (
  partial:
    | KCodeSessionStoreState
    | Partial<KCodeSessionStoreState>
    | ((state: KCodeSessionStoreState) => KCodeSessionStoreState | Partial<KCodeSessionStoreState>),
) => void;
...
function normalizeThoughtLevelConfigOption(option: KCodeConfigOption): KCodeConfigOption {
  if (
    option.type !== "select" ||
...
function normalizeConfigOptions(options: KCodeConfigOption[]): KCodeConfigOption[] {
  return options.map(normalizeThoughtLevelConfigOption);
}

function cloneConfigOptions(options: readonly KCodeConfigOption[]): KCodeConfigOption[] {
...
function isModeConfigOption(option: KCodeConfigOption): boolean {
  return option.category === "mode" && option.type === "select";
}

function createFallbackModeOption(params: {
  currentModeId: string;
  options?: NonNullable<KCodeConfigOption["options"]>;
}): KCodeConfigOption {
...
function replaceModeConfigOption(
  currentOptions: KCodeConfigOption[] | null,
  nextModeOption: KCodeConfigOption | null,
): KCodeConfigOption[] | null {
...
function updateActiveTaskConfigOptions(
  current: WorkspaceKCodeUIState,
  updater: (options: KCodeConfigOption[] | null) => KCodeConfigOption[] | null,
): Pick<WorkspaceKCodeUIState, "taskConfigOptionsByTaskId"> | null {
...
function resolveActiveTaskConfigOptionsOnSwitch(
  current: WorkspaceKCodeUIState,
  taskId: string,
): { configOptions: KCodeConfigOption[]; status: ConfigOptionsStatus } | null {
  ...
  return {
    ...
    promoteGroupedDraftTask: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const optimisticTask: KCodeTaskMeta = {
              taskId,
              traceId: `session-${taskId}` as KCodeTaskMeta["traceId"],
              title: "",
              workspacePath,
    ...
    startDraft: (
      ...
      const normalizedProvider = provider
        ? normalizeAgentProviderToKCodeAgent(provider)
        : undefined;
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          workspacePath,
          (current) => {
            const nextSelectedProvider = normalizeAgentProviderToKCodeAgent(
    ...
    bindRuntimeProvider: (
      ...
      set((state) =>
        updateWorkspaceState(
          ...
          (current) => ({
            ...current,
            selectedProvider: normalizeAgentProviderToKCodeAgent(provider),
          }),
          workspaceIdentity,
        ),
      ),
