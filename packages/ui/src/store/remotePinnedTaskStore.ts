...
interface RemotePinnedTaskState {
  ...
  refreshWorkspace: (params: {
    ...
    kcodeTaskService: Pick<IKCodeTaskService, "listPinnedTasks">;
  }) => Promise<void>;
  upsertTask: (task: KCodeTaskMeta) => void;
...
function sortPinnedTasks(tasks: KCodeTaskMeta[]): KCodeTaskMeta[] {
...
export const useRemotePinnedTaskStore = create<RemotePinnedTaskState>()((set) => ({
  ...
  errorByWorkspaceKey: {},
  async refreshWorkspace({ workspacePath, workspaceIdentity, kcodeTaskService }) {
    ...
    try {
      const items = await kcodeTaskService.listPinnedTasks({
        workspacePath,
