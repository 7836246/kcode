/**
 * KCode Session Store 选择器与内部辅助函数
 *
 * 从 kcodeSessionStore.ts 拆分出来，包含 workspace 状态读取/更新辅助函数，
 * 以及所有按 task 粒度的只读访问器和独立选择器。
 */
import type { KCodeTaskRuntimeStatus, KCodeTaskMeta } from "@kcode/shared";
...
import {
  ...
  type KCodeSessionStoreState,
  type WorkspaceInitState,
  type TaskRuntimeState,
  type WorkspaceKCodeUIState,
} from "./kcodeSessionStoreTypes.js";
...
function collectIdentityTaskIds(
  baseState: WorkspaceKCodeUIState,
  workspaceIdentity: string,
): Set<string> {
...
function createIdentityWorkspaceStateSeed(
  baseState: WorkspaceKCodeUIState | undefined,
  workspaceIdentity?: string,
...
export function updateWorkspaceState(
  state: KCodeSessionStoreState,
  workspacePath: string,
  updater: (current: WorkspaceKCodeUIState) => WorkspaceKCodeUIState,
  workspaceIdentity?: string,
  ...
  if (nextWorkspaceState === current) {
    // 单 KCode Agent 迁移后旧 provider 选择都会归一为 glm，很多调用实际不会改变状态。
...
export function getTaskRuntimeState(
  workspaceState: WorkspaceKCodeUIState,
  taskId: string,
): TaskRuntimeState {
...
interface WorkspaceDisplayedTaskState {
  taskStatus: KCodeTaskRuntimeStatus;
...
export function getTaskUiState(workspaceState: WorkspaceKCodeUIState, taskId: string) {
...
export function getTaskMeta(
  workspaceState:
    | WorkspaceKCodeUIState
    | Partial<Pick<WorkspaceKCodeUIState, "optimisticTaskListByTaskId" | "taskListCache">>,
  taskId: string,
): KCodeTaskMeta | null {
...
export function getVisibleTaskMetas(
  ...
  const taskById = new Map<string, KCodeTaskMeta>();
...
export function getTaskUnreadIndicator(
  workspaceState:
    ...
    | Partial<
      ...
      >,
  taskId: string,
  fallbackTask?: Pick<KCodeTaskMeta, "unreadAt">,
): boolean {
...
export function selectWorkspaceKCodeState(
  state: KCodeSessionStoreState,
  workspacePath: string,
  workspaceIdentity?: string,
) {
...
export function getWorkspaceInitState(
  workspaceState: Pick<WorkspaceKCodeUIState, "workspaceInit">,
): WorkspaceInitState {
