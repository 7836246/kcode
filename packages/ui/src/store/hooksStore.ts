...
import type { Hook, HookConfig } from "@kcode/shared";
import type { IHooksService } from "@kcode/services";
...
function buildKCodeHookLocation(
...
function hookFromConfig(config: HookConfig, workspacePath: string): Hook {
  return {
    ...
    location: buildKCodeHookLocation(workspacePath, config.storageLevel),
  };
}
...
export const useHooksStore = create<HooksStoreState>((set, get) => ({
  ...
  importHook: async (id, hooksService) => {
    ...
    if (!source || source.location?.source === "kcode") throw new Error("Hook is not importable");
    const imported: Hook = {
      ...source,
      id: `hook-${crypto.randomUUID()}`,
      enabled: true,
      location: buildKCodeHookLocation(workspacePath, source.location?.scope ?? "user"),
    };
