import type { KCodeModelTrajectoryMessage } from "@kcode/services";

export function trajectoryToolOutputs(message: KCodeModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-result" ? [formatTrajectoryToolPayload(part.output)] : [],
  );
}

export function trajectoryToolCallInputs(message: KCodeModelTrajectoryMessage): string[] {
  return message.parts.flatMap((part) =>
    part.kind === "tool-call" ? [formatTrajectoryToolPayload(part.input)] : [],
  );
}

export function trajectoryToolHasError(message: KCodeModelTrajectoryMessage): boolean {
...
export function trajectoryToolMetadata(message: KCodeModelTrajectoryMessage): {
  names: string;
  ids: string;
} {
