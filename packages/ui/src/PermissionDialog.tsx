...
import { MAX_PERMISSION_FEEDBACK_CHARS } from "@kcode/shared/kcode-protocol-v4";
...
function readPermissionRuleScopes(option: KCodePermissionOption): PermissionRuleScope[] {
...
function isOfficialCuaProjectPermission(option: KCodePermissionOption): boolean {
 ...
 */
function isWorkflowRefineOption(option: KCodePermissionOption): boolean {
...
const PROVIDER_PERMISSION_OPTION_NAME_LABELS: Partial<
  Record<KCodeProvider, Record<string, string>>
> = {
  glm: {
    // GLM/KCode Agent 通过 KCode Agent 发来的项目级记忆授权文案是英文原文。
...
function getPermissionDisplayReason(request: KCodePermissionRequest): string | null {
...
function buildPermissionToolCall(
  request: KCodePermissionRequest,
  preview: ReturnType<typeof getPermissionRequestPreview>,
): TaskChatToolCall {
  const rawInput = readRawToolCallInput(request.raw);
...
export function PermissionDialog({
  ...
  onRespond: (requestId: string, option: KCodePermissionOption, feedback?: string) => void;
  workspacePath: string;
  ...
  // 当前 KCode Agent 的 ExitPlanMode 权限请求不再传 legacy switch_mode。
            ...
            >
              {orderedOptions.map((option, index) => {
                ...
                const knownNameLabel = nameMessageIds
                  ...
                  : null;
                // KCode Agent 协议里 option.name 才是给用户看的真实选项文案，kind 只表示按钮语义。
