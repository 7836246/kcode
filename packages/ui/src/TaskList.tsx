...
import { TID_TASK_LIST, TID_TASK_EMPTY, TID_TASK_SETTINGS_BUTTON } from "@kcode/shared";
...
export const TaskList = memo(function TaskList({
  ...
  hasMore?: boolean;
  onShowMore?: () => void;
  onRenameTask: (taskId: string, title: string) => Promise<KCodeTaskMeta | null>;
  onSetTaskPinned: (taskId: string, pinned: boolean) => Promise<KCodeTaskMeta | null>;
  onArchiveTask: (taskId: string) => Promise<KCodeTaskMeta | null>;
  onSetTaskUnread: (taskId: string, unread: boolean) => Promise<KCodeTaskMeta | null>;
  ...
  const startDraft = useKCodeSessionStore((state) => state.startDraft);
  ...
  const handleSubmitRenameTask = useCallback(
    async (taskId: string) => {
      ...
      let renamedTask: KCodeTaskMeta | null;
      try {
  ...
  const visibleSourceTasks = useMemo(() => {
    const orderedTasks =
      sortBy === "manual"
        ? tasks
        : [...tasks].sort((left, right) => compareKCodeTaskListItems(left, right, sortBy));
    return orderedTasks;
  }, [sortBy, tasks]);
  useEffect(() => {
