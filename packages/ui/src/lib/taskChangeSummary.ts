import type {
  ...
  KCodeTaskChangedFileSummary,
  KCodeTaskMeta,
} from "@kcode/shared";
import { computeLineChangeStat } from "@kcode/shared";
...
export function getTaskChangeSummary(
  task: Pick<KCodeTaskMeta, "changeSummary"> | null | undefined,
): KCodeTaskChangeSummary | null {
...
function formatTaskChangeStats(
  summary: KCodeTaskChangeSummary,
...
export function buildTaskChangeSummary(
  fileChanges: readonly KCodePersistedFileChange[] | undefined,
): KCodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: KCodeTaskChangedFileSummary[] = Array.from(changedFileMap.values())
    .map((file) => {
...
export function buildTurnChangeSummary(
  turn: KCodePersistedFileChange | null | undefined,
): KCodeTaskChangeSummary | null {
  ...
  let added = 0;
  let removed = 0;
  const files: KCodeTaskChangedFileSummary[] = Array.from(filesByPath.entries())
    .map(([path, snapshot]) => {
...
export function buildPerTurnChangeSummaries(
  fileChanges: readonly KCodePersistedFileChange[] | undefined,
): Map<number, KCodeTaskChangeSummary> {
  const result = new Map<number, KCodeTaskChangeSummary>();
  if (!fileChanges || fileChanges.length === 0) {
    return result;
  }
