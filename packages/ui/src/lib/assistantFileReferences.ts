...
import { extractConversationPreviewFileReferences } from "@kcode/shared";
import { decodeFilePathUriEscapes, joinFilePath } from "@/lib/path.js";
import { MEDIA_PREVIEW_FORMATS } from "@kcode/shared";
import {
  extractKCodeFileCitationDirectives,
  resolveKCodeFileCitationPreviewKind,
} from "@/lib/kcodeFileCitation.js";
import {
...
export function extractAssistantFileReferences(
  ...
  for (const citation of extractKCodeFileCitationDirectives(content)) {
  ...
  for (const match of content.matchAll(FILE_PATH_RE)) {
    ...
    // 明确的 kcode-file-citation 仍在上面的专用分支处理。
    if (/^~[\\/]/.test(raw)) {
      protectedRanges.push([start, end]);
      continue;
    }
