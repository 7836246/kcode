...
// sessions-index 只补实时 activity/detail，最终拼出旧 KCodeGroupedTaskView 同形结果。
...
function taskKeyOf(task: KCodeTaskMeta): string {
...
function taskOrderMapKey(task: KCodeTaskMeta): string {
...
function nodeMapKey(node: KCodeGroupedTaskViewNode): string {
...
function sortGroupTasks(
  ...
  membersByTaskKey: Map<string, KCodeGroupedTaskViewStructureMember>,
): KCodeTaskListItem[] {
  const resolvedOrderByTaskKey = new Map<string, number>();
  let maxOrder = 0;
  const missing: Array<{ task: KCodeTaskListItem; addedAt: number; key: string }> = [];
...
export function buildGroupedTaskViewFromSessions(
  ...
  const activeTaskByKey = new Map<string, KCodeTaskListItem>();
  ...
  const membersByGroupId = new Map<string, KCodeGroupedTaskViewStructureMember[]>();
  ...
  const nodes: KCodeGroupedTaskViewNode[] = structure.groups.map((group) => {
