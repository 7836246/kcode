...
import { type KCodeProviderAccountAccess, kcodeProviderAccountAccessSchema } from "@kcode/shared";
...
export function resolveEntitledAccountProviderAccess(
  ...
  const parsed = kcodeProviderAccountAccessSchema.safeParse(provider.effectiveConfig.access);
