...
import { extractKCodeFileCitations } from "@/lib/kcodeFileCitation.js";
...
function projectCitationTextNode(
  ...
  const citations = extractKCodeFileCitations(value);
...
export function createKCodeFileCitationRemarkPlugin(
  workspacePath: string,
  homePath?: string,
): Plugin {
  return function kcodeFileCitationRemarkPlugin() {
    return (tree: unknown) => {
