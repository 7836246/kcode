import type { KCodeTaskRuntimeStatus } from "@kcode/shared";
import { getWorkspaceDisplayedTaskState } from "@/store/kcodeSessionStore.js";
import type { KCodeSessionStoreState, WorkspaceKCodeUIState } from "@/store/kcodeSessionStore.js";
...
interface MarkRemoteWorkspaceRunningTasksFailedParams {
  ...
  getWorkspaceState: KCodeSessionStoreState["getWorkspaceState"];
  setTaskRuntimeState: KCodeSessionStoreState["setTaskRuntimeState"];
  reason: string;
}

function isRunningRuntimeStatus(status: KCodeTaskRuntimeStatus): boolean {
...
function collectRemoteWorkspaceRunningTaskIds(workspaceState: WorkspaceKCodeUIState): string[] {
