...
> &
  Partial<Pick<IKCodeTaskService, "listDeletedTaskIds">>;
...
interface TaskListMembershipSets {
  ...
  terminalStatusByTaskId: Map<string, Extract<KCodeTaskMeta["status"], "completed" | "error">>;
  /**
...
function collectUnreadAt(unreadAtByTaskId: Map<string, number>, tasks: KCodeTaskMeta[]): void {
...
function mergeTaskIndexItems(lists: KCodeTaskMeta[][]): KCodeTaskMeta[] {
  const itemByEntityKey = new Map<string, KCodeTaskMeta>();
...
async function fetchTaskListMembershipSetsForEndpoints(
  ...
  const taskIndexItemByEntityKey = new Map<string, KCodeTaskMeta>();
