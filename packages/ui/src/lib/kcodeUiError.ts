import type { KCodeError, TraceId } from "@kcode/shared";
import { errorAttributionSchema, type ErrorAttribution } from "@kcode/shared/kcode-protocol-v4";

export interface KCodeUiError extends KCodeError {
...
interface NormalizeKCodeUiErrorOptions {
  fallbackCode?: string;
...
const GENERIC_KCODE_UI_ERROR_MESSAGES = new Set([
  "Internal error",
  "Turn execution failed",
  "Compact failed",
  "Rewind failed",
  "KCode session failed",
]);
...
function collectMessageCandidatesFromRecord(record: Record<string, unknown>): string[] {
  ...
  const messagePaths: Array<readonly string[]> = [
    ...
    ["data", "message"],
    ["data", "detail"],
    // KCode Agent 常把可读原因放在 data.details（复数）里；之前只识别 detail，
    ...
    ["data", "error", "detail"],
    ["data", "error", "details"],
    // kcode-cli 会把模型/网络错误摘要放在 data.kcode.error 下。
    // 之前 UI 只读 data.error，导致已经结构化好的 provider 根因仍被 “Internal error” 盖住。
    ["data", "kcode", "error", "message"],
    ["data", "kcode", "error", "detail"],
    ["data", "kcode", "error", "details"],
  ];
...
export function normalizeKCodeUiError(
  error: unknown,
  options: NormalizeKCodeUiErrorOptions = {},
): KCodeUiError {
  const candidates = collectMessageCandidates(error);
  // kcode-cli 已经把 provider/network 根因放进 detail 或 data.kcode.error，
  // 外层仍可能保留 "Internal error" 这类包装文案。主提示优先选非泛化候选，避免根因被盖住。
  const primaryMessage =
    candidates.find((candidate) => !GENERIC_KCODE_UI_ERROR_MESSAGES.has(candidate)) ??
    candidates[0] ??
    options.fallbackMessage ??
    "Internal error";
  const detailMessage = candidates.find(
    (candidate) => candidate !== primaryMessage && !GENERIC_KCODE_UI_ERROR_MESSAGES.has(candidate),
  );
  ...
  const underlyingErrorMessage = readFirstStringFromPaths(error, [
    ...
    ["data", "kcode", "error", "underlyingErrorMessage"],
  ]);
  const underlyingErrorDetail = readFirstStringFromPaths(error, [
    ...
    ["data", "kcode", "error", "underlyingErrorDetail"],
  ]);
