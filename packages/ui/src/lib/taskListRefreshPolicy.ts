import type { KCodeWorkspaceTaskListChanged } from "@kcode/shared";
...
const TASK_LIST_MEMBERSHIP_REASONS = new Set<KCodeWorkspaceTaskListChanged["reason"]>([
  "task_archived",
  "task_unarchived",
  "task_pinned",
  "task_unpinned",
...
function isTaskListMembershipWorkspaceEvent<
  T extends Pick<KCodeWorkspaceTaskListChanged, "reason">,
>(
  event: T,
): event is T & {
...
export function shouldRefetchTaskListMembershipForWorkspaceEvent(
  event: Pick<KCodeWorkspaceTaskListChanged, "reason">,
): boolean {
