// re-home 产物（为删除 kcodeChatMessages 铺路，模式同 kcode-task-types-core）。
// 本文件承载旧消息转换层（kcodeChatMessages/kcodeChatMessageHelpers）中仍被存活栈
...
// 旧消息拼装运行时（kcodeChatMessages.ts）删除后，这里是该类型面的唯一事实源。
import type {
  KCodeAssistantMessagePart,
  KCodeAssistantMessageFeedback,
  KCodePromptAttachment,
  KCodeTimelineMeta,
  KCodeTaskSnapshotBodyRef,
  KCodeTaskSnapshotToolFieldRef,
  KCodeTaskSnapshotToolSlice,
} from "@kcode/shared";
...
export interface TaskChatToolCall {
  ...
  /** KCode 固定工具名；kind 仍保留给旧 KCode Agent 快照和 UI 聚合分类兼容。 */
  toolName?: string;
  kind: string;
  title?: string;
  input: unknown;
...
export type TaskChatMessagePart = KCodeAssistantMessagePart;
