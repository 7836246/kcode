...
export function mergeWorkspaceTaskListItemsWithOptimistic(params: {
  items: readonly KCodeTaskMeta[];
  optimisticTasks: readonly KCodeTaskMeta[];
  activeTaskId: string | null;
  ...
  const sortedItems = [...taskByKey.values()].sort((left, right) =>
    compareKCodeTaskListItems(left, right, params.sortBy),
  );
...
export function useWorkspaceTaskOptimisticOverlayByWorkspaceKey(
  ...
  const optimisticTaskListSignature = useKCodeSessionStore((state) =>
    JSON.stringify(
  ...
  return useMemo(() => {
    const state = useKCodeSessionStore.getState();
