...
import type { BackgroundBashOutput, BackgroundBashOutputResult } from "@kcode/shared";
...
export function useBackgroundBashOutput(target: BackgroundBashOutputTarget, visible: boolean) {
  ...
  const service = useKCodeAgentService(workspacePath, remoteSessionId, workspaceIdentity);
