...
import { mergeTaskWithOptimisticMeta } from "@/lib/kcodeTaskMetaMerge.js";
import {
...
function collectViewWorkspaceScopes(
  ...
  const addTask = (task: KCodeTaskMeta) => {
...
function mergeGroupedTaskViewWithOptimistic(params: {
  ...
  const optimisticTaskByKey = new Map<string, KCodeTaskMeta>();
  const placementTaskByKey = new Map<string, KCodeTaskMeta>();
  ...
  const groupMissingTasksByGroupId = new Map<string, KCodeTaskMeta[]>();
  const topMissingTasks: KCodeTaskMeta[] = [];
...
function collectGroupedViewTaskKeys(view: KCodeGroupedTaskView): Set<string> {
...
function groupedNodeIdentityKey(node: KCodeGroupedTaskView["nodes"][number]): string {
  if (node.type === "group") {
...
function areGroupedNodesEquivalent(
  previous: KCodeGroupedTaskView["nodes"][number],
  next: KCodeGroupedTaskView["nodes"][number],
): boolean {
...
function stabilizeGroupedView(
  previous: KCodeGroupedTaskView,
  next: KCodeGroupedTaskView,
): KCodeGroupedTaskView {
...
function nodeToTopLevelRef(
  node: KCodeGroupedTaskView["nodes"][number],
): KCodeGroupedTaskViewTopLevelNodeRef {
  if (node.type === "group") {
...
function viewToOrderInput(params: { view: KCodeGroupedTaskView }): KCodeGroupedTaskViewOrderInput {
  return {
...
const groupedViewCacheBySignature = new Map<string, KCodeGroupedTaskView>();

function readCachedGroupedView(signature: string): KCodeGroupedTaskView | undefined {
  return groupedViewCacheBySignature.get(signature);
}

function writeCachedGroupedView(signature: string, view: KCodeGroupedTaskView): void {
...
export function useGroupedTaskView(params: { workspaceTabs: WorkspaceTabState[] }) {
  ...
  const [view, setView] = useState<KCodeGroupedTaskView>(
  ...
  const clearPromotedGroupedDraftTask = useKCodeSessionStore(
  ...
  const displayedViewRef = useRef<KCodeGroupedTaskView>({ nodes: [] });
  ...
  const [remoteDataLoader] = useState(
    () =>
      new GroupedRemoteDataSingleFlight<{
        structure: KCodeGroupedTaskViewStructure;
    ...
    try {
      ...
      const remoteData = await remoteDataLoader.load(remoteDataKey, async () => {
        const [structureResult, membershipResult] = await Promise.all([
          services.kcodeTaskService.listGroupedTaskViewStructure({
            workspaceScopes: scopes,
          }),
          fetchTaskListMembershipSets({
            service: services.kcodeTaskService,
  ...
  useEffect(() => {
    ...
    const promotedGroupTasks: KCodeTaskMeta[] = [];
    const settledRootTasks: KCodeTaskMeta[] = [];
    ...
    void services.kcodeTaskService
  ...
  useEffect(() => {
    const disposables = scopes.map((scope) =>
      services.kcodeTaskService.onDynamicWorkspaceEvent(scope)((event) => {
  ...
  }, [invalidateRemoteData, scopes, services.kcodeTaskService]);
  ...
  const createGroup = useCallback(async (): Promise<KCodeTaskGroup> => {
    setSaving(true);
    try {
      const group = await services.kcodeTaskService.createTaskGroup();
  ...
  }, [invalidateRemoteData, refresh, services.kcodeTaskService]);
  ...
  const renameGroup = useCallback(
    async (groupId: string, title: string) => {
      ...
      const optimisticView: KCodeGroupedTaskView = {
      ...
      setSaving(true);
      try {
        const renamedGroup = await services.kcodeTaskService.renameTaskGroup({
          groupId,
          title: nextTitle,
    ...
    [invalidateRemoteData, scopes, services.kcodeTaskService, view],
  );

  const updateGroupColor = useCallback(
    async (groupId: string, color: KCodeTaskGroupColor) => {
      const previousView = view;
      const groupNode = view.nodes.find(
        (node) => node.type === "group" && node.group.id === groupId,
      );
      ...
      setSaving(true);
      try {
        const updatedGroup = await services.kcodeTaskService.updateTaskGroupColor({
          groupId,
          color,
          workspaceScopes: collectViewWorkspaceScopes(optimisticView),
        });
  ...
  const applyOrder = useCallback(
    async (nextView: KCodeGroupedTaskView) => {
      const previousView = view;
      setView(nextView);
      setSaving(true);
      try {
        ...
        await services.kcodeTaskService.applyGroupedTaskViewOrder(
          viewToOrderInput({
            view: nextView,
          }),
        );
    ...
    [invalidateRemoteData, refresh, services.kcodeTaskService, view],
  );
  ...
  const ungroupGroup = useCallback(
    async (groupId: string) => {
      ...
      const nextView: KCodeGroupedTaskView = {
      ...
      try {
        ...
        await services.kcodeTaskService.deleteTaskGroup({
          groupId,
    ...
    [applyOrder, invalidateRemoteData, refresh, scopes, services.kcodeTaskService, view],
  );
