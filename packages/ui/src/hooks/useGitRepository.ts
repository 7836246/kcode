...
function createLastTurnChange(
  workspacePath: string,
  snapshotByPath: Map<string, KCodePersistedFileSnapshot>,
  file: KCodeTaskChangeSummary["files"][number],
): GitPaneFileChange {
...
function buildLastTurnDataset(options: {
  workspacePath: string;
  turnIndex: number | null;
  fileChange: KCodePersistedFileChange | null;
  summary: KCodeTaskChangeSummary | null;
}): GitPaneDataset {
  const snapshotByPath = new Map<string, KCodePersistedFileSnapshot>(
  ...
  return {
    ...
    // 关键业务逻辑：上一轮更改继续优先复用 KCode Agent 已持久化的单轮文件快照，
