...
interface PluginReferenceCatalogState {
  entries: KCodePluginReferenceCatalogEntry[];
...
let sessionCatalogRequests = new WeakMap<
  object,
  Map<string, Promise<KCodePluginsReferenceCatalogResult>>
>();

function releaseSessionCatalogRequest(
  services: object,
  serviceCache: Map<string, Promise<KCodePluginsReferenceCatalogResult>>,
  requestKey: string,
  request: Promise<KCodePluginsReferenceCatalogResult>,
): void {
...
export function usePluginReferenceCatalog(
  ...
  useEffect(() => {
    ...
    const subscription = services.kcodeAgentService.onAgentRuntimeRestarted((event) => {
  ...
  useEffect(() => {
    ...
    let request: Promise<KCodePluginsReferenceCatalogResult>;
    let requestServiceCache: Map<string, Promise<KCodePluginsReferenceCatalogResult>> | undefined;
