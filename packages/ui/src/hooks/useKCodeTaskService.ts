...
import type { IKCodeTaskService } from "@kcode/services";
import type { KCodeTaskSnapshot } from "@kcode/shared";
import { uiMemoryDiagnosticsRegistry } from "@/lib/memoryDiagnostics.js";

type GetTaskSnapshotParams = Parameters<IKCodeTaskService["getTaskSnapshot"]>[0];
type GetTaskSnapshotResult = Promise<KCodeTaskSnapshot | null>;
type GetTaskSnapshotWithEtagParams = Parameters<IKCodeTaskService["getTaskSnapshotWithEtag"]>[0];

const kcodeTaskServiceProxyCache = new WeakMap<IKCodeTaskService, IKCodeTaskService>();
...
const snapshotCacheByService = new WeakMap<
  IKCodeTaskService,
  Map<string, { etag: string; snapshot: KCodeTaskSnapshot }>
>();
const SNAPSHOT_CACHE_STORAGE_KEY = "kcode-task-snapshot-cache:v1";
...
let latestSnapshotCache: Map<string, { etag: string; snapshot: KCodeTaskSnapshot }> | undefined;
...
function getOrCreateSnapshotCacheMap(
  ...
  const created = new Map<string, { etag: string; snapshot: KCodeTaskSnapshot }>();
...
function writePersistedSnapshotEntry(key: string, etag: string, snapshot: KCodeTaskSnapshot): void {
  ensurePersistedSnapshotCacheLoaded();
...
function createKCodeTaskServiceProxy(service: IKCodeTaskService): IKCodeTaskService {
...
/** 获取 KCode task wrapper 服务实例 */
export function useKCodeTaskService(
  workspacePath?: string,
  preferredRemoteSessionId?: string | null,
  workspaceIdentity?: string | null,
): IKCodeTaskService {
  // KCode task 服务按 workspace 身份解析，保证所有 task RPC 都落到对应的 host。
  const services = workspacePath
    ? useWorkspaceServices(workspacePath, preferredRemoteSessionId, workspaceIdentity)
    : useServices();
  const rawService = services.kcodeTaskService;
  if (!rawService || typeof rawService !== "object") {
    return rawService;
  }
  const cachedProxy = kcodeTaskServiceProxyCache.get(rawService);
  if (cachedProxy) {
    return cachedProxy;
  }
  const nextProxy = createKCodeTaskServiceProxy(rawService);
  kcodeTaskServiceProxyCache.set(rawService, nextProxy);
  return nextProxy;
}
