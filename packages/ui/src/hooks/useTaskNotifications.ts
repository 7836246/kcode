...
import type { IPlatformService, TaskNotificationPayload } from "@kcode/shared";
import type { ConversationSnapshot, SessionSummary } from "@kcode/shared/kcode-protocol-v4";
...
export function useWorkspaceTerminalTaskNotifications({
  ...
  useEffect(() => {
    ...
    const store = acquireSessionsIndex(registryScope, kcodeAgentService);
    const syncState = () => {
      setIndexState({
        signature,
