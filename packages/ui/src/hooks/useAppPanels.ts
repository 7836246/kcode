...
import { createUuid } from "@kcode/shared";
import type { EmbeddedBrowserOpenUrlRequest, IPlatformService } from "@kcode/shared";
...
export function useAppPanels(options: {
  ...
  const { kcodeAgentService, kcodeSessionService } = useServices();
    ...
    (request: OpenScopedWorkflowArtifactSideTabRequest) => {
      ...
      void (async () => {
        try {
          ...
          // 这里只带 workspacePath。
          const result = await kcodeAgentService.conversationWorkflowRunArtifactsV4({
            workspacePath: request.workspacePath,
