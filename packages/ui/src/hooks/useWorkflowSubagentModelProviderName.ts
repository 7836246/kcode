 ...
 * （见 kcodeSessionSettingsToConfigOptions），那种「名字」在这里就当作没有——屏幕上绝不出现
 * providerId（团队套餐的它是一个 UUID）。
 */
function workflowSubagentProviderNameLookup(
  configOptions: readonly KCodeConfigOption[] | null | undefined,
...
export function useWorkflowSubagentModelProviderName(
  ...
  const configOptions = useKCodeSessionStore((state) =>
    workspacePath === undefined
      ? undefined
      : selectWorkspaceKCodeState(state, workspacePath, workspaceIdentity).configOptions,
  );
