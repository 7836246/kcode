...
function areResolvedTaskMetasEqual(left: KCodeTaskMeta | null, right: KCodeTaskMeta | null) {
  if (left === right) {
    return true;
  }
...
function useStableResolvedActiveTaskMeta(taskMeta: KCodeTaskMeta | null) {
  const stableTaskMetaRef = useRef<KCodeTaskMeta | null>(null);
...
export function useWorkspaceActiveTaskState({
  ...
  const workspaceState = useKCodeSessionStore((state) =>
    selectWorkspaceKCodeState(state, workspaceAbsPath, workspaceIdentity),
  );
  ...
  const activeTaskMeta = useMemo(() => {
    ...
    // App 以前依赖 kcodeTaskMetaMerge 同时拉普通列表和 pinned 列表，只是为了给当前激活 task
