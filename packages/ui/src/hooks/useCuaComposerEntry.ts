...
import { isRemoteWorkspaceIdentity, KCODE_CUA_OFFICIAL_PLUGIN_ID } from "@kcode/shared";
...
import { getVisibleTaskMetas, getWorkspaceState } from "@/store/kcodeSessionStoreSelectors.js";
import { useOptionalTabStore } from "@/store/TabStoreProvider.js";

/** 与 kcodeSessionStoreTaskSlice 的 isRunningStatus 同口径。 */
...
export function useCuaComposerEntry({
  ...
  // 失败都写）。只有失败操作的目标是 kcode-cua 时才映射为本按钮的错误态，
  ...
  const workspaceSessionBusy = useKCodeSessionStore((state) => {
  ...
  const view = useMemo(
    () =>
      resolveCuaComposerEntryView({
        ...
        pluginEnabled,
        pluginToggling: togglingPluginId === KCODE_CUA_OFFICIAL_PLUGIN_ID,
        pluginError:
          Boolean(pluginStoreError) && lastFailedPluginId === KCODE_CUA_OFFICIAL_PLUGIN_ID,
