...
import type { KCodeSessionEndedSubagent, KCodeSessionSubagentsResult } from "@kcode/shared";
...
interface SessionSubagentsViewState {
  revision: number;
  ended: KCodeSessionSubagentsResult["ended"];
  error: string | null;
  loading: boolean;
}
...
function mergeEndedSubagentPages(
  current: readonly KCodeSessionEndedSubagent[],
  incoming: readonly KCodeSessionEndedSubagent[],
): KCodeSessionEndedSubagent[] {
...
export function useSessionSubagents(options: {
  ...
  const kcodeAgentService = services.kcodeAgentService;
  ...
  const requestPage = useCallback(
    (endedLimit: number, endedCursor?: string): Promise<KCodeSessionSubagentsResult> => {
      if (!options.sessionId) return Promise.reject(new Error("session_id_missing"));
      if (typeof kcodeAgentService?.listSessionSubagents !== "function") {
      ...
      return kcodeAgentService.listSessionSubagents({
