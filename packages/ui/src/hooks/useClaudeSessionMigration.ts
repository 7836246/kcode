...
import type { KCodeImportSessionsResult, KCodeImportableSessionCandidate } from "@kcode/shared";
import { logger } from "@/logger.js";
import { useKCodeTaskService } from "@/hooks/useKCodeTaskService.js";
...
export function useClaudeSessionMigration(params: {
  ...
  const kcodeTaskService = useKCodeTaskService(
  ...
  const [candidates, setCandidates] = useState<KCodeImportableSessionCandidate[]>([]);
  const [selectedSessionIds, setSelectedSessionIds] = useState<string[]>([]);
  const [scanError, setScanError] = useState<string | null>(null);
  const [importError, setImportError] = useState<string | null>(null);
  const [lastImportResult, setLastImportResult] = useState<KCodeImportSessionsResult | null>(null);
  ...
  const scan = useCallback(async () => {
    ...
    try {
      ...
      const nextCandidates = await kcodeTaskService.scanImportableClaudeSessions({
  ...
  const importSessions = useCallback(
    async (sessionIds: string[]) => {
      ...
      try {
        ...
        const result = await kcodeTaskService.importClaudeSessions({
          workspacePath: effectiveWorkspacePath,
          ...(effectiveWorkspaceIdentity ? { workspaceIdentity: effectiveWorkspaceIdentity } : {}),
          sessionIds,
        });
