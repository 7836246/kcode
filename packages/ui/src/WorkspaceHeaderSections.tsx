...
export function WorkspaceHeaderTitleSection({
  ...
  const buildHeaderTaskSnapshot = (
    fallbackTask: KCodeTaskMeta,
    overrides?: Partial<KCodeTaskMeta>,
  ): KCodeTaskMeta => {
    if (activeTaskMeta) {
      return {
        ...activeTaskMeta,
  ...
  const handleSubmitRenameTask = async () => {
    ...
    try {
      const renamedTask = await services.kcodeTaskService.renameTask({
  ...
  const handleArchiveTask = async () => {
    ...
    void services.kcodeTaskService
      ...
      .then((meta) => {
        // Header 更多菜单不能只依赖 kcodeTaskMetaMerge：归档后只有旧列表状态被更新。
