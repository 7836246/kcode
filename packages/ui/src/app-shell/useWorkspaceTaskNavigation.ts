...
export function useWorkspaceTaskNavigation({
  ...
  const setActiveTaskId = useKCodeSessionStore((s) => s.setActiveTaskId);
  const taskNavHistory = useKCodeSessionStore((s) => s.taskNavHistory);
  const taskNavPushAutomations = useKCodeSessionStore((s) => s.taskNavPushAutomations);
  const taskNavPushPluginStore = useKCodeSessionStore((s) => s.taskNavPushPluginStore);
  const taskNavGoBack = useKCodeSessionStore((s) => s.taskNavGoBack);
  const taskNavGoForward = useKCodeSessionStore((s) => s.taskNavGoForward);
  const removeTaskFromNavHistory = useKCodeSessionStore((s) => s.removeTaskFromNavHistory);
  ...
  const handleSelectTask = useCallback(
    (
      ...
      const targetWorkspaceState = useKCodeSessionStore
        .getState()
      ...
      if (shouldClearUnread) {
        ...
        if (!targetServices) {
          ...
          void targetServices.kcodeTaskService
            .setTaskUnread({
              ...targetTask,
              unread: false,
              expectedUnreadAt,
  ...
  const handleTaskNavBack = useCallback(() => {
    const currentWorkspaceState = useKCodeSessionStore
      .getState()
    ...
    while (entry) {
      ...
      const navWorkspaceState = useKCodeSessionStore
        .getState()
  ...
  const currentWorkspaceState = useKCodeSessionStore.getState().getWorkspaceState(workspaceAbsPath);
