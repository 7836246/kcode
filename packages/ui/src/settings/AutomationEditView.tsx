...
interface AutomationEditViewProps {
  ...
  onBack: () => void;
  /** 编辑态:立即运行 / 启停 / 删除。 */
  onRunNow?: (automation: KCodeAutomation) => Promise<void> | void;
...
function resolveRunStatus(run: KCodeAutomationRun): RunStatusKind {
...
function normalizeAutomationScheduleRule(
  rule: KCodeAutomationScheduleRule | null | undefined,
...
export function AutomationEditView({
  ...
  const modelSelectGroups = useMemo(() => {
    ...
    return buildAutomationModelSelectGroups({
      selectedProvider: KCODE_AGENT_PROVIDER,
      labels: {
  ...
  const currentScheduleRule = useMemo<KCodeAutomationScheduleRule | undefined>(
    () =>
