 ...
 * Supports the unified KCode Agent MCP source backed by settings directories.
 */
import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import {
  convertToKCodeAgentMcpServer,
...
import { isKCodeAgentMcpStatusModeUnsupportedError, type IMcpSyncService } from "@kcode/services";
...
const DEFAULT_MCP_SOURCE: ServerScope = "kcodeagentmcp";
...
async function refreshMcpServerStatusList({
  ...
  // mcp/list 收敛到 IMcpSyncService——UI 不直接触达 kcodeAgentService。
  ...
  try {
    ...
    if (refreshMode === "status" && isKCodeAgentMcpStatusModeUnsupportedError(error)) {
...
function buildMcpOAuthAuthorizationStatusRefreshServers(
  ...
  for (const server of servers) {
    if (server.source !== "kcodeagentmcp" || !server.enabled) {
      continue;
    }
    ...
    result.push(kcodeAgentServer);
  }

  return result;
}

function buildMcpOAuthAuthorizationStatusRefreshOptions(mcpServers: KCodeAgentMcpServer[]): {
  mode: "status";
  mcpServers: KCodeAgentMcpServer[];
} {
  return {
...
function buildMcpServerStatusListKey(servers: KCodeMcpServer[]): string {
  return servers
    .filter((server) => server.source === "kcodeagentmcp")
    .map((server) => {
...
function buildPluginMcpServerStatusListKey(plugins: KCodePluginInfo[]): string {
  return plugins
    .map((plugin) => {
...
function PluginMcpServerList({
  ...
  pluginListingById: ReadonlyMap<string, KCodeAvailablePluginSummary["listing"]>;
...
export function McpSettingsSection({
  ...
  const [editingServer, setEditingServer] = useState<KCodeMcpServer | null>(null);
  ...
  const requestMcpServerStatusList = useCallback(
    async (options?: {
      ...
      const requestedMcpServers =
        options?.mcpServers ?? useMcpStore.getState().getEnabledMcpServersForKCode("kcode");
  ...
  async function handleSave(form: FormState, prev?: KCodeMcpServer) {
    if (!activeWorkspacePath) {
      return;
    }
    const loaded = await ensureLoadedForWorkspace(
  ...
  async function handleDelete(server: KCodeMcpServer) {
  ...
  function handleEdit(server: KCodeMcpServer) {
  ...
  function handleOpenAuthorization(server: KCodeMcpServer) {
