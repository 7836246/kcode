import type {
  ...
  KCodePluginInfo,
  KCodePluginComponentKind,
  KCodePluginsDescribeResult,
} from "@kcode/shared";
import { isPluginCommand, isUserCommand, KCODE_COMMAND_AGENT_SOURCE } from "@kcode/shared";
...
const COMPONENT_KIND_ORDER: KCodePluginComponentKind[] = [
  "agent",
  "command",
  "skill",
  "hook",
...
export function describeResultToDisplayGroups(
  result: KCodePluginsDescribeResult,
): PluginComponentDisplayGroup[] {
  const byKind = new Map<KCodePluginComponentKind, PluginComponentDisplayGroup>();
...
export function groupCommandsByPlugin(
  ...
  for (const command of commands) {
    if (
      ...
      command.agentSource === KCODE_COMMAND_AGENT_SOURCE &&
...
export function filterLocalMcpServers(servers: KCodeMcpServer[], query: string): KCodeMcpServer[] {
...
function toPluginMcpServerDisplayName(plugin: KCodePluginInfo, serverName: string): string {
...
function resolvePluginMcpRuntimeServerName(plugin: KCodePluginInfo, displayName: string): string {
...
function mapPluginRuntimeStatus(
  snapshot: KCodeMcpServerStatusSnapshot | undefined,
): Pick<
  PluginMcpServerItem,
