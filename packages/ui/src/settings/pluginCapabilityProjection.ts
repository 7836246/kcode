...
import { compareDocumentPluginPriority, isPluginCommand, isUserCommand } from "@kcode/shared";
...
export function selectBuiltInPlugins(
  ...
  installedPlugins: readonly KCodeInstalledPluginSummary[],
): KCodePluginInfo[] {
...
interface PluginSettingsGroups {
  installed: KCodePluginInfo[];
  builtIn: KCodePluginInfo[];
}
...
export function selectPluginsForScope(
  plugins: readonly KCodePluginInfo[],
  _installedPlugins: readonly KCodeInstalledPluginSummary[],
  _scope: KCodePluginScope,
): KCodePluginInfo[] {
...
export function selectSkillsForScope(
  ...
  scopedPlugins: readonly Pick<KCodePluginInfo, "id" | "name" | "enabled">[],
...
export function selectCommandsForScope(
  commands: readonly KCodeCommand[],
  scopedPlugins: readonly Pick<KCodePluginInfo, "id" | "name" | "enabled">[],
  scope: KCodePluginScope,
): KCodeCommand[] {
  const enabledPlugins = scopedPlugins.filter((plugin) => plugin.enabled);
