...
// 也便于单测这些无 React 的判定。
import { resolveWorkspaceKey, type KCodeSavedWorkflowRun } from "@kcode/shared";
import type { KCodeAgentSavedWorkflowTarget } from "@kcode/services";
...
export const GLOBAL_SAVED_WORKFLOW_TARGET: KCodeAgentSavedWorkflowTarget = { scope: "global" };
...
export function buildGlobalRunProjectResolver(
  localProjects: readonly AutomationWorkspaceOption[],
): (run: KCodeSavedWorkflowRun) => SavedWorkflowRunProject | null {
  return (run) => {
