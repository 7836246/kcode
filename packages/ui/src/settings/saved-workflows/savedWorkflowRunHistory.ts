...
import type { KCodeSavedWorkflowRun, KCodeSavedWorkflowRunStatus } from "@kcode/shared";
...
): Map<string, KCodeSavedWorkflowRun> {
  const byName = new Map<string, KCodeSavedWorkflowRun>();
...
export function savedWorkflowRunBadgeKind(
  status: KCodeSavedWorkflowRunStatus | "errored" | "stopped" | "failed" | "cancelled" | undefined,
...
export function savedWorkflowRunDurationMs(run: KCodeSavedWorkflowRun, now: number): number {
