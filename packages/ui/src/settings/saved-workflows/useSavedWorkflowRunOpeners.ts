...
import type { KCodeSavedWorkflowRun } from "@kcode/shared";
import type {
...
export function useSavedWorkflowRunOpeners(options: {
  ...
  resolveTarget: (run: KCodeSavedWorkflowRun) => SavedWorkflowRunOpenTarget | null;
  ...
  handleOpenRun: (run: KCodeSavedWorkflowRun, workflowName: string) => void;
  handleOpenArtifact: (run: KCodeSavedWorkflowRun, artifactId: string) => void;
} {
  const { onOpenWorkflowArtifact, onOpenWorkflowRun, resolveTarget } = options;

  const handleOpenRun = useCallback(
    (run: KCodeSavedWorkflowRun, workflowName: string) => {
  ...
  const handleOpenArtifact = useCallback(
    (run: KCodeSavedWorkflowRun, artifactId: string) => {
      if (!run.parentSessionId) return;
      const target = resolveTarget(run);
      if (!target) return;
      // 行上的 chip 载荷（`KCodeSavedWorkflowRun.artifacts`）带最新版的 `contentType`：
