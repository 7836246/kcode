...
interface OffPeakEditViewProps {
  ...
  onDeleteHistory?: (task: KCodeOffPeakTask) => void;
...
export function OffPeakEditView({
  ...
  const thoughtLevelOption = useMemo<KCodeConfigOption | null>(
    () =>
    ...
    >
      ...
      {tab === "history" ? (
        <OffPeakHistoryTab
          ...
          {...(onOpenSession
            ? {
                onOpenSession: (task: KCodeOffPeakTask) =>
                  task.sessionId
                    ? onOpenSession({
