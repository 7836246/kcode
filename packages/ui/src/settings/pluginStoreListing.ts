...
export function resolveUniquePluginListingByName(
  plugins: readonly Pick<KCodeAvailablePluginSummary, "name" | "listing">[],
  name: string,
): KCodePluginStoreListing | undefined {
  const normalizedName = name.trim().toLocaleLowerCase();
  let match: KCodePluginStoreListing | undefined;
  let count = 0;
...
export interface StorePluginItem {
  ...
  orphaned: boolean;
  listing?: KCodePluginStoreListing;
  summary?: KCodeAvailablePluginSummary;
  /** 运行时信息（仅已发现的已安装插件有）：启用态、组件、manifest 回退字段。 */
  info?: KCodePluginInfo;
  installedMeta?: KCodeInstalledPluginSummary;
}

export type PluginUpdateStatus = NonNullable<KCodeInstalledPluginSummary["updateStatus"]>;
...
const OFFICIAL_MARKETPLACE_ORDER: readonly string[] = [KCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID];
...
export function sortMarketplaceSources(
  marketplaces: readonly KCodePluginMarketplaceSummary[],
  locale: string,
): KCodePluginMarketplaceSummary[] {
...
export function buildStoreItems(input: {
  ...
  availablePlugins: KCodeAvailablePluginSummary[];
  installedPlugins: KCodeInstalledPluginSummary[];
  plugins: KCodePluginInfo[];
  restorableBuiltins: KCodeAvailablePluginSummary[];
}): StorePluginItem[] {
