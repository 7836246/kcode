...
import type { CodingPlanWebviewLocale } from "@kcode/shared";
...
const CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY = "kcode:coding-plan:report-context";
...
export function resolveCodingPlanEmbeddedOrigin({
  ...
  if (
    ...
    return normalizeKCodeEndpointOrigin(normalizedOverride);
  }
  const normalizedEndpointOrigin = normalizeKCodeEndpointOrigin(endpointOrigin);
...
export function buildCodingPlanEmbeddedWebviewUrl({
  ...
  const url = new URL("/coding-plan", normalizeKCodeEndpointOrigin(origin));
...
export function createCodingPlanAuthInjectionScript({
  ...
  // App 当前 locale，写入 window.__kcodeLang__ 供 kcodeBridge.getLang() 读取，
  ...
  const values: Record<string, string | null> =
    provider === "zai"
      ? {
          ...
          kcodejwttoken: credentials.kcodeJwtToken?.trim() || null,
          "oauth:bigmodel:access_token": null,
        }
      : {
          "oauth:zai:access_token": null,
          // kcodejwttoken 是 zcode-plan 域通用凭证（BigModel OAuth callback 同样落盘），
  ...
  return `(() => {
  ${storageUpdates}
  const kcodeTheme = ${JSON.stringify(theme)};
  document.documentElement.classList.toggle("dark", kcodeTheme === "zai-dark");
  document.documentElement.classList.toggle("theme-zai-light", kcodeTheme === "zai-light");
  document.documentElement.classList.toggle("theme-zai-dark", kcodeTheme === "zai-dark");
  localStorage.setItem("kcode-theme", kcodeTheme);
  localStorage.setItem("kcode:coding-plan:embedded", "app");
  // 写入当前 App locale，供官网 kcodeBridge.getLang() 读取。
  // 注意：这是注入 webview 执行的原始 JS，不能用 TS 语法（如 as any）。
  window.__kcodeLang__ = ${JSON.stringify(resolvedLocale)};
  const kcodeReportContext = ${JSON.stringify(normalizedReportContext)};
  window.__kcodeReportContext__ = kcodeReportContext;
  localStorage.setItem(${JSON.stringify(CODING_PLAN_REPORT_CONTEXT_STORAGE_KEY)}, JSON.stringify(kcodeReportContext));
  window.dispatchEvent(new CustomEvent("kcode-coding-plan-auth-ready", {
    detail: { ...${JSON.stringify({ provider, locale: resolvedLocale })}, reportContext: kcodeReportContext },
  }));
})()`;
}
...
export function createCodingPlanCredentialClearScript(): string {
  ...
  delete window.__kcodeReportContext__;
})()`;
}

export function createCodingPlanScrollbarHideScript(): string {
  return `(() => {
  const styleId = "kcode-coding-plan-hide-scrollbar";
 ...
 * 重写 window.__kcodeLang__ 并派发 kcode-coding-plan-lang-change 事件，
 * 官网侧（kcodeBridge.onLangChange 或 window 监听）据此无感切换语言。
 */
export function createCodingPlanLangInjectionScript(locale: CodingPlanWebviewLocale): string {
  ...
  window.__kcodeLang__ = ${JSON.stringify(resolvedLocale)};
  window.dispatchEvent(new CustomEvent("kcode-coding-plan-lang-change", {
...
export function getCodingPlanCredentialKeys(provider: CodingPlanWebsiteProvider): string[] {
  // kcodejwttoken 对两个 provider 都加载：它是 zcode-plan 域通用凭证，
  // BigModel OAuth callback 同样落盘（见 resolveBigModelStartPlanZcodeJwt）。
  return provider === "zai"
    ? [`oauth:${ZAI_PROVIDER_ID}:access_token`, "kcodejwttoken"]
    : [`oauth:${BIGMODEL_PROVIDER_ID}:access_token`, "kcodejwttoken"];
}
