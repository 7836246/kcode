...
- 公开目录只精确匹配模型 ID（全名、`provider/model` 的裸 ID、`name:tag` 去 tag）；前缀相似不算命中，避免 `gpt-4-turbo` 吃到 `gpt-4-turbo-preview` 的窗口
- 供应商 `/models` 条目按该响应自己的 id 关联，不走公开目录那套匹配
- 公开目录成功响应进程内缓存；HTTP 明确失败后本次进程不再打该源；超时或网络错误下次仍可重试
