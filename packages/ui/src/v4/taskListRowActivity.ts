...
const TASK_LIST_ROW_ACTIVITY_FIELD = "__kcodeSessionActivity" as const;
...
export type TaskListMetaWithActivity = KCodeTaskMeta & {
  [TASK_LIST_ROW_ACTIVITY_FIELD]: TaskListRowActivity;
};

export function attachTaskListRowActivity<T extends KCodeTaskMeta>(
  task: T,
...
export function getTaskListRowActivity(task: KCodeTaskMeta): TaskListRowActivity | null {
...
function isTaskListRowRunning(task: KCodeTaskMeta): boolean {
 ...
 */
export function isTaskListRowActive(task: KCodeTaskMeta): boolean {
...
export function mergeTaskListMembershipFields(
  activityTask: KCodeTaskMeta,
  membershipTask: KCodeTaskMeta,
): KCodeTaskMeta {
