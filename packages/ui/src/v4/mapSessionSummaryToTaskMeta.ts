// sessions-index 的 SessionSummary → 侧栏实时 detail KCodeTaskMeta 映射。
...
import type { TraceId, KCodeProvider, KCodeTaskMeta } from "@kcode/shared";
import type { SessionSummary } from "@kcode/shared/kcode-protocol-v4";
import {
...
function phaseToStatus(phase: SessionSummary["phase"]): KCodeTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
interface MapSessionSummaryOptions {
  ...
  previous?: KCodeTaskMeta;
}

/**
 * SessionSummary → 实时 detail KCodeTaskMeta。sessions-index 不携带的字段
  ...
  return attachTaskListRowActivity(
    {
      ...
      ...(previous?.provider ? { provider: previous.provider as KCodeProvider } : {}),
      ...(status ? { status } : {}),
