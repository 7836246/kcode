...
// 复用 treemappingActivity 的既有解析规则，不复活 kcodeChatMessages。
//
...
import type { ConversationRow, ToolCallRow } from "@kcode/shared/kcode-protocol-v4";
  ...
  const layer = useMemo(
    () =>
      new SessionDataLayer({
        transport: createAgentConversationTransport(kcodeAgentService, {
          workspacePath,
          workspaceIdentity,
        }),
      }),
    [kcodeAgentService, workspacePath, workspaceIdentity],
  );
  useEffect(() => {
