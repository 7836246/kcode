...
import { KCODE_AGENT_PROVIDER, resolveExecutionState } from "@kcode/shared";
...
import type { SessionConfigState } from "@kcode/shared/kcode-protocol-v4";
import type { IModelSelectionService } from "@kcode/services";
...
import { submissionModeSchema } from "@kcode/shared/kcode-protocol-v4";
...
function shouldHydrateWorkspaceCatalog(params: {
  ...
  slashCommands: readonly KCodeSlashCommand[];
}): boolean {
...
export function useDraftConfigControl(params: {
  ...
  const kcodeSessionService = useKCodeSessionService(workspacePath, null, workspaceIdentity);
  ...
  useEffect(() => {
    ...
    const flight = prepareWorkspaceWithKCodeSessionService({
      workspacePath,
      workspaceIdentity,
      ...
      .then((prepareResult) => {
        ...
        const latest = useKCodeSessionStore.getState();
