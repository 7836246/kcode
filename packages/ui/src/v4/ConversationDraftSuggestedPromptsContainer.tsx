  ...
  const revalidateAndPrependPlugin = useCallback(
    async (current: DraftSuggestedPluginFlow, requestVersion: number) => {
      await invalidateDeferredDraftSessionForSkillChange({
        kcodeSessionService: resolution.services.kcodeSessionService,
        workspacePath,
        workspaceIdentity,
    ...
    [
      ...
      resolution.services.kcodeSessionService,
      targetParams,
      workspaceIdentity,
      workspacePath,
    ],
      ...
      if (
        ...
        (kind === "install" &&
          (!flow.result?.pluginName ||
            flow.result.marketplace !== KCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID ||
