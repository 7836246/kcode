...
import { resolveWorkspaceTelemetryDetail, type IPlatformService } from "@kcode/shared";
import { createConversationTelemetryService, type IServiceAccessor } from "@kcode/services";
...
function acquireSupervisor(
  ...
  const key = attachmentScopeKey(scope, services.kcodeAgentService);
...
function ensureSupervisorSubscription(
  ...
  const telemetryService = createConversationTelemetryService(services.kcodeAgentService);
...
export function ConversationTelemetryWorkspaceAttachment({
  ...
  const lease = useMemo(() => {
    const agentService = services.kcodeAgentService as object | null | undefined;
