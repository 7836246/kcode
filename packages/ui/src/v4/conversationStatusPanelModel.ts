import type { GitRepositorySummary } from "@kcode/shared";
import type {
...
import { workflowRunStepCounts } from "@kcode/shared/kcode-protocol-v4";
...
function buildRunningWorkflowRuns(
  ...
  for (const run of runs ?? []) {
    ...
    // 计数与聊天紧凑卡同源（唯一实现在 @kcode/shared 的 workflowRunStepCounts：表内 + 表外）。
