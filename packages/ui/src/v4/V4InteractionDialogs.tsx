...
import type { KCodeElicitationRequest, KCodePermissionOption, KCodeProvider } from "@kcode/shared";
...
} from "@/store/kcodeSessionStore.js";
import type { ElicitationFormDraft } from "@/store/kcodeSessionStoreTypes.js";
...
function resolveV4ElicitationRequest(
  projected: KCodeElicitationRequest | null,
  botProgress: KCodeElicitationRequest | null,
): KCodeElicitationRequest | null {
...
function buildV4ElicitationProgressKey(request: KCodeElicitationRequest): string {
...
export function V4InteractionDialogs({
  ...
  const notificationEnabled = useKCodeStoreWithDefault((state) => state.notificationEnabled, true);
  const botElicitationProgress = useKCodeSessionStore(
    (state) =>
      getTaskUiState(getWorkspaceState(state, workspacePath, workspaceIdentity), sessionId)
        .elicitationRequest,
  );
  const localElicitationDraft = useKCodeSessionStore((state) => {
  ...
  useEffect(() => {
    ...
    const taskUiState = getTaskUiState(
      getWorkspaceState(useKCodeSessionStore.getState(), workspacePath, workspaceIdentity),
      sessionId,
    );
  ...
  if (pending.payload.kind === "permission") {
    ...
    return (
      ...
      <PermissionDialog
        ...
        onRespond={(_requestId, option: KCodePermissionOption, feedback?: string) => {
