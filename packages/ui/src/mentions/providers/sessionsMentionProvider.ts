// composer parity：`#` 会话候选的数据源从旧 kcodeSessionStore/taskQueryCache/remote*
...
import type { KCodeProvider, KCodeTaskMeta } from "@kcode/shared";
...
function getSessionLabel(task: KCodeTaskMeta): string {
  const title = task.title.replace(/^#sess_[a-zA-Z0-9._-]+\s*/, "").trim();
  return title || "Untitled session";
}

function getWorkspaceLabel(task: KCodeTaskMeta): string {
  const raw = task.workspacePath.trim() || task.workspaceIdentity?.trim() || "";
  return raw.split(/[\\/]/).filter(Boolean).at(-1) ?? raw;
}

function mapTaskToMentionItem(task: KCodeTaskMeta, provider: KCodeProvider): SessionMentionItem {
...
function collectSessionMentionItems(
  ...
  const currentWorkspaceKey = options.workspacePath
    ...
    : undefined;
  const taskBySessionId = new Map<string, KCodeTaskMeta>();
...
function buildSessionMentionScopes(params: {
  ...
  const currentAgentService = params.currentServices.kcodeAgentService;
  ...
  for (const candidate of candidates) {
    ...
    if (!resolved || resolved.services.kcodeAgentService !== currentAgentService) {
      continue;
    }
    ...
    scopes.push({
      ...
      agentService: resolved.services.kcodeAgentService,
    });
  }
      ...
      return [
        {
          ...
          agentService: workspaceServices.kcodeAgentService,
        },
      ];
    }
