...
import { extractPlanStepsFromToolInput, extractPlanStepsFromToolOutput } from "@kcode/shared";
import type { KCodePlanStep } from "@kcode/shared";
...
const todoTextClasses: Record<KCodePlanStep["status"], string> = {
  pending: "text-foreground-subtle",
  in_progress: "text-foreground",
  completed: "text-foreground-subtlest line-through",
};

function readTodoPlan(context: ToolCallBlockRenderContext): KCodePlanStep[] | null {
...
function TodoStatusIcon({ status }: { status: KCodePlanStep["status"] }) {
  if (status === "completed") {
