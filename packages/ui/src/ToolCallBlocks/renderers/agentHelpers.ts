...
type AgentIntl = ReturnType<typeof useKCodeIntl>["intl"];
...
export function getAgentKindLabel(
  ...
  const rawName =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "agentType"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "agent_type"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "subagent_type"]);
...
export function getAgentColor(toolCall: AgentToolCall) {
  ...
  const inputRecord = isPlainRecord(toolCall.input) ? toolCall.input : null;
  const rawColor =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "color"]) ??
...
export function readBackgroundAgentInfo(toolCall: AgentToolCall) {
  ...
  const kcode = meta && isPlainRecord(meta.kcode) ? meta.kcode : null;
  const kcodeBackgroundAgent =
    kcode && isPlainRecord(kcode.backgroundAgent) ? kcode.backgroundAgent : null;
  const taskNotification =
    kcode && isPlainRecord(kcode.taskNotification) ? kcode.taskNotification : null;
  const backgroundAgent = kcodeBackgroundAgent;
...
export function getAgentActivityContent(toolCall: AgentToolCall) {
  const taskNotificationResult =
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "taskNotification", "result"]) ??
    readStringFromNestedRecord(toolCall.raw, ["_meta", "kcode", "taskNotification", "summary"]);
