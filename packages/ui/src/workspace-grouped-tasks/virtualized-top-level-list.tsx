...
import type { KCodeGroupedTaskViewNode } from "@kcode/services";
...
function getTopLevelNodeKey(node: KCodeGroupedTaskViewNode | undefined, index: number): string {
  if (!node) {
...
function estimateTopLevelNodeSize(
  node: KCodeGroupedTaskViewNode | undefined,
...
function VirtualizedGroupedTopLevelList({
  ...
  isGroupCollapsed: (groupId: string) => boolean;
  renderNode: (node: KCodeGroupedTaskViewNode, index: number) => ReactNode;
}) {
