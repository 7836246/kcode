...
} from "@/lib/kcodeTaskUsageFallback.js";
import { normalizeKCodeUiError } from "@/lib/kcodeUiError.js";
...
export function syncBotTaskConfigOptionsToStore(params: {
  kcodeSessionStore: Pick<
    ReturnType<typeof useKCodeSessionStore.getState>,
  ...
  params.kcodeSessionStore.setTaskConfigOptions(
...
export function shouldMirrorBotTaskStreamToStore(params: {
  ...
  // Bugfix: 远端 Bot task 的 stream 来自 bot runtime host，不一定会被当前 ChatView 的 KCode Agent stream 订阅收到。
...
export function useBotBroadcastEffects(
  ...
  useEffect(() => {
    const disposable = services.broadcastService.onMessage((message) => {
      ...
      if (stream) {
        const kcodeSessionStore = useKCodeSessionStore.getState();
        const workspaceState = kcodeSessionStore.getWorkspaceState(
        ...
        switch (event.type) {
          ...
          case "tool_call":
            kcodeSessionStore.setTaskRuntimeState(
              stream.workspacePath,
              stream.taskId,
              "streaming",
              undefined,
          ...
          case "permission_request":
            kcodeSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, event, stream.workspaceIdentity);
            kcodeSessionStore.setTaskRuntimeState(
              stream.workspacePath,
              stream.taskId,
              "streaming",
          ...
          case "task_complete":
            ...
            kcodeSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            kcodeSessionStore.setTaskError(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            break;
          case "task_error": {
            const normalizedError = normalizeKCodeUiError(
              {
                message: event.error,
                detail: event.detail,
                code: event.code,
            ...
            kcodeSessionStore.setTaskPermissionRequest(stream.workspacePath, stream.taskId, null, stream.workspaceIdentity);
            kcodeSessionStore.setTaskError(stream.workspacePath, stream.taskId, normalizedError, stream.workspaceIdentity);
            break;
          }
          case "task_warning":
            kcodeSessionStore.setTaskError(
              stream.workspacePath,
              stream.taskId,
              normalizeKCodeUiError(
                {
          ...
          case "usage_update":
            {
              ...
              kcodeSessionStore.setTaskContextWindow(
                stream.workspacePath,
                stream.taskId,
                event.size,
                stream.workspaceIdentity,
              );
              kcodeSessionStore.setTaskUsage(
                stream.workspacePath,
                stream.taskId,
                nextUsage,
                stream.workspaceIdentity,
          ...
          case "session_info_update":
            if (event.apiRetry !== undefined) {
              kcodeSessionStore.setTaskApiRetryStatus(
                stream.workspacePath,
                stream.taskId,
                event.apiRetry ?? null,
                stream.workspaceIdentity,
      ...
      if (provider && shouldSyncVisibleTaskConfig) {
        // Bugfix: /model、/mode 可以从第三方 Bot 修改当前 task 的真实 KCode Agent 状态。
        // 这些操作不经过 ChatInputToolbar，本地 store 以前不会同步 provider/configOptions，
        // 导致 Bot 回复已切换但 UI 下拉仍显示旧状态。
        kcodeSessionStore.bindRuntimeProvider(
          refresh.workspacePath,
          provider,
          refresh.workspaceIdentity,
        );
      ...
      // 会自然出现该消息；本地拼装面（kcodeChatMessages）随旧 ChatView 退役。
      if (refresh.event === "permission_request" && refresh.permissionRequest) {
        kcodeSessionStore.setTaskPermissionRequest(
          refresh.workspacePath,
          refresh.taskId,
        ...
        kcodeSessionStore.removeTaskPermissionRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
        ...
        // 当前 UI 窗口不一定有同一条 KCode Agent stream 订阅，必须把新的 elicitation_request 显式写回 store。
        kcodeSessionStore.setTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
        ...
        // Bugfix: Bot 代用户提交 AskUserQuestion 时，当前 UI 窗口不一定能收到 KCode Agent stream 的
        // elicitation_response。通过 bots:task 明确同步 requestId 出队，避免问答弹窗一直挂着。
        kcodeSessionStore.removeTaskElicitationRequest(
          refresh.workspacePath,
          refresh.taskId,
          refresh.requestId,
          refresh.workspaceIdentity,
      ...
      if (shouldRefreshBotTaskList(refresh.event, Boolean(refresh.task))) {
        kcodeSessionStore.bumpTaskListVersion(refresh.workspacePath, refresh.workspaceIdentity);
      }
    });
