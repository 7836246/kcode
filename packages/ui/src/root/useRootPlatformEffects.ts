...
export function useRootPlatformEffects({
  ...
  initialWorkspacePurpose?: import("@kcode/shared").WorkspacePurpose;
  ...
  isDesktop?: boolean;
  locale: ReturnType<typeof import("@/i18n/IntlProvider.js").useKCodeIntl>["locale"];
  tabs: WindowTabState[];
  ...
  intl: ReturnType<typeof import("@/i18n/IntlProvider.js").useKCodeIntl>["intl"];
}) {
  ...
  useEffect(() => {
    // 启动时必须先完成供应商可用性门禁，再恢复历史/初始 workspace。
    // 如果这里抢先 addTab，未配置 API Key 的用户会先看到主界面，之后才被登录页覆盖。
  ...
  useEffect(() => {
    ...
    const disposeNotificationClick = platform.onTaskNotificationClick((taskId: string) => {
      ...
      const workspaces = useKCodeSessionStore.getState().workspaces;
  ...
  useEffect(() => {
    ...
    void baseServices.conversationShareService
      ...
      .then((result) => {
        ...
        const sessionStore = useKCodeSessionStore.getState();
  ...
  useEffect(() => {
    ...
    syncActiveSession();
    return useKCodeSessionStore.subscribe(syncActiveSession);
