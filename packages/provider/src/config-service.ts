...
export interface ProviderConfigServiceDependencies {
    readonly kcodeBuiltinSource: ProviderSource<ProviderConfigLayerSnapshot>;
...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
    ...
    constructor(dependencies: ProviderConfigServiceDependencies) {
        this.#kcodeBuiltinSource = dependencies.kcodeBuiltinSource;
        this.#personalRepository = dependencies.personalRepository;
        this.#sourceDisposers = [
            this.#kcodeBuiltinSource.onDidChange((reason) => this.#emit(`kcodeBuiltin:${reason}`)),
    ...
    async read(): Promise<ProviderConfigSnapshot> {
        ...
        const [kcodeBuiltin, personal] = await Promise.all([
            this.#kcodeBuiltinSource.read(),
    ...
    async savePersonalProviderOverlay(
        ...
        const kcodeBuiltin = await this.#kcodeBuiltinSource.read();
        return this.#updatePersonal((current) => {
            assertMembershipCurrent(membership, providerId, current);
            const builtin = kcodeBuiltin.providers.get(providerId);
            const currentPersonal = current.providers.get(providerId);
            const currentEffectiveProviders = kcodeBuiltin.providers.overlay(current.providers);
            ...
            if (builtin?.access?.type === "zhipu-account" && config.access !== undefined) {
                ...
                throw new Error(
                    `固定 Account Provider 的 Access 只能由 KCode Built-in Config 声明: ${providerId}`,
                );
            }
            ...
            if (builtin) {
                if (normalized.group != null && normalized.group !== builtin.group) {
                    throw new Error(
                        `Personal Overlay 不能改写 Built-in Provider group: ${providerId}`,
                    );
                }
                ...
                if (group !== "standard-personal") {
                    throw new Error(
                        `Personal-only Provider 必须使用 standard-personal group: ${providerId}`,
                    );
                }
                normalized = normalized.overlay(new ProviderConfigValue({ group }));
            }
            const membershipBaseline =
                builtin ?? resolveTemplateBaseline(kcodeBuiltin, current.providers, providerId);
            ...
            const nextEffectiveProviders = kcodeBuiltin.providers.overlay(providers);
    ...
    async createPersonalProvider(
        ...
        const template = templateId ? kcodeBuiltin.providerTemplates?.get(templateId) : undefined;
    ...
    async reorderPersonalModels(
        ...
        return this.#updatePersonal((current) => {
            ...
            const builtinProvider =
                kcodeBuiltin.providers.get(providerId) ??
                resolveTemplateBaseline(kcodeBuiltin, current.providers, providerId);
            const provider = writableProviderOverlay(kcodeBuiltin, current, providerId);
    ...
    async addPersonalModel(
        ...
        return this.#updatePersonal((current) => {
            ...
            const provider = writableProviderOverlay(kcodeBuiltin, current, normalizedProviderId);
            const builtinModelIds =
                membership?.inheritedModelIds ??
                resolveProviderBuiltinModelIds(
                    kcodeBuiltin,
                    current.providers,
            ...
            return {
                providers: current.providers.set(
                    normalizedProviderId,
                    provider
                        .withPersonalModelIds([...currentModelIds, normalizedModelId])
                        .withModelOrder(
            ...
            return {
                providers: current.providers.set(
                    ...
                    provider
                        ...
                        .withModelOrder(
                            normalizeModelOrder(builtinModelIds, modelIds, requestedOrder),
                        ),
                ),
    ...
    async savePersonalModelDraft(
        ...
        return this.#updatePersonal((current) => {
            ...
            if (
                originalId !== nextId &&
                (personalModelIds.includes(nextId) || builtinSet.has(nextId))
            ) {
                throw new Error(`Model 已存在: ${normalizedProviderId}/${nextId}`);
            }
            ...
            return {
                providers: current.providers.set(
                    ...
                    provider
                        ...
                        .withModelOrder(
                            normalizeModelOrder(inherited, plan.nextPersonalModelIds, [
                                ...(provider.modelOrder ?? []),
                                ...plan.addedModelIds,
...
function normalizeProviderOrder(
    _kcodeBuiltinProviders: ProviderConfigMap,
...
function appendCurrentProviderOrder(
    kcodeBuiltinProviders: ProviderConfigMap,
    personalProviders: ProviderConfigMap,
    currentOrder: readonly ProviderId[] | undefined,
    addedProviderId: ProviderId,
): ProviderId[] {
    const current = normalizeProviderOrder(
        kcodeBuiltinProviders,
        personalProviders,
        currentOrder ?? [],
    );
    return normalizeProviderOrder(kcodeBuiltinProviders, personalProviders, [
...
function resolvePersonalProviderBaselines(
    ...
    const personal = personalProviders.mapConfigs((config, _id, rule) => {
        const template = rule.templateId
            ? kcodeBuiltin.providerTemplates?.get(rule.templateId)
            : undefined;
        return template ? template.config.overlay(config) : config;
    });
    return kcodeBuiltin.providers.overlay(personal);
}
