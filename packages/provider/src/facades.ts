...
export class ProviderSettingsFacade {
    ...
    getView(): ProviderSettingsView {
        ...
        return createProviderSettingsView({
            ...
            kcodeBuiltinProviders: snapshot.config.kcodeBuiltinProviders,
            kcodeBuiltinProviderTemplates: snapshot.config.kcodeBuiltinProviderTemplates,
    ...
    resolveModelConfig(input: ResolveModelConfigInput): ModelConfigResolution {
        ...
        if (!("personalConfig" in input)) {
            const modelRules = ModelConfigRules.composeEffective(
                snapshot.config.kcodeBuiltinModelRules,
        ...
        const inheritedConfig = snapshot.config.kcodeBuiltinModelRules.resolve({
            providerId: input.providerId,
            templateId: provider.templateId,
            modelId: input.modelId,
            apiType: provider.config.api?.type,
        ...
        personalRules = personalRules.setExact(
            input.providerId,
            input.modelId,
            personalConfig,
            true,
    ...
    async importRemoteModels(
        ...
        const view = await this.#mutateProvider(
            providerId,
    ...
    getView(
        ...
        const resolveLegacyReasoningLevel =
            snapshot && this.#resolveLegacyReasoningLevel
                ? (selection: ModelSelection) =>
                      this.#resolveLegacyReasoningLevel!(snapshot, selection)
                : undefined;
...
function createProviderSettingsView(input: {
    revision: number;
    kcodeBuiltinProviders: ProviderRegistryServiceSnapshot["config"]["kcodeBuiltinProviders"];
    kcodeBuiltinProviderTemplates: ProviderRegistryServiceSnapshot["config"]["kcodeBuiltinProviderTemplates"];
    ...
    const projectProvider = (provider: (typeof input.resolution.resolvedProviders)[number]) => {
        ...
        return Object.freeze({
            ...
            ...(provider.templateConfig
                ? { templateConfig: provider.templateConfig.toJSON() }
                : {}),
            ...
            models: Object.freeze(
                provider.models.map((model) => {
                    ...
                    return Object.freeze({
                        ...
                        ...(personalModelConfig
                            ? { personalExactConfig: personalModelConfig.toJSON() }
                            : {}),
                        ...(personalModelRule
                            ? {
                                  useRecommendedConfig:
                                      personalModelRule.type !== "manual-provider-model",
                              }
                            : {}),
    ...
    return Object.freeze({
        ...
        providerTemplates: Object.freeze(
            (input.kcodeBuiltinProviderTemplates ?? ProviderTemplateMap.empty())
                .entries()
