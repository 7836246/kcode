...
} from "@kcode/shared/model-config";
...
export class ModelConfigRules {
    ...
    resolve(input: ModelConfigRuleResolutionInput): ModelConfig {
        ...
        const baseUrl =
            input.baseUrl == null ? undefined : normalizeBaseURLForRuleMatch(input.baseUrl);
        for (const rule of this.#rules) {
            ...
            if (
                ...
                if (input.apiType == null || !matchesRule(rule.apiTypeMatch, input.apiType))
                    continue;
            }
            if (
    ...
    toKCodeBuiltinJSON(): BuiltinModelConfigRulesData {
    ...
    #collect<T extends ModelConfigRule["type"]>(type: T) {
        return this.#rules
            ...
            .map(({ type: _type, config, ...identity }) => ({
                ...identity,
