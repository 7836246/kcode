import type { KCodePersistedMessage, KCodePersistedMessagePart } from "./kcode-task-types-core.js";

// KCode runtime 把同一个 user turn 里**每一轮 LLM 调用**落成独立的 assistant 消息（各带 time.created/completed）。
...
export function coalesceConsecutiveKCodeAssistants(
  messages: readonly KCodePersistedMessage[],
): KCodePersistedMessage[] {
  const merged: KCodePersistedMessage[] = [];
...
function mergeAssistantPair(
  first: KCodePersistedMessage,
  next: KCodePersistedMessage,
): KCodePersistedMessage {
  ...
  const shiftedNextParts: KCodePersistedMessagePart[] = (next.parts ?? []).map((part) =>
    part.type === "tool-call"
