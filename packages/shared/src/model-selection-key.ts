...
export function buildNativeSupplierKey(kcodeProvider: KCodeProvider): string {
  return `${NATIVE_SUPPLIER_KEY_PREFIX}${kcodeProvider}`;
}
...
export function resolveSupplierKeyFromModelDisplayValue(
  ...
  return buildNativeSupplierKey(kcodeProvider);
}
