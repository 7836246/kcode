import type {
  ...
  TraceId,
  KCodeStreamEvent,
  KCodeTaskNetworkDebugStatusType,
} from "./kcode-task-types-core.js";

const MODEL_NETWORK_STATUS_TYPES = new Set<KCodeTaskNetworkDebugStatusType>([
...
export function kcodeTaskNetworkDebugStatusFromPayload(params: {
  taskId: string;
  traceId: TraceId;
  inputId?: InputId;
  queryId?: QueryId;
...
}): Extract<KCodeStreamEvent, { type: "task_network_debug_status" }> | null {
...
function networkStatusTypeValue(value: unknown): KCodeTaskNetworkDebugStatusType | undefined {
  return typeof value === "string" && MODEL_NETWORK_STATUS_TYPES.has(value as never)
    ? (value as KCodeTaskNetworkDebugStatusType)
    : undefined;
}
