...
import { KCODE_AGENT_PROVIDER, KCODE_AGENT_PROVIDER_LABEL } from "./kcode-agent-policy.js";
import type {
  KCodeConfigOption,
...
export type KCodeAutomationBotDeliveryTarget = z.infer<
  typeof kcodeAutomationBotDeliveryTargetSchema
>;
...
export interface BotTaskSummary {
  taskId: string;
  title: string;
  status: KCodeTaskRuntimeStatus | "persisted-completed" | "persisted-error" | "unknown";
  workspacePath: string;
  workspaceIdentity?: string;
  provider?: KCodeProvider;
  model?: string;
...
export interface BotTaskBroadcastPayload {
  ...
  permissionRequest?: KCodePermissionRequest;
  elicitationRequest?: KCodeElicitationRequest;
  requestId?: string;
  error?: string;
}
...
export const botCurrentOptionsSchema = z
  .object({
    ...
    // 兼容旧 bot-config.json；CLI provider 现在统一由 KCode Protocol 侧配置决定。
    cli: z.literal(KCODE_AGENT_PROVIDER).optional(),
  })
  .strict();

export const botDraftOptionsSchema = z
  .object({
    provider: z.literal(KCODE_AGENT_PROVIDER),
    modelSelection: modelSelectionSchema.optional(),
    mode: z.string().min(1).optional(),
  })
  .strict();
...
export const BOT_KCODE_PROVIDER_OPTIONS: Array<{
  id: KCodeProvider;
  label: string;
}> = [{ id: KCODE_AGENT_PROVIDER, label: KCODE_AGENT_PROVIDER_LABEL }];
