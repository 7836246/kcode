import type { KCodeApiRetryStatus } from "./kcode-task-types-core.js";

export function normalizeKCodeApiRetryStatus(
  value: unknown,
...
export function kcodeApiRetryFromModelNetworkStatusPayload(
  payload: Record<string, unknown>,
): KCodeApiRetryStatus | null | undefined {
  const type = stringValue(payload.type);
  if (type === "model_retry_scheduled") {
    return normalizeKCodeApiRetryStatus(payload);
  }
  if (type === "model_request_started") {
    const streamRecoveryRetry = kcodeApiRetryFromStreamRecoveryPayload(payload.streamRecovery);
...
export function isKCodeModelRetryRecoveryProgressPayload(
  payload: Record<string, unknown>,
): boolean {
...
export function kcodeApiRetryFromStreamRecoveryPayload(
  value: unknown,
): KCodeApiRetryStatus | undefined {
  const record = asRecord(value);
