export type KCodeBackgroundTaskControlStatus =
  | "pending"
  | "running"
  | "completed"
  | "failed"
...
export interface KCodeBackgroundTaskControlItem {
  jobId: string;
  toolCallId?: string;
  command: string;
  taskKind: "agent" | "bash";
  cancellable?: boolean;
  title?: string;
  status: KCodeBackgroundTaskControlStatus;
  startedAt?: number;
  elapsedMs?: number;
  pid?: number;
  stdoutTail?: string;
...
const ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES = new Set<KCodeBackgroundTaskControlStatus>([
  "running",
]);
...
type BackgroundTaskControlKind = KCodeBackgroundTaskControlItem["taskKind"];
 ...
 */
export function resolveKCodeBackgroundTaskControlKind(
  value: unknown,
): BackgroundTaskControlKind | undefined {
...
function normalizeStatus(value: string | undefined): KCodeBackgroundTaskControlStatus {
...
function readJobId(record: Record<string, unknown>, command: string): string {
  const explicitId = readStringField(record, [
    // KCode Protocol 后台任务取消入口按 taskId 查找 runtime task 记录；
...
export function parseKCodeBackgroundTaskControlItems(
  value: unknown,
): KCodeBackgroundTaskControlItem[] {
  if (!Array.isArray(value)) {
    return [];
  }
  const jobsById = new Map<string, KCodeBackgroundTaskControlItem>();
  for (const item of value) {
    if (!isPlainRecord(item)) {
      continue;
    }
    const taskKind = resolveKCodeBackgroundTaskControlKind(item);
    if (!taskKind) {
      continue;
    }
    ...
    const job: KCodeBackgroundTaskControlItem = {
      jobId,
...
export function isActiveKCodeBackgroundTaskControlItem(
  job: KCodeBackgroundTaskControlItem,
): boolean {
  return ACTIVE_BACKGROUND_TASK_CONTROL_STATUSES.has(job.status);
}

export function getKCodeBackgroundTaskControlItemElapsedMs(
  job: KCodeBackgroundTaskControlItem,
  now = Date.now(),
): number {
...
export function collectVisibleKCodeBackgroundTaskControlItems(
  jobs: readonly KCodeBackgroundTaskControlItem[],
  now = Date.now(),
  thresholdMs = 30_000,
): Array<KCodeBackgroundTaskControlItem & { elapsedMs: number }> {
  return jobs
    .filter(isActiveKCodeBackgroundTaskControlItem)
    .map((job) => ({
      ...job,
      elapsedMs: getKCodeBackgroundTaskControlItemElapsedMs(job, now),
    }))
