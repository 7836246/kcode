...
/** KCode 运行中继续输入时的交互行为 */
export type KCodeInteractionBehavior = "queue" | "guide";
...
export interface ResourceUsageProcess {
  pid: number;
  /** 进程显示名，如 kcode-main / kcode-agent-kcode-demo / node_repl */
  name: string;
...
export interface AppSettings {
   ...
   * 只影响内置浏览器出口，不影响 KCode 自身对后端与模型 API 的请求。默认关闭，重启后生效。
   */
  ...
  /** 数据目录的根路径（替代 homedir），默认为 os.homedir()；.kcode/v2 后缀不变 */
  dataBaseDir?: string;
