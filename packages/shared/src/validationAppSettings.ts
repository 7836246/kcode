...
import { normalizeKCodeEndpointOrigin } from "./kcodeEndpoint.js";
import {
...
const kcodeInteractionBehaviorSchema = z.enum(["queue", "guide"]);
...
const kcodeEndpointOriginSchema = z.preprocess((value) => {
  ...
  try {
    return normalizeKCodeEndpointOrigin(trimmed);
  } catch {
    return undefined;
  }
}, z.string().optional());

function sanitizeKCodeEndpointOrigin(value: unknown): unknown {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return value;
  }
  const raw = value as Record<string, unknown>;
  if (!("kcodeEndpointOrigin" in raw)) {
    return value;
  }
  const parsed = kcodeEndpointOriginSchema.safeParse(raw.kcodeEndpointOrigin);
  if (parsed.success && typeof parsed.data === "string") {
    return { ...raw, kcodeEndpointOrigin: parsed.data };
  }
  const { kcodeEndpointOrigin: _kcodeEndpointOrigin, ...next } = raw;
        ...
        migrateMessageStreamShowReasoningDefault(
          migrateCloseToTrayOnWindowsDefault(
            migrateLegacyLocalePreference(
              sanitizeKCodeEndpointOrigin(migrateLegacyWorkspaceSession(value)),
            ),
          ),
        ),
      ),
