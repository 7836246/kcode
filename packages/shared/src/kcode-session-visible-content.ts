...
// 消费者：kcodeTaskServiceAdapter/kcodeSessionProjection/kcodeTaskIndexSyncer、
// CLI bootstrap session-mapper。textFromKCodeMessageParts 已迁
// kcode-protocol-legacy-types.ts（幸存面）。本文件与旧投影栈同生命周期。
import type { KCodeSessionGoal } from "./kcode-protocol/index.js";
...
export function isKCodeGoalContinuationReminderText(text: string): boolean {
...
export function isKCodeGoalContinuationReminderMessage(message: KCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (message.info.source === "goal-continuation" ||
      String(message.info.metadata?.["source"] ?? "") === "goal-continuation" ||
      isKCodeGoalContinuationReminderText(textFromKCodeMessageParts(message.parts)))
  );
}

export function isKCodeGoalStateReminderText(text: string): boolean {
  const normalized = text.trimStart();
  return normalized.startsWith("<system-reminder>") && normalized.includes(GOAL_STATE_TEXT_MARKER);
}

export function isKCodeGoalModelOnlyReminderMessage(message: KCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
    (isKCodeGoalContinuationReminderMessage(message) ||
      isKCodeGoalStateReminderText(textFromKCodeMessageParts(message.parts)))
  );
}

export function isKCodeModelOnlySyntheticUserMessage(message: KCodeMessageWithParts): boolean {
  if (
    message.info.role === "user" &&
...
export function isKCodeCompactSummaryMessage(message: KCodeMessageWithParts): boolean {
  return (
    message.info.role === "user" &&
...
export function getKCodeUserVisibleMessages(
  messages: readonly KCodeMessageWithParts[],
  _options: { target?: KCodeSessionGoal | null } = {},
): KCodeMessageWithParts[] {
  const visibleMessages: KCodeMessageWithParts[] = [];
  for (const message of messages) {
    if (isKCodeModelOnlySyntheticUserMessage(message) || isKCodeCompactSummaryMessage(message)) {
...
export function getKCodeGoalIterationByAssistantMessageId(
  messages: readonly KCodeMessageWithParts[],
  options: { maxGoalIteration?: number; target?: KCodeSessionGoal | null } = {},
): Map<string, number> {
  ...
  const sortedMessages = [...messages].sort(compareKCodeMessagesByCreatedTime);
  ...
  for (const message of sortedMessages) {
    if (message.info.role === "user") {
      if (isKCodeGoalContinuationReminderMessage(message)) {
...
export interface KCodeGoalIterationCountTimelineItem {
  goalIteration?: number;
  status: "started" | "completed" | "failed_closed" | "cancelled";
  verification?: { passed?: boolean | null } | null;
}

export function getKCodeGoalActiveIterationCount(input: {
  targetStatus?: string | null;
  timeline?: readonly KCodeGoalIterationCountTimelineItem[] | null;
}): number {
...
function compareKCodeMessagesByCreatedTime(
  left: KCodeMessageWithParts,
  right: KCodeMessageWithParts,
) {
...
function isVisibleRealGoalUserMessage(message: KCodeMessageWithParts, target: KCodeSessionGoal) {
  if (
    message.info.role !== "user" ||
...
export function resolveKCodeVisibleSessionTitle(input: {
  title?: string;
  messages: readonly KCodeMessageWithParts[];
  target?: KCodeSessionGoal | null;
  fallback?: string;
}): string {
  const normalizedTitle = input.title?.trim() ?? "";
  if (
    normalizedTitle &&
    !isKCodeGoalContinuationReminderText(normalizedTitle) &&
    !isKCodeGoalStateReminderText(normalizedTitle)
  ) {
  ...
  const firstUser = input.messages.find(
    (message) =>
      message.info.role === "user" &&
      getConversationMessageProjectionPolicy(message) === "realUserInput" &&
      !isKCodeGoalModelOnlyReminderMessage(message),
  );
  const firstUserText = textFromKCodeMessageParts(firstUser?.parts ?? []).trim();
  if (firstUserText) {
