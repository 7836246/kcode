...
import { z } from "zod";
import type { KCodeTaskMigrationSource, KCodeTaskMode } from "./kcode-task-types-core.js";
import { kcodeAgentProviderSchema } from "./kcode-agent-policy.js";
import { kcodePermissionResponseSchema } from "./kcode-protocol-legacy-types.js";
// merge 冲突解决：两侧分别在相邻行新增独立 import（本分支 hook trust review
// 决策 schema、staging telemetry error attribution schema），二者无语义交集，均保留。
import { workspaceHookReviewDecisionSchema } from "./kcode-protocol-v4/workspace-hook-review.js";
import { errorAttributionSchema } from "./kcode-protocol-v4/snapshot.js";

const nonEmptyString = z.string().trim().min(1);
const kcodeTaskModeRealtimeValues = [
  "yolo",
  "plan",
  "edit",
  "auto",
  "autoEdit",
  "build",
] as const satisfies readonly KCodeTaskMode[];
const kcodeTaskMigrationSourceRealtimeValues = [
  "claudeCode",
] as const satisfies readonly KCodeTaskMigrationSource[];
const kcodeTaskChangeSummaryRealtimeSchema = z
  .object({
...
const taskMetaRealtimeSchema = z.object({
  ...
  // schema 推导类型因此无法回到 KCodeTaskMeta，host typecheck 也就无法覆盖这条链路。
  mode: z.enum(kcodeTaskModeRealtimeValues),
  model: z.string().optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: kcodeAgentProviderSchema.optional(),
  migrationSource: z.enum(kcodeTaskMigrationSourceRealtimeValues).optional(),
  ...
  lastError: z
    ...
    .optional(),
  changeSummary: kcodeTaskChangeSummaryRealtimeSchema.optional(),
});
...
export const workspaceTaskListInvalidatedEventSchema = taskRealtimeInvalidationBaseEventSchema
  ...
  .strict();
const kcodePromptAttachmentSchema = z.discriminatedUnion("kind", [
  z
    .object({
...
const kcodeTaskRuntimeCommandBaseSchema = z
  .object({
  ...
  .strict();
const kcodeTaskRuntimeCommandSchema = z.discriminatedUnion("type", [
  kcodeTaskRuntimeCommandBaseSchema
    .extend({
...
const taskEnqueueCommandOwnerCommandRequestSchema = taskOwnerCommandBaseSchema
  .extend({
    ...
    taskCommand: kcodeTaskRuntimeCommandSchema,
  })
  .strict();
...
export const taskOwnerCommandResultSchema = z.discriminatedUnion("success", [
  z
    .object({
      ...
      taskCommand: kcodeTaskRuntimeCommandSchema.optional(),
    })
    .strict(),
  z
    .object({
