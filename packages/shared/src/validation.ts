...
import { z } from "zod";
import { kcodeProcessDiagnosticSchema } from "./process-diagnostic.js";
...
import { kcodeProviderSchema } from "./providers.js";
import { kcodeAgentProviderSchema } from "./kcode-agent-policy.js";
...
} from "./kcode-protocol/index.js";
import { kcodeTaskModeSchema } from "./kcode-task-mode-schema.js";
import { PROTOCOL_V4_LIMITS } from "./kcode-protocol-v4/core.js";
import { errorAttributionSchema } from "./kcode-protocol-v4/snapshot.js";
import { sessionWorkflowActivitySchema } from "./kcode-protocol-v4/sessions-index-workflow-activity.js";
import {
...
export { kcodeTaskModeSchema } from "./kcode-task-mode-schema.js";
...
export const helloAckMessageSchema = z.object({
  type: z.literal("kcode-hello-ack"),
  version: z.string(),
...
export const hostInitLocalMessageSchema = z.object({
  ...
  kcodeBuiltinProviderConfigFilePath: nonEmptyStringSchema,
...
export const hostOffPeakRunMessageSchema = z.object({
  ...
  workspaceIdentity: z.string().optional(),
  prompt: nonEmptyStringSchema,
  // 权限四档映射现有 KCodeTaskMode；与 cron-run 的 mode 同样按宽松 string 传输
...
export { kcodeProviderSchema };

export const kcodeTaskMigrationSourceSchema = z.enum(["claudeCode"]);
...
export const hostAgentProcessExceptionResponseSchema = z
  .object({
    ...
    runtimeGeneration: z.number().int().positive(),
    runtimeInstanceId: nonEmptyStringSchema,
    diagnostic: kcodeProcessDiagnosticSchema,
  })
  .strict();
...
export const agentLaneResourceSampleSchema = kcodeProcessResourceSampleSchema
...
export const nodeSelfResourceSampleSchema = z
  .object({
     ...
     * 上限刻意放宽（与 CLI 的 `kcodeProcessResourceSampleSchema` 同口径）：读数异常时宁可让
...
export const hostMcpResourceSamplesResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    environmentKey: resourceTelemetryEnvironmentKeySchema.optional(),
    samples: kcodeMcpResourceSamplesSchema,
  })
  .strict();
...
export const hostToolExecResourceResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    sample: kcodeToolExecResourceSchema,
  })
  .strict();
...
export const hostMcpTelemetryResponseSchema = z
  .object({
    ...
    runtimeSurface: z.enum(["local", "remote"]),
    event: kcodeMcpTelemetryEventSchema,
  })
  .strict();
...
export const kcodeTaskPersistStatusSchema = z.enum(["running", "completed", "error"]);

export const kcodePromptImageAttachmentSchema = z.object({
...
export const kcodePromptVideoAttachmentSchema = z.object({
...
export const kcodePromptPdfAttachmentSchema = z.object({
  kind: z.literal("pdf"),
  filename: z.string(),
  mimeType: z.string(),
  sizeBytes: z.number().int().nonnegative().optional(),
...
export const kcodePromptFileAttachmentSchema = z.object({
  kind: z.literal("file"),
  filename: z.string(),
  mimeType: z.string(),
...
export const kcodePromptAttachmentSchema = z.discriminatedUnion("kind", [
  kcodePromptImageAttachmentSchema,
  kcodePromptVideoAttachmentSchema,
  kcodePromptPdfAttachmentSchema,
  kcodePromptFileAttachmentSchema,
]);

export const kcodePersistedToolCallSchema = z.object({
...
const kcodePersistedMessagePartSchema = z.discriminatedUnion("type", [
...
export const kcodePersistedMessageSchema = z.object({
  ...
  attachments: z.array(kcodePromptAttachmentSchema).optional(),
  tools: z.array(kcodePersistedToolCallSchema).optional(),
  thought: z.string().optional(),
  parts: z.array(kcodePersistedMessagePartSchema).optional(),
...
export const kcodeTaskGoalStatusSchema = z.enum(["active", "paused", "budget_limited", "complete"]);

export const kcodeTaskTargetChangedActionSchema = z.enum([
  "set",
  "status_updated",
  "cleared",
  "usage_accounted",
...
export const kcodeTaskTargetChangedSourceSchema = z.enum(["command", "tool", "runtime"]);

export const kcodeTaskGoalSchema = z.object({
  sessionID: nonEmptyStringSchema,
  ...
  status: kcodeTaskGoalStatusSchema,
...
export const kcodeTaskGoalChangedPatchSchema = z.object({
  action: kcodeTaskTargetChangedActionSchema,
  source: kcodeTaskTargetChangedSourceSchema,
  target: kcodeTaskGoalSchema.nullable(),
  previousTarget: kcodeTaskGoalSchema.nullable().optional(),
});

export const kcodeTaskTargetStatusSchema = kcodeTaskGoalStatusSchema;
export const kcodeTaskTargetSchema = kcodeTaskGoalSchema;
export const kcodeTaskTargetChangedPatchSchema = kcodeTaskGoalChangedPatchSchema;

export const kcodeTaskMetaSchema = z.object({
  taskId: nonEmptyStringSchema,
  ...
  mode: kcodeTaskModeSchema,
  model: z.string().optional(),
  thoughtLevel: nonEmptyStringSchema.optional(),
  runtimeEpoch: z.number().int().nonnegative().optional(),
  provider: kcodeAgentProviderSchema.optional(),
  migrationSource: kcodeTaskMigrationSourceSchema.optional(),
  ...
  status: kcodeTaskPersistStatusSchema.optional(),
  lastError: z
    .object({
  ...
  changeSummary: z
    ...
    .optional(),
  target: kcodeTaskGoalSchema.nullable().optional(),
});

export const kcodeTaskIndexEntrySchema = z.object({
  workspaceHash: nonEmptyStringSchema,
  taskId: nonEmptyStringSchema,
});

export const kcodePinnedTasksFileSchema = z.object({
  version: z.literal("1"),
  tasks: z.array(kcodeTaskIndexEntrySchema),
});

const kcodePersistedFileSnapshotSchema = z.object({
  path: z.string(),
...
const kcodePersistedFileChangeSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  snapshots: z.array(kcodePersistedFileSnapshotSchema),
  fileState: z.enum(["applied", "reverted"]).optional(),
});

const kcodePersistedTurnCheckpointSchema = z.object({
  turnIndex: z.number().int().nonnegative(),
  baseFileCheckpointId: nonEmptyStringSchema,
  resultFileCheckpointId: nonEmptyStringSchema.optional(),
});

export const kcodeSessionFileSchema = z.object({
  meta: kcodeTaskMetaSchema,
  messages: z.array(kcodePersistedMessageSchema),
  fileChanges: z.array(kcodePersistedFileChangeSchema).optional(),
  turnCheckpoints: z.array(kcodePersistedTurnCheckpointSchema).optional(),
});
