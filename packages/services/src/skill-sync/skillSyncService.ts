...
async function collectUserSkillCandidates(): Promise<SkillSyncCandidate[]> {
  ...
  const kcodeDirectoryNames = await collectCoveredSkillDirectoryNamesInRoot(userZcodeSkillRoot);
  const kcodeSkillNameKeys = new Set(
    rawZcodeCandidates.map((candidate) => normalizeSkillNameKey(candidate.name)),
  );
  const kcodeCandidates = await dedupeCandidatesByCanonicalSkillPath(
    rawZcodeCandidates,
    seenSkillRealpaths,
  );
  const agentsCandidates = await dedupeCandidatesByCanonicalSkillPath(
    (await collectUserSkillCandidatesInRoot(getUserAgentsSkillRoot())).filter(
      (candidate) =>
        !kcodeDirectoryNames.has(candidate.directoryName) &&
        !kcodeSkillNameKeys.has(normalizeSkillNameKey(candidate.name)),
    ),
    seenSkillRealpaths,
  );

  return [...kcodeCandidates, ...agentsCandidates].sort((left, right) =>
...
async function importArchive(
  ...
  try {
    ...
    // 远端 SkillsService 会同时读取用户级 .kcode/skills 和 .agents/skills。
    // 同名 skill 已在兼容目录存在时也必须跳过，避免同步后在 .kcode 下生成重复来源。
    const existingSkillPathByName = await collectUserSkillDirectoryPathByName();
