...
export interface HostResourceUsageAgent {
  ...
  children: readonly KCodeProcessChildProcess[];
}
...
type Owner =
  ...
  | { kind: "mcp"; child: KCodeProcessChildProcess }
...
function ownerToRow(
  ...
  if (owner.kind === "agent") {
    return {
      pid: sample.pid,
      name: isOwnerRoot
        ? formatKCodeAgentProcessName(owner.agent.provider, owner.agent.workspacePath)
