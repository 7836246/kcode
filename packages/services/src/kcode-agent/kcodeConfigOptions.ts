import type {
  ...
  ModelSelection,
  KCodeSessionMode,
  KCodeSessionSettingsState,
  KCodeSessionStateSnapshot,
  KCodeTaskModeInfo,
} from "@kcode/shared";
import {
  formatModelPickerValue as formatSharedModelSelection,
  getKCodeAgentAvailableModes as getSharedKCodeAgentAvailableModes,
  normalizeAvailableKCodeMode as normalizeSharedAvailableKCodeMode,
...
function resolveTaskMetaModelSelectionFromSnapshot(
  snapshot: Pick<KCodeSessionStateSnapshot, "messages" | "settings">,
): ModelSelection | undefined {
...
export function normalizeAvailableKCodeMode(mode: KCodeSessionMode): string {
  return normalizeSharedAvailableKCodeMode(mode);
}

export function getKCodeAgentAvailableModes(): KCodeTaskModeInfo[] {
  return getSharedKCodeAgentAvailableModes();
}

export function settingsToConfigOptions(settings: KCodeSessionSettingsState): KCodeConfigOption[] {
  // service 侧曾经维护了一份三项 mode 白名单，edit 模式上线后没有同步，
  // 切换模式时 mode_update 会把 UI 菜单覆盖成 build/plan/yolo。这里统一复用 shared 的事实源。
  return kcodeSessionSettingsToKCodeConfigOptions(settings);
}
