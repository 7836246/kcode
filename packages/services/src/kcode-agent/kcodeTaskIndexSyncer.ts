...
import { repairSubagentTaskIndex } from "#src/kcode-agent/repairSubagentTaskIndex.js";
import {
  deriveKCodeTaskStatusFromSessionSnapshot,
  generateTraceId,
  getKCodeUserVisibleMessages,
  isKCodeGoalContinuationReminderText,
  isKCodeModelOnlySyntheticUserMessage,
  resolveWorkspaceKey,
...
import { Emitter, type Event, type IDisposable } from "@kcode/rpc";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { KCodeWorkspaceEventSubscriptionParams } from "#src/session/kcodeTaskListTypes.js";
import type {
  IKCodeAgentService,
...
import { formatTaskMetaModelSelectionFromSnapshot } from "./kcodeConfigOptions.js";

const logger = createServiceLogger("kcode-task-index-syncer");
...
export interface KCodeTaskIndexTerminalEvent {
...
export interface KCodeTaskIndexReadyEvent {
  target: KCodeAgentSessionTarget;
  /** v4 phase 终态映射：agent 收口后可接受下一条输入的 ready 边界。 */
  reason: "prompt_completed" | "prompt_failed";
}

export interface KCodeTaskIndexSyncer {
  /**
   ...
   */
  ensureWorkspaceSubscription(target: KCodeAgentWorkspaceTarget): void;
  /**
   ...
   * desktop-continuous 路径上，kcodeSessionService 在 createSession/resumeSession/setModel
  ...
  syncSnapshotAndBroadcast(
    ...
    options: {
      ...
      unreadSignal?: KCodeWorkspaceTaskListChanged["unreadSignal"];
      /**
       ...
       */
      broadcastReason: KCodeWorkspaceTaskListChanged["reason"];
    },
   ...
   */
  syncTaskModel(target: KCodeAgentSessionTarget, model: string): Promise<KCodeTaskMeta | null>;
  /**
  ...
  emitWorkspaceTaskListChanged(
    ...
    meta: KCodeTaskMeta | undefined,
    reason: KCodeWorkspaceTaskListChanged["reason"],
    options?: Pick<KCodeWorkspaceTaskListChanged, "unreadSignal">,
  ): void;
  /**
...
): KCodeWorkspaceTaskListChanged["unreadSignal"] {
...
function taskStatusFromSummaryPhase(phase: SessionPhase): KCodeTaskMeta["status"] {
  switch (phase) {
    case "running":
    case "prewarming":
      return "running";
...
function buildBaselineMetaFromSummary(
  target: KCodeAgentWorkspaceTarget,
...
export function createKCodeTaskIndexSyncer(
  options: CreateKCodeTaskIndexSyncerOptions,
  ...
  const workspaceEmitters = new Map<string, Emitter<KCodeWorkspaceEvent>>();
  const terminalEventEmitter = new Emitter<KCodeTaskIndexTerminalEvent>();
  const readyEventEmitter = new Emitter<KCodeTaskIndexReadyEvent>();
  let disposed = false;
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<KCodeWorkspaceEvent> {
    ...
    if (!emitter) {
      emitter = new Emitter<KCodeWorkspaceEvent>();
  ...
  function broadcastTargetFrom(target: KCodeAgentSessionTarget): WorkspaceBroadcastTarget {
    return {
      workspacePath: target.workspacePath,
      workspaceIdentity: target.workspaceIdentity,
  ...
  function emitTerminalAndReady(target: KCodeAgentSessionTarget, summary: SessionSummary): void {
  ...
  function applyTerminalTransition(
    ...
    void taskIndexRepo
      ...
      .then((meta) => {
        ...
        // 不经过 kcodeSessionService 的 op 驱动 snapshot 同步，行缺失/正文搜索/lastError
  ...
  function applyTitleChange(target: KCodeAgentSessionTarget, title: string): void {
    const updatedAt = Date.now();
    void taskIndexRepo
      .applyAgentPatch({
        workspacePath: target.workspacePath,
      ...
      .then((meta) => {
        if (meta) {
          ...
          // v4 createSession 不经过 kcodeSessionService.createSession；首个标题到达且
  ...
  function processSummary(
    ...
    if (becameVisibleTask) {
      ...
      // 不经过 kcodeSessionService.createSession。此处是最早且不依赖标题时序的新任务边界；
  ...
  async function seedMissingRowsFromInitialSnapshot(
    ...
    // 纯 V4 UI 不经过 kcodeSessionService.initializeWorkspace；若把首帧
  ...
  function applyWorkspaceConfigFrame(
    ...
    if (!config || config.configOptions.length === 0) {
      // 空目录（无 live session 的订阅种子）不下发：下游 useKCodeConfig 收到空
      // configOptions 会把聊天工具栏的模型目录清掉。
      completeConfigRecoveryFrame(state, deliveryKind);
      return;
    }
    // v4 载荷与 KCodeConfigOption 结构对齐（shared 黄金测试背书），零映射直通，
    // 下游 workspace_config_options_update 消费面（useKCodeConfig 等）不改。
    getWorkspaceEmitter({
  ...
  function ensureWorkspaceSubscription(target: KCodeAgentWorkspaceTarget): void {
    if (disposed) {
      return;
    }
...
function hasUserVisibleContent(snapshot: KCodeSessionStateSnapshot): boolean {
  const title = snapshot.session.title?.trim() ?? "";
  if (title && !isKCodeGoalContinuationReminderText(title)) {
    return true;
  }
  ...
  return snapshot.messages.some((message) => {
    ...
    if (isKCodeModelOnlySyntheticUserMessage(message)) return false;
...
// 与 UI 默认折叠后展示的范围一致。
function buildSearchableTextFromSnapshot(snapshot: KCodeSessionStateSnapshot): string {
  const parts: string[] = [];
  let total = 0;
  // /goal 自动续跑会把内部 system-reminder 作为 runtime user turn 持久化；
  // 这条内容是 model-only 输入，不是用户可见 query，不能写进侧边栏搜索正文。
  for (const message of getKCodeUserVisibleMessages(snapshot.messages, {
    target: snapshot.projection.target,
  })) {
    const textParts = message.parts.filter(
      (part): part is Extract<KCodeMessagePart, { type: "text" }> => part.type === "text",
    );
...
function fromKCodeGoal(
  goal: NonNullable<KCodeSessionStateSnapshot["projection"]["target"]>,
): KCodeTaskGoal {
  return {
