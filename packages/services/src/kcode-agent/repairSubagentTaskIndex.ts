import { resolveWorkspaceKey, KCODE_AGENT_PROVIDER } from "@kcode/shared";
import type { TaskIndexRepo } from "#src/session/taskIndexRepo.js";
import type { IKCodeAgentService, KCodeAgentWorkspaceTarget } from "#src/kcode-agent/kcodeAgent.js";
...
export async function repairSubagentTaskIndex(params: {
  ...
  agentService: Pick<IKCodeAgentService, "listSessions">;
  taskIndexRepo: Pick<TaskIndexRepo, "listTaskMetas" | "updateTaskState">;
  isCurrent: () => boolean;
  onRemoved: () => void;
}): Promise<void> {
  const { target, taskIndexRepo, agentService, isCurrent } = params;
  const rows = await taskIndexRepo.listTaskMetas({ ...target, provider: KCODE_AGENT_PROVIDER });
  const ids = rows
