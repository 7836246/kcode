import { resolveKCodeAgentSpawnCwd } from "#src/kcode-agent/kcodeAgentSpawnCwd.js";
import type { KCodeAgentStorageStartupSnapshot } from "#src/kcode-agent/kcodeAgent.js";
/* eslint-disable max-lines -- kcodeAgentProcessManager 集中维护 agent 子进程启动、复用、超时回收和 runtime identity，拆分会扩大进程生命周期状态同步面 */
...
import {
  parseKCodeProcessDiagnostic,
  KCODE_AGENT_LIFECYCLE_LOG_MARKER,
  KCODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS,
  KCODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS,
  KCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS,
} from "@kcode/shared/process-diagnostic";
import {
  KCODE_AGENT_RUNTIME,
  KCODE_AGENT_PROVIDER,
  KCODE_RUNTIME_ENV_KEY,
  resolveWorkspaceKey,
  resolveKCodeRuntimeEnv,
  sanitizeKCodeRuntimeEnv,
} from "@kcode/shared";
import {
  findKCodeAgentRuntimeBinary,
  findKCodeAgentRuntimeNodeBundle,
...
import { KCodeProtocolClient } from "./kcodeProtocolClient.js";
import { KCodeStdioTransport } from "./kcodeStdioTransport.js";
import { readKCodeStdioTapDevState } from "./kcodeStdioTapDevConfig.js";
import type { KCodeAgentPresentationSurface } from "./kcodeAgentPresentationSurface.js";
...
export interface KCodeAgentCommand {
...
export interface KCodeAgentCommandResolverContext {
  presentationSurface?: KCodeAgentPresentationSurface;
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
}

export type KCodeAgentCommandResolver = (
  context: KCodeAgentCommandResolverContext,
) => Promise<KCodeAgentCommand | null> | KCodeAgentCommand | null;

export interface KCodeAgentProcessManagerOptions {
  commandResolver?: KCodeAgentCommandResolver;
  presentationSurface?: KCodeAgentPresentationSurface;
...
interface ManagedKCodeAgentProcess {
  ...
  runtimeIdentity: KCodeAgentRuntimeIdentity;
...
function buildE2EAgentCoverageEnv(env: NodeJS.ProcessEnv = process.env): Record<string, string> {
  const artifactDir = env.KCODE_E2E_ARTIFACT_DIR?.trim();
  if (env.KCODE_E2E_COVERAGE !== "1" || !artifactDir) {
    return {};
  }
  const directory = resolve(artifactDir, "coverage", "raw", "cli");
  mkdirSync(directory, { recursive: true });
  const preloadPath = resolve(directory, "kcode-e2e-coverage-preload.cjs");
 ...
 */
interface KCodeAgentRuntimeRestartedEvent {
  workspaceKey: string;
  runtimeIdentity: KCodeAgentRuntimeIdentity;
}

export interface KCodeAgentRuntimeLifecycleEvent {
  workspacePath: string;
  workspaceIdentity?: string;
  workspaceKey: string;
  runtimeIdentity: KCodeAgentRuntimeIdentity;
...
interface KCodeAgentRuntimeIdentity {
  generation: number;
  identity: string;
  processId?: number;
  workspaceKey: string;
...
interface KCodeAgentSpawnPreflight {
  command: string;
  args: string[];
  requestedCwd: string;
  cwd: string;
...
const serviceLog = createServiceLogger("kcode-agent");
...
function parseArgsJson(raw: string | undefined): string[] | undefined {
  ...
  if (!Array.isArray(parsed) || parsed.some((item) => typeof item !== "string")) {
    throw new Error("KCODE_AGENT_SERVER_ARGS_JSON must be a JSON string array");
  }
  return parsed;
}
...
async function buildKCodeAgentSpawnPreflight(
  command: KCodeAgentCommand,
  workspacePath: string,
  spawnFallbackCwd?: string,
): Promise<KCodeAgentSpawnPreflight> {
  ...
  const {
    ...
    cwdExists,
  } = await resolveKCodeAgentSpawnCwd({ requestedCwd, workspacePath, spawnFallbackCwd });
  return {
...
function resolveBundledWorkspaceKCodeAgentCommand(
  context: KCodeAgentCommandResolverContext,
): KCodeAgentCommand | null {
  const distEntrypoint = findUpward("apps/kcode-cli/packages/cli/dist/kcode.cjs");
  if (distEntrypoint) {
    const useBytecode =
      process.versions.electron && process.env.KCODE_DESKTOP_AGENT_BYTECODE === "1";
    const entrypoint = useBytecode
      ? join(dirname(distEntrypoint), "kcode.bytecode.cjs")
      : distEntrypoint;
    ...
    return {
      ...
      // 这里显式启用 Node 运行模式，避免内置 kcode-agent 被当成 Electron/Chromium 子进程启动并卡在 GPU 初始化。
      env: { ELECTRON_RUN_AS_NODE: "1" },
    };
  }

  const sourceEntrypoint = findUpward("apps/kcode-cli/packages/cli/src/main.ts");
...
function resolveDeployedKCodeAgentBinaryCommand(
  context: KCodeAgentCommandResolverContext,
): KCodeAgentCommand | null {
  // 旧 resolver 只识别 KCODE_AGENT_SERVER_COMMAND env 和 monorepo 源码树。
  // SSH 远端把 kcode-server.cjs 单文件部署到 ~/.kcode/server/，宿主进程的 cwd 不在仓库内、
  // env 也不会被 ssh exec 继承，即使 kcode-agent 已经部署到 ~/.kcode/server/agents/glm/，
  // resolver 也找不到，第一次 getClient 就抛 "KCode agent server command is not configured"。
  // 这里复用 findKCodeAgentRuntimeBinary 的候选链（含 GLM_BINARY_PATH env、
  // packagedResourcesPath、~/.kcode/server/agents/glm、bundled-agents 等），
  // 把已部署的原生 binary 当成最终兜底，远端/桌面打包形态都能命中。
  const binaryPath = findKCodeAgentRuntimeBinary();
  if (!binaryPath) {
    return null;
  }
  return {
    command: binaryPath,
    args: KCODE_AGENT_RUNTIME.spawnArgs,
    cwd: context.workspacePath,
  };
}

function resolveElectronRuntimeKCodeAgentCommand(
  context: KCodeAgentCommandResolverContext,
): KCodeAgentCommand | null {
  // 桌面打包态：host 跑在 Electron utility process 里，process.execPath 指向 Electron Helper，
  // 它内置的 Node runtime 与 kcode-cli 目标版本一致（Electron 41 = Node 24.x）。
  // 这里直接用 app 自带的 Electron Node 执行打进 resources/glm 的 kcode.cjs，
  ...
  const bundlePath = findKCodeAgentRuntimeNodeBundle();
  if (!bundlePath) {
    return null;
  }
  return {
    command: process.execPath,
    args: [bundlePath, ...KCODE_AGENT_RUNTIME.spawnArgs],
...
export function resolveDefaultKCodeAgentCommand(
  context: KCodeAgentCommandResolverContext,
): KCodeAgentCommand | null {
  const command = process.env.KCODE_AGENT_SERVER_COMMAND?.trim();
  if (command) {
    return applyPresentationSurfaceToCommand(
      {
        command,
        args: parseArgsJson(process.env.KCODE_AGENT_SERVER_ARGS_JSON) ?? ["app-server", "--stdio"],
        cwd: process.env.KCODE_AGENT_SERVER_CWD?.trim() || context.workspacePath,
      },
  ...
  // 抢先匹配）→ 桌面打包态 Electron Node runtime 跑 kcode.cjs → 已部署 native binary（远端 SSH 兜底）。
  const bundled =
    resolveBundledWorkspaceKCodeAgentCommand(context) ??
    resolveElectronRuntimeKCodeAgentCommand(context);
  return applyPresentationSurfaceToCommand(
    bundled
      ? { ...bundled, supportsStorageStartup: true }
      : resolveDeployedKCodeAgentBinaryCommand(context),
    context.presentationSurface,
  );
}

function applyPresentationSurfaceToCommand(
  command: KCodeAgentCommand | null,
  presentationSurface: KCodeAgentCommandResolverContext["presentationSurface"],
): KCodeAgentCommand | null {
...
function wrapKCodeAgentCommandWithStdioTapDevProxy(
  command: KCodeAgentCommand,
  workspaceKey: string,
): KCodeAgentCommand {
  const tapState = readKCodeStdioTapDevState();
  if (!tapState.enabled) {
    return command;
  }

  const tapScript = findUpward("scripts/dev/kcode-stdio-tap.mjs");
  if (!tapScript) {
    debugLog("KCode stdio tap proxy enabled but script not found");
    return command;
  }
...
export class KCodeAgentProcessManager {
  private readonly processesByWorkspaceKey = new Map<string, ManagedKCodeAgentProcess>();
  private readonly ownedProcesses = new Set<ManagedKCodeAgentProcess>();
  private readonly startingByWorkspaceKey = new Map<string, Promise<KCodeProtocolClient>>();
  ...
  private readonly storageStartupEmitter = new Emitter<{
    workspaceKey: string;
    snapshot: KCodeAgentStorageStartupSnapshot;
  }>();
  ...
  getStorageStartupState(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): KCodeAgentStorageStartupSnapshot | null {
  ...
  private readonly commandResolver: KCodeAgentCommandResolver;
  private readonly presentationSurface: KCodeAgentProcessManagerOptions["presentationSurface"];
  private readonly requestTimeoutMs: number | undefined;
  private readonly processLifecycleReporter: RuntimeProcessLifecycleReporter | undefined;
  private readonly resolveSpawnEnv: KCodeAgentProcessManagerOptions["resolveSpawnEnv"];
  private readonly waitForSpawnAdmission: KCodeAgentProcessManagerOptions["waitForSpawnAdmission"];
  ...
  private readonly runtimeRestartedEmitter = new Emitter<KCodeAgentRuntimeRestartedEvent>();
  private readonly runtimeLifecycleEmitter = new Emitter<KCodeAgentRuntimeLifecycleEvent>();
  private disposeAllInFlight: Promise<void> | undefined;
  private disposed = false;

  /** 进程换代通知（generation>1 时触发）；v4 订阅方据此重订，见 KCodeAgentRuntimeRestartedEvent。 */
  ...
  constructor(options?: KCodeAgentProcessManagerOptions) {
    this.commandResolver = options?.commandResolver ?? resolveDefaultKCodeAgentCommand;
  ...
  private reportProcessLifecycle(
    ...
    try {
      ...
      warnLog("KCode agent process lifecycle reporter failed", error);
    }
  }

  private clearIdleTimer(managed: ManagedKCodeAgentProcess): void {
   ...
   */
  private scheduleIdleReclaim(workspaceKey: string, managed: ManagedKCodeAgentProcess): void {
    ...
    const timer = setTimeout(() => {
      ...
      log("KCode agent process idle timeout; reclaiming", {
        workspaceKey,
  ...
  private recordTerminationIntent(
    managed: ManagedKCodeAgentProcess,
  ...
  private reportRuntimeUnavailable(managed: ManagedKCodeAgentProcess): void {
  ...
  private reportRuntimeReady(managed: ManagedKCodeAgentProcess): void {
    if (
      !managed.spawned ||
      managed.exited ||
  ...
  private cleanupManagedProcess(
    ...
    const cleanupPromise = managed.client
      ...
      .then(() => {
        ...
        log("KCode agent process cleanup completed", {
    ...
    if (options.reportError !== false) {
      void cleanupPromise.catch((error) => {
        errorLog("KCode agent process cleanup failed", {
          workspaceKey: managed.runtimeIdentity.workspaceKey,
          pid: managed.child.pid,
          runtimeIdentity: managed.runtimeIdentity.identity,
          reason,
  ...
  private async cleanupManagedProcessForShutdown(managed: ManagedKCodeAgentProcess): Promise<void> {
  ...
  private async cleanupManagedProcessWithRetry(
    ...
    try {
      ...
      warnLog(`KCode agent process cleanup retrying during ${retryScope}`, {
        workspaceKey: managed.runtimeIdentity.workspaceKey,
        pid: managed.child.pid,
        runtimeIdentity: managed.runtimeIdentity.identity,
        cleanupStage: reason,
  ...
  async getClient(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KCodeProtocolClient> {
    if (this.disposed) {
      throw new Error("KCode agent process manager is disposed.");
    }
    ...
    if (starting) {
      ...
      log("KCode agent process start already in progress", {
        workspaceKey,
      });
      const client = await starting;
      log("KCode agent process start wait completed", {
        workspaceKey,
  ...
  private async startClient(
    ...
    if (!command) {
      throw new Error(
        "KCode agent server command is not configured. Set KCODE_AGENT_SERVER_COMMAND before integration.",
      );
    }
    if (admissionSignal.aborted) {
      throw admissionSignal.reason ?? new Error("KCode agent process start was cancelled.");
    }
    const effectiveCommand = wrapKCodeAgentCommandWithStdioTapDevProxy(command, workspaceKey);
    log("KCode agent command resolved", {
      workspaceKey,
      command: command.command,
    ...
    if ((this.restartGenerationByWorkspaceKey.get(workspaceKey) ?? 0) !== startGeneration) {
      ...
      throw new Error("KCode agent process start was cancelled.");
    }
    // cwd 探测也让出事件循环，必须放在最终 admission 与销毁/代际检查之前。
    const spawnPreflight = await buildKCodeAgentSpawnPreflight(
      effectiveCommand,
    ...
    // app 以本地开发方式启动时，让 agent 子进程也带上 KCODE_RUNTIME_ENV=development；
    // 不再传 NODE_ENV，避免用户 shell/runtime 变量影响 KCode 运行模式或泄漏到 Bash 工具。
    const runtimeEnv = resolveKCodeRuntimeEnv(process.env);
    log("KCode agent spawn preflight", {
      workspaceKey,
      spawnPreflight,
    });
    ...
    const child = spawn(effectiveCommand.command, spawnPreflight.args, {
      ...
      env: {
        ...sanitizeKCodeRuntimeEnv(process.env),
        [KCODE_RUNTIME_ENV_KEY]: runtimeEnv,
        ...spawnEnv,
    ...
    const transport = new KCodeStdioTransport(child, {
      onStderrLine: (line) => {
        const diagnostic = parseKCodeProcessDiagnostic(line);
    ...
    const client = new KCodeProtocolClient(transport, {
    ...
    const runtimeIdentity: KCodeAgentRuntimeIdentity = {
    ...
    const managed: ManagedKCodeAgentProcess = {
      child,
      client,
      exited: false,
      readyReported: false,
    ...
    child.once("spawn", () => {
      ...
      this.reportRuntimeReady(managed);
      log("KCode agent process started", {
        workspaceKey,
        command: effectiveCommand.command,
        cwd: spawnPreflight.cwd,
        pid: child.pid,
    ...
    child.once("error", (error) => {
      errorLog(
        `KCode agent process error${this.processLifecycleReporter?.onError ? ` ${KCODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
        {
          workspaceKey,
          pid: child.pid,
    ...
    child.once("exit", async (code, signal) => {
      ...
      log("KCode agent process exited", exitContext);
      if (terminationKind === "unexpected") {
        ...
        errorLog(
          `KCode agent process exited unexpectedly${this.processLifecycleReporter ? ` ${KCODE_AGENT_LIFECYCLE_LOG_MARKER}` : ""}`,
          {
            ...exitContext,
            stderr,
          },
    ...
    client.onRequestTimeout((event) => {
      ...
      if (event.method === "workspace/cancelGenerateText") {
        warnLog(
          "KCode agent cancel notification timed out; keeping client (best-effort control plane)",
          {
            workspaceKey,
            method: event.method,
      ...
      warnLog("KCode agent request timed out; disposing stale protocol client", {
        workspaceKey,
        method: event.method,
        requestId: event.requestId,
        timeoutMs: event.timeoutMs,
    ...
    client.onClose(() => {
      ...
      log("KCode agent protocol client closed", {
        workspaceKey,
        pid: child.pid,
        runtimeIdentity: runtimeIdentity.identity,
        wasActiveClient,
  ...
  async getRuntimeIdentity(params: {
    workspacePath: string;
    workspaceIdentity?: string;
  }): Promise<KCodeAgentRuntimeIdentity> {
    const workspaceKey = resolveWorkspaceKey(params);
    ...
    if (!managed || managed.exited || managed.child.killed) {
      throw new Error("KCode agent runtime identity is unavailable.");
    }
    ...
    try {
      ...
      return command
        ...
        : {
            available: false,
            workspaceKey,
            reason: "KCODE_AGENT_SERVER_COMMAND is not configured",
          };
    } catch (error) {
      return {
        available: false,
  ...
  private abortPendingStarts(
    workspaceKey: string,
    reason = new Error("KCode agent process start was cancelled."),
  ): void {
  ...
  private abortAllPendingStarts(
    reason = new Error("KCode agent process manager is disposed."),
  ): void {
  ...
  async disposeAllAndWait(): Promise<void> {
    ...
    // 依赖 unref timer，host 自己退出后 timer 不会再执行，kcode-cli 会残留为孤儿进程。
