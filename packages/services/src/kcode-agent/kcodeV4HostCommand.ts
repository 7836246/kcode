...
class KCodeV4CommandRejectedError extends Error {
  readonly code = "KCODE_V4_COMMAND_REJECTED";
  ...
  constructor(
    ...
    this.name = "KCodeV4CommandRejectedError";
  }
}
...
export function assertV4CommandAckOk(
  ...
  throw new KCodeV4CommandRejectedError(commandType, ack, contextMessage);
}
...
export async function sendHostCasCommandV4<T extends CommandType>(input: {
  ...
  throw new KCodeV4CommandRejectedError(
    input.type,
    lastAck ?? {
      commandId: "",
      status: "stale",
