import { KCodeStorageStartupGate } from "#src/kcode-agent/kcodeStorageStartupGate.js";
...
import type { V4Method } from "@kcode/shared/kcode-protocol-v4";
import type { z } from "zod";
import type { KCodeProtocolTransport } from "./kcodeProtocolTransport.js";

/** 客户端可发的方法名：旧 kcodeProtocolMethods + v4/*（并存，收敛为 v4）。 */
type KCodeProtocolClientMethod = KCodeProtocolMethod | V4Method;

interface KCodeProtocolClientOptions {
  requireStorageStartup?: boolean;
  requestTimeoutMs?: number;
}

interface KCodeProtocolRequestTimeoutEvent {
  method: KCodeProtocolClientMethod;
  requestId: KCodeProtocolRequestId;
  timeoutMs: number;
}
...
interface KCodeProtocolClientRequestOptions {
...
const DEFAULT_KCODE_PROTOCOL_REQUEST_TIMEOUT_MS = 3 * 60_000;

class KCodeProtocolClientError extends Error {
  constructor(
    message: string,
    ...
    super(message);
    this.name = "KCodeProtocolClientError";
  }
}

export class KCodeProtocolRequestTimeoutError extends Error {
  constructor(
    readonly method: KCodeProtocolClientMethod,
    readonly requestId: KCodeProtocolRequestId,
    readonly timeoutMs: number,
  ) {
    super(`KCode Protocol request timed out: ${method}`);
    this.name = "KCodeProtocolRequestTimeoutError";
  }
}

export class KCodeProtocolClient implements IDisposable {
  readonly storageStartup: KCodeStorageStartupGate;
  private readonly pending = new Map<string, PendingRequest<unknown>>();
  private readonly notificationEmitter = new Emitter<KCodeProtocolNotification>();
  private readonly requestEmitter = new Emitter<KCodeProtocolRequest>();
  private readonly requestTimeoutEmitter = new Emitter<KCodeProtocolRequestTimeoutEvent>();
   ...
   * 否则会立即抛 "KCode Protocol client is disposed"。
   */
  ...
  constructor(
    private readonly transport: KCodeProtocolTransport,
    options?: KCodeProtocolClientOptions,
  ) {
    this.storageStartup = new KCodeStorageStartupGate(options?.requireStorageStartup ?? false);
    this.requestTimeoutMs = options?.requestTimeoutMs ?? DEFAULT_KCODE_PROTOCOL_REQUEST_TIMEOUT_MS;
    this.disposables.push(
      transport.onMessage((message) => this.handleMessage(message)),
      transport.onClose((event) => {
        this.storageStartup.dispose();
        const suffix = event.reason ? `: ${event.reason}` : "";
        this.rejectAll(new Error(`KCode agent transport closed${suffix}`));
  ...
  async request<T = unknown>(
    ...
    const resultPromise = new Promise<T>((resolve, reject) => {
      const expire = () => {
        ...
        const error = new KCodeProtocolRequestTimeoutError(method, id, requestTimeoutMs);
  ...
  async notify(method: KCodeProtocolClientMethod, params?: unknown): Promise<void> {
    this.assertNotDisposed();
    await this.transport.send({ method, params });
  }

  async respond(id: KCodeProtocolRequestId, result: unknown): Promise<void> {
    this.assertNotDisposed();
  ...
  private handleMessage(message: unknown): void {
    ...
    if ("result" in message && "id" in message) {
      this.resolveResponse(
        (message as { id: KCodeProtocolRequestId; result: unknown }).id,
    ...
    if ("method" in message && "id" in message) {
      this.requestEmitter.fire(message as KCodeProtocolRequest);
      return;
    }
    ...
    if ("method" in message) {
      if (
        ...
        this.storageStartup.accept((message as KCodeProtocolNotification).params)
      ) {
      ...
      this.notificationEmitter.fire(message as KCodeProtocolNotification);
    }
  }

  private resolveResponse(id: KCodeProtocolRequestId, result: unknown): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
    ...
    try {
      ...
      pending.reject(
        error instanceof Error
          ? error
          : new Error(`KCode Protocol response parse failed: ${pending.method}`),
      );
    }
  }

  private rejectResponse(id: KCodeProtocolRequestId, error: Error): void {
    const requestKey = String(id);
    const pending = this.pending.get(requestKey);
    if (!pending) {
      return;
  ...
  private disposeLocalResources(): void {
    this.rejectAll(new Error("KCode Protocol client disposed"));
  ...
  private assertNotDisposed(): void {
    if (this.disposed) {
      throw new Error("KCode Protocol client is disposed");
    }
  }
}
