import type { KCodeSessionEvent } from "@kcode/shared";
import type { KCodeAgentServiceEvent } from "#src/kcode-agent/kcodeAgent.js";
...
type SessionServiceEvent = Extract<KCodeAgentServiceEvent, { type: "session.event" }>;
...
interface BackgroundSessionEventCoalescer {
  accept(event: KCodeAgentServiceEvent): void;
  flush(): void;
  dispose(): void;
}
...
function getBackgroundSessionEventCoalesceKey(event: KCodeSessionEvent): string | null {
...
function mergeBackgroundSessionEvents(
  current: KCodeSessionEvent,
  next: KCodeSessionEvent,
): KCodeSessionEvent {
...
export function createBackgroundSessionEventCoalescer(params: {
  emit: (event: KCodeAgentServiceEvent) => void;
  flushDelayMs?: number;
  maxItems?: number;
