...
import { Emitter, Event as RpcEvent, type Event, type IDisposable } from "@kcode/rpc";
import {
...
export type KCodeAgentV4ClientMode = "desktop-continuous" | "web-remote-replayable";

export interface KCodeAgentV4ConnectionContext {
  connectionId: string;
  clientMode: KCodeAgentV4ClientMode;
...
const TRUSTED_CONNECTION_FIELD = "__kcodeTrustedV4Connection";
const TRUSTED_UNSUBSCRIBE_ROUTE_FIELD = "__kcodeTrustedV4UnsubscribeRoute";

type TrustedConnectionCarrier = {
  [TRUSTED_CONNECTION_FIELD]?: KCodeAgentV4ConnectionContext;
};

interface TrustedKCodeAgentV4UnsubscribeRoute {
  topic: string;
  connectionId: string;
}

type TrustedUnsubscribeRouteCarrier = {
  [TRUSTED_UNSUBSCRIBE_ROUTE_FIELD]?: TrustedKCodeAgentV4UnsubscribeRoute;
};

export function readTrustedKCodeAgentV4UnsubscribeRoute(
  value: unknown,
): TrustedKCodeAgentV4UnsubscribeRoute | null {
 ...
 */
export function readTrustedKCodeAgentV4Connection(
  value: unknown,
): KCodeAgentV4ConnectionContext | null {
  if (typeof value !== "object" || value === null) return null;
...
function withTrustedConnection<T extends object>(
  value: T,
  context: KCodeAgentV4ConnectionContext,
): T {
...
function withTrustedUnsubscribeRoute<T extends object>(
  value: T,
  route: TrustedKCodeAgentV4UnsubscribeRoute,
): T {
  const forwarded = {
    ...(value as unknown as Record<string, unknown>),
  };
...
function workspaceKey(target: KCodeAgentWorkspaceTarget): string {
  return target.workspaceIdentity?.trim() || target.workspacePath;
}

function workspaceTarget(target: KCodeAgentWorkspaceTarget): KCodeAgentWorkspaceTarget {
  return {
...
interface ConnectionFlowRoute {
  key: string;
  target: KCodeAgentWorkspaceTarget;
  connection: KCodeAgentV4ConnectionContext;
}
...
function deliveryProfileFor(clientMode: KCodeAgentV4ClientMode): HelloMessage["deliveryProfile"] {
  return clientMode === "desktop-continuous" ? "continuous" : "replayable";
}

function createHello(context: KCodeAgentV4ConnectionContext): HelloMessage {
...
export interface KCodeAgentConnectionScope {
  readonly service: IKCodeAgentService;
...
export function createKCodeAgentConnectionScope(
  base: IKCodeAgentService,
  context: KCodeAgentV4ConnectionContext,
  ...
  const forwardedConnection = (params: unknown): KCodeAgentV4ConnectionContext => {
    const downstream = readTrustedKCodeAgentV4Connection(params);
  ...
  const ownershipKey = (kind: SubscriptionKind, target: KCodeAgentWorkspaceTarget, topic: string) =>
    `${kind}\0${workspaceKey(target)}\0${topic}`;
  const routedEventKey = (kind: SubscriptionKind, target: KCodeAgentWorkspaceTarget) =>
    `${kind}\0${workspaceKey(target)}`;
  const flowRouteKey = (target: KCodeAgentWorkspaceTarget, connectionId: string) =>
  ...
  const flowRouteForEntry = (entry: OwnedSubscription): ConnectionFlowRoute => ({
    ...
    } satisfies KCodeAgentV4ConnectionContext,
  });
  ...
  const findBySubscription = (
    ...
    target: KCodeAgentWorkspaceTarget,
    subscriptionId: string,
    expectedRoute: TrustedKCodeAgentV4UnsubscribeRoute | null,
  ...
  const forwardedUnsubscribeRoute = (
    ...
    const downstream = readTrustedKCodeAgentV4UnsubscribeRoute(params);
  ...
  const unsubscribeBase = (
    ...
    const params: KCodeAgentConversationUnsubscribeParams = {
      ...entry.target,
  ...
  const resyncBase = (entry: OwnedSubscription, params: KCodeAgentConversationResyncParams) => {
    const forwarded = withTrustedUnsubscribeRoute(params, {
      topic: entry.topic,
      connectionId: entry.connectionId,
    });
  ...
  const resyncOwned = (kind: SubscriptionKind, params: KCodeAgentConversationResyncParams) => {
    assertReady();
  ...
  const overrides: Partial<IKCodeAgentService> = {
