/* oxlint-disable eslint(max-lines) -- 迁移期需要在一个门面里集中维护旧 task projection 到 KCode session 的协议适配。 */
...
import {
  coalesceConsecutiveKCodeAssistants,
  createSessionTraceId,
  decodeCustomModelValue,
  deriveKCodeTaskStatusFromSessionSnapshot,
  extractPlanStepsFromToolInput,
  extractPlanStepsFromToolOutput,
  generateTraceId,
  getKCodeGoalActiveIterationCount,
  getKCodeGoalIterationByAssistantMessageId,
  getKCodeUserVisibleMessages,
  isMainAgentToolProjectionSource,
  normalizeKCodeApiRetryStatus,
  attachKCodeBackgroundTaskNotificationToRaw,
  collectKCodeBackgroundTaskNotificationsByToolUseId,
  mergeKCodeBackgroundTaskControlItems,
  parseKCodeBackgroundTaskControlItems,
  parseKCodeBackgroundTaskNotificationText,
  parseModelPickerValue as parseSharedModelSelection,
  resolveWorkspaceKey,
  resolveKCodeVisibleSessionTitle,
  textFromKCodeMessageParts,
  KCODE_AGENT_PROVIDER,
  kcodeBackgroundTaskNotificationToolUpdateStatus,
  appendKCodeStreamingToolInputDelta,
  buildKCodeStreamingToolInputPreview,
  createKCodeToolProjectionMemory,
  finalizeKCodeToolProjectionInput,
  forgetKCodeToolProjectionMetadata,
  isKCodeModelRetryRecoveryProgressPayload,
  markKCodeStreamingToolInputPreviewMaterialized,
  resolveKCodeToolProjectionMetadata,
  kcodeApiRetryFromModelNetworkStatusPayload,
  kcodeApiRetryFromStreamRecoveryPayload,
  kcodeTaskNetworkDebugStatusFromPayload,
  shouldMaterializeKCodeStreamingToolInputPreview,
  type KCodeApiRetryStatus,
  type KCodeAssistantMessageFeedback,
  ...
  type KCodeError,
  type KCodeGoalVerificationTimelineMeta,
  type KCodeImportSessionsResult,
  type KCodeImportableSessionCandidate,
  type KCodeUsage,
  type KCodePersistedMessage,
  type KCodePersistedMessagePart,
  type KCodePersistedToolCall,
  type KCodePromptAttachment,
  type KCodeProvider,
  type KCodeSessionFile,
  type KCodeTaskGoal,
  type KCodeTaskGoalStats,
  type KCodeTaskMode,
  type KCodePlanStep,
  type KCodeSlashCommand,
  type KCodeStreamEvent,
  type KCodeTaskCreateResult,
  type KCodeTaskMeta,
  type KCodeTaskClientMode,
  type KCodeTaskRuntimeCommand,
  type KCodeTaskSnapshot,
  type KCodeTaskSnapshotBody,
  type KCodeTaskSnapshotRefContent,
  type KCodeTaskSnapshotToolCallsSlice,
  type KCodeTaskTokenUsageResult,
  type KCodeTodoGroup,
  type KCodeTurnSteerCommandKind,
  type KCodeTurnSteerSource,
  type KCodeWorkspaceEvent,
  type KCodeWorkspaceTaskListChanged,
  type InputId,
  type TraceId,
  kcodeContextUsageBreakdownSchema,
  kcodeSessionSettingsStateSchema,
  type KCodeDeliveryKind,
  type KCodeMessagePart,
  type KCodeMessageWithParts,
  type ModelSelection,
  type KCodePermissionOption,
  type KCodePermissionRequestParams,
  type KCodePermissionRequest,
  type KCodeSessionEvent,
  type KCodeSessionMode,
  type KCodeSessionSettingsState,
  type KCodeSessionStateSnapshot,
  type KCodeStateUpdatedNotification,
  type KCodeContextCompactionTimelineMeta,
  type KCodeTimelineMeta,
  type KCodeTimelineStatus,
  type KCodeTimelineTrigger,
  type KCodeToolProjectionMemory,
  type KCodeUserInputRequestParams,
  type KCodeUserInputResponse,
  type KCodeAgentMcpServer,
} from "@kcode/shared";
import type {
  KCodeTaskListQuery,
  KCodeTaskListResult,
  KCodeWorkspaceEventSubscriptionParams,
  IKCodeTaskService,
  KCodeArchivedTaskDeletionResult,
  KCodeTaskReadyOutcome,
  KCodeTaskTerminalOutcome,
...
import type {
  KCodeTaskIndexReadyEvent,
  KCodeTaskIndexSyncer,
  KCodeTaskIndexTerminalEvent,
} from "./kcodeTaskIndexSyncer.js";
import { readModelTrajectory } from "./modelTrajectory.js";
import { errorAttributionSchema, type CommandPayloadMap } from "@kcode/shared/kcode-protocol-v4";
import {
  assertV4CommandAckOk,
  createHostCommandEnvelope,
  sendHostCasCommandV4,
} from "./kcodeV4HostCommand.js";
...
import {
  ...
  normalizeAvailableKCodeMode,
  settingsToConfigOptions,
} from "./kcodeConfigOptions.js";
...
interface CreateKCodeTaskServiceAdapterOptions {
  kcodeAgentService: IKCodeAgentService;
  ...
  taskIndexSyncer: KCodeTaskIndexSyncer;
...
type WorkspaceEventInput = string | KCodeWorkspaceEventSubscriptionParams;
type KCodeSendPromptRuntimeCommand = Extract<KCodeTaskRuntimeCommand, { type: "send_prompt" }>;
type KCodeTerminalStreamEvent =
  | Extract<KCodeStreamEvent, { type: "task_complete" }>
  | Extract<KCodeStreamEvent, { type: "task_error" }>;

const GLM_PROVIDER: KCodeProvider = KCODE_AGENT_PROVIDER;
const EMPTY_SLASH_COMMANDS: KCodeSlashCommand[] = [];
const logger = createServiceLogger("kcode-task-service");
...
function formatKCodeAgentLogDate(now: Date): string {
  return [now.getFullYear(), padDatePart(now.getMonth() + 1), padDatePart(now.getDate())].join("-");
}

function resolveKCodeAgentCurrentLogFilePath(now = new Date()): string {
  const configuredLogDir = process.env.KCODE_LOG_DIR?.trim();
  const logDir = configuredLogDir || join(homedir(), ".kcode", "cli", "log");
  return join(logDir, `kcode-${formatKCodeAgentLogDate(now)}.jsonl`);
}

export function createKCodeTaskServiceAdapter(
  options: CreateKCodeTaskServiceAdapterOptions,
): IKCodeTaskService {
  const errorEmitter = new Emitter<KCodeError>();
  const taskEmitters = new Map<string, Emitter<KCodeStreamEvent>>();
  const globalTaskEmitters = new Map<string, Emitter<KCodeStreamEvent>>();
  const overlays = new Map<string, TaskOverlay>();
  const taskTargets = new Map<string, TaskTarget>();
  const runtimeCommands = new Map<string, KCodeTaskRuntimeCommand[]>();
  const runtimeCommandDrains = new Map<string, Promise<void>>();
  const apiRetryByTaskKey = new Map<string, KCodeApiRetryStatus | null>();
  const backgroundTaskControlsByTaskKey = new Map<string, KCodeBackgroundTaskControlItem[]>();
  const streamedTurnKeys = new Set<string>();
  const activePromptInputIds = new Map<string, InputId>();
  const toolProjectionMemoryByTaskKey = new Map<string, KCodeToolProjectionMemory>();
  ...
  function unsupported(name: string): never {
    throw Object.assign(
      new Error(`KCode task service adapter does not support IKCodeTaskService.${name} yet.`),
      {
        code: "KCODE_AGENT_UNSUPPORTED_LEGACY_TASK_METHOD",
      },
    );
  }
  ...
  function turnAttributionOf(
    params: KCodeBackgroundTurnAttribution,
  ): KCodeBackgroundTurnAttribution {
  ...
  async function resolveProductMcpServers(
    servers: KCodeAgentMcpServer[] | undefined,
  ): Promise<KCodeAgentMcpServer[] | undefined> {
  ...
  async function sendPromptToAgent(
    ...
    // KCode task wrapper 的字段仍叫 traceId，但这里语义已经是单次输入 inputId。
    // 先记录 inputId，后续 KCode session 事件回投 KCode Agent 时才能让 UI 终态按输入轮次收口。
    activePromptInputIds.set(taskKey(target), params.traceId);
    logger.info(params.traceId, "KCode task facade sendPrompt 开始", {
    ...
    try {
      ...
      if (params.attachments?.length) {
        ...
        await options.kcodeAgentService.sendPrompt({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
        ...
        const ack = await options.kcodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          ...(target.remoteSessionId ? { remoteSessionId: target.remoteSessionId } : {}),
          ...(params.clientMode ? { clientMode: params.clientMode } : {}),
      ...
      logger.info(params.traceId, "KCode task facade sendPrompt ACK", {
      ...
      logger.warn(params.traceId, "KCode task facade sendPrompt 失败", {
        durationMs: Date.now() - startedAt,
  ...
  function setRuntimeCommands(params: TaskTarget, commands: KCodeTaskRuntimeCommand[]): void {
    const key = taskKey(params);
  ...
  function markRuntimeCommandRunning(
    params: TaskTarget,
    command: KCodeSendPromptRuntimeCommand,
  ): KCodeSendPromptRuntimeCommand {
    const key = taskKey(params);
    const runningCommand: KCodeSendPromptRuntimeCommand = {
      ...command,
      status: "running",
      updatedAt: Date.now(),
    };
  ...
  function markRuntimeCommandFailed(
    ...
    const key = taskKey(params);
    const failedCommand: KCodeSendPromptRuntimeCommand = {
      ...command,
      status: "failed",
      updatedAt: Date.now(),
      error: error instanceof Error ? error.message : String(error),
  ...
  function completeRuntimeCommandByInputId(
    ...
    logger.info(command.traceId, "KCode task command 终态收口", {
  ...
  function completeRuntimeCommandForTerminalEvent(
    params: TaskTarget,
    event: KCodeTerminalStreamEvent,
  ): void {
  ...
  async function drainRuntimeCommands(params: TaskTarget, reason: string): Promise<void> {
    ...
    const command = commands.find(
      (candidate): candidate is KCodeSendPromptRuntimeCommand =>
    ...
    logger.info(runningCommand.traceId, "KCode task command drain 开始", {
    ...
    try {
      ...
      logger.warn(runningCommand.traceId, "KCode task command drain 失败", {
        commandId: runningCommand.commandId,
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
  ...
  async function drainRuntimeCommandsSafely(params: TaskTarget, reason: string): Promise<void> {
    try {
      ...
      logger.warn(undefined, "KCode task command drain 调度失败", {
        error: error instanceof Error ? error.message : String(error),
        reason,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
  ...
  function persistedStatusFromToolUpdate(
    status: Extract<KCodeStreamEvent, { type: "tool_call_update" }>["status"],
  ): KCodePersistedToolCall["status"] | undefined {
  ...
  function rememberLiveToolProjection(params: TaskTarget, event: KCodeStreamEvent): void {
    ...
    const nextTool: KCodePersistedToolCall =
  ...
  function persistedToolId(tool: KCodePersistedToolCall): string | undefined {
  ...
  function mergePersistedToolCall(
    snapshotTool: KCodePersistedToolCall | undefined,
    liveTool: KCodePersistedToolCall,
  ): KCodePersistedToolCall {
    if (!snapshotTool) {
      return liveTool;
    }
    return {
  ...
  function findToolLocation(
    messages: readonly KCodePersistedMessage[],
    toolId: string,
  ...
  function latestAssistantMessageIndex(messages: readonly KCodePersistedMessage[]): number {
  ...
  function upsertPersistedToolPart(
    parts: readonly KCodePersistedMessagePart[] | undefined,
    toolIndex: number,
  ): KCodePersistedMessagePart[] {
  ...
  function updateSnapshotMessageTool(
    message: KCodePersistedMessage,
    toolIndex: number,
    tool: KCodePersistedToolCall,
  ): KCodePersistedMessage {
  ...
  function mergeLiveToolProjectionIntoSnapshotMessages(
    ...
    if (changed) {
      logger.debug(undefined, "KCode snapshot 合并 live tool projection", {
        event: "kcode_task.snapshot.live_tool_projection.merged",
  ...
  function getTaskTarget(taskId: string): TaskTarget {
    ...
    if (!target) {
      throw Object.assign(new Error(`KCode session target is not loaded: ${taskId}`), {
        code: "KCODE_SESSION_TARGET_NOT_FOUND",
      });
    }
    return target;
  }
  ...
  function getToolProjectionMemory(params: TaskTarget): KCodeToolProjectionMemory {
    const key = taskKey(params);
    let memory = toolProjectionMemoryByTaskKey.get(key);
    if (!memory) {
      memory = createKCodeToolProjectionMemory();
  ...
  function getWorkspaceEmitter(workspace: WorkspaceEventInput): Emitter<KCodeWorkspaceEvent> {
    return taskIndexSyncer.getWorkspaceEmitter(workspace);
  }

  function getTaskEmitter(params: TaskTarget): Emitter<KCodeStreamEvent> {
    const key = taskKey(params);
    let emitter = taskEmitters.get(key);
    if (!emitter) {
      emitter = new Emitter<KCodeStreamEvent>();
      taskEmitters.set(key, emitter);
    }
    return emitter;
  }

  function getGlobalTaskEmitter(taskId: string): Emitter<KCodeStreamEvent> {
  ...
  function emitTaskEvent(params: TaskTarget, event: KCodeStreamEvent): void {
  ...
  function emitWorkspaceTaskListChanged(
    ...
    taskMeta: KCodeTaskMeta | undefined,
    reason: KCodeWorkspaceTaskListChanged["reason"],
  ) {
  ...
  function emitWorkspaceConfig(
    ...
    settings: KCodeSessionSettingsState,
  ) {
  ...
  async function resumeSnapshot(
    ...
    return options.kcodeAgentService.resumeSession({
      workspacePath: params.workspacePath,
      workspaceIdentity: params.workspaceIdentity,
  ...
  async function sendConfigCasCommandV4<T extends "switchModelConfig" | "switchCollaborationMode">(
    ...
    await sendHostCasCommandV4({
      send: (envelope) =>
        options.kcodeAgentService.sendConversationCommandV4({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          envelope,
        }),
   ...
   * command.ts 裁决），而旧协议 KCodeSessionMode 含 auto 且旧 op 接受它——为 UI 行为
  ...
  async function switchCollaborationModeViaProtocol(
    ...
    if (mode === "auto") {
      await options.kcodeAgentService.setMode({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: target.taskId,
        mode,
  ...
  async function repairEmptyImportedClaudeSnapshot(
    ...
    const repaired = await repairImportedClaudeSessionSnapshot({
      snapshot,
      target: params,
      createSession: (input) => options.kcodeAgentService.createSession(input),
      onRepair: (history) => {
        logger.warn(
          undefined,
  ...
  function snapshotToMeta(snapshot: KCodeSessionStateSnapshot): KCodeTaskMeta {
    const target = {
    ...
    return applyOverlayToMeta(
      {
        ...
        mode: fromKCodeMode(snapshot.session.mode),
        ...
        status: deriveKCodeTaskStatusFromSessionSnapshot(snapshot),
        ...
        target: snapshot.projection.target
          ? fromKCodeGoal(snapshot.projection.target)
  ...
  function rememberIndexedTaskMeta(meta: KCodeTaskMeta): KCodeTaskMeta {
  ...
  async function resolveTaskIndexResumeHints(
    ...
    if (model || thoughtLevel) {
      ...
      logger.info(undefined, "从 task index 回填 KCode session resume 配置", {
        model: model || null,
  ...
  async function syncTaskIndexMeta(meta: KCodeTaskMeta): Promise<KCodeTaskMeta> {
  ...
  async function updateIndexedTaskState(
    ...
    try {
      ...
      // 旧 KCode session 可能还没有轻量 task index 行。
  ...
  function updateTaskIndexFromStreamEvent(params: TaskTarget, event: KCodeStreamEvent): void {
  ...
  function updateTaskApiRetryFromStreamEvent(params: TaskTarget, event: KCodeStreamEvent): void {
    const key = taskKey(params);
  ...
  function snapshotToKCode(
    ...
    const goalIterationByAssistantMessageId = getKCodeGoalIterationByAssistantMessageId(
      snapshot.messages,
      {
    ...
    const backgroundTaskNotifications = collectKCodeBackgroundTaskNotificationsByToolUseId(
      snapshot.messages,
    );
      ...
      normalizeGoalVerificationTimelineMessageOrder(
        addGoalVerificationTimelineSnapshotFallback(
          addSessionForkSnapshotFallback(
            coalesceConsecutiveKCodeAssistants(
              getKCodeUserVisibleMessages(snapshot.messages, {
                target: snapshot.projection.target,
              }).map((message) =>
                mapMessage(
                  message,
    ...
    const pendingPermissions: KCodePermissionRequest[] = snapshot.projection.pendingPermissions
    ...
    const pendingElicitations = snapshot.projection.pendingPermissions
      ...
      .filter(
        (event): event is Extract<KCodeStreamEvent, { type: "elicitation_request" }> =>
          event !== null,
      );
    const backgroundTaskControls = parseKCodeBackgroundTaskControlItems(
  ...
  function mapServiceEvent(params: TaskTarget, event: KCodeAgentServiceEvent): void {
    if (event.type === "snapshot") {
      void syncTaskIndexSnapshot(event.snapshot).catch((error) => {
        logger.warn(undefined, "同步 KCode snapshot 到 task index 失败", error);
      });
      const snapshotEvent: KCodeStreamEvent = {
        type: "task_snapshot_updated",
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: workspaceKey(params),
  ...
  function handleTaskIndexTerminalEvent(event: KCodeTaskIndexTerminalEvent): void {
  ...
  function handleTaskIndexReadyEvent(event: KCodeTaskIndexReadyEvent): void {
    const params: TaskTarget = {
      taskId: event.target.sessionId,
      workspacePath: event.target.workspacePath,
      workspaceIdentity: event.target.workspaceIdentity,
  ...
  async function disposeKCodeAgentServiceAndWait(): Promise<void> {
    const agentService = options.kcodeAgentService as IKCodeAgentService & {
  ...
  const service: IKCodeTaskService & {
    disposeAll(): void;
    disposeAllAndWait(): Promise<void>;
  } = {
    async initialize(params) {
      const result = await options.kcodeAgentService.initialize(params);
      return {
    ...
    async releaseWorkspacePreparation(params): Promise<void> {
      ...
      await options.kcodeAgentService.disposeWorkspace(normalizeWorkspaceParams(params));
    },

    async createTask(params): Promise<KCodeTaskCreateResult> {
      ...
      const mcpServers = await resolveProductMcpServers(params.mcpServers);
      let snapshot: KCodeSessionStateSnapshot | null = null;
        ...
        try {
          ...
          if (requestedSelection) {
            if (!sameModelSelection(snapshot.settings.model.current, requestedSelection)) {
              ...
              snapshot = await options.kcodeAgentService.setModel({
                ...target,
                sessionId: draftSessionId,
          ...
          if (
            ...
            snapshot = await options.kcodeAgentService.setThoughtLevel({
              ...target,
              sessionId: draftSessionId,
    ...
    async enqueueTaskCommand(params): Promise<KCodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const workspaceKeyValue = workspaceKey(params);
      const command: KCodeTaskRuntimeCommand = {
    ...
    async promoteTaskCommand(params): Promise<KCodeEnqueueTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
    ...
    async cancelTaskCommand(params): Promise<KCodeCancelTaskCommandResult> {
      assertCurrentOwnerRun(params, params.ownerRunId);
      const key = taskKey(params);
      const commands = runtimeCommands.get(key) ?? [];
      const command = commands.find((candidate) => candidate.commandId === params.commandId);
      if (!command) {
        logger.info(undefined, "KCode task command 取消时已不存在", {
          commandId: params.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      if (command.status === "running") {
        logger.info(command.traceId, "KCode task command 已开始运行，跳过取消", {
          commandId: command.commandId,
          taskId: params.taskId,
          workspaceIdentity: params.workspaceIdentity ?? null,
          workspaceKey: resolveWorkspaceKey(params),
      ...
      logger.info(command.traceId, "KCode task command 已取消", {
        commandId: command.commandId,
        status: command.status,
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
    ...
    async stopGeneration(params): Promise<void> {
      ...
      logger.info(params.runId, "KCode task facade stopGeneration 开始", {
      ...
      logger.info(params.runId, "KCode task facade stopGeneration ACK", {
        durationMs: Date.now() - startedAt,
        taskId: params.taskId,
        workspaceIdentity: target.workspaceIdentity ?? null,
        workspaceKey: resolveWorkspaceKey(target),
    ...
    async compactSession(params) {
      ...
      try {
        const result = await options.kcodeAgentService.compactSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
    ...
    async goalSession(params) {
      ...
      logger.info(params.inputId, "[kcode-task-service] goalSession start", {
      ...
      const result = await options.kcodeAgentService.goalSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
        inputId: params.inputId,
      ...
      logger.info(params.inputId, "[kcode-task-service] goalSession agent 返回", {
        action: params.action,
        durationMs: Date.now() - startedAt,
      ...
      emitWorkspaceTaskListChanged(target, meta, "task_status_changed");
      logger.info(params.inputId, "[kcode-task-service] goalSession task index 同步完成", {
        action: params.action,
        durationMs: Date.now() - startedAt,
        startedTurn: result.startedTurn,
        status: result.snapshot.session.status,
    ...
    async closeTask(params): Promise<void> {
      ...
      await options.kcodeAgentService.closeSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async resumeTask(params): Promise<KCodeTaskMeta> {
    ...
    async listTasks(params): Promise<KCodeTaskMeta[]> {
    ...
    async listPinnedTasks(params): Promise<KCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async listTaskList(params: KCodeTaskListQuery): Promise<KCodeTaskListResult> {
    ...
    async listArchivedTasks(params): Promise<KCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async archiveStaleTasks(params): Promise<KCodeTaskMeta[]> {
    ...
    async archiveWorkspaceTasks(params): Promise<KCodeTaskMeta[]> {
      const tasks = await taskIndexRepo.listTaskMetas({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        provider: GLM_PROVIDER,
    ...
    async getTaskSnapshot(params): Promise<KCodeTaskSnapshot | null> {
      const startedAt = Date.now();
      ...
      const kcodeSnapshot = limitTaskSnapshotMessages(
        snapshotToKCode(snapshot, {
      ...
      const indexedMeta = await syncTaskIndexMeta(kcodeSnapshot.meta);
      const indexDurationMs = Date.now() - indexStartedAt;
      logger.info(undefined, "[kcode-task-service] 历史快照读取完成", {
        ...
        stats: getTaskSnapshotMessageDiagnostics(kcodeSnapshot),
        taskId: params.taskId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
      return { ...kcodeSnapshot, meta: indexedMeta };
    },
    ...
    async getTaskSnapshotWithEtag(params) {
      ...
      if (!snapshot) {
        logger.info(undefined, "[kcode-task-service] 历史快照 ETag 读取为空", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          messageLimit: params.messageLimit ?? null,
          taskId: params.taskId,
      ...
      if (params.ifNoneMatch && params.ifNoneMatch === etag) {
        logger.info(undefined, "[kcode-task-service] 历史快照 ETag 命中缓存", {
          clientMode: params.clientMode ?? "unknown",
          durationMs: Date.now() - startedAt,
          etagDurationMs,
          messageLimit: params.messageLimit ?? null,
      ...
      logger.info(undefined, "[kcode-task-service] 历史快照 ETag 生成完成", {
        clientMode: params.clientMode ?? "unknown",
        durationMs: Date.now() - startedAt,
        etagDurationMs,
        messageLimit: params.messageLimit ?? null,
    ...
    async getTaskSnapshotBody(): Promise<KCodeTaskSnapshotBody | null> {
      return null;
    },

    async getTaskSnapshotRef(): Promise<KCodeTaskSnapshotRefContent | null> {
      return null;
    },

    async getTaskSnapshotToolCallsSlice(): Promise<KCodeTaskSnapshotToolCallsSlice | null> {
      return null;
    },

    async getTaskMeta(params): Promise<KCodeTaskMeta | null> {
      const meta = await taskIndexRepo.getTaskMeta(params);
      return meta ? rememberIndexedTaskMeta(meta) : null;
    },

    async getTaskConfigOptions(params): Promise<KCodeConfigOption[]> {
    ...
    async getTaskModelSelection(params): Promise<ModelSelection | null> {
      const target = getTaskTarget(params.taskId);
      const snapshot = await options.kcodeAgentService.readSession({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async setAssistantMessageFeedback(params): Promise<KCodeSessionFile> {
      const snapshot = await service.getTaskSnapshot(params);
      if (!snapshot) {
      ...
      if (targetMessage) {
        targetMessage.feedback = params.feedback as KCodeAssistantMessageFeedback | undefined;
      }
      return snapshot;
    },
    ...
    async scanImportableClaudeSessions(params: {
      ...
      limit?: number;
    }): Promise<KCodeImportableSessionCandidate[]> {
    ...
    async importClaudeSessions(params: {
      ...
      workspaceIdentity?: string;
      sessionIds: string[];
    }): Promise<KCodeImportSessionsResult> {
      return importClaudeNativeSessions({
        ...
        createImportedSession: async (source) => {
          const targetWorkspaceIdentity = params.workspacePath
            ...
            : undefined;
          const snapshot = await options.kcodeAgentService.createSession({
          ...
          // 导入后的任务必须是真实 KCode session，setModel/sendPrompt 才能继续命中 runtime。
    ...
    async setMode(params): Promise<void> {
      ...
      const target = getTaskTarget(params.taskId);
      await switchCollaborationModeViaProtocol(target, toKCodeMode(params.mode) ?? "build");
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    },

    async setConfigOption(params): Promise<KCodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
      ...
      if (params.configId === THOUGHT_LEVEL_CONFIG_ID) {
        ...
        const current = await options.kcodeAgentService.readSession({
          workspacePath: target.workspacePath,
          workspaceIdentity: target.workspaceIdentity,
          sessionId: params.taskId,
        });
        ...
        await switchCollaborationModeViaProtocol(
          target,
          toKCodeMode(params.value as KCodeTaskMode) ?? "build",
        );
      }
      const snapshot = await resumeSnapshot(target);
      await syncTaskIndexSnapshot(snapshot);
    ...
    async setModel(params): Promise<KCodeConfigOption[]> {
      ...
      const target = getTaskTarget(params.taskId);
      await options.kcodeAgentService.setModel({
        workspacePath: target.workspacePath,
        workspaceIdentity: target.workspaceIdentity,
        sessionId: params.taskId,
    ...
    async setAutomationSessionConfig(params): Promise<KCodeConfigOption[]> {
      const target = getTaskTarget(params.taskId);
    ...
    async getTaskNativeSessionLogFile() {
      const path = resolveKCodeAgentCurrentLogFilePath();
      // 返回 KCode Agent 的结构化日志 JSONL；日志行中的 sessionId 用于按当前任务排查。
      return { provider: GLM_PROVIDER, path, exists: existsSync(path) };
    },

    async getModelTrajectory(params) {
      // KCode Agent 把 taskId 当作 sessionId 落盘 model-io（见本文件其它 sessionId: params.taskId 用法），
      // 这里按 sessionId 还原该 task 的模型调用轨迹，供 UI 侧边栏可视化。
      const trajectory = await readModelTrajectory(params.taskId, params.limit);
      logger.info(
        `[KCodeTaskService] getModelTrajectory taskId=${params.taskId} records=${trajectory.records.length} files=${trajectory.sourceFiles.length} truncated=${trajectory.truncated}`,
      );
      return trajectory;
    },

    async getTaskTokenUsage(params): Promise<KCodeTaskTokenUsageResult> {
      // 摘要面板需要展示 task 的累计模型消耗，不能复用 usage_update 的 context window。
      // 这里通过 KCode Protocol 读 agent SQLite 的 model_usage 聚合，保持桌面和远控同一事实源。
      return options.kcodeAgentService.getTaskTokenUsage({
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        sessionId: params.taskId,
      });
    ...
    async getTaskSessionFilePath(params) {
      return {
        path: `${params.workspacePath}/${params.taskId}.kcode-session`,
        exists: false,
      };
    },
    ...
    async deleteArchivedTasks(params): Promise<KCodeArchivedTaskDeletionResult> {
      const result: KCodeArchivedTaskDeletionResult = {
        deletedTaskIds: [],
        skippedTaskIds: [],
        failedTaskIds: [],
      };
    ...
    async renameTask(params): Promise<KCodeTaskMeta> {
      const renamedAt = Date.now();
      logger.info(undefined, "[KCodeTaskService] renameTask start", {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
        workspaceKey: resolveWorkspaceKey(params),
      ...
      try {
        ...
        logger.info(undefined, "[KCodeTaskService] renameTask overlay set", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        ...
        logger.info(undefined, "[KCodeTaskService] renameTask index updated", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        ...
        logger.info(undefined, "[KCodeTaskService] renameTask event emitted", {
          taskId: params.taskId,
          workspaceKey: resolveWorkspaceKey(params),
        });
        return meta;
      } catch (error) {
        logger.error(undefined, "[KCodeTaskService] renameTask failed", {
          taskId: params.taskId,
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          workspaceKey: resolveWorkspaceKey(params),
    ...
    async setTaskPinned(params): Promise<KCodeTaskMeta> {
    ...
    async setTaskUnread(params): Promise<KCodeTaskMeta> {
    ...
    async archiveTask(params): Promise<KCodeTaskMeta> {
    ...
    async unarchiveTask(params): Promise<KCodeTaskMeta> {
    ...
    async branchTaskFromPrompt(): Promise<KCodeTaskCreateResult> {
      unsupported("branchTaskFromPrompt");
    },

    onDynamicStreamEvent(taskId: string): Event<KCodeStreamEvent> {
      return getGlobalTaskEmitter(taskId).event;
    },

    onDynamicTaskTerminalOutcome(taskId: string): Event<KCodeTaskTerminalOutcome> {
    ...
    onDynamicTaskReady(taskId: string): Event<KCodeTaskReadyOutcome> {
      return (listener) =>
    ...
    onDynamicTaskEvent(params): Event<KCodeStreamEvent> {
      const target = {
        taskId: params.taskId,
        workspacePath: params.workspacePath,
        workspaceIdentity: params.workspaceIdentity,
      ...
      return (listener) => {
        ...
        // 已收敛 v4 命令；读路径消费方是
        // host 镜像 taskRealtimePort（词表同为 KCodeStreamEvent），镜像换 v4 帧 = relay
        // 协议与手机 store 整链重做。
        // 过渡归宿 = replayable 读路径 v4 store，与 host/index.ts 镜像、
        // mapStateUpdated/mapServiceEvent、agentService.onDynamicSessionEvent 同批摘除。
        const upstreamDisposable = options.kcodeAgentService.onDynamicSessionEvent({
          workspacePath: params.workspacePath,
          workspaceIdentity: params.workspaceIdentity,
          sessionId: params.taskId,
          deliveryKind: toKCodeDeliveryKind(params.deliveryKind),
    ...
    onDynamicWorkspaceEvent(workspace): Event<KCodeWorkspaceEvent> {
    ...
    disposeAll(): void {
      ...
      options.kcodeAgentService.disposeAll();
    ...
    async disposeAllAndWait(): Promise<void> {
      ...
      // 否则 host 退出时会把 kcode-cli 的 SIGKILL 兜底 timer 一起带走。
      taskIndexSyncer.disposeAll();
      await disposeKCodeAgentServiceAndWait();
      disposeLocalTaskState();
    },
  };
...
function applyOverlayToMeta(meta: KCodeTaskMeta, overlay: TaskOverlay): KCodeTaskMeta {
  return {
    ...meta,
...
function deriveTitleFromSnapshot(snapshot: KCodeSessionStateSnapshot): string {
  return resolveKCodeVisibleSessionTitle({
...
function buildSnapshotHistory(
  ...
  messageLimit: number,
): NonNullable<KCodeTaskSnapshot["history"]> {
  return {
...
function getTaskSnapshotMessageDiagnostics(snapshot: KCodeTaskSnapshot) {
...
function parseModelPickerValue(value: string): ModelSelection {
  ...
  if (customModel?.providerId && customModel.modelName) {
    // UI 下拉的 custom:provider:model 只是展示态，不能原样传给 kcode-cli。
...
function toKCodeMode(mode: KCodeTaskMode | undefined): KCodeSessionMode | undefined {
  switch (mode) {
    case "plan":
      return "plan";
    case "edit":
...
function fromKCodeMode(mode: KCodeSessionMode): KCodeTaskMode {
...
function addSessionForkSnapshotFallback(
  ...
  return [
    ...messages,
    {
      id: `kcode-timeline-fork-${parentSessionId}-`,
      role: "user",
      content: "",
...
function addGoalVerificationTimelineSnapshotFallback(
  ...
  const timelineMessages = timeline
    ...
    .map<KCodePersistedMessage>((item) => ({
...
function goalVerificationTimelineIdentityKey(
  item: Extract<KCodeGoalVerificationTimelineMeta, { type: "goal_verification" }>,
): string {
...
function goalVerificationTimelineMessageId(
  ...
  if (typeof item.goalIteration === "number") {
    return `kcode-goal-verification-${item.targetId}-${item.goalIteration}`;
  }
  return `kcode-goal-verification-${item.verificationId}`;
}

function insertGoalVerificationTimelineMessages(
  messages: KCodePersistedMessage[],
  timelineMessages: KCodePersistedMessage[],
): KCodePersistedMessage[] {
...
function getSnapshotGoalActiveIterationCount(snapshot: KCodeSessionStateSnapshot): number {
  ...
  return getKCodeGoalActiveIterationCount({
...
function backgroundTaskNotificationToolUpdateFromInput(params: {
  ...
  taskId: string;
  traceId: TraceId;
}): Extract<KCodeStreamEvent, { type: "tool_call_update" }> | null {
  const parsed = parseKCodeBackgroundTaskNotificationText(params.input);
  if (!parsed) {
    return null;
  }
  const status = kcodeBackgroundTaskNotificationToolUpdateStatus(parsed.notification.status);
  return {
    ...
    ...(status === "failed" && parsed.notification.error
      ...
      : {}),
    raw: attachKCodeBackgroundTaskNotificationToRaw(
...
function mapMessage(
  message: KCodeMessageWithParts,
  goalIteration?: number,
  backgroundTaskNotifications?: ReadonlyMap<string, KCodeBackgroundTaskNotificationInfo>,
): KCodePersistedMessage {
  const tools: KCodePersistedToolCall[] = [];
  const parts: KCodePersistedMessagePart[] = [];
  const attachments =
    message.info.role === "user" ? mapPromptAttachmentsFromParts(message.parts) : undefined;
  let syntheticTimeline: KCodeTimelineMeta | undefined;
...
function mapPromptAttachmentsFromParts(
  parts: readonly KCodeMessagePart[],
): KCodePromptAttachment[] | undefined {
  const attachments = parts
    .filter((part): part is Extract<KCodeMessagePart, { type: "file" }> => part.type === "file")
    .map(mapPromptAttachmentFromFilePart)
    .filter((attachment): attachment is KCodePromptAttachment => attachment !== undefined);
  return attachments.length > 0 ? attachments : undefined;
}

function mapPromptAttachmentFromFilePart(
  part: Extract<KCodeMessagePart, { type: "file" }>,
): KCodePromptAttachment | undefined {
...
function extractSyntheticTimelineFromTextPart(
  part: Extract<KCodeMessagePart, { type: "text" }>,
): KCodeTimelineMeta | undefined {
...
function synthesizeCompactionTimeline(
  part: Extract<KCodeMessagePart, { type: "compaction" }>,
): KCodeTimelineMeta | undefined {
  const metadata = asRecord(part.metadata);
...
function mapToolPart(
  part: Extract<KCodeMessagePart, { type: "tool" }>,
  backgroundTaskNotifications?: ReadonlyMap<string, KCodeBackgroundTaskNotificationInfo>,
): KCodePersistedToolCall {
  const state = part.state;
  const taskNotification = backgroundTaskNotifications?.get(part.callId);
  // KCode Protocol 的 part.callId 是实时流和终态 snapshot 共同的工具身份。
  // 以前只保存 metadata 会丢掉 toolCallId，手机 replayable 里 result-only 临时工具就无法被终态快照覆盖。
  const raw = attachKCodeBackgroundTaskNotificationToRaw(
  ...
  if (state.status === "completed") {
    ...
    const notificationStatus = taskNotification?.status
      ? kcodeBackgroundTaskNotificationToolUpdateStatus(taskNotification.status)
      : undefined;
...
function textFromParts(parts: readonly KCodeMessagePart[]): string {
  return textFromKCodeMessageParts(parts);
}

function reasoningFromParts(parts: readonly KCodeMessagePart[]): string | undefined {
  const text = parts
    .filter(
      (part): part is Extract<KCodeMessagePart, { type: "reasoning" }> => part.type === "reasoning",
    )
    .map((part) => part.text)
    .join("");
...
function fromKCodeGoal(goal: unknown): KCodeTaskGoal {
  ...
  return {
    ...
    status: isKCodeTaskGoalStatus(status) ? status : "active",
...
function isKCodeTaskGoalStatus(status: string | undefined): status is KCodeTaskGoal["status"] {
  return (
    status === "active" ||
    status === "paused" ||
...
function sessionTodosToPlanSteps(
  todos: KCodeSessionStateSnapshot["todos"],
): KCodePlanStep[] | null {
...
function sessionGoalStatsToRuntime(
  stats: KCodeSessionStateSnapshot["goalStats"],
): KCodeTaskGoalStats | null {
  return stats ? { ...stats } : null;
}

function sessionTodoGroupsToRuntime(
  groups: KCodeSessionStateSnapshot["todoGroups"],
): KCodeTodoGroup[] | null {
...
function mapStateUpdated(
  ...
  const parsedSettings = kcodeSessionSettingsStateSchema.safeParse(notification.patch);
  ...
  return [
    {
      ...
      taskId: params.taskId,
      traceId,
      currentModeId: normalizeAvailableKCodeMode(parsedSettings.data.mode.current),
      availableModes: getKCodeAgentAvailableModes(),
    },
    {
...
function mapSessionEvent(
  ...
  toolProjectionMemory?: KCodeToolProjectionMemory,
  backgroundTaskControlsByTaskKey?: Map<string, KCodeBackgroundTaskControlItem[]>,
  ...
  // KCode Protocol runtime trace 只在事件没有 inputId 时兜底，避免同一轮 chunk/tool/complete 被拆成不同 trace。
  ...
  if (eventInputId && eventInputId !== protocolTraceId) {
    logger.debug(
      eventInputId,
      `对齐 KCode prompt inputId eventType=${event.type} protocolTrace=${protocolTraceId}`,
    );
  }
  ...
  if (event.type === "turn.started") {
    ...
    const runStartedEvent: KCodeStreamEvent = {
    ...
    if (stringValue(payload.turnPhase) === "compact") {
      return [
        compactFailureToTimelineEvent(
          ...
          traceId,
          eventInputId,
          stringValue(errorPayload.message) ?? "KCode compact failed",
        ),
      ];
    }
    ...
    return [
      {
        ...
        traceId,
        ...(eventInputId ? { inputId: eventInputId } : {}),
        error: stringValue(errorPayload.message) ?? "KCode session failed",
...
): Extract<KCodeStreamEvent, { type: "session_info_update" }> | null {
  if (!hasActiveApiRetry || !isKCodeModelRetryRecoveryProgressPayload(payload)) {
    return null;
  }
...
function mapToolInputStreaming(
  ...
  if (kind === "tool_input_delta") {
    const state = appendKCodeStreamingToolInputDelta(
      streamingToolInputById?.get(toolId),
      stringValue(payload.delta) ?? "",
    );
    streamingToolInputById?.set(toolId, state);
    if (!shouldMaterializeKCodeStreamingToolInputPreview(state, { toolName })) {
    ...
    const preview = buildKCodeStreamingToolInputPreview(state.rawInput);
    markKCodeStreamingToolInputPreviewMaterialized(state);
  ...
  if (kind === "tool_call") {
    ...
    const preview = buildKCodeStreamingToolInputPreview(
      rawInput,
    ...
    if ((hasCompleteInput || preview.complete) && toolProjectionMemory) {
      finalizeKCodeToolProjectionInput(toolId, preview.input, toolProjectionMemory);
    }
...
function logStreamingToolInputProjection(
  ...
  logger.debug(traceId, "KCode streaming tool input projected", {
    ...details,
    event: "kcode.task.streaming_tool_input.projected",
  });
}
...
function mapCompactTimelinePayload(
  ...
  inputId: InputId | undefined,
  payload: Record<string, unknown>,
): Extract<KCodeStreamEvent, { type: "agent_message_chunk" }> | null {
...
function mapSyntheticTimelinePartPayload(
  ...
  const timeline = extractSyntheticTimelineFromTextPart(
    part as Extract<KCodeMessagePart, { type: "text" }>,
  );
  if (!timeline) {
    return null;
  }
...
function compactTimelineMetaFromPayload(
  payload: Record<string, unknown>,
  inputId: InputId | undefined,
): KCodeContextCompactionTimelineMeta | null {
...
function compactFailureToTimelineEvent(
  ...
  inputId: InputId | undefined,
  reason: string,
): Extract<KCodeStreamEvent, { type: "agent_message_chunk" }> {
  return {
    type: "agent_message_chunk",
    taskId,
    traceId,
...
function mapToolUpdated(
  ...
  const rememberedTool = resolveKCodeToolProjectionMetadata(payload, toolId, memory);
  ...
  if ("input" in payload && "toolName" in payload) {
    ...
    toolNameById?.set(toolId, toolName);
    finalizeKCodeToolProjectionInput(toolId, payload.input, memory);
    const toolEvent: KCodeStreamEvent = {
      type: "tool_call",
      taskId,
      traceId,
      ...(inputId ? { inputId } : {}),
  ...
  if ("result" in payload) {
    ...
    // KCode Protocol 的 ToolCallResult 只有 toolCallId/result，不再重复带 toolName。
    // 去掉 KCode Agent 后如果不记住前序 ToolCallScheduled 的 TodoWrite 名称，result 里的 todos
    ...
    if (status !== "in_progress") {
      forgetKCodeToolProjectionMetadata(toolId, memory);
    }
    const planSteps = isMainAgentToolProjectionSource(payload)
      ? extractPlanStepsFromToolOutput({
          title: toolName,
...
function parentToolUseIdFromToolPayload(payload: Record<string, unknown>): string | null {
  // KCode Protocol 发送的父级字段叫 parentToolCallId；
...
function permissionRequestToStreamEvent(
  taskId: string,
  request: KCodePermissionRequestParams,
): KCodePermissionRequest {
  return {
...
function pendingPermissionToStreamEvent(
  taskId: string,
  permission: KCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): KCodePermissionRequest {
  return {
    type: "permission_request",
    taskId,
...
function userInputRequestToElicitationStreamEvent(
  taskId: string,
  request: KCodeUserInputRequestParams,
): KCodeStreamEvent {
  const questions =
...
): Extract<KCodeStreamEvent, { type: "elicitation_response" }> {
  return {
...
function pendingUserInputBackedPermissionToElicitationEvent(
  taskId: string,
  permission: KCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<KCodeStreamEvent, { type: "elicitation_request" }> | null {
...
function pendingExitPlanModeToElicitationEvent(
  taskId: string,
  permission: KCodeSessionStateSnapshot["projection"]["pendingPermissions"][number],
): Extract<KCodeStreamEvent, { type: "elicitation_request" }> {
...
function setTaskBackgroundTaskControlCache(
  cache: Map<string, KCodeBackgroundTaskControlItem[]> | undefined,
  cacheKey: string,
  jobs: KCodeBackgroundTaskControlItem[],
) {
...
function updateTaskBackgroundTaskControlCacheFromPayload(
  ...
  cacheKey: string,
  payload: Record<string, unknown>,
): KCodeBackgroundTaskControlItem[] | null {
  const parsedJobs = parseKCodeBackgroundTaskControlItems([payload]);
  ...
  const nextJobs = mergeKCodeBackgroundTaskControlItems(cache.get(cacheKey) ?? [], parsedJobs);
...
function permissionOptionsFromPayload(payload: Record<string, unknown>): KCodePermissionOption[] {
  return Array.isArray(payload.options) ? (payload.options as KCodePermissionOption[]) : [];
}
...
function permissionResolvedPayloadToStreamEvents(
  ...
  } as Extract<KCodeStreamEvent, { type: "permission_response" }>;
...
function mapSessionInfoLikePayload(
  ...
  const networkDebugStatus = kcodeTaskNetworkDebugStatusFromPayload({
    taskId,
    traceId,
    ...(inputId ? { inputId } : {}),
    ...(eventId ? { eventId } : {}),
  ...
  if ("target" in payload && ("action" in payload || "source" in payload)) {
    events.push({
      ...
      target: {
        ...
        target: payload.target ? fromKCodeGoal(payload.target as never) : null,
        previousTarget: payload.previousTarget
          ? fromKCodeGoal(payload.previousTarget as never)
          : undefined,
      },
    });
  }
  ...
  if (Array.isArray(projection.backgroundJobs)) {
    const jobs = parseKCodeBackgroundTaskControlItems(projection.backgroundJobs);
    setTaskBackgroundTaskControlCache(
      backgroundTaskControlsByTaskKey,
...
function taskTokenUsageDeltaFromPayload(
  ...
  eventId: string | undefined,
  payload: Record<string, unknown>,
): Extract<KCodeStreamEvent, { type: "task_token_usage_delta" }> | null {
...
function apiRetryFromSessionInfoPayload(
  payload: Record<string, unknown>,
): KCodeApiRetryStatus | null | undefined {
  if ("apiRetry" in payload) {
    return normalizeKCodeApiRetryStatus(payload.apiRetry);
  }

  const runtimeRetry = normalizeKCodeApiRetryStatus(asRecord(payload.runtime).apiRetry);
  if (runtimeRetry !== undefined) {
    return runtimeRetry;
  }

  const metaRetry = normalizeKCodeApiRetryStatus(asRecord(asRecord(payload._meta).kcode).apiRetry);
  if (metaRetry !== undefined) {
    return metaRetry;
  }

  return (
    kcodeApiRetryFromStreamRecoveryPayload(payload) ??
    kcodeApiRetryFromModelNetworkStatusPayload(payload)
  );
}

function recordAgentModelNetworkTelemetry(event: KCodeSessionEvent): void {
...
function buildAgentModelNetworkInterface(payload: Record<string, unknown>): string {
  ...
  return `kcode_agent.model.${providerKind}.${transport}.${base}`;
}
...
type ContextUsageUpdate = Pick<
  Extract<KCodeStreamEvent, { type: "usage_update" }>,
  "used" | "size" | "cost" | "cache" | "breakdown"
>;

function contextUsageFromProjection(
  projection: KCodeSessionStateSnapshot["projection"],
...
function contextUsageFromRuntime(
  runtimeUsage: KCodeSessionStateSnapshot["runtime"]["contextUsage"] | undefined,
): ContextUsageUpdate | null {
...
function contextUsageFromPayload(payload: Record<string, unknown>): ContextUsageUpdate | null {
  ...
  // KCode Protocol 的 session.updated 里 contextUsed/contextWindow 是 projection 事实源；
...
function optionalContextUsageBreakdownFromPayload(
  ...
  const parsed = kcodeContextUsageBreakdownSchema.safeParse(payload.contextUsageBreakdown);
...
function usageFromPayload(value: unknown): KCodeUsage | undefined {
...
function timelineStatusValue(value: unknown): KCodeTimelineStatus | undefined {
...
function timelineTriggerValue(value: unknown): KCodeTimelineTrigger | undefined {
  return value === "manual" ||
    value === "auto" ||
    value === "reactive" ||
    value === "partial" ||
...
function turnSteerSourceValue(value: unknown): KCodeTurnSteerSource | undefined {
  return value === "plan_approval_feedback" || value === "workflow_refine_feedback"
    ? value
    : undefined;
}

function turnSteerCommandKindValue(value: unknown): KCodeTurnSteerCommandKind | undefined {
