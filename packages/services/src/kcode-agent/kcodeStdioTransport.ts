...
import { kcodeProtocolMessageSchema } from "@kcode/shared";
import { createServiceLogger } from "#src/logger/serviceLogger.js";
import type {
  KCodeProtocolTransport,
  KCodeProtocolTransportClosedEvent,
} from "./kcodeProtocolTransport.js";
import {
...
interface KCodeStdioTransportOptions {
...
const processTreeLogger = createServiceLogger("kcode-agent-process-tree");

export class KCodeStdioTransport implements KCodeProtocolTransport {
  readonly kind = "stdio" as const;

  private readonly messageEmitter = new Emitter<KCodeProtocolMessage>();
  private readonly closeEmitter = new Emitter<KCodeProtocolTransportClosedEvent>();
  ...
  constructor(
    ...
    private readonly options?: KCodeStdioTransportOptions,
  ) {
    this.stderrCollector = new AgentStderrCollector(child.stderr, options?.onStderrLine);

    // KCode Protocol stdio 帧边界只认 LF。Node readline 会把 U+2028/U+2029
  ...
  async send(message: KCodeProtocolMessage): Promise<void> {
    if (this.disposed || this.closed || this.child.killed || !this.child.stdin.writable) {
      throw new Error("KCode agent stdio transport is closed");
    }
  ...
  private async disposeAndWaitOnce(): Promise<void> {
    ...
    if (!this.hasChildExited()) {
      ...
      const configuredEofWaitMs =
        process.env.KCODE_E2E_COVERAGE === "1"
  ...
  private handleStdoutFrame(frame: string): void {
    ...
    try {
      const parsed = kcodeProtocolMessageSchema.parse(JSON.parse(line));
  ...
  private handleStreamError(stream: "stdin" | "stdout", error: Error): void {
    ...
    // kcode-server 会因未处理的 EPIPE 直接崩溃，UI 只能看到远端连接断开而不是协议请求失败。
    // 这里把 stream error 视为 transport 已关闭，阻止后续继续向失效 agent 写入。
    this.fireClose({ reason: `${stream}_error: ${error.message}` });
  }

  private fireClose(event: KCodeProtocolTransportClosedEvent): void {
    if (this.closed) {
      return;
    }
    this.closed = true;
