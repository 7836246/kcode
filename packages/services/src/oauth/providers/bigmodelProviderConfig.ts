import { BIGMODEL_PROVIDER_ID, buildBigModelApiUrl } from "@kcode/shared";
...
const BIGMODEL_OAUTH_PROVIDER_CONFIG: Omit<OAuthProviderRuntimeConfig, "appSecret"> = {
  ...
  userinfoUrl: buildBigModelApiUrl({ KCODE_ENV: "production" }, BIGMODEL_USERINFO_PATH),
  appId: "zcode",
  redirectUri: "kcode://oauth/callback",
};
...
export function createBigModelProviderRuntimeConfig(
  ...
  return {
    ...
    tokenUrl:
      ...
      buildKCodeApiUrlFromEnv(env, "/api/v1/oauth/token"),
    ...
    // 当前 BigModel callback 只消费 kcode OAuth token 路由，显式 appSecret 仅保留给
