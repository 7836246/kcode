...
// to @kcode/kcode-cua/src/broker/ports.ts. The descriptor registration itself stays in
// services (host control plane — depends on services' createServiceDescriptor
// + @kcode/shared ServiceChannels), so services internal callers (node.ts,
// accessor.ts, services/index.ts) and ui consumers (via @kcode/services root
...
// at compile time; Vite never resolves @kcode/kcode-cua for these).
import type {
  CuaPermissionState,
  CuaPermissionStatus,
...
} from "@kcode/kcode-cua/broker/ports";
