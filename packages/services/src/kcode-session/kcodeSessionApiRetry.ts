import {
  ...
  resolveWorkspaceKey,
  type KCodeSessionApiRetryStatus,
  type KCodeSessionStateSnapshot,
  kcodeApiRetryFromModelNetworkStatusPayload,
  kcodeApiRetryFromStreamRecoveryPayload,
} from "@kcode/shared";
import type { KCodeSessionServiceEvent, KCodeTaskTarget } from "#src/kcode-session/kcodeSession.js";

export function createKCodeSessionApiRetryRuntimeTracker(): {
  trackApiRetryFromSessionEvent: (
    params: KCodeTaskTarget,
    event: KCodeSessionServiceEvent,
  ) => KCodeSessionServiceEvent;
  withApiRetryRuntime: (snapshot: KCodeSessionStateSnapshot) => KCodeSessionStateSnapshot;
} {
  const apiRetryBySessionKey = new Map<string, KCodeSessionApiRetryStatus | null>();
  ...
  function trackApiRetryFromSessionEvent(
    ...
    if (
      ...
      if (apiRetry !== undefined) {
        ...
        isKCodeModelRetryRecoveryProgressPayload(asRecord(event.event.payload))
      ) {
  ...
  function withApiRetryRuntime(snapshot: KCodeSessionStateSnapshot): KCodeSessionStateSnapshot {
  ...
  function sessionKey(params: KCodeTaskTarget): string {
...
): KCodeSessionApiRetryStatus | null | undefined {
