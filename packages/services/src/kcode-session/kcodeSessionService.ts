/* eslint-disable max-lines -- kcodeSessionService 聚合 desktop-continuous session 操作、draft lifecycle 和 task index 同步边界，拆分需要单独设计。 */
import type { IKCodeAgentService } from "#src/kcode-agent/kcodeAgent.js";
import type { KCodeTaskIndexSyncer } from "#src/kcode-agent/kcodeTaskIndexSyncer.js";
...
import type {
  ...
  KCodeSessionReadWorkspacePresentationParams,
...
import { formatModelPickerValue } from "#src/kcode-agent/kcodeConfigOptions.js";
import { createKCodeSessionApiRetryRuntimeTracker } from "#src/kcode-session/kcodeSessionApiRetry.js";
import { appendWorkspaceToFilesystemMcpServers } from "#src/session/mcpWorkspaceScope.js";
import { repairEmptyImportedClaudeSessionSnapshot } from "#src/kcode-session/importedClaudeSessionRepair.js";
import { createKCodeDeferredDraftRegistry } from "#src/kcode-session/kcodeSessionDraftRegistry.js";
...
interface CreateKCodeSessionServiceOptions {
...
export function createKCodeSessionService({
  agentService,
  taskIndexSyncer,
  cuaProductMcpServerResolver,
}: CreateKCodeSessionServiceOptions): IKCodeSessionService {
  const { withApiRetryRuntime } = createKCodeSessionApiRetryRuntimeTracker();
  const deferredDraftSessions = createKCodeDeferredDraftRegistry();

  function notifySyncer(
    target: KCodeTaskTarget,
    options?: {
  ...
  function getSessionSnapshotDiagnostics(snapshot: KCodeSessionStateSnapshot) {
  ...
  // kcodeSessionService 是 desktop-continuous 主路径，状态变更后必须
  ...
  async function broadcastSnapshot(
    ...
    try {
      ...
      logger.warn(
        undefined,
        `[kcode-session-service] ${tag} syncSnapshotAndBroadcast 失败 taskId=${snapshot.session.sessionId}`,
        error,
      );
    }
  }
  ...
  async function repairEmptyImportedClaudeSession(
    snapshot: KCodeSessionStateSnapshot,
    params: KCodeSessionResumeParams | KCodeSessionReadParams,
  ...
  async function withResolvedMcpServers<
    T extends KCodeSessionCreateParams | KCodeSessionResumeParams,
  >(params: T): Promise<T> {
  ...
  return {
    async initializeWorkspace(params: KCodeSessionWorkspaceTarget) {
    ...
    getWorkspaceRuntimeIdentity(params: KCodeSessionWorkspaceTarget) {
      return agentService.getWorkspaceRuntimeIdentity(params);
    },

    readWorkspacePresentation(params: KCodeSessionReadWorkspacePresentationParams) {
      return agentService.readWorkspacePresentation(params);
    },

    async createSession(params: KCodeSessionCreateParams) {
      ...
      logger.info(sessionTraceId, "[kcode-session-service] createSession 分配 session trace", {
      ...
      logger.info(sessionTraceId, "[kcode-session-service] createSession agent 返回", {
      ...
      // desktop-continuous 路径不会经过 KCode task adapter，sqlite 的 task index 全靠
      ...
      logger.info(sessionTraceId, "[kcode-session-service] createSession task index 同步完成", {
    ...
    async resumeSession(params: KCodeSessionResumeParams) {
      const startedAt = Date.now();
      ...
      if (
        ...
        if (
          ...
          logger.warn(
            undefined,
            "[kcode-session-service] resumeSession 跳过不支持的 task 思考强度",
            {
      ...
      if (thoughtLevelOverride && snapshot.settings.thoughtLevel.current !== thoughtLevelOverride) {
        logger.info(undefined, "[kcode-session-service] resumeSession 重放 task 思考强度", {
      ...
      logger.info(undefined, "[kcode-session-service] resumeSession 历史恢复完成", {
        agentDurationMs,
    ...
    listSessions(params: KCodeSessionListParams) {
      return agentService.listSessions(params);
    },

    async readSession(params: KCodeSessionReadParams) {
      const startedAt = Date.now();
      const snapshot = await repairEmptyImportedClaudeSession(
        withApiRetryRuntime(await agentService.readSession(params)),
        params,
      );
      logger.info(undefined, "[kcode-session-service] readSession 历史快照读取完成", {
    ...
    readSessionMessages(params: KCodeSessionMessagesParams) {
      return agentService.readSessionMessages(params);
    },

    readSessionEvents(params: KCodeSessionEventsParams) {
      return agentService.readSessionEvents(params);
    },

    promoteDeferredDraftSession(params: KCodeTaskTarget) {
      ...
      notifySyncer(params);
      logger.info(undefined, "[kcode-session-service] deferred draft session 已提升为 task", {
        sessionId: params.sessionId,
        workspaceIdentity: params.workspaceIdentity ?? null,
        workspacePath: params.workspacePath,
      });
    ...
    closeSession(params: KCodeTaskTarget) {
      deferredDraftSessions.forget(params);
      return agentService.closeSession(params).then(() => undefined);
    },

    async closeDeferredDraftSession(params: KCodeTaskTarget) {
      try {
        ...
        logger.warn(
          undefined,
          "[kcode-session-service] 条件关闭 deferred draft 失败，保留旧 session",
          {
    ...
    async setModel(params: KCodeSessionSetModelParams) {
    ...
    async setThoughtLevel(params: KCodeSessionSetThoughtLevelParams) {
      return withApiRetryRuntime(await agentService.setThoughtLevel(params));
    },

    async setMode(params: KCodeSessionSetModeParams) {
