import { formatLogPrefix, type TraceId } from "@kcode/shared";
...
export function createServiceLogger(
  ...
  function write(
    ...
    // 服务层日志过去常被复用到 KCode Agent 命名 logger，导致新 KCode 路径继续依赖 KCode Agent 目录。
    // 这里把通用分级日志抽到独立模块，后续删除 KCode Agent runtime 时不会牵连非 KCode Agent 服务。
