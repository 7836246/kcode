import type { KCodeConfigOption, KCodeProvider, KCodeTaskMode } from "@kcode/shared";

const CANONICAL_SESSION_MODES = new Set<KCodeTaskMode>([
  "yolo",
  "plan",
  "edit",
  "auto",
...
function getModeConfigOption(options: readonly KCodeConfigOption[]): KCodeConfigOption | undefined {
  return options.find((option) => option.category === "mode" && option.type === "select");
}

function normalizePersistedSessionMode(
  modeId: string | null | undefined,
  _provider?: KCodeProvider,
): KCodeTaskMode | undefined {
  const trimmedModeId = readTrimmedString(modeId);
  if (!trimmedModeId) {
    return undefined;
  }

  if (CANONICAL_SESSION_MODES.has(trimmedModeId as KCodeTaskMode)) {
    return trimmedModeId as KCodeTaskMode;
  }
