...
export function buildSearchableTextFromMessages(messages: KCodeSessionFile["messages"]): string {
  const parts: string[] = [];
  let total = 0;
...
export async function writeImportedClaudeTaskSnapshot(params: {
  ...
  // ~/.kcode/v2/sessions/{hash}/{taskId}.json。现在真实 KCode session 承担续聊，legacy snapshot
