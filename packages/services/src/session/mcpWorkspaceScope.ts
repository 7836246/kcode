...
import type { KCodeAgentMcpServer } from "@kcode/shared";
...
function isFilesystemServer(
  server: KCodeAgentMcpServer,
): server is Extract<KCodeAgentMcpServer, { command: string }> {
  return (
    "command" in server &&
...
export function appendWorkspaceToFilesystemMcpServers(
  mcpServers: KCodeAgentMcpServer[] | undefined,
  workspacePath: string,
): KCodeAgentMcpServer[] | undefined {
