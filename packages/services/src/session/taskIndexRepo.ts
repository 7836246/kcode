...
import type {
  ...
  KCodeTaskListResult,
  KCodeTaskListItem,
} from "#src/session/kcodeTaskListTypes.js";
import type {
  KCodeGroupedTaskRef,
...
function appendKCodeAgentIndexedProviderFilter(
  where: string[],
...
interface TaskIndexStatePatch {
  ...
  unreadAt?: number;
  model?: string;
  status?: KCodeTaskMeta["status"];
  lastError?: KCodeTaskMeta["lastError"];
  target?: KCodeTaskMeta["target"];
  updatedAt?: number;
}
...
const GROUPED_WORKSPACE_BOOTSTRAP_ONCE_KEY = "__kcode_internal_grouped_workspace_bootstrap_once__";
const DEFAULT_TASK_GROUP_COLOR: KCodeTaskGroupColor = "gray";
...
function isTerminalTaskStatus(status: KCodeTaskMeta["status"]): boolean {
  return status === "completed" || status === "error";
}

function shouldPreserveNewerTerminalStatus(
  existingMeta: KCodeTaskMeta | null,
  incomingMeta: KCodeTaskMeta,
): boolean {
...
function rowToMeta(row: TaskIndexRow): KCodeTaskMeta {
  const workspaceIdentity = resolveTaskIndexRowWorkspaceIdentity(row);
  try {
    const parsed = kcodeTaskMetaSchema.safeParse(JSON.parse(row.meta_json));
    if (parsed.success) {
      return {
        ...(parsed.data as KCodeTaskMeta),
  ...
  return {
    ...
    mode: row.mode as KCodeTaskMeta["mode"],
    model: row.model ?? undefined,
    provider: row.provider === KCODE_AGENT_PROVIDER ? KCODE_AGENT_PROVIDER : undefined,
    migrationSource: (row.migration_source as KCodeTaskMeta["migrationSource"]) ?? undefined,
    ...
    unreadAt: row.unread_at ?? undefined,
    status: (row.task_status as KCodeTaskMeta["status"]) ?? undefined,
  };
}

/** 序列化 meta 到 meta_json。cron 身份随 meta 一起写入（单一来源），另在 writeRecord 投影到 cron_automation_id 索引列。 */
function serializeMetaJson(meta: KCodeTaskMeta): string {
...
function normalizeWorkspaceBootstrapScopes(
  scopes: Array<{
    workspacePath: string;
    workspaceIdentity?: string;
    workspacePurpose?: import("@kcode/shared").WorkspacePurpose;
  }>,
...
function rowToTaskListItem(row: TaskIndexRow, search: string | null): KCodeTaskListItem {
...
function isTaskGroupColor(value: string): value is KCodeTaskGroupColor {
  return (
    value === "gray" ||
    value === "red" ||
    value === "orange" ||
...
function rowToTaskGroup(row: TaskGroupRow): KCodeTaskGroup {
  return {
    id: row.group_id,
    title: row.title,
...
function workspaceGroupColor(targetWorkspaceKey: string): KCodeTaskGroupColor {
...
function groupedTopNodeOrderRef(node: KCodeGroupedTaskViewNode): {
  nodeType: "group" | "task";
  nodeKey: string;
  mapKey: string;
} {
...
function compareGroupedNodes(
  left: KCodeGroupedTaskViewNode,
  right: KCodeGroupedTaskViewNode,
): number {
...
function compareCronGroupTasks(left: KCodeTaskListItem, right: KCodeTaskListItem): number {
...
function compareGroupTasks(
  left: KCodeTaskListItem,
  right: KCodeTaskListItem,
...
export class TaskIndexRepo {
  ...
  async archiveStaleTasks(params: {
    ...
    if (params.provider) {
      appendKCodeAgentIndexedProviderFilter(where, args, params.provider);
    }
  ...
  private writeRecord(record: TaskIndexWriteRecord): KCodeTaskMeta {
  ...
  async syncTaskMetaAtGroupedTop(params: {
    ...
    titleOverridden?: boolean;
    searchableText?: string;
  }): Promise<{ meta: KCodeTaskMeta; initializedGroupedOrder: boolean }> {
      ...
      try {
        ...
        const meta: KCodeTaskMeta = {
          ...
          target: Object.prototype.hasOwnProperty.call(params.meta, "target")
            ? params.meta.target
            : existingMeta?.target,
          // Claude Code 导入升级成真实 KCode session 后，protocol snapshot
          // 本身不知道迁移来源。同步运行态快照时保留已有 migrationSource，避免
          // 列表过滤和后续切模型把导入任务重新当成普通 KCode 任务。
   ...
   */
  private ensureCronGroupMembership(meta: KCodeTaskMeta): void {
  ...
  private ensureOffPeakGroupMembership(
    meta: Pick<KCodeTaskMeta, "workspacePath" | "workspaceIdentity" | "taskId">,
  ): void {
   ...
   */
  async seedTaskMetaIfMissing(meta: KCodeTaskMeta): Promise<KCodeTaskMeta> {
    await this.ensureReady();
  ...
  }): Promise<{ meta: KCodeTaskMeta; cleared: boolean }> {
    await this.ensureReady();
   ...
   */
  async listSessionsByAutomation(automationId: string): Promise<KCodeTaskMeta[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
        `SELECT
  ...
  async queryTaskList(
    params: KCodeTaskListQuery & { provider?: KCodeProvider },
  ): Promise<KCodeTaskListResult> {
    await this.ensureReady();
  ...
  async renameTaskGroup(params: { groupId: string; title: string }): Promise<KCodeTaskGroup> {
    await this.ensureReady();
    const title = params.title.trim() || "New Group";
    const now = Date.now();
    const database = this.getDatabase();
  ...
  async initializeGroupedTaskAtTop(params: KCodeGroupedTaskRef): Promise<boolean> {
    await this.ensureReady();
    return this.initializeGroupedTaskAtTopReady(params);
  }

  private initializeGroupedTaskAtTopReady(params: KCodeGroupedTaskRef): boolean {
    const row = this.getTaskRow(params);
  ...
  async queryGroupedTaskView(
    params: KCodeGroupedTaskViewQuery & { provider?: KCodeProvider },
  ): Promise<KCodeGroupedTaskView> {
    await this.ensureReady();
    ...
    if (params.provider) {
      // grouped 和 workspace 都是 KCode Agent 任务列表入口，必须共享旧 provider
      // 残留过滤口径；否则历史 claude/codex/gemini 索引行会只在 grouped 里冒出来。
      appendKCodeAgentIndexedProviderFilter(activeTaskWhere, activeTaskArgs, params.provider);
    }
    const activeTasks =
  ...
  async queryGroupedTaskViewStructure(params: {
  ...
  }): Promise<KCodeGroupedTaskViewStructure> {
    await this.ensureReady();
    ...
    const memberRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupMemberRow[];
    const members: KCodeGroupedTaskViewStructureMember[] = memberRows.map((row) => ({
    ...
    const orderRows = this.getDatabase()
      ...
      .all() as unknown as TaskGroupViewNodeOrderRow[];
    const topLevelOrders: KCodeGroupedTaskViewStructureTopOrder[] = [];
  ...
  async applyGroupedTaskViewOrder(
    params: KCodeGroupedTaskViewOrderInput & { provider?: KCodeProvider },
  ): Promise<KCodeGroupedTaskView> {
    await this.ensureReady();
    ...
    const validateTaskRef = (task: KCodeGroupedTaskRef): string | null => {
      ...
      if (params.provider && row.provider !== params.provider) {
        ...
        // 带 provider 的 KCode Agent 视图只接受当前 glm task；旧 provider 引用作为不可见遗留数据跳过。
        return null;
      }
      return key;
    };
    ...
    const visibleTopLevelNodes: KCodeGroupedTaskViewTopLevelNodeRef[] = [];
    const visibleGroups: Array<{ groupId: string; taskRefs: KCodeGroupedTaskRef[] }> = [];
    ...
    for (const group of params.groups) {
      ...
      const visibleTaskRefs: KCodeGroupedTaskRef[] = [];
