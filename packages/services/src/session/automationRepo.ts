...
import {
  ...
  resolveWorkspaceKey,
  kcodeAutomationBotDeliveryTargetSchema,
  modelSelectionSchema,
  kcodeTaskModeSchema,
  type KCodeAutomation,
  type KCodeAutomationBotDeliveryTarget,
  type KCodeAutomationCreateParams,
  type KCodeAutomationDispatchStatus,
  type KCodeAutomationLifecycleStatus,
  type ModelSelection,
  type KCodeAutomationRun,
  type KCodeAutomationRunDispatchStatus,
  type KCodeAutomationRunOutcome,
  type KCodeAutomationTrigger,
  type KCodeAutomationUpdateParams,
} from "@kcode/shared";
...
function rowToAutomation(row: AutomationRow): KCodeAutomation {
  ...
  return {
    ...
    scheduleRule: row.schedule_rule
      ? (JSON.parse(row.schedule_rule) as KCodeAutomation["scheduleRule"])
      : undefined,
    ...(row.schedule_edited_by_user === 1 ? { scheduleEditedByUser: true } : {}),
    runCount: row.run_count,
    enabled: row.enabled === 1,
    lifecycleStatus: row.lifecycle_status as KCodeAutomationLifecycleStatus,
    nextRunAt: row.next_run_at ?? undefined,
    lastRunAt: row.last_run_at ?? undefined,
    dispatchStatus: row.dispatch_status as KCodeAutomationDispatchStatus,
...
function readAutomationModelSelection(row: AutomationRow): KCodeAutomation["modelSelection"] {
...
function serializeAutomationModelSelection(
  selection: KCodeAutomation["modelSelection"],
): string | null {
...
function normalizeAutomationMode(mode: string | null): KCodeAutomation["mode"] | undefined {
  const parsed = kcodeTaskModeSchema.safeParse(mode);
  return parsed.success ? parsed.data : undefined;
}

function assertValidAutomationMode(mode: unknown): void {
  if (mode === undefined || mode === null) return;
  if (!kcodeTaskModeSchema.safeParse(mode).success) {
    // 读取兼容历史脏数据不代表允许继续写脏数据；Repo 是绕过 RPC 时的最终持久化边界。
    throw new Error(`Invalid automation mode: ${String(mode)}`);
  }
}

function rowToRun(row: AutomationRunRow): KCodeAutomationRun {
  ...
  return {
    ...
    trigger: row.trigger as KCodeAutomationTrigger,
    ...(modelSelection ? { modelSelection } : {}),
    dispatchStatus: row.dispatch_status as KCodeAutomationRunDispatchStatus,
    outcome: (row.outcome as KCodeAutomationRunOutcome | null) ?? undefined,
...
export class AutomationRepo {
  ...
  // 互相覆盖全局值，导致 repo 与裸 SQL 操作在并发窗口内写进真实库 ~/.kcode/v2（历史脏数据
  ...
  async create(
    params: KCodeAutomationCreateParams,
    options: { nextRunAt: number | null; lifecycleStatus?: KCodeAutomationLifecycleStatus },
  ): Promise<KCodeAutomation> {
  ...
  async getBotDeliveryTarget(
    automationId: string,
    workspaceKey?: string,
  ): Promise<KCodeAutomationBotDeliveryTarget | undefined> {
    await this.ensureReady();
    const raw = this.getRow(automationId, workspaceKey)?.bot_delivery_target;
    if (!raw) return undefined;
    try {
      const parsed = kcodeAutomationBotDeliveryTargetSchema.safeParse(JSON.parse(raw));
      return parsed.success ? parsed.data : undefined;
    } catch {
  ...
  async get(automationId: string, workspaceKey?: string): Promise<KCodeAutomation | null> {
    await this.ensureReady();
    const row = this.getRow(automationId, workspaceKey);
  ...
  async update(
    automationId: string,
    params: KCodeAutomationUpdateParams,
    options?: {
      nextRunAt?: number | null;
      lifecycleStatus?: KCodeAutomationLifecycleStatus;
      resetRetry?: boolean;
    },
    workspaceKey?: string,
  ): Promise<KCodeAutomation | null> {
    assertValidAutomationMode(params.mode);
    await this.ensureReady();
   ...
   */
  async claimDue(now: number): Promise<KCodeAutomation[]> {
    await this.ensureReady();
    const db = this.getDatabase();
    db.exec("BEGIN IMMEDIATE");
    try {
  ...
  async ensureRunClaimed(params: {
    ...
    workspaceKey: string;
    scheduledAt: number | null;
    trigger: KCodeAutomationTrigger;
  }): Promise<void> {
    await this.ensureReady();
    const now = Date.now();
    this.getDatabase()
  ...
  async markRunDispatch(params: {
    runId: string;
    dispatchStatus: KCodeAutomationRunDispatchStatus;
  ...
  async listRuns(automationId: string, workspaceKey?: string): Promise<KCodeAutomationRun[]> {
    await this.ensureReady();
    const rows = this.getDatabase()
      .prepare(
  ...
  async getRun(runId: string): Promise<KCodeAutomationRun | null> {
    await this.ensureReady();
    const row = this.getDatabase()
