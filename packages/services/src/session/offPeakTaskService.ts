...
import {
  ...
  type KCodeOffPeakTaskCreateParams,
} from "@kcode/shared";
...
import type { ModelSelection, ModelSelectionValidation } from "@kcode/provider";
...
function isValidCreateParams(params: KCodeOffPeakTaskCreateParams): boolean {
  return (
...
export class OffPeakTaskService implements IOffPeakTaskService {
  ...
  async createTask(params: KCodeOffPeakTaskCreateParams): Promise<OffPeakTaskCreateResult> {
    let providerName = "";
    try {
    ...
    const normalizedParams: KCodeOffPeakTaskCreateParams = {
      ...params,
   ...
   */
  async cancelTask(offPeakTaskId: string): Promise<KCodeOffPeakTask | null> {
  ...
  async pauseTask(offPeakTaskId: string): Promise<KCodeOffPeakTask | null> {
   ...
   */
  async continueTask(offPeakTaskId: string): Promise<KCodeOffPeakTask | null> {
  ...
  async deleteHistory(offPeakTaskId: string): Promise<KCodeOffPeakTask | null> {
  ...
  async list(): Promise<KCodeOffPeakTask[]> {
    return this.projectModelSelectionIssues(await this.deps.repo.list());
  }

  async get(offPeakTaskId: string): Promise<KCodeOffPeakTask | null> {
  ...
  ): Promise<KCodeOffPeakTask[]> {
    const repaired: KCodeOffPeakTask[] = [];
  ...
  private async settleOne(task: KCodeOffPeakTask): Promise<void> {
