...
export interface ProviderConfigRuntimeOptions {
  ...
  readonly kcodeBuiltinRemote?: NodeProviderConfigRuntimeOptions["kcodeBuiltinRemote"];
  readonly kcodeBuiltinEnvironment?: NodeProviderConfigRuntimeOptions["kcodeBuiltinEnvironment"];
 ...
 * 配置迁移保留 KCode 用户的供应商数据，文件运行时由 @kcode/provider-node 唯一实现。
 */
export class ProviderConfigRuntime {
  ...
  constructor(options: ProviderConfigRuntimeOptions) {
    const runtimeOptions: NodeProviderConfigRuntimeOptions = {
      kcodeBuiltinFilePath: options.kcodeBuiltinFilePath,
      kcodeBuiltinActiveFilePath: options.kcodeBuiltinActiveFilePath,
      kcodeBuiltinRemote: options.kcodeBuiltinRemote,
      kcodeBuiltinEnvironment: options.kcodeBuiltinEnvironment,
      onKCodeBuiltinRefreshError: options.onKCodeBuiltinRefreshError,
  ...
  resolveKCodeBuiltinActiveFilePath(): Promise<string> {
    return this.#runtime.resolveKCodeBuiltinActiveFilePath();
  }

  refreshKCodeBuiltin(options?: { readonly force?: boolean }) {
    return this.#runtime.refreshKCodeBuiltin(options);
  }

  onDidCheckKCodeBuiltin(listener: () => Promise<void>): () => void {
    return this.#runtime.onDidCheckKCodeBuiltin(listener);
  }
