...
import { fetchProviderRemoteModels, ProviderRemoteModelsError } from "./providerRemoteModels.js";
...
export interface IProviderSettingsService {
    ...
    listRemoteProviderModels(
        providerId: ProviderId,
    ): Promise<ProviderSettingsRemoteModelsListResult>;
...
export function createProviderSettingsService(
    ...
    return {
        ...
        resolveModelConfig: async (input) => {
            ...
            const inheritedConfig = ModelConfig.fromData(resolution.inheritedConfig)
                .overlay(patch)
                .toJSON();
