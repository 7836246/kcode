...
interface TeamPlanRequestKeyDependencies {
  ...
  readonly access: Extract<KCodeAccountAccess, { planKind: "team-coding-plan" }>;
}
...
export async function resolveAccountTeamPlanRuntimeApiKey(
  ...
  const kcodeJwtToken = (await params.credentialService?.load(KCODE_JWT_TOKEN_KEY))?.trim() ?? "";
  if (family === "bigmodel" && kcodeJwtToken && token === kcodeJwtToken) {
    // BigModel /api/biz 只接受登录 access token，不能使用旧版本误存的 KCode JWT。
    log.warn(undefined, "Team Plan runtime key projection skipped: stale kcode JWT token", {
      family,
      projectId,
    });
    return null;
