/* oxlint-disable eslint(max-lines) -- 已发布旧 KCode config.json 的多版 Provider 结构读取集中在同一边界。 */
...
interface KCodeConfigFile {
  $schema?: string;
  provider?: Record<string, KCodeOpenCodeProviderConfig>;
  [key: string]: unknown;
}

interface KCodeOpenCodeProviderConfig {
  api?: string;
  name?: string;
  env?: string[];
  id?: string;
  ...
  createdAt?: number;
  updatedAt?: number;
  models?: Record<string, KCodeOpenCodeModelConfig>;
  kcode?: KCodeProviderConfigExtension;
  [key: string]: unknown;
}

interface KCodeOpenCodeModelConfig {
  id?: string;
  name?: string;
  family?: string;
  release_date?: string;
  attachment?: boolean;
  reasoning?: boolean | KCodeReasoningConfigExtension;
  temperature?: boolean;
  tool_call?: boolean;
  interleaved?: unknown;
  cost?: JsonObject;
  ...
  modified?: boolean;
  deleted?: boolean;
  kcode?: KCodeModelConfigExtension;
  [key: string]: unknown;
}

interface KCodeReasoningConfigExtension {
  enabled?: boolean;
  variants?: string[];
...
interface KCodeProviderConfigExtension {
  enabled?: boolean;
  systemDisabledReason?: ModelProviderSystemDisabledReason;
  endpoints?: ModelProviderEndpoints;
  apiFormat?: ModelProviderApiFormat;
...
interface KCodeModelConfigExtension {
  kinds?: ModelProviderKind[];
  defaultKind?: ModelProviderKind;
  modelIdByKind?: Partial<Record<ModelProviderKind, string>>;
  disabledReason?: string;
...
const kcodeModelProviderModalitySchema = z.enum(["text", "image", "video", "audio", "pdf"]);
const kcodeProviderModelMappingsSchema = z.record(z.string(), z.unknown());

const kcodeReasoningConfigExtensionSchema = z
  .object({
...
const kcodeModelConfigExtensionSchema = z
  .object({
...
const kcodeOpenCodeModelConfigSchema: z.ZodType<KCodeOpenCodeModelConfig> = z
  .object({
    id: z.string().optional(),
    name: z.string().optional(),
    reasoning: z.union([z.boolean(), kcodeReasoningConfigExtensionSchema]).optional(),
    limit: z
      .object({
    ...
    modalities: z
      .object({
        input: z.array(kcodeModelProviderModalitySchema).optional(),
        output: z.array(kcodeModelProviderModalitySchema).optional(),
      })
      .optional(),
    ...
    modified: z.boolean().optional(),
    deleted: z.boolean().optional(),
    kcode: kcodeModelConfigExtensionSchema.optional(),
  })
  .passthrough();

const kcodeProviderConfigExtensionSchema = z
  .object({
    enabled: z.boolean().optional(),
    ...
    defaultKind: modelProviderKindSchema.optional(),
    providerMappings: kcodeProviderModelMappingsSchema.optional(),
...
const kcodeOpenCodeProviderConfigSchema: z.ZodType<KCodeOpenCodeProviderConfig> = z
  .object({
    ...
    createdAt: z.number().optional(),
    updatedAt: z.number().optional(),
    models: z.record(z.string(), kcodeOpenCodeModelConfigSchema).optional(),
    kcode: kcodeProviderConfigExtensionSchema.optional(),
  })
  .passthrough();

const kcodeConfigFileSchema = z
  .object({
    $schema: z.string().optional(),
    provider: z.record(z.string(), kcodeOpenCodeProviderConfigSchema).optional(),
  })
  .passthrough();
...
function inferModelProviderKindFromOpenCodeProvider(
  provider: KCodeOpenCodeProviderConfig,
): ModelProviderKind {
  if (provider.kind) {
...
function hasOpenCodeProviderRuntimeFields(provider: KCodeOpenCodeProviderConfig): boolean {
  // config.json 的权威运行态字段是 kind/options.baseURL。
  // 旧 endpoints/apiFormat/defaultKind/kcode 可能是历史迁移残留，若继续优先读取会覆盖新配置。
  return Boolean(
    provider.kind ||
...
function resolveOpenCodeProviderEndpoints(
  ...
  return provider.endpoints ?? provider.kcode?.endpoints ?? {};
}

function stripKCodePrefixedProviderMappings(
  ...
  const entries = Object.entries(providerMappings ?? {}).filter(
    ([key]) => !key.toLowerCase().startsWith("kcode"),
  );
  return Object.fromEntries(entries);
}

function openCodeReasoningToModelReasoning(
  model: KCodeOpenCodeModelConfig,
  ...
  if (!options.preferOpenCodeFields && model.kcode?.reasoning) {
    return stripModelProviderReasoningPatches(model.kcode.reasoning);
  }
...
function openCodeModelToModelProviderModel(
  ...
  const kcode = options.preferOpenCodeFields ? undefined : model.kcode;
  const kcodeExtensions = model.kcode;
  const contextWindow =
  ...
  return createModelProviderModelConfig({
    ...
    priority: readFiniteNumber(model.priority) ?? readFiniteNumber(kcodeExtensions?.priority),
    disabledReason: model.disabledReason ?? kcode?.disabledReason,
    supportsTools: model.supportsTools ?? kcode?.supportsTools,
    supportsStructuredOutput: model.supportsStructuredOutput ?? kcode?.supportsStructuredOutput,
    modified: model.modified ?? kcodeExtensions?.modified,
    deleted: model.deleted ?? kcodeExtensions?.deleted,
  });
}
...
function openCodeProviderToModelProviderConfig(
  ...
  for (const deletedModelId of normalizeDeletedModelIds(kcode?.deletedModels)) {
  ...
  const apiKeyRequired =
    provider.apiKeyRequired ?? kcode?.apiKeyRequired ?? readBoolean(options.apiKeyRequired);
  const providerMappings = preferOpenCodeFields
    ? undefined
    : stripKCodePrefixedProviderMappings(provider.providerMappings ?? kcode?.providerMappings);
  return normalizeProviderForStore({
    ...
    ...(provider.enabled !== undefined
      ...
      : kcode?.enabled !== undefined
        ? { enabled: kcode.enabled }
        : {}),
    ...((provider.systemDisabledReason ?? kcode?.systemDisabledReason)
      ? {
          systemDisabledReason: provider.systemDisabledReason ?? kcode?.systemDisabledReason,
        }
      : {}),
    endpoints,
    apiFormat: preferOpenCodeFields
      ? resolveModelProviderKindApiFormat(defaultKind)
      : (provider.apiFormat ?? kcode?.apiFormat ?? resolveModelProviderKindApiFormat(defaultKind)),
    ...((provider.source ?? kcode?.source)
      ? { source: provider.source ?? kcode?.source }
      : { source: "custom" }),
    ...((provider.catalogSourceId ?? kcode?.catalogSourceId)
      ? { catalogSourceId: provider.catalogSourceId ?? kcode?.catalogSourceId }
      : {}),
    ...((provider.catalogProviderId ?? kcode?.catalogProviderId)
      ? {
          catalogProviderId: provider.catalogProviderId ?? kcode?.catalogProviderId,
        }
      : {}),
    ...((provider.modelsDevProviderId ?? kcode?.modelsDevProviderId)
      ? {
          modelsDevProviderId: provider.modelsDevProviderId ?? kcode?.modelsDevProviderId,
        }
      : {}),
    ...(apiKeyRequired !== undefined ? { apiKeyRequired } : {}),
    ...((provider.headers ?? kcode?.headers ?? readStringRecord(options.headers))
      ? {
          headers: provider.headers ?? kcode?.headers ?? readStringRecord(options.headers),
        }
      : {}),
    ...((provider.logoUrl ?? kcode?.logoUrl)
      ? { logoUrl: provider.logoUrl ?? kcode?.logoUrl }
      : {}),
    apiKey,
    ...((provider.apiKeyUrl ?? kcode?.apiKeyUrl)
      ? { apiKeyUrl: provider.apiKeyUrl ?? kcode?.apiKeyUrl }
      : {}),
    defaultKind,
    models,
    ...(providerMappings ? { providerMappings } : {}),
    createdAt:
      (!preferOpenCodeFields ? (provider.createdAt ?? kcode?.createdAt) : undefined) ?? now,
    updatedAt:
      (!preferOpenCodeFields ? (provider.updatedAt ?? kcode?.updatedAt) : undefined) ?? now,
  });
}

async function readRawKCodeConfigFile(): Promise<KCodeConfigFile | null> {
  const filePath = getKCodeConfigFilePath();
  let raw: string;
  try {
  ...
  const parsed = kcodeConfigFileSchema.safeParse(json);
  if (!parsed.success) {
...
async function readKCodeConfigProviders(): Promise<ModelProviderConfig[] | null> {
  const config = await readRawKCodeConfigFile();
  if (!config?.provider) {
    return null;
  }
...
function normalizeProviderForStore(provider: ModelProviderConfig): ModelProviderConfig {
  ...
  return {
    ...
    providerMappings: stripKCodePrefixedProviderMappings(
...
function applyProviderStoreMigrations(providers: ModelProviderConfig[]): ModelProviderConfig[] {
  ...
  // reasoning mapping 由 KCode Built-in/Personal ModelConfigRules 在 Registry 中解析；
...
export async function readLegacyKCodeConfigProviders(): Promise<ModelProviderConfig[]> {
  const configProviders = await readKCodeConfigProviders();
