...
export class ProviderRuntime {
    ...
    constructor(dependencies: ProviderRuntimeDependencies) {
        ...
        this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckKCodeBuiltin(async () => {
            ...
            if (
                !this.#disposed &&
                config.kcodeBuiltinRevision !== account.basedOnKCodeBuiltinRevision
            ) {
...
function createSettingsMutationTarget(
    ...
    return {
        ...
        reorderPersonalProviders: (providerIds) =>
            configService.reorderPersonalProviders(providerIds),
        ...
        importRemoteModels: (providerId, remoteModelIds, membership, modelConfigs) =>
            configService.importRemoteModels(providerId, remoteModelIds, membership, modelConfigs),
        ...
        refreshSources: async (reason) => {
            const sourceResults = await Promise.allSettled([
                configRuntime.refreshKCodeBuiltin({ force: true }),
