...
import { DATA_BASE_DIR_FORBIDDEN_WINDOWS_INSTALL_DIR_ERROR_CODE } from "@kcode/shared";

let _dataBaseDir: string | null = null;
export const KCODE_WINDOWS_APP_INSTALL_DIR_ENV = "KCODE_WINDOWS_APP_INSTALL_DIR";
const envDataBaseDir = process.env.KCODE_DATA_BASE_DIR?.trim() || null;
...
/** Get the current base directory. Priority: setDataBaseDir() > env KCODE_DATA_BASE_DIR > homedir(). */
...
/** {dataBaseDir}/.kcode */
export function getKCodeDataRootDir(): string {
  return join(getDataBaseDir(), ".kcode");
}

/** 非项目对话共享的真实工作目录；默认 ~/.kcode/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getKCodeDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.kcode/v2 */
export function getAppConfigDir(): string {
  return join(getKCodeDataRootDir(), "v2");
}
...
function collectWindowsForbiddenAppInstallDirs(
  ...
  const candidates = [
    ...
    readEnvValue(env, KCODE_WINDOWS_APP_INSTALL_DIR_ENV),
    programFiles ? win32.join(programFiles, "KCode") : null,
    programFilesX86 ? win32.join(programFilesX86, "KCode") : null,
    programW6432 ? win32.join(programW6432, "KCode") : null,
    localAppData ? win32.join(localAppData, "Programs", "KCode") : null,
  ];
...
export function getExportLogStageDir(): string {
  return join(getKCodeDataRootDir(), "export-log-stage");
}
...
/** 与 KCode session 持久化一致：使用 workspaceKey 的 SHA-256 前 12 位 */
...
/** ~/.kcode/v2/sessions/{workspaceHash} */
...
/** ~/.kcode/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.kcode/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .kcode/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".kcode", "v2");
  const newDir = join(newBaseDir, ".kcode", "v2");
