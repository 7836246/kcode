...
import { DEFAULT_ENABLED_OFFICIAL_PLUGIN_IDS } from "@kcode/shared";
...
const DEFAULT_COMMAND_AGENT_SOURCE: CommandAgentSource = KCODE_COMMAND_AGENT_SOURCE;
const COMMAND_AGENT_SOURCE_ORDER: readonly CommandAgentSource[] = KCODE_COMMAND_AGENT_SOURCES;
const ENABLE_OVERRIDE_KEY = "enable";
const HOME_PREFIX = "~/";
const KCODE_OFFICIAL_PLUGIN_MARKETPLACE = "kcode-plugins-official";
const KCODE_INLINE_PLUGIN_MARKETPLACE = "inline";
const KCODE_PLUGIN_MANIFEST_PATH = join(".kcode-plugin", "plugin.json");
...
const KCODE_COMMAND_DESCRIPTOR: CommandAgentSourceDescriptor = {
  agentSource: "kcodeAgent",
  directorySource: "kcode",
  userDirectorySegments: [".kcode", "commands"],
  workspaceDirectorySegments: [".kcode", "commands"],
  fileExtension: ".md",
  format: "markdown",
  namespaceSeparator: "/",
...
function getUserCliConfigPath(): string {
  return join(resolveUserHomeDir(), ".kcode", "cli", "config.json");
}
...
async function scanOfficialPluginCacheRoots(pluginStorageRoot: string): Promise<string[]> {
  const cacheRoot = join(pluginStorageRoot, "cache", KCODE_OFFICIAL_PLUGIN_MARKETPLACE);
...
async function findPluginManifestPath(rootPath: string): Promise<string | null> {
  for (const manifestPath of [
    join(rootPath, KCODE_PLUGIN_MANIFEST_PATH),
...
async function resolvePluginCommandRootDescriptors(): Promise<PluginCommandRootDescriptor[]> {
  ...
  const candidates: PluginRootCandidate[] = [
    ...config.dirs.map((dir) => ({
      defaultEnabled: true,
      marketplace: KCODE_INLINE_PLUGIN_MARKETPLACE,
  ...
  for (const candidate of candidates) {
    ...
    // 仍会从 cache 贡献命令。
    if (
      candidate.marketplace === KCODE_OFFICIAL_PLUGIN_MARKETPLACE &&
...
export function createCommandsService(_options?: CommandsServiceOptions): ICommandsService {
  async function list(params: {
    ...
    // KCode Agent 需要先合并所有 workspace 目录，再合并所有 user 目录；
    // 按每个目录交错读取 project/user 会让 user .kcode 抢在 workspace .agents 前面。
    for (const agentSource of agentSources) {
      const descriptors =
        agentSource === KCODE_COMMAND_AGENT_SOURCE
    ...
    const pluginCommands =
      !params.agentSource || params.agentSource === KCODE_COMMAND_AGENT_SOURCE
        ? await discoverPluginCommands(enabledOverrides)
        : [];

    return {
      commands: [...dedupedUserCommands, ...pluginCommands] as KCodeCommand[],
...
async function discoverPluginCommandsRecursive(
  ...
  for (const entry of entries) {
    ...
    if (!entry.toLowerCase().endsWith(KCODE_COMMAND_DESCRIPTOR.fileExtension)) {
      continue;
    }
    ...
    try {
      ...
      const parsed = CommandFileParser.parseCommandFile(
        content,
        fullPath,
        KCODE_COMMAND_DESCRIPTOR.format,
      );
      if (!parsed) {
        continue;
      }
      const name = getCommandName(rootDir, fullPath, KCODE_COMMAND_DESCRIPTOR);
...
async function discoverCommandsFromDirectorySources(params: {
  ...
  for (const descriptor of params.descriptors) {
    ...
    // `.kcode` 是强优先级来源；只要读到有效命令，同 scope 的 `.agents` 就不再参与。
    if (descriptor.directorySource === "kcode" && discoveredCount > 0) {
      break;
    }
  }
}
