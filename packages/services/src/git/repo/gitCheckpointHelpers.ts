...
import type { GitCheckpointDiff, GitCheckpointFileDiff } from "@kcode/shared";
...
export function getCheckpointRefName(workspacePath: string, checkpointId: string): string {
  ...
  return `refs/kcode/checkpoints/${workspaceHash}/${checkpointId}`;
}
...
export function buildCheckpointEnv(tempIndexPath: string): NodeJS.ProcessEnv {
  return {
    ...
    GIT_AUTHOR_NAME: "KCode Checkpoint",
    GIT_AUTHOR_EMAIL: "checkpoint@kcode.local",
    GIT_COMMITTER_NAME: "KCode Checkpoint",
    GIT_COMMITTER_EMAIL: "checkpoint@kcode.local",
  };
}
