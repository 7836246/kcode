...
import type { IKCodeSessionService } from "#src/kcode-session/kcodeSession.js";
...
interface ConversationShareServiceOptions {
  kcodeAgentService: ConversationShareAgentService;
  ...
  kcodeSessionService?: Pick<IKCodeSessionService, "createSession" | "listSessions">;
...
function hasUnsupportedArtifactReference(value: unknown): boolean {
  if (typeof value === "string") return /^kcode-artifact:\/\//iu.test(value);
 ...
 * 仅在 fault 码缺席时保留一条 errno 文本兜底，用于尚未带结构化码的旧 kcode-cli。
 */
function isDefiniteMissingAttachment(error: unknown): boolean {
  const faultCode = readKCodeAttachmentFaultCode(error);
  if (faultCode) {
    return (
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareStatNotFound ||
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.statNotFile
    );
  }
...
function isAttachmentAuthorizationError(error: unknown): boolean {
  ...
  if (faultCode) {
    return (
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareStatNotAuthorized ||
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareReadNotAuthorized ||
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareStatConnectionUntrusted ||
      faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareReadConnectionUntrusted
    );
  }
  const message = error instanceof Error ? error.message : String(error);
...
function isAttachmentTooLargeError(error: unknown): boolean {
  const faultCode = readKCodeAttachmentFaultCode(error);
  return (
    faultCode === KCODE_ATTACHMENT_FAULT_CODES.shareStatTooLarge ||
    faultCode === KCODE_ATTACHMENT_FAULT_CODES.previewTooLarge
  );
}
...
export class ConversationShareService implements IConversationShareService {
  private readonly kcodeAgentService: ConversationShareAgentService;
  ...
  >();
  private readonly kcodeSessionService?: Pick<
  ...
  constructor(options: ConversationShareServiceOptions) {
    this.kcodeAgentService = options.kcodeAgentService;
    ...
    this.kcodeSessionService = options.kcodeSessionService;
    ...
    // 配置的 KCode origin）导入后回链仍指向生产站，点分割线打开的是另一个环境的分享。
    // 改用与 API base 同一个环境解析器（buildRuntimeKCodeApiUrl 也走它），保证同环境。
    // 优先级不变：显式 option > KCODE_CONVERSATION_SHARE_WEB_URL > 按环境推导。
    this.shareWebUrl = (
      options.shareWebUrl ??
      process.env.KCODE_CONVERSATION_SHARE_WEB_URL ??
      `${resolveRuntimeKCodeEndpointOrigin(process.env)}/cn/share`
    ).replace(/\/+$/u, "");
    this.importIndexPath = join(this.conversationWorkspaceRoot, ".kcode-share-imports.json");
  ...
  async preflight(
    ...
    try {
      return await this.preflightWithAgent(input, this.kcodeAgentService);
    } catch (error) {
  ...
  private async importShareInternal(
    ...
    const shareRoot = join(workspacePath, ".kcode-share");
    const importRoot = join(shareRoot, sanitizeFileSegment(continuation.share.share_id));
    const markerPath = join(importRoot, ".kcode-share-import.json");
    ...
    try {
      ...
      if (
        ...
        const sessions = await this.kcodeSessionService.listSessions({ workspacePath, limit: 100 });
    ...
    try {
      ...
      for (const artifact of continuation.artifacts) {
        ...
        installedArtifacts.push({
          ...
          workspaceRelativePath: `.kcode-share/${sanitizeFileSegment(continuation.share.share_id)}/shared-artifacts/${fileName}`,
      ...
      const snapshot = await this.kcodeSessionService.createSession({
        workspacePath,
        ...(workspaceIdentity ? { workspaceIdentity } : {}),
        sessionId,
   ...
   *
   * 目录名用的是 share_id 而不是 contextId（二者不等价），所以扫 .kcode-share/ 下各
  ...
  async getImportedConversation(input: {
    ...
    const shareRoot = join(input.workspacePath, ".kcode-share");
    let entries: Dirent[];
    try {
  ...
  async publish(input: PublishTextConversationInput, operationId: string) {
    return this.publishWithAgent(input, operationId, this.kcodeAgentService);
  }
  ...
  private async publishInternal(
    input: PublishTextConversationInput,
    operationId?: string,
    agentService: ConversationShareAgentService = this.kcodeAgentService,
    ...
    const artifactSnapshot = await buildConversationShareArtifactSnapshot({
      kcodeAgentService: agentService,
  ...
  private async cleanupAbandonedImports(): Promise<void> {
    ...
    for (const entry of imports) {
      ...
      if (typeof marker.sessionId !== "string" || !marker.sessionId.startsWith("share-import-"))
        continue;
      try {
        const sessions = await this.kcodeSessionService.listSessions({
