...
import { type IKCodeTaskService as IKCodeTaskServiceShape } from "../session/kcodeTaskService.js";
...
export function createBotRemoteWorkspaceService(params: {
  ...
  return {
    ...
    async getKCodeTaskService(target: {
      workspacePath: string;
      workspaceIdentity: string;
    }): Promise<IKCodeTaskServiceShape | null> {
      return (await getRuntimeServices(target))?.kcodeTaskService ?? null;
    },
    ...
    async syncAppRuntimePreferences(preferences: KCodeAgentAppRuntimePreferences): Promise<void> {
      ...
      await Promise.all(
        Array.from(runtimeServicesByWorkspaceKey.values()).map((services) =>
          services.kcodeAgentService.syncAppRuntimePreferences(preferences),
        ),
      );
    },
    dispose(): void {
  ...
  async function getRuntimeServices(target: {
    ...
    // task wrapper 命令走 IKCodeTaskService，session 主状态走 KCode session facade。
    ...
    while (true) {
      ...
      const preferences: KCodeAgentAppRuntimePreferences = cachedPreferences
        ? cachedPreferences
      ...
      await services.kcodeAgentService.syncAppRuntimePreferences(preferences);
