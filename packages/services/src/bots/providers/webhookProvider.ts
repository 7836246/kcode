...
function parseWebhookElicitationResponse(
  ...
  if (payload.type !== "kcode.bot.elicitation_response") {
    return undefined;
  }
  const requestId =
...
export function createWebhookBotProvider(
  ...
  return {
      ...
      if (bot.webhookUrl) {
        ...
        const response = await postWebhookWithRetry(bot.webhookUrl, {
          ...
          headers: {
            ...
            [bot.webhookAuthHeaderName || "x-kcode-bot-secret"]: secret ?? "",
          },
          body: JSON.stringify({
            type: "kcode.bot.test",
            botId: bot.id,
    ...
    async send(bot, message) {
      ...
      if (secret) {
        headers[bot.webhookAuthHeaderName || "x-kcode-bot-secret"] = secret;
      }
      const response = await postWebhookWithRetry(bot.webhookUrl, {
        method: "POST",
        headers,
        body: JSON.stringify({
          type: "kcode.bot.message",
          botId: bot.id,
          provider: "webhook",
          ...
          ...(message.elicitation
            ? { type: "kcode.bot.elicitation_request" }
            : {}),
          sentAt: Date.now(),
        }),
      });
