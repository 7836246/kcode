...
export function taskStatus(task: KCodeTaskMeta): string {
...
function readRunningTaskStartedAt(snapshot: KCodeSessionFile | null, task: KCodeTaskMeta): number | null {
...
export function formatStatusTaskLine(task: KCodeTaskMeta, label = "Task"): string {
  return `${label}: ${task.title} (${task.taskId})`;
}

export function readTaskWorkedDurationMs(snapshot: KCodeSessionFile | null, task: KCodeTaskMeta): number | null {
...
export function readLatestAssistantTurnChangeSummary(snapshot: KCodeSessionFile | null): KCodeTaskMeta["changeSummary"] | null {
...
function formatStatusToolProgress(tool: KCodePersistedToolCall | undefined): string | null {
  if (!tool) {
    return null;
  }
...
export function formatStatusStreamToolProgress(event: Extract<KCodeStreamEvent, { type: "tool_call" | "tool_call_update" }>): string | null {
...
export function readLatestTaskProgress(snapshot: KCodeSessionFile | null): string | null {
  const messages = snapshot?.messages ?? [];
