/* eslint-disable max-lines -- Bots 服务仍复用原 RPC 文件名，先把鉴权、命令路由、KCode Agent 桥接收口集中在同一服务内。 */
...
import {
  ...
  type KCodeElicitationRequest,
  type KCodeElicitationQuestion,
  type KCodePermissionOption,
  type KCodePermissionRequest,
  type KCodePromptAttachment,
  type KCodeTaskMode,
  type KCodeAssistantMessagePart,
...
import type { IKCodeTaskService } from "../session/kcodeTaskService.js";
...
import type { KCodeAgentAppRuntimePreferences } from "../kcode-agent/kcodeAgent.js";
...
function formatBotModelSelectionValue(selection: ModelSelection | undefined): string | undefined {
  ...
  return selection.providerId === KCODE_AGENT_PROVIDER
    ? selection.modelId
...
function parseBotModelOptionValue(value: string): ModelSelection | undefined {
  ...
  return value.trim() ? { providerId: KCODE_AGENT_PROVIDER, modelId: value.trim() } : undefined;
}
...
interface BotsServiceDeps {
  ...
  kcodeTaskService: IKCodeTaskService;
...
interface BotRemoteWorkspaceService {
  ...
  getKCodeTaskService?(target: BotRemoteWorkspaceTarget): Promise<IKCodeTaskService | null>;
...
interface PreparedBotMessageContent {
  content: string;
  kcodeAttachments: KCodePromptAttachment[];
}
...
function sortBotPermissionOptions(
  options: readonly KCodePermissionOption[],
): KCodePermissionOption[] {
...
function formatBotPermissionOptionLabel(option: KCodePermissionOption, locale?: Locale): string {
...
function formatBotPermissionOptionDescription(
  option: KCodePermissionOption,
  request: Pick<KCodePermissionRequest, "title" | "description" | "kind" | "raw">,
  locale?: Locale,
): string | undefined {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
function isBotPermissionRejectOption(option: KCodePermissionOption): boolean {
  const displayKind = getBotPermissionOptionDisplayKind(option);
...
const DEFAULT_BOT_KCODE_PROVIDER: KCodeProvider = KCODE_AGENT_PROVIDER;
...
export function createBotsService(
  ...
  async function prepareBotMessageContent(
    ...
    const kcodeAttachments: KCodePromptAttachment[] = [];
    ...
    for (const rawAttachment of rawAttachments) {
      ...
      if (cached.kind === "image" || cached.kind === "audio") {
        kcodeAttachments.push({
          kind: cached.kind,
          filename: cached.filename,
          mimeType: cached.mimeType,
          dataBase64,
          // Bugfix：Bot 已把附件缓存到本地，KCodePromptAttachment 也必须携带该路径。
          // 只在 prompt 文本里描述路径会让下游附件策略无法选择本地文件读取。
          localPath: cached.localPath,
        });
        // Bugfix: bot 附件已经被 gateway 下载并缓存到本地。只把图片作为 KCode Agent image block 传入时，
  ...
  async function resolveKCodeTaskServiceForContext(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
  ): Promise<IKCodeTaskService> {
    if (!context.workspaceIdentity) {
      return deps.kcodeTaskService;
    }
    const remoteKCodeTaskService = await deps.remoteWorkspaceService?.getKCodeTaskService?.({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    if (remoteKCodeTaskService) {
      return remoteKCodeTaskService;
    }
    // Bugfix: 远端 workspace 的 bot 请求不能缺 runtime 时静默走本地 kcodeTaskService。
  ...
  async function listActiveTaskConfigOptions(
    ...
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(context);
    return kcodeTaskService.getTaskConfigOptions({ taskId });
  }

  function findSelectConfigOption(
    options: readonly KCodeConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
  ): (KCodeConfigOption & { type: "select" }) | undefined {
    const category = configId === "thoughtLevel" ? "thought_level" : configId;
    return options.find(
      (item): item is KCodeConfigOption & { type: "select" } =>
  ...
  function listConfigSelectOptions(
    options: readonly KCodeConfigOption[],
    configId: "model" | "mode" | "thoughtLevel",
    context: { locale?: Locale; provider?: KCodeProvider } = {},
  ): BotModelOption[] {
  ...
  function getModeDisplayLabel(
    ...
    const labels: Partial<Record<KCodeProvider, Record<string, string>>> = {
      glm: {
  ...
  async function listModelProviderOptionsForActiveTask(
    task: Pick<KCodeTaskMeta, "model" | "workspacePath" | "workspaceIdentity">,
    _activeProvider: KCodeProvider,
  ): Promise<BotModelProviderOption[]> {
  ...
  async function listProviderConfigOptionsForActiveTask(
    task: Pick<KCodeTaskMeta, "workspacePath" | "workspaceIdentity">,
    activeProvider: KCodeProvider,
  ): Promise<KCodeConfigOption[]> {
  ...
  function normalizeBotDraftOptions(draftOptions: BotDraftOptions): BotDraftOptions {
    ...
    // 如果直接复用，/new 后首条消息会重新创建第三方 runtime，绕过 KCode Agent 单一事实源。
    return {
      ...draftOptions,
      provider: normalizeAgentProviderToKCodeAgent(draftOptions.provider),
    };
  }
  ...
  async function buildInitializedDraftOptions(
    ...
    const requestedProvider = normalizeAgentProviderToKCodeAgent(
      provider ?? DEFAULT_BOT_KCODE_PROVIDER,
    );
    if (context.workspaceIdentity && !(await isRemoteWorkspaceConnected(context))) {
      // Bugfix: 远端断连时初始化草稿也不能偷偷申请远端 KCode Agent runtime。
  ...
  async function buildActiveTaskDraftOptions(context: BotContextState): Promise<BotDraftOptions> {
    ...
    const resolvedProvider = normalizeAgentProviderToKCodeAgent(activeTask.provider);
  ...
  async function applyDraftConfigOptions(
    ...
    if (modeOption?.id && forcedDraftMode) {
      const kcodeTaskService = await resolveKCodeTaskServiceForContext(context);
      await kcodeTaskService.setMode({
        taskId,
        mode: forcedDraftMode as KCodeTaskMode,
      });
    } else if (modeOption?.id) {
      // provider 不支持 yolo（非 KCode Agent）：保持其自身默认模式，避免首条消息回调失败。
      botsLogger.debug(
        traceId,
  ...
  function updateLiveStatusProgress(event: KCodeStreamEvent): void {
  ...
  async function readTaskMeta(
    context: Pick<BotContextState, "workspacePath" | "workspaceIdentity">,
    taskId: string,
  ): Promise<KCodeTaskMeta | null> {
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(context);
    const tasks = await kcodeTaskService.listTasks({
      workspacePath: context.workspacePath,
      workspaceIdentity: context.workspaceIdentity,
    });
    ...
    const entries = (
      await Promise.all(
        workspaces.map(async (workspace) => {
          const kcodeTaskService = await resolveKCodeTaskServiceForContext(workspace);
          const tasks = await kcodeTaskService
            .listTasks({
              workspacePath: workspace.workspacePath,
              workspaceIdentity: workspace.workspaceIdentity,
            })
  ...
  async function readContextActiveTaskMeta(
    ...
    return (
      (
        await (
          await resolveKCodeTaskServiceForContext(context)
        )
          .getTaskSnapshot({
  ...
  async function processProviderCallback(
    ...
    const preparedPayload = callbackBot
      ? ((await adapter.prepareCallbackPayload?.(callbackBot, payload).catch((error: unknown) => ({
          kcodeCallbackPrepareError: error instanceof Error ? error.message : String(error),
        }))) ?? payload)
      : payload;
    if (
      isRecord(preparedPayload) &&
      typeof preparedPayload.kcodeCallbackPrepareError === "string"
    ) {
      return {
        ok: false,
        replies: [],
        responseBody: { error: preparedPayload.kcodeCallbackPrepareError },
        status: 401,
      };
    }
    ...
    const parsePayload =
      isFeishuBotProvider(provider) && isRecord(preparedPayload)
        ? { kcodeProvider: provider, ...preparedPayload }
        : preparedPayload;
    ...
    for (const inbound of parsedInboundMessages) {
      ...
      if (bot) {
        ...
        const handledByFeishuSynchronousCardAction =
          ...
          isRecord(preparedPayload) &&
          preparedPayload.kcodeFeishuSynchronousCardAction === true &&
          Boolean(outbound[0]);
  ...
  function normalizeBotElicitationQuestions(
    event: Extract<KCodeStreamEvent, { type: "elicitation_request" }>,
    locale: Locale | undefined,
  ): KCodeElicitationQuestion[] {
  ...
  function mergeElicitationFormValues(
    question: KCodeElicitationQuestion,
  ...
  async function submitPendingElicitation(
    ...
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(auth.context);
    const submitted = await kcodeTaskService.respondElicitation({
      taskId: pending.taskId,
      workspacePath: auth.context.workspacePath,
  ...
  async function handlePendingElicitationValue(
    ...
    if (question.multiSelect) {
      ...
      await writeContext({ ...auth.context, pendingElicitation: nextPending });
      // Bugfix: 多选题在 Bot 里 toggle 后不会触发 KCode Agent response，必须主动同步草稿给 UI。
      await broadcastPendingElicitationProgress(auth.context, nextPending);
      return createElicitationReply(actor, nextPending, auth.locale);
    }
    return advancePendingElicitation(auth, actor, pending, {
  ...
  async function handleElicitationRequest(
    ...
    // Bugfix: Bot 原先只消费 permission_request，没有把 KCode Agent 的
  ...
  async function watchTaskStream(
    ...
    let assistantParts: KCodeAssistantMessagePart[] = [];
    ...
    const handleStreamEvent = async (
      event: KCodeStreamEvent | TaskStreamMirrorableEvent,
      ...
      if (event.type === "permission_request") {
        ...
        // Bugfix: UI 会把 KCode Agent 原始权限选项规整成“允许/始终允许/拒绝”的固定顺序和文案；
      ...
      if (event.type === "task_complete" || event.type === "task_error") {
        ...
        // Bugfix: KCode Agent 终态事件可能先于 task index/meta 落盘广播到 Bots。
        ...
        const mode = getMode();
        const locale = await readMessageLocale();
        const completedSnapshot = await kcodeTaskService
          .getTaskSnapshot({
            taskId: event.taskId,
            workspacePath: context.workspacePath,
            workspaceIdentity: context.workspaceIdentity,
    ...
    const enqueueStreamEvent = (
      ...
      // Bugfix: KCode Agent 事件分发不保证等待 async listener。微信这类离散消息如果并发发送，
    ...
    const dynamicTaskEvent = (
      kcodeTaskService as Partial<Pick<IKCodeTaskService, "onDynamicTaskEvent">>
    ).onDynamicTaskEvent;
    ...
    const streamDisposable = dynamicTaskEvent
      ...
      : kcodeTaskService.onDynamicStreamEvent(context.activeTaskId)(enqueueStreamEvent);
  ...
  function sendPromptInBackground(
    ...
    void resolveKCodeTaskServiceForContext(context)
      .then((kcodeTaskService) =>
        kcodeTaskService.sendPrompt({
          taskId,
          traceId,
          content,
  ...
  async function handleMessage(message: BotInboundMessage): Promise<BotOutboundMessage[]> {
    ...
    if (auth.context.mode === "task" && auth.context.activeTaskId) {
      const taskService = await resolveKCodeTaskServiceForContext(auth.context);
    ...
    if (auth.context.mode === "draft" || !auth.context.activeTaskId) {
      ...
      const kcodeTaskService = await resolveKCodeTaskServiceForContext(auth.context);
      const task = await kcodeTaskService.createTask({
        workspacePath: auth.context.workspacePath,
        workspaceIdentity: auth.context.workspaceIdentity,
      ...
      const taskTitle = deriveTaskTitle(preparedMessage.content, preparedMessage.kcodeAttachments);
      ...
      await broadcastTaskListChange(context, task.taskId, "prompt_sent", {
        ...
        prompt: {
          ...
          attachments:
            preparedMessage.kcodeAttachments.length > 0
              ? preparedMessage.kcodeAttachments
              : undefined,
    ...
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(auth.context);
    await kcodeTaskService.resumeTask({
      taskId: auth.context.activeTaskId,
      workspacePath: auth.context.workspacePath,
      workspaceIdentity: auth.context.workspaceIdentity,
    });
    ...
    const originalSelection = await kcodeTaskService.getTaskModelSelection({
      taskId: auth.context.activeTaskId,
    });
  ...
  async function resolveTaskSelectionEntry(
    ...
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(context);
    const snapshot = await kcodeTaskService
      .getTaskSnapshot({
        taskId: value.trim(),
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
  ...
  async function isContextActiveTaskRunning(context: BotContextState): Promise<boolean> {
    ...
    const kcodeTaskService = await resolveKCodeTaskServiceForContext(context);
    const activeTaskSnapshot = await kcodeTaskService
      .getTaskSnapshot({
        taskId: context.activeTaskId,
        workspacePath: context.workspacePath,
        workspaceIdentity: context.workspaceIdentity,
    ...
    if (
      ...
      // KCode Agent 历史任务的 status 为空也可能只是旧数据，不代表 UI 仍在运行；只有本进程确实发起
  ...
  function warnAutomationDeliveryOnce(params: {
    target: KCodeAutomationBotDeliveryTarget;
    reason: string;
  }): void {
      ...
      return enqueueInboundProcessing(message.actor, async () => {
        ...
        switch (command.type) {
          ...
          case "model.list": {
            ...
            const activeProvider = normalizeAgentProviderToKCodeAgent(active.task.provider);
            if (!activeProvider) {
              return [createOutbound(message.actor, msg(auth.locale, "modelProviderMissing"))];
            }
            const providers = await listModelProviderOptionsForActiveTask(
          ...
          case "model.set": {
            ...
            const kcodeTaskService = await resolveKCodeTaskServiceForContext(active.task);
            const configOptions = await kcodeTaskService.setModel({
            ...
            const optionSource =
              commandName === "mode" && active.task.provider
                ? await listProviderConfigOptionsForActiveTask(
                    active.task,
                    normalizeAgentProviderToKCodeAgent(active.task.provider),
                  )
                : active.configOptions;
            const currentValue =
              commandName === "mode"
            ...
            const currentLabel =
              commandName === "mode"
                ? readConfigSelectLabelForValue(optionSource, commandName, currentValue, {
                    locale: auth.locale,
                    provider: normalizeAgentProviderToKCodeAgent(active.task.provider),
                  })
          ...
          case "thoughtLevel.set": {
            ...
            const traceId = generateTraceId(active.taskId);
            const kcodeTaskService = await resolveKCodeTaskServiceForContext(auth.context);
            const configOptions = await kcodeTaskService.setConfigOption({
              taskId: active.taskId,
              traceId,
          ...
          case "permission.respond": {
            ...
            const kcodeTaskService = await resolveKCodeTaskServiceForContext(auth.context);
            const submitted = await kcodeTaskService.respondPermission({
              taskId: auth.context.activeTaskId,
