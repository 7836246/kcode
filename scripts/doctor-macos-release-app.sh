...
APP_PATH="${1:-${KCODE_MACOS_RELEASE_APP_PATH:-/Applications/KCode.app}}"
# 安装包身份与后端环境分轴：KCODE_PREVIEW_IDENTITY=1 让生产后端的构建仍是 KCode Preview。
# 只认 "1"，与 CI workflow / release 门的精确比较同一套语义（其它拼写一律视为未开启）。
is_preview_identity_requested() {
  [[ "${KCODE_PREVIEW_IDENTITY:-}" = "1" ]]
}
APP_BUNDLE_NAME="$(basename "$APP_PATH")"
APP_DISPLAY_NAME="${APP_BUNDLE_NAME%.app}"
APP_EXECUTABLE_NAME="${KCODE_APP_EXECUTABLE_NAME:-$APP_DISPLAY_NAME}"

if [ "${APP_PATH:-}" = "--help" ] || [ "${APP_PATH:-}" = "-h" ]; then
  cat <<'USAGE'
Usage:
  bash scripts/doctor-macos-release-app.sh /Applications/KCode.app
  KCODE_MACOS_RELEASE_APP_PATH=/Applications/KCode.app pnpm run doctor:macos-release
...
run_quiet_validation() {
  ...
  shift
  local output_file
  output_file="$(mktemp "${TMPDIR:-/tmp}/kcode-macos-release-doctor.XXXXXX")"
