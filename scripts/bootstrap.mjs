...
function runBootstrapServerBuild() {
  ...
  // bootstrap:with-remote 的最终构建过去复用 build:bootstrap，导致 @kcode/server build
...
function runBootstrapWithRemoteBuild() {
  for (const filter of ["@kcode/rpc", "@kcode/web", "@kcode/formal-proof"]) {
...
if (existsSync(gitmodulesPath) && readFileSync(gitmodulesPath, "utf8").includes("apps/kcode-cli")) {
  runGit(["submodule", "update", "--init", "--recursive", "apps/kcode-cli"]);
} else {
  console.log("[bootstrap] skip submodule init; apps/kcode-cli is a regular directory");
}
