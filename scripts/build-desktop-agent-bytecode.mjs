...
const defaultEntryPath = join(repoRoot, "apps/kcode-cli/packages/cli/dist/kcode.cjs");
...
export async function buildDesktopAgentBytecode({
  ...
  if (env.KCODE_E2E_COVERAGE === "1") throw new Error("coverage 构建不能启用字节码试验");
  const directory = dirname(entryPath);
  const temporary = join(directory, `.bytecode-${randomUUID()}`);
  const loaderPath = join(directory, "kcode.bytecode.cjs");
  try {
    ...
    const bytecodeFile = `kcode.bytecode-${metadata.bytecodeSha256}.jsc`;
    ...
    const runtimeFile = `kcode.bytecode-runtime-${runtimeHash}.cjs`;
