...
const agentEntry = join(root, "agent", "kcode.cjs");

function usage() {
  return `Usage:
  kcode --web [--host <host>] [--port <port>] [--workspace <path>] [--open|--no-open] [--token <token>|--no-token]
  kcode --version
`;
}
...
async function serve(options) {
  ...
  const child = spawn(process.execPath, [serverEntry], {
    ...
    env: {
      ...process.env,
      PORT: String(port),
      KCODE_AGENT_SERVER_ARGS_JSON: JSON.stringify([agentEntry, "app-server", "--stdio"]),
      KCODE_AGENT_SERVER_COMMAND: process.execPath,
      KCODE_SERVER_HOST: options.host,
      KCODE_SERVER_WORKSPACE: options.workspace,
      KCODE_WEB_STATIC_ROOT: webRoot,
      // 显式关闭 token 时必须清空继承值，否则 --no-token 仍会开启后端鉴权。
      KCODE_SERVER_AUTH_TOKEN: token,
    },
  ...
  console.log("");
  console.log("KCode Web is running");
...
try {
  ...
  if (argv.length === 1 && ["--version", "-v"].includes(argv[0])) {
    ...
    if (argv.length === 1 && ["--help", "-h"].includes(argv[0])) {
      console.log("Web mode: kcode --web [options] (kcode --web --help for details)\n");
    }
