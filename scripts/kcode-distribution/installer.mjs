...
BASE_URL="\${KCODE_DIST_BASE_URL:-${baseUrl}}"
INSTALL_DIR="\${KCODE_DIST_HOME:-$HOME/.kcode/runtime}"
BIN_DIR="\${KCODE_DIST_BIN_DIR:-$HOME/.local/bin}"

need_cmd() {
  if ! command -v "$1" >/dev/null 2>&1; then
    echo "kcode install requires $1" >&2
    exit 1
  fi
}
...
cat > "$BIN_DIR/kcode" <<SH
#!/usr/bin/env sh
exec node "$INSTALL_DIR/current/bin/kcode.mjs" "\\$@"
SH
chmod +x "$BIN_DIR/kcode"

echo "KCode $VERSION installed."
echo "Run: kcode (TUI) or kcode --web (Web)"
case ":$PATH:" in
  *":$BIN_DIR:"*) ;;
