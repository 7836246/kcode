...
export async function resolveBuiltinProviderBuildEnvironment({
  ...
  if (!value?.trim()) {
    ...
    for (const file of files) {
      ...
      if (parsed.KCODE_ENV !== undefined) value = parsed.KCODE_ENV;
    }
  }
  const normalized = value?.trim().toLowerCase() || "test";
  if (normalized !== "test" && normalized !== "production") {
    throw new Error(`Invalid KCODE_ENV for Built-in Provider build: ${normalized}`);
  }
  return normalized;
}
...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.KCODE_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/kcode-builtin.json",
  );
  try {
    ...
    const { decodeKCodeBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/kcode-builtin-release.ts"))
        .href,
      import.meta.url,
    );
    decodeKCodeBuiltinRelease(JSON.parse(content));
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "kcode-builtin.json"), config.content, "utf8");
  return config;
}
