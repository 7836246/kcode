  ...
  return new Promise((resolveRun, rejectRun) => {
    ...
    const child = spawn(command, spawnArgs, {
      ...
      env: withPinnedNodePath(
        {
          ...process.env,
          KCODE_ENV: requestedEnv,
          KCODE_DESKTOP_AGENT_BYTECODE: agentBytecode ? "1" : "0",
        },
        process.execPath,
      ),
      stdio: "inherit",
...
try {
  ...
  await run(pnpmCommand, ["--filter", "@kcode/desktop", "pre-dev"]);
  ...
  await run(pnpmCommand, ["--filter", "@kcode/desktop", "dev:runtime"]);
} catch (error) {
