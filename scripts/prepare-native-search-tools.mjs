...
export async function prepareNativeSearchTools({
  platform = process.env.KCODE_TARGET_OS || process.platform,
  arch = process.env.KCODE_TARGET_ARCH || process.arch,
  outputDir,
  dependenciesDir,
  prebuiltPlan,
} = {}) {
...
async function main() {
  await prepareNativeSearchTools({
    platform: readOption("platform") ?? process.env.KCODE_TARGET_OS ?? process.platform,
    arch: readOption("arch") ?? process.env.KCODE_TARGET_ARCH ?? process.arch,
