...
const zh: Record<DocSlug, DocPage> = {
  ...
  install: {
    ...
    blocks: [
      ...
      {
        type: "note",
        text: "安装包未签名。SmartScreen 若提示无法识别，选「更多信息」→「仍要运行」。",
      },
      { type: "h2", text: "Linux" },
      {
        type: "ol",
...
const en: Record<DocSlug, DocPage> = {
  ...
  install: {
    ...
    blocks: [
      ...
      {
        type: "note",
        text: "Builds are unsigned. If SmartScreen says it does not recognize the app, choose More info → Run anyway.",
      },
      { type: "h2", text: "Linux" },
  ...
  faq: {
    ...
    blocks: [
      ...
      { type: "p", text: "Builds are unsigned. Run this in Terminal:" },
      { type: "code", text: "xattr -dr com.apple.quarantine /Applications/KCode.app" },
