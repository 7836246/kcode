...
interface KCodeBootstrapModule {
  createModelAdapter(input: unknown): unknown;
  createKCodeApp(input: unknown): Promise<{
    close?: () => Promise<void>;
...
interface KCodeContractsModule {
...
export async function recordPromptTrajectoryFromFixture(input: {
  ...
  try {
    const { bootstrap, contracts, provider } = await loadKCodeModules();
    ...
    const app = await bootstrap.createKCodeApp({
...
async function loadKCodeModules(): Promise<{
  bootstrap: KCodeBootstrapModule;
  contracts: KCodeContractsModule;
  provider: KCodeProviderModule;
}> {
  ...
  try {
    const [bootstrap, contracts, providerConfig, providerRegistry] = await Promise.all([
      import(bootstrapPath) as Promise<KCodeBootstrapModule>,
      import(contractsPath) as Promise<KCodeContractsModule>,
      import(providerConfigPath) as Promise<Omit<KCodeProviderModule, "ProviderRegistry">>,
      import(providerRegistryPath) as Promise<Pick<KCodeProviderModule, "ProviderRegistry">>,
    ]);
    return {
      bootstrap,
      contracts,
    ...
    throw new Error(
      "Missing built kcode-cli packages. Run `pnpm --filter @kcode/bootstrap^... build && pnpm --filter @kcode/bootstrap build` before `record`.",
      { cause: error },
    );
  }
}
