# @kcode/prompt-trajectory

OpenAI protocol trajectory recorder for inspecting kcode-cli prompt assembly.
...
```bash
pnpm --filter @kcode/bootstrap^... build
pnpm --filter @kcode/bootstrap build

pnpm --filter @kcode/prompt-trajectory record -- \
  --fixture /path/to/recording.json \
  --out /tmp/kcode-prompt-trajectory/basic-live

pnpm --filter @kcode/prompt-trajectory record:prompt -- \
  --prompt "Say hello in one short sentence."

pnpm --filter @kcode/prompt-trajectory derive -- \
  --out /tmp/kcode-prompt-trajectory/basic-live

pnpm --filter @kcode/prompt-trajectory model-io -- \
  --input ~/.kcode/cli/debug/model-io-<session>.jsonl \
  --out /tmp/kcode-prompt-trajectory/model-io-session
```
...
uses the same kcode model config resolution as the CLI. The upstream request is
...
`model-io` reads a real KCode `model-io-*.jsonl` file and turns the main
