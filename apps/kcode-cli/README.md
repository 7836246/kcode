...
- `npm run build` produces `dist/kcode.cjs`, which works anywhere Node.js 24.14.0 is installed.
...
kcode plugins are local bundles that can contribute skills, custom commands, and MCP servers.

Plugin state lives under `~/.kcode/cli/plugins`:
...
- `marketplaces/kcode-plugins-official/`: bundled and CDN partitions plus the merged metadata for the single official marketplace.

This repository also ships built-in official plugins as workspace packages. The bundled Browser Use, Document Skills, Skill Creator, and KCode Guide content plugins are default-enabled and appear as `browser-use@kcode-plugins-official`, `document-skills@kcode-plugins-official`, `skill-creator@kcode-plugins-official`, and `kcode-guide@kcode-plugins-official`. Runtime-heavy official plugins, and local-data migration plugins such as `ios-simulator@kcode-plugins-official`, `android-emulator@kcode-plugins-official`, and `restore-legacy-sessions@kcode-plugins-official`, are discovered by kcode but stay disabled until the user enables them.

```sh
kcode plugins list
kcode plugins enable ios-simulator
kcode plugins disable browser-use
kcode plugins enable restore-legacy-sessions
kcode plugins disable ios-simulator
```
...
MCP config can live directly in `.kcode-plugin/plugin.json` through `mcpServers`. A plugin may provide both `.mcp.json` and manifest `mcpServers`; when the same server name appears in both places, `mcpServers` from the selected manifest wins.

Supported fields in the current kcode plugin surface:
...
Example `.kcode-plugin/plugin.json` with inline MCP config:
...
```json
{
  ...
  "mcpServers": {
    "ios-simulator": {
      "command": "node",
      "args": ["${KCODE_PLUGIN_ROOT}/dist/mcp/server.js"],
      "cwd": "${KCODE_PROJECT_DIR}",
      "env": {
        "PLUGIN_DATA": "${KCODE_PLUGIN_DATA}",
...
Only environment variables with the `KCODE_` prefix are expanded. Missing variables disable the affected MCP server and produce a plugin diagnostic.
...
For MCP servers, prefer Node's normal package build and `bin` output when targeting kcode-cli, and keep all process/file/network side effects inside the MCP server boundary.

## MCP Configuration

kcode reads MCP servers from the main JSON config. The default user config path is `~/.kcode/cli/config.json`; MCP entries live under `mcp.servers`. MCP is enabled by default, so `features.mcp` only needs to be set when you want an explicit on/off switch. The current CLI does not auto-discover standalone `mcp.json` or `.mcp.json` files outside enabled plugins.
...
- `stdio`: requires `command`; accepts `args`, `cwd`, `env`, `enabled`, and `timeoutMs`. `cwd` is resolved from the active working directory, and the server process inherits kcode's environment plus any `env` overrides.
...
kcode reads hooks from the same main JSON config file as MCP, usually `~/.kcode/cli/config.json`. Hooks are disabled by default; set `hooks.enabled` to `true` and add process hooks under `hooks.events`.
