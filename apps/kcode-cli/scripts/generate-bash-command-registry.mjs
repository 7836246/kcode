...
if (check) {
  const temporaryDirectory = await mkdtemp(join(tmpdir(), "kcode-bash-registry-"));
  ...
  try {
    ...
    if (!actual.equals(expected)) {
      throw new Error(
        "Generated Bash command registry is stale. Run `pnpm --dir apps/kcode-cli registry:generate`.",
      );
    }
  } finally {
