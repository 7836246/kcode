...
`${KCODE_SKILL_DIR}/examples.md` §5. Read it when you want to see a whole script's arc; §7
...
an inline `script` is written under `.kcode/workflow-drafts/` before it is even compiled and
...
**Editing a draft asks nothing.** `.kcode/workflow-drafts/` is machine-owned and
...
- `${KCODE_SKILL_DIR}/patterns.md` — the topology catalogue: fan-out/fan-in, review sweeps,
...
- `${KCODE_SKILL_DIR}/examples.md` — complete worked scripts, including a two-subagent
...
- `script`: a one-off workflow written inline. It is saved to a file under
  `.kcode/workflow-drafts/` and the result names that file, whether the script compiled or
...
- `path`: a script file on disk, relative to the working directory or absolute — normally
  ...
  arguments in its `/* kcode-workflow` block.
...
`.kcode/workflows/<name>.dwf.ts`, committed with the repository and visible only inside it;
global definitions go in `~/.kcode/workflows/<name>.dwf.ts` and are available from every
...
- `script` or `script_path`, never both. `script_path` saves a draft a `CreateWorkflow` or
  `AmendWorkflow` result named without re-emitting it; a `/* kcode-workflow` block in that
...
The saved file is valid TypeScript: a `/* kcode-workflow` block of YAML metadata, then the
