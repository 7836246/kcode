...
import { KCODE_OFFICIAL_PLUGIN_MARKETPLACE, isOfficialMarketplaceId } from "@kcode/contracts";
import { KCODE_CUA_OFFICIAL_PLUGIN_ID, isKCodeCuaInternalFeatureEnabled } from "@kcode/shared";
...
export interface ResolveKCodePluginsOptions {
...
export interface ListKCodePluginsOptions extends ResolveKCodePluginsOptions {}

export interface SetKCodePluginEnabledOptions extends ResolveKCodePluginsOptions {
  enabled: boolean;
  plugin: string;
  scope?: "user" | "workspace";
}

export interface SetKCodePluginEnabledResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface KCodeMarketplaceSummaryData {
  id: string;
  name: string;
...
export interface KCodeAvailablePluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface KCodeInstalledPluginData {
  id: string;
  name: string;
  marketplace: string;
  description?: string;
...
export interface KCodePluginsOverviewData {
  marketplaces: KCodeMarketplaceSummaryData[];
  availablePlugins: KCodeAvailablePluginData[];
  installedPlugins: KCodeInstalledPluginData[];
  restorableBuiltins: KCodeAvailablePluginData[];
  diagnostics: PluginLoadOutcome["diagnostics"];
}

export interface KCodeMarketplaceUpdateData {
  diagnostics: PluginLoadOutcome["diagnostics"];
  marketplaces: KCodeMarketplaceSummaryData[];
}

export interface AddKCodeMarketplaceOptions extends ResolveKCodePluginsOptions {
...
export interface RemoveKCodeMarketplaceOptions extends ResolveKCodePluginsOptions {
  marketplace: string;
}

export interface UpdateKCodeMarketplaceOptions extends ResolveKCodePluginsOptions {
  abortSignal?: AbortSignal;
  marketplace?: string;
}

export interface InstallKCodeMarketplacePluginOptions extends ResolveKCodePluginsOptions {
  abortSignal?: AbortSignal;
  dryRun?: boolean;
  marketplace: string;
  pluginName: string;
...
export interface UninstallKCodeMarketplacePluginOptions extends ResolveKCodePluginsOptions {
  pluginId?: string;
  pluginName?: string;
  marketplace?: string;
  removeCache?: boolean;
  /** 保留 data/<plugin-id> 用户数据目录（`kcode plugins uninstall --keep-data`）。 */
  keepData?: boolean;
}

export interface UpdateKCodeMarketplacePluginOptions extends ResolveKCodePluginsOptions {
  abortSignal?: AbortSignal;
  pluginId: string;
}

export interface ValidateKCodePluginPathOptions extends ResolveKCodePluginsOptions {
  abortSignal?: AbortSignal;
  path: string;
}

export interface KCodePluginUpdateData extends KCodePluginInstallData {
  previousVersion: string;
}

interface RestoreBuiltinPluginOptions extends ResolveKCodePluginsOptions {
  pluginId: string;
}

interface ConfigureKCodePluginOptions extends ResolveKCodePluginsOptions {
...
interface ResetKCodePluginConfigOptions extends ResolveKCodePluginsOptions {
  pluginId: string;
  scope?: "user" | "workspace";
}

interface ValidateKCodePluginOptions extends ResolveKCodePluginsOptions {
  marketplace?: string;
  pluginName?: string;
  source?: string;
}

interface DescribeKCodePluginOptions extends ResolveKCodePluginsOptions {
  marketplace: string;
  pluginName: string;
}

export interface KCodePluginInstallData {
...
function countVisibleMarketplacePlugins(
  ...
  if (marketplaceId !== KCODE_OFFICIAL_PLUGIN_MARKETPLACE) return plugins.length;
  return plugins.filter((entry) => entry.name !== OFFICIAL_NODE_REPL_HOST_PLUGIN_NAME).length;
}

export function resolveKCodePlugins(options: ResolveKCodePluginsOptions = {}): PluginLoadOutcome {
...
export function getKCodePluginsOverview(
  options: ResolveKCodePluginsOptions = {},
): KCodePluginsOverviewData {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  const outcome = resolveKCodePlugins({
    ...options,
    configResult,
    pluginStorageRoot,
  });
  ...
  const restorableBuiltins: KCodeAvailablePluginData[] = OFFICIAL_PLUGIN_DEFINITIONS.filter(
    (def) =>
      suppressed.has(`${def.name}@${KCODE_OFFICIAL_PLUGIN_MARKETPLACE}`) &&
      // computer-use 的恢复入口需要 internal 特性开启（与 restoreBuiltinPluginCore 同口径）。
      (def.name !== "computer-use" || isKCodeCuaInternalFeatureEnabled(options.env ?? process.env)),
  ).map((def) => {
    const listing = def.listing
      ? parseEntryStoreListing({ name: def.name, ...def.listing })
      : undefined;
    return {
      id: `${def.name}@${KCODE_OFFICIAL_PLUGIN_MARKETPLACE}`,
      name: def.name,
...
export function listKCodePlugins(options: ListKCodePluginsOptions = {}): PluginLoadOutcome {
  const outcome = resolveKCodePlugins(options);
...
function loadPluginListingsById(storageRoot: string): Record<string, PluginStoreListing> {
  ...
  for (const definition of OFFICIAL_PLUGIN_DEFINITIONS) {
    ...
    if (listing) {
      listings.set(`${definition.name}@${KCODE_OFFICIAL_PLUGIN_MARKETPLACE}`, listing);
    }
  }
...
export async function setKCodePluginEnabled(
  options: SetKCodePluginEnabledOptions,
): Promise<SetKCodePluginEnabledResult> {
...
export async function addKCodePluginMarketplace(
  options: AddKCodeMarketplaceOptions,
): Promise<KCodeMarketplaceSummaryData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function removeKCodePluginMarketplace(
  options: RemoveKCodeMarketplaceOptions,
): Promise<void> {
  const { pluginStorageRoot } = resolvePluginContext(options);
...
export async function updateKCodePluginMarketplace(
  options: UpdateKCodeMarketplaceOptions,
): Promise<KCodeMarketplaceUpdateData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
...
export async function installKCodeMarketplacePlugin(
  options: InstallKCodeMarketplacePluginOptions,
): Promise<KCodePluginInstallData> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.dryRun === true) {
  ...
  const isSuppressedBundledOfficial =
    options.marketplace === KCODE_OFFICIAL_PLUGIN_MARKETPLACE &&
  ...
  if (options.marketplace === KCODE_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function uninstallKCodeMarketplacePlugin(
  options: UninstallKCodeMarketplacePluginOptions,
): Promise<KCodeInstalledPluginData | null> {
  const { configResult, pluginStorageRoot, workingDirectory } = resolvePluginContext(options);
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const pluginId = resolvePluginIdForMutation(options);

    // 官方 CDN marketplace 与内置插件共享 kcode-plugins-official id 空间，且其缓存
...
/**
 * `kcode plugins update <plugin>`：先刷新所属 marketplace 目录，再按同一条目重装。
 * cacheMarketplacePlugin 对已存在的安装记录做原地覆盖并保留 installedAt；启用态只会给
 * 用户配置里尚未显式声明的 id 补默认值，因此更新不会改变用户已经做过的开关选择。
 */
export async function updateKCodeMarketplacePlugin(
  options: UpdateKCodeMarketplacePluginOptions,
): Promise<KCodePluginUpdateData> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ...
  const refreshed = await updateKCodePluginMarketplace({
    ...options,
  ...
  const installed = await installKCodeMarketplacePlugin({
    ...options,
    marketplace: record.marketplace,
...
/** `kcode plugins validate <path>`：只读校验本地插件目录或 marketplace 目录。 */
export async function validateKCodePluginPath(
  options: ValidateKCodePluginPathOptions,
...
async function restoreBuiltinPluginCore(options: RestoreBuiltinPluginOptions): Promise<void> {
  const kcodeCuaPluginId = KCODE_CUA_OFFICIAL_PLUGIN_ID;
  if (
    options.pluginId === kcodeCuaPluginId &&
    !isKCodeCuaInternalFeatureEnabled(options.env ?? process.env)
  ) {
    // overview 虽然隐藏了恢复入口，但协议调用仍可绕过 UI 写用户配置。
    // 功能开关关闭时在写盘前失败，确保用户配置与插件缓存都保持零痕迹。
    throw new Error("computer-use built-in plugin requires KCODE_CUA_PRODUCT_HELPER to be enabled");
  }
...
export async function configureKCodePlugin(options: ConfigureKCodePluginOptions): Promise<void> {
...
export async function resetKCodePluginConfig(
  options: ResetKCodePluginConfigOptions,
...
export async function validateKCodePlugin(
  options: ValidateKCodePluginOptions,
): Promise<PluginLoadOutcome["diagnostics"]> {
  const { pluginStorageRoot } = resolvePluginContext(options);
  ensureDefaultPluginMarketplaces(pluginStorageRoot);
  if (options.source) {
...
export async function describeKCodePlugin(
  options: DescribeKCodePluginOptions,
...
function resolvePluginContext(options: ResolveKCodePluginsOptions): {
  configResult: ConfigResult;
  pluginStorageRoot: string;
  workingDirectory: string;
} {
...
function resolvePluginIdForMutation(options: UninstallKCodeMarketplacePluginOptions): string {
...
function resolvePluginConfigPath(
  options: ResolveKCodePluginsOptions & { scope?: "user" | "workspace" },
  configResult: ConfigResult,
  workingDirectory: string,
): string {
  if (options.scope !== "workspace") {
  ...
  // Workspace Plugin 配置固定落在当前 `<workspace>/.kcode/config.json`。嵌套 workspace
  // 可能同时发现仓库根与自身的配置，读取端 innermost 优先；写入端也必须锁定当前
  // workspace，不能用 project discovery 的第一个 outermost 文件。
  const workspaceConfigPath = join(workingDirectory, ".kcode", "config.json");
