...
export interface ProcessProviderRegistryRuntimeOptions {
  ...
  readonly standalone?: {
    ...
    readonly onBuiltinRefreshResult?: (event: KCodeBuiltinRefreshEvent) => void;
  };
}
...
export async function startProcessProviderRegistryRuntime(
  ...
  if (!paths) {
    throw new Error("缺少进程 Provider Registry 的 KCode Built-in / Personal Config 路径");
  }

  const accountSource = new MutableAccountProviderConfigSource();
  const credentialStore = options.standalone
    ? (options.standalone.credentialStore ?? createSharedKCodeCredentialStore({ env: { ...env } }))
    : undefined;
  let standaloneAccount: AccountProviderService | undefined;
  const bundledFile = options.standalone
    ? env[KCODE_BUILTIN_PROVIDER_BUNDLED_CONFIG_FILE_ENV]?.trim()
    : undefined;
  const runtime = new NodeProviderRegistryRuntime({
    ...paths,
    ...(bundledFile
      ? {
          kcodeBuiltinFilePath: bundledFile,
          kcodeBuiltinActiveFilePath: paths.kcodeBuiltinFilePath,
          kcodeBuiltinRemote: {
            controlFilePath: join(
              dirname(paths.kcodeBuiltinFilePath),
              "kcode-builtin-refresh.json",
            ),
            resolveEndpointKey: () => resolveRuntimeKCodeEndpointOrigin(env),
            fetchRelease: (endpointOrigin, signal) =>
              downloadKCodeBuiltinRelease({
                endpointOrigin,
                signal,
                appVersion: KCODE_VERSION,
                platform: resolveKCodeBuiltinClientPlatform(),
      ...
      : {}),
    onKCodeBuiltinRefreshError: options.standalone?.onBuiltinRefreshError,
    accountSource,
    ...(credentialStore
      ? {
  ...
  const disposeRecovery = standaloneAccount
    ? runtime.onDidCheckKCodeBuiltin(async () => {
        const [config, account] = await Promise.all([
          runtime.configService.read(),
          standaloneAccount!.read(),
        ]);
        if (config.kcodeBuiltinRevision !== account.basedOnKCodeBuiltinRevision)
...
export function parseProcessAccountProviderConfigSnapshot(input: {
  ...
  const basedOnKCodeBuiltinRevision = input.basedOnKCodeBuiltinRevision.trim();
  if (!basedOnKCodeBuiltinRevision) {
