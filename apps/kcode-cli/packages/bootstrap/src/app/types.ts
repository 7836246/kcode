import type { KCodeToolExecResource, BackgroundBashOutputResult } from "@kcode/shared";
import type { AiSdkModelAdapter } from "@kcode/adapters/model";
import type {
  AgentRuntime,
  AgentRuntimeConfig,
  ExecuteTurnOptions,
...
import type { KCodeModelOption } from "@kcode/shared";
import type { EffectiveModelSelectionResult } from "@kcode/shared/model-selection";
export type { KCodeModelOption } from "@kcode/shared";
import type { ModelProviderSourceTitle } from "../model-config.js";
import type { KCodeInstalledPluginData } from "../plugins.js";
import type {
  AutomationPort,
  OffPeakPort,
...
import type { AgentTelemetryRuntimeOwner, WorkspaceHookPolicy } from "@kcode/contracts";
...
export type KCodeAppRuntimeConfigInput = AgentRuntimeConfig;

export interface KCodeAppOptions {
  sessionId?: SessionId;
  resume?: boolean;
  version?: string;
  traceContext?: TraceContext;
  runtimeConfig?: KCodeAppRuntimeConfigInput;
  /**
  ...
  onToolExecResource?: (sample: KCodeToolExecResource) => void;
...
export type SubmitPromptOptions = SubmitPromptOptionsBase &
  import("@kcode/contracts").TurnBackgroundAttribution;
...
export interface KCodePluginSetResult {
  enabled: boolean;
  path: string;
  plugin: PluginMetadata;
}

export interface KCodePluginUninstallResult {
  // null 表示该 plugin id 当前未安装（幂等 no-op），调用方据此提示"未安装"。
  removed: KCodeInstalledPluginData | null;
}
...
export interface KCodeApp {
  ...
  getCurrentModelOption?(): KCodeModelOption | undefined;
  /** 只读 Registry 元数据；不要求选项完整，也不绑定执行模型。 */
  getModelOption?(selection: ModelSelection): KCodeModelOption | undefined;
  ...
  readSubagents(input?: {
    endedCursor?: string;
    endedLimit?: number;
  }): Promise<import("@kcode/shared").KCodeSessionSubagentsResult>;
  ...
  listModels(): KCodeModelOption[];
  listThoughtLevels(): string[];
  listPlugins(): Promise<PluginLoadOutcome>;
  setPluginEnabled(plugin: string, enabled: boolean): Promise<KCodePluginSetResult>;
  uninstallPlugin(plugin: string): Promise<KCodePluginUninstallResult>;
  /**
...
export interface RunKCodeProtocolAgentOptions {
...
export interface ListKCodeSessionsOptions {
  directory?: string;
  env?: NodeJS.ProcessEnv;
  limit?: number;
  sessionStore?: SessionStorePort;
