import { updateUiLocaleInFileConfig, type ConfigResult } from "@kcode/adapters/config";
import type { AgentRuntime } from "@kcode/core";
import { resolveLocale } from "@kcode/i18n";
import { normalizeModelSelection, type ModelSelection } from "@kcode/provider";
import {
...
import type { PrepareUserExecutionBoundary, KCodeApp } from "./types.js";
...
export function createSessionFacade(deps: CreateSessionFacadeDeps): SessionFacade {
  ...
  return {
    ...
    setTarget: async (input) =>
      (await setTargetStatus("set", input)) as Awaited<ReturnType<KCodeApp["setTarget"]>>,
    updateTargetStatus: async (status) =>
      (await setTargetStatus("status_updated", { status })) as Awaited<
        ReturnType<KCodeApp["updateTargetStatus"]>
      >,
