...
} from "@kcode/adapters/exec";
import { createNodeFileSystemAdapter } from "@kcode/adapters/fs";
import { createNodeWebFetchHttpClientAdapter } from "@kcode/adapters/http";
import { createJimpImageProcessorAdapter } from "@kcode/adapters/image";
import { createPopplerPdfDocumentAdapter } from "@kcode/adapters/pdf";
import { createNodeSessionMailboxAdapter } from "@kcode/adapters/mailbox";
import { createNodeContextSourceAdapter } from "@kcode/adapters/context";
import { createNodeSkillAdapter } from "@kcode/adapters/skills";
import { createMcpAdapter } from "@kcode/adapters/mcp";
import {
  AgentRuntime,
  PermissionService,
...
import { createModelTelemetry } from "@kcode/telemetry";
import {
...
import { isRemoteWorkspaceIdentity, resolveKCodeRuntimeEnv } from "@kcode/shared";
import {
...
import { resolveKCodeCustomCommandPrompt } from "../custom-command-prompt.js";
import { resolveKCodeBuiltinPromptCommand } from "../builtin-prompt-command.js";
import { collectDisabledPaths } from "../skill-command-overrides.js";
import { loadPluginAgentProfiles, loadKCodeAgentProfiles } from "../subagents.js";
...
export async function createKCodeApp(options: KCodeAppOptions): Promise<KCodeApp> {
  if (!options?.providerRegistry) {
    throw new Error("createKCodeApp requires a Provider Registry");
  }
  ...
  const modelLogger = loggerFactory.createLogger("kcode").child({
    ...traceContextToLogContext(traceContext),
  ...
  try {
    ...
    const modelIoDir = getModelIoDir(
      cliStorageRoot,
      resolveKCodeRuntimeEnv(options.env ?? process.env) === "development",
    );
    const kcodeSubagentProfileOutcome = await loadKCodeAgentProfiles({
      logger,
      storageRoot,
      workingDirectory,
    });
    const kcodeSubagentProfiles = kcodeSubagentProfileOutcome.profiles;
    ...
    const pluginSubagentProfiles = loadPluginAgentProfiles({
      ...
      reservedProfileNames: kcodeSubagentProfiles.map((profile) => profile.name),
      modelSelectionOverrides: kcodeSubagentProfileOutcome.pluginAgentModelSelectionOverrides,
    }).profiles;
    ...
    const subagentProfiles = [...kcodeSubagentProfiles, ...pluginSubagentProfiles];
    ...
    let { configuredMcpServers, runtimeConfig, untrustedProjectMcpServers } =
      resolveAppRuntimeConfig({
        ...
        builtInSubagentModelSelectionOverrides:
          kcodeSubagentProfileOutcome.builtInModelSelectionOverrides,
    ...
    startupTimer.mark("KCode runtime configuration resolved", {
      ...
      ...(options.workspaceHookReviewHost
        ? {
            emitReviewEvent: async (event) => {
              if (!runtime) throw new Error("KCode runtime is not initialized yet.");
    ...
    const sessionMailboxPort =
      ...
      (messageEnabled
        ? createNodeSessionMailboxAdapter({
            rootDir: resolvePath(
              (options.env ?? process.env).KCODE_MAILBOX_ROOT ?? "~/.kcode/mailbox",
            ),
          })
        : undefined);
    markStorageAdaptersInitialized({
    ...
    const inputFacade = createInputFacade({
      ...
      customCommandPromptResolver: async (text, resolverOptions) => {
        const builtinPrompt = resolveKCodeBuiltinPromptCommand(text, {
          workingDirectory,
        });
        if (builtinPrompt !== undefined) {
          return builtinPrompt;
        }
        return await resolveKCodeCustomCommandPrompt(text, {
      ...
      if (input.messageId && input.attachmentIndex !== undefined) {
        ...
        if (persistedAttachment?.type === "file") {
          ...
          artifactUri =
            ...
            (persistedAttachment.url.startsWith("kcode-artifact://")
    ...
    return {
      ...
      readPromptAttachment: async (input) => {
        ...
        if (ref.startsWith("kcode-artifact://")) {
      ...
      statPromptAttachment: async (input) => {
        ...
        if (artifactUri) {
          if (!artifactStore.statToolResultArtifact) {
            throw new KCodeAttachmentFaultError(KCODE_ATTACHMENT_FAULT_CODES.statUnsupported);
          }
        ...
        if (result.kind !== "file") {
          ...
          throw new KCodeAttachmentFaultError(KCODE_ATTACHMENT_FAULT_CODES.statNotFile);
        }
        return {
    ...
    startupTimer.fail("KCode app startup failed", error, {
