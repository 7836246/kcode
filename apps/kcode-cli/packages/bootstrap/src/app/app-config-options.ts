...
import { detectLocale, resolveLocale } from "@kcode/i18n";
import type { RuntimeConfigPatch, SupportedLocale, UiLocale } from "@kcode/contracts";
import type { KCodeAppOptions } from "./types.js";

export function isMessageEnabled(env: NodeJS.ProcessEnv): boolean {
  return env.KCODE_MESSAGE_ENABLED === "1" || env.KCODE_MESSAGE_ENABLED === "true";
}

export function createConfigCliOverrides(options: KCodeAppOptions): RuntimeConfigPatch | undefined {
...
function detectAppLocale(options: KCodeAppOptions): SupportedLocale | undefined {
