...
import type { Logger, SkillRoot } from "@kcode/contracts";
import { DYNAMIC_WORKFLOW_SKILL_NAME } from "@kcode/contracts";
import { candidateBaseDirs } from "./bundled-plugins.js";

/**
 * 随 CLI 内置的技能包（apps/kcode-cli/packages/bundled-skills）。
 *
 ...
 * - SEA 二进制：资产内嵌在 `kcode-bundled-skills/` 前缀下，首启按内容 hash 解压到
 *   `<cli storage>/bundled-skills/<hash>/`；目录名即内容身份，重复启动幂等，并发只会有一个赢家。
 * - 远端主机：prepare-prebuilds 把目录 stage 到远端 kcode.cjs 旁，与桌面同路。
 */
...
export const SEA_BUNDLED_SKILL_ASSET_PREFIX = "kcode-bundled-skills/";
const SEA_BUNDLED_SKILL_MANIFEST_ASSET_KEY = `${SEA_BUNDLED_SKILL_ASSET_PREFIX}manifest.json`;
const SEED_MARKER_FILE = ".kcode-bundled-skills-seed.json";
