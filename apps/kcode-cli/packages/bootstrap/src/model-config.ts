...
export function createRuntimeAiSdkModelExecutionConfig(
  ...
  return {
    defaultHeaders: buildCliKCodeSourceHeaders(env, options),
    env,
    ...(network ? { network } : {}),
  };
}
...
function buildCliKCodeSourceHeaders(
  env: EnvRecord,
  ...
  return {
    "HTTP-Referer": resolveRuntimeKCodeEndpointOrigin(env),
    "User-Agent": `KCode/${appVersion ?? "unknown"}`,
    ...(appVersion ? { "X-KCode-App-Version": appVersion } : {}),
    "X-Title": `Z Code@${sourceTitle}`,
    "X-Release-Channel": resolveRuntimeKCodeEnv(env),
...
function resolveAppVersionForHeaders(
  ...
  return normalizePrintableHeaderValue(env[KCODE_APP_VERSION_ENV] ?? options.appVersion);
}
