import { PERMISSION_FULL_ACCESS_OPTION_ID } from "@kcode/shared/kcode-protocol-v4";
...
import {
  ...
  isKCodeFileStreamingToolInputPreviewTool,
  parseKCodeBackgroundTaskNotificationText,
  resolveKCodeBackgroundTaskControlKind,
...
export class ProductProjection {
  ...
  private applyBackgroundTaskNotification(fact: CanonicalUserIntentFact): ConversationDelta[] {
    const parsed = parseKCodeBackgroundTaskNotificationText(fact.input);
    if (!parsed) return [];
    const row = this.findToolRow(parsed.toolUseId);
    if (!row) return [];

    const notificationStatus = kcodeBackgroundTaskNotificationToolUpdateStatus(
  ...
  private openToolRow(
    ...
    if (isKCodeFileStreamingToolInputPreviewTool(row.toolName)) {
  ...
  private appendStreamingToolInput(
    ...
    if (
      state.lastPublishedAt !== null &&
      now - state.lastPublishedAt < KCODE_FILE_STREAMING_TOOL_INPUT_PREVIEW_MIN_INTERVAL_MS
    ) {
      return [];
    }
  ...
  private onBackgroundTaskLifecycle(event: SessionEvent): ConversationDelta[] {
    ...
    const legacyKind = resolveKCodeBackgroundTaskControlKind(payload);
  ...
  //
  // 归约本体在 @kcode/shared 的 workflow-runs-reducer（与状态 schema 同居）：TUI 镜像要用
  // 同一份归约，两处各写一份就是两个时钟。
