...
import { getModelUsageTotalTokens, SessionEventType } from "@kcode/contracts";
import { parseAutomationRunId } from "@kcode/shared";
...
export function streamingParentToolCallId(payload: Record<string, unknown>): string | undefined {
  ...
  const kcode = recordValue(meta.kcode);
  return (
    ...
    optionalString(kcode.parentToolCallId) ??
    optionalString(kcode.parentToolUseId)
  );
}
