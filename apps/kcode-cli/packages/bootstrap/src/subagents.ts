...
import { migrateUserSubagentMarkdown, migrateSubagentStateFile } from "@kcode/shared/node";
import {
  parseAgentProfileFromMarkdown,
  type AgentProfile,
  type AgentProfileParseDiagnostic,
} from "@kcode/core";
import type { Logger, PluginMetadata } from "@kcode/contracts";
import {
  createAgentStateId,
...
interface LoadKCodeAgentProfilesInput {
  logger?: Logger;
  storageRoot: string;
  workingDirectory: string;
}

interface LoadKCodeAgentProfilesResult {
...
export async function loadKCodeAgentProfiles(
  input: LoadKCodeAgentProfilesInput,
): Promise<LoadKCodeAgentProfilesResult> {
  ...
  const roots = [
    ...
    { path: join(input.workingDirectory, ".kcode", "agents"), source: "project" as const },
  ];
...
function sanitizeProjectAgentProfile(profile: AgentProfile): AgentProfile {
  ...
  // 项目级 .kcode/agents/*.md 是仓库内容，不能通过 frontmatter
