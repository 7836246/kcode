...
import { openStartupSqliteSessionStore } from "@kcode/adapters/storage";
import type { SessionInfo } from "@kcode/contracts";
import type { ListKCodeSessionsOptions, ResolveLatestSessionOptions } from "./app/types.js";
...
export async function listKCodeSessions(
  options: ListKCodeSessionsOptions = {},
): Promise<SessionInfo[]> {
  const ownsStore = options.sessionStore === undefined;
  let sessionStore = options.sessionStore;
  if (!sessionStore) {
