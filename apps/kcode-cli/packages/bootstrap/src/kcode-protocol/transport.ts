...
import { kcodeProtocolMessageSchema, kcodeProtocolMethods } from "@kcode/shared";
import type { Logger } from "@kcode/contracts";

type KCodeProtocolOutgoingMessage =
  | KCodeProtocolError
  | KCodeProtocolNotification
  | KCodeProtocolRequest
  | KCodeProtocolResponse;

type KCodeProtocolMessageHandler = (
  message: KCodeProtocolMessage,
) => Promise<KCodeProtocolOutgoingMessage | undefined>;

const PROTOCOL_EOF_DRAIN_MS = 100;

interface KCodeProtocolNdjsonConnectionOptions {
  signal?: AbortSignal;
  clearPostResponseMessages?: () => void;
  handleMessage: KCodeProtocolMessageHandler;
  ...
  ) => readonly KCodeProtocolOutgoingMessage[];
  takePostResponseBatch?: (requestId: KCodeProtocolRequestId) => {
    readonly messages: readonly KCodeProtocolOutgoingMessage[];
    commit(): boolean;
  } | null;
}

export class KCodeProtocolNdjsonConnection {
  private buffer = "";
  ...
  constructor(private readonly options: KCodeProtocolNdjsonConnectionOptions) {
  ...
  send(message: KCodeProtocolOutgoingMessage): void {
  ...
  private readonly onClose = (): void => {
    ...
    this.draining = true;
    this.notifyTransportClosed(new Error("KCode Protocol client connection closed"));
  ...
  private readonly onAbort = (): void => {
    this.notifyTransportClosed(new Error("KCode Protocol runtime stopping"));
    this.finish();
  };
  ...
  private decodeLine(line: string): KCodeProtocolMessage | null {
    let decoded: unknown;
    try {
      decoded = JSON.parse(line);
    } catch (error) {
      this.options.logger?.warn("KCode Protocol JSON parse failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "kcode_protocol.parse.failed",
    ...
    const parsed = kcodeProtocolMessageSchema.safeParse(decoded);
    if (!parsed.success) {
      this.sendError("invalid-message", -32600, "Invalid KCode Protocol message", {
  ...
  private shouldBypassProcessingQueue(message: KCodeProtocolMessage): boolean {
    // 模型任务占住串行队列时，停止/取消请求必须仍能进入 server，
    // 才能把底层 AbortSignal 传给真实模型请求。控制面只旁路当前执行，普通请求仍保持串行。
    return (
      "id" in message &&
      "method" in message &&
      (message.method === kcodeProtocolMethods.sessionStop ||
        message.method === kcodeProtocolMethods.workspaceCancelGenerateText)
    );
  }

  private async handleMessage(message: KCodeProtocolMessage): Promise<void> {
    if (this.terminal) return;
