import type { OffPeakPort, OffPeakTaskSummary } from "@kcode/contracts";
import {
  kcodeOffPeakCreateResultSchema,
  kcodeOffPeakListResultSchema,
  kcodeProtocolMethods,
  type KCodeOffPeakTaskProtocolSnapshot,
} from "@kcode/shared";
import type {
  ...
  return {
    async create(input, createContext) {
      ...
      if (boundSessionId) {
        ...
        try {
          const listed = await context.requestClient(
            kcodeProtocolMethods.offPeakList,
            {},
            kcodeOffPeakListResultSchema,
          );
      ...
      const result = await context.requestClient(
        kcodeProtocolMethods.offPeakCreate,
        {
          title: input.title,
          prompt: input.prompt,
...
function toOffPeakTaskSummary(input: KCodeOffPeakTaskProtocolSnapshot): OffPeakTaskSummary {
  return {
