...
import { SqliteSessionStore, type SqliteMigrationProgress } from "@kcode/adapters/storage";
import {
  classifyDatabaseStartupError,
  databaseStartupErrorDetails,
  kcodeProtocolNotifications,
  kcodeStorageStartupStateSchema,
  type KCodeStorageStartupState,
} from "@kcode/shared";

export async function openProtocolStartupStorage(options: {
  dbPath: string;
  output: NodeJS.WritableStream;
  onProgress?: (progress: KCodeStorageStartupState) => void;
  ...
  const report = async (progress: SqliteMigrationProgress) => {
    const params = kcodeStorageStartupStateSchema.parse({
      schemaVersion: 1,
      attemptId,
      databaseId,
    ...
    await new Promise<void>((resolve, reject) => {
      options.output.write(
        `${JSON.stringify({ method: kcodeProtocolNotifications.storageStartup, params })}\n`,
...
export async function prepareProtocolStartupStorage(options: {
  ...
  const { kcodeStoragePathReadySchema, classifyDatabaseStartupError } =
    await import("@kcode/shared");
  ...
  const acknowledgement = new Promise<boolean>((resolve, reject) => {
    ...
    lines.once("line", (line) => {
      try {
        ...
        const ack = kcodeStoragePathReadySchema.parse(JSON.parse(line));
