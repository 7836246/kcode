...
import {
  ...
  kcodeProviderRuntimeHeadersResponseSchema,
  type KCodeWorkspaceRef,
} from "@kcode/shared";
import type { KCodeAppOptions } from "../app/types.js";
import {
...
): NonNullable<KCodeAppOptions["providerRuntimeHeadersPort"]> {
  return {
    ...
    async refreshBeforeModelRequest(input) {
      ...
      try {
        result = await context.requestClient(
          kcodeProtocolMethods.interactionRequestProviderRuntimeHeaders,
          {
        ...
        if (input.abortSignal?.aborted || timedOut) {
          context.notify({
            method: kcodeProtocolNotifications.providerRuntimeHeadersCancelled,
            trace: protocolTraceFromTraceContext(input.traceContext),
