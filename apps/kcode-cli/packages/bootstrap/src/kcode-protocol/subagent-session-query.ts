...
import type { KCodeSessionEndedSubagent, KCodeSessionRunningSubagent } from "@kcode/shared";
...
interface SessionSubagentProjection {
  revision: number;
  running: KCodeSessionRunningSubagent[];
  ended: KCodeSessionEndedSubagent[];
}
...
function runningStatus(input: {
  ...
  parentProjection?: SessionProjection;
}): KCodeSessionRunningSubagent["status"] | undefined {
...
): KCodeSessionEndedSubagent["status"] | undefined {
  switch (task?.status) {
    case "completed":
      return "success";
    case "cancelled":
...
function endedStatus(input: {
  ...
  childOutcome: ReturnType<typeof lastChildOutcome>;
  childProjection?: SessionProjection;
}): KCodeSessionEndedSubagent["status"] {
...
export function projectSessionSubagents(
  ...
  const running: KCodeSessionRunningSubagent[] = [];
  const ended: KCodeSessionEndedSubagent[] = [];
...
function encodeCursor(item: KCodeSessionEndedSubagent): string {
  return Buffer.from(
...
export function paginateEndedSubagents(
  ended: readonly KCodeSessionEndedSubagent[],
  options: { cursor?: string; limit: number },
): { items: KCodeSessionEndedSubagent[]; nextCursor?: string } {
