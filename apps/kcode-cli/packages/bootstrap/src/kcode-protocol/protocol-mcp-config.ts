...
import type { KCodeProtocolMcpServer } from "@kcode/shared";

export function protocolMcpServersToRuntimeMcpConfig(
  servers: KCodeProtocolMcpServer[] | undefined,
  ...
  for (const server of servers) {
    if ("command" in server) {
      runtimeServers[server.name] = {
        ...
        // KCode Protocol 的 mcpServers 是 session/runtime 和状态查询的完整覆盖配置。
