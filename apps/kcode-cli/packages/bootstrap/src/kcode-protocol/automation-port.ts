...
import {
  ...
  kcodeAutomationCheckTaskBindingResultSchema,
  kcodeAutomationCreateResultSchema,
  kcodeAutomationDeleteResultSchema,
  kcodeAutomationListResultSchema,
  kcodeAutomationUpdateResultSchema,
  kcodeProtocolMethods,
  type KCodeAutomationProtocol,
} from "@kcode/shared";
import {
...
export function createProtocolAutomationPort(
  ...
  return {
    async create(input, createContext) {
        ...
        try {
          try {
            const result = await context.requestClient(
              kcodeProtocolMethods.automationCheckTaskBinding,
            ...
            const legacyResult = await context.requestClient(
              kcodeProtocolMethods.automationList,
              {},
              kcodeAutomationListResultSchema,
            );
      ...
      try {
        result = await context.requestClient(
          kcodeProtocolMethods.automationCreate,
          {
    ...
    async update(input) {
      const hasIntervalCarrier = input.intervalUnit !== undefined && input.interval !== undefined;
      const result = await context.requestClient(
        kcodeProtocolMethods.automationUpdate,
        {
    ...
    async delete(input) {
      const result = await context.requestClient(
        kcodeProtocolMethods.automationDelete,
        { automationId: input.id },
        kcodeAutomationDeleteResultSchema,
      );
...
function normalizeCronAutomationMode(
  mode: KCodeAutomationProtocol["mode"],
): CronAutomation["mode"] {
  switch (mode) {
    case undefined:
      return undefined;
...
function toCronAutomation(input: KCodeAutomationProtocol): CronAutomation {
  return {
