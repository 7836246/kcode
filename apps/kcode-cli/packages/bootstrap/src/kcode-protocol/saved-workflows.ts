...
// 解析器与序列化器只从 @kcode/core 取：这里不解析 frontmatter，也不拼 YAML。
//
// 全局作用域：五个方法的 params 收可选 `scope`（缺省
// `project`）。`global` 时改按本机全局根（`~/.kcode/workflows/`）操作，`workspace` 只是**载体**——
// 处理器对全局档不读它的路径。`workflows/move` 把全局档搬回 `workspace` 项目（只此一向）。
import { unlink, writeFile } from "node:fs/promises";
import { SavedWorkflowMetaSchema, isValidSavedWorkflowName } from "@kcode/contracts";
import {
  listSavedWorkflows,
  moveSavedWorkflow,
...
import {
  KCODE_WORKFLOWS_RUNS_MAX_LIMIT,
  kcodeWorkflowsDeleteParamsSchema,
  kcodeWorkflowsGetParamsSchema,
  kcodeWorkflowsListParamsSchema,
  kcodeWorkflowsMoveParamsSchema,
  kcodeWorkflowsRunsParamsSchema,
  kcodeWorkflowsUpdateMetaParamsSchema,
  type KCodeSavedWorkflowRun,
  type KCodeSavedWorkflowScope,
  type KCodeWorkflowsDeleteResult,
  type KCodeWorkflowsGetResult,
  type KCodeWorkflowsListResult,
  type KCodeWorkflowsMoveResult,
  type KCodeWorkflowsRunsResult,
  type KCodeWorkflowsUpdateMetaResult,
} from "@kcode/shared";
import type { JournalStorePort } from "@kcode/dynamic-workflow";
...
function scopeOf(params: { scope?: KCodeSavedWorkflowScope }): KCodeSavedWorkflowScope {
...
export async function listSavedWorkflowsOp(
  ...
  rawParams: unknown,
): Promise<KCodeWorkflowsListResult> {
  const params = parseParams(kcodeWorkflowsListParamsSchema, rawParams);
...
export async function getSavedWorkflowOp(
  _context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodeWorkflowsGetResult> {
  const params = parseParams(kcodeWorkflowsGetParamsSchema, rawParams);
...
export async function updateSavedWorkflowMetaOp(
  ...
  const params = parseParams(kcodeWorkflowsUpdateMetaParamsSchema, rawParams);
...
export async function deleteSavedWorkflowOp(
  ...
  const params = parseParams(kcodeWorkflowsDeleteParamsSchema, rawParams);
...
export async function listSavedWorkflowRunsOp(
  ...
  const params = parseParams(kcodeWorkflowsRunsParamsSchema, rawParams);
  ...
  const limit = Math.min(KCODE_WORKFLOWS_RUNS_MAX_LIMIT, params.limit);
  ...
  const runs: KCodeSavedWorkflowRun[] = page.map((row) => ({
    runId: row.runId,
...
): Map<string, KCodeSavedWorkflowRun["artifacts"]> {
  const byRun = new Map<string, KCodeSavedWorkflowRun["artifacts"]>();
...
export async function moveSavedWorkflowOp(
  _context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodeWorkflowsMoveResult> {
  const params = parseParams(kcodeWorkflowsMoveParamsSchema, rawParams);
