...
import {
  createKCodeProcessResourceSampler,
  type KCodeProcessResourceSampler,
...
export function startProtocolResourceSampler(
  server: KCodeProtocolAgentServer,
  send: (message: KCodeProtocolNotification) => void,
  logger: Logger,
): KCodeProcessResourceSampler | undefined {
  try {
    // 本地内存诊断日志：复用同一 60s 节拍，
    // 变化/心跳门控后才写一行，避免与消息流同频刷盘。
    const memoryDiagnosticsGate = createMemorySampleWriteGate();
    const sampler = createKCodeProcessResourceSampler({
      onSample: (sample, memoryUsage) => {
        send({
          method: kcodeProtocolNotifications.processResourceSample,
          params: sample,
        });
        ...
        try {
          ...
          if (reason) {
            ...
            logger.info("Process memory sample", {
              event: "kcode_protocol.process.memory_sample",
              reason,
              ...memoryFields,
              counters,
            });
