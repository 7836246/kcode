...
import {
  ...
  kcodeUserInputResponseSchema,
  type KCodePermissionOption,
  type KCodePermissionResponse,
  type KCodeUserInputQuestion,
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        kcodeProtocolMethods.interactionRequestPermission,
        {
          input: request.input,
    ...
    // v4 answer → KCodePermissionResponse：optionId 语义来自 v4 reducer 合成的
...
function v4AnswerToPermissionResponse(
  ...
  permissionOptions: KCodePermissionOption[],
  toolName: string,
  ...
  if (exact) {
    ...
    // option.response——wire 上 kcodePermissionUpdateSchema 是 strict，旧桌面多一个字段就丢事件。
...
async function requestUserInput(
  ...
  const response = await raceClientRequestWithV4Interaction(
    ...
    (signal) =>
      context.requestClient(
        kcodeProtocolMethods.interactionRequestUserInput,
        {
          input: request.input,
...
function v4AnswerToUserInputResponse(answer: V4InteractionAnswer): KCodeUserInputResponse {
...
function v4AnswerToPlanApprovalResponse(answer: V4InteractionAnswer): KCodeUserInputResponse {
...
function createExitPlanModeApprovalQuestion(): KCodeUserInputQuestion {
  return {
    header: "Plan",
    options: [
      {
...
function mapAskUserQuestion(question: AskUserQuestion): KCodeUserInputQuestion {
  return {
...
async function readPersistedAutoResolution(
  ...
  try {
    ...
    context.logger?.warn("Failed to restore user input auto-resolution state", {
      ...
      event: "kcode_protocol.user_input_auto_resolution_restore_failed",
      interactionId: request.requestId,
