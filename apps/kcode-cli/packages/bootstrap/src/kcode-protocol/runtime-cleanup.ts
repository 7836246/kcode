import type { Logger, McpPort } from "@kcode/contracts";
import type { McpConnectionPool, McpTelemetryTracker } from "@kcode/adapters/mcp";
import type { SqliteSessionStore } from "@kcode/adapters/storage";
import { shutdownPreparedModelTelemetry } from "@kcode/telemetry";
...
import type { KCodeProtocolAgentServer } from "./server.js";
...
export async function cleanupProtocolRuntime(options: {
  logger: Logger;
  deadlineAt?: number;
  server?: Pick<KCodeProtocolAgentServer, "shutdown" | "disposeProjections">;
  processResourceSampler?: Pick<KCodeProcessResourceSampler, "stop">;
  ...
  const step = async (resource: string, cleanup: () => unknown | Promise<unknown>) => {
    ...
    try {
      ...
      options.logger.warn(`KCode Protocol ${resource} shutdown failed`, {
        errorType: error instanceof Error ? error.name : typeof error,
        event: `kcode_protocol.${resource}.shutdown.failed`,
