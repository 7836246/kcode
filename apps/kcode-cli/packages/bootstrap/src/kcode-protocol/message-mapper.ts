import type { KCodeMessagePart, KCodeMessageWithParts, KCodeToolState } from "@kcode/shared";
import type { MessagePart, MessageWithParts, ToolState } from "@kcode/contracts";
import { shouldHideInvalidToolCallFromProduct } from "../tool-call-product-visibility.js";

export function mapMessageWithParts(message: MessageWithParts): KCodeMessageWithParts {
  return {
    info:
...
function mapMessagePart(part: MessagePart): KCodeMessagePart {
  const base = {
...
function mapToolState(state: ToolState): KCodeToolState {
  switch (state.status) {
    case "pending":
...
function mapCompletedToolStateMetadata(
  ...
  // KCode protocol 的 completed tool state 要求 metadata 必须是 object。
  return visibleMetadata;
}
