...
import { workspaceHookPolicySchema } from "@kcode/contracts";
import type { WorkspaceHookPolicyProvider } from "@kcode/core";
import {
  kcodeWorkspaceHookTrustGrantParamsSchema,
  kcodeWorkspaceHookTrustGrantReasonCodeSchema,
  kcodeWorkspaceHookTrustGrantResultSchema,
  type KCodeWorkspaceHookTrustGrantReasonCode,
  type KCodeWorkspaceHookTrustGrantResult,
} from "@kcode/shared";
import {
...
export async function grantWorkspaceHookTrustForProtocol(
  ...
  const params = kcodeWorkspaceHookTrustGrantParamsSchema.parse(rawParams);
  ...
  if (policyRejection) {
    return kcodeWorkspaceHookTrustGrantResultSchema.parse({
      accepted: false,
...
): KCodeWorkspaceHookTrustGrantReasonCode | undefined {
  try {
...
function toPublicReasonCode(error: unknown): KCodeWorkspaceHookTrustGrantReasonCode {
  const candidate = error instanceof Error ? error.message : error;
  const parsed = kcodeWorkspaceHookTrustGrantReasonCodeSchema.safeParse(candidate);
