/* eslint-disable max-lines -- KCode Protocol 的 session/workspace 方法共享同一个 server context 与 snapshot helpers，迁移期先集中维护。 */
import { observeSessionDebug } from "./session-debug.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../kcode-protocol-v4/task-list-session-membership.js";
import { resolveEffectiveBashShellSelection } from "@kcode/adapters/exec";
import { inputIntentMetadata } from "../kcode-protocol-v4/commands/input-intent.js";
import { createModelExecutionContext } from "./model-execution.js";
import type { SendInputOptions } from "../app/types.js";
import { repairPersistedRemoteSessionPaths, type TurnAttachment } from "@kcode/core";
import {
  CoreErrorType,
...
import {
  ...
  kcodeProtocolErrorCodes,
  kcodeProtocolMethods,
  kcodeSessionCancelBackgroundTaskParamsSchema,
  kcodeSessionCompactParamsSchema,
  kcodeSessionCloseParamsSchema,
  kcodeSessionCreateParamsSchema,
  kcodeSessionEventsParamsSchema,
  kcodeSessionForkParamsSchema,
  kcodeSessionGoalParamsSchema,
  kcodeSessionListParamsSchema,
  kcodeSessionMessagesParamsSchema,
  kcodeSessionReadParamsSchema,
  kcodeSessionRuntimePreferencesResultSchema,
  kcodeSessionResumeParamsSchema,
  kcodeSessionSendParamsSchema,
  kcodeSessionSetModeParamsSchema,
  kcodeSessionSetModelParamsSchema,
  kcodeSessionSetThoughtLevelParamsSchema,
  kcodeSessionStopParamsSchema,
  kcodeSessionSubscribeParamsSchema,
  kcodeSessionSubagentsParamsSchema,
  kcodeTaskTokenUsageParamsSchema,
  kcodeUsageStatsParamsSchema,
  kcodeWorkspaceGenerateTextParamsSchema,
  getConversationMessageProjectionPolicy,
  parseRemoteWorkspaceIdentity,
  type KCodeAutomationBotDeliveryTarget,
  type KCodeSessionCreateParams,
  type KCodeDeliveryKind,
  type IntegratedTerminalShellSelection,
  type KCodeSessionRuntimePreferencesScope,
  type KCodeSessionRuntimePreferencesResult,
  type KCodeModelContextBudgetStrategy,
  type KCodeProtocolTrace,
  type KCodeSessionEvent,
  type KCodeSessionHistoryTarget,
  type KCodeSessionResumeParams,
  type KCodeSessionPersistence,
...
import {
  ...
  type KCodeProtocolSessionRecord,
  type KCodeProtocolToolInputTransmissionState,
} from "./server-types.js";
import { createWorkspaceKCodeApp, ensureSessionModelAvailable } from "./workspace-model-runtime.js";
...
import { runSessionModelConfigMutation } from "../kcode-protocol-v4/model-config-mutation.js";
...
type ProtocolGoalTarget = NonNullable<
  Awaited<ReturnType<NonNullable<KCodeProtocolSessionRecord["app"]["readTarget"]>>>
>;

type KCodeSessionRecordParams = (
  | KCodeSessionCreateParams
  | (KCodeSessionResumeParams & {
      mode?: KCodeSessionCreateParams["mode"];
      model?: KCodeSessionCreateParams["model"];
      parentSessionId?: KCodeSessionCreateParams["parentSessionId"];
      thoughtLevel?: KCodeSessionCreateParams["thoughtLevel"];
      // 自动化会话会在创建期关闭标题二次生成，而 create/resume
      // 共用 record 初始化函数；resume 兼容分支也必须声明该策略字段。
      titleGenerationEnabled?: KCodeSessionCreateParams["titleGenerationEnabled"];
    })
) & { taskType?: SessionTaskType };

interface SessionStartupPreferences {
  memoryEnabled: boolean;
  modelContextBudgetStrategy: KCodeModelContextBudgetStrategy;
...
type SessionStartupPreferencesSource =
  | { kind: "host" }
  | { kind: "inherit"; parent: KCodeProtocolSessionRecord };

function resolveSupportedAppThoughtLevel(
  app: Pick<KCodeProtocolSessionRecord["app"], "listThoughtLevels">,
...
function protocolEventSequenceKey(deliveryKind?: KCodeDeliveryKind): string {
  return deliveryKind ?? DEFAULT_PROTOCOL_EVENT_SEQUENCE_KEY;
}

function getProtocolEventSequenceState(
  record: KCodeProtocolSessionRecord,
...
function getProtocolToolInputTransmissionState(
  record: KCodeProtocolSessionRecord,
  deliveryKind?: KCodeDeliveryKind,
): KCodeProtocolToolInputTransmissionState {
  const key = protocolEventSequenceKey(deliveryKind);
  const existing = record.protocolToolInputTransmissions.get(key);
  if (existing) {
    return existing;
  }
  const created: KCodeProtocolToolInputTransmissionState = {
...
function readProtocolStreamingParentToolUseId(
  ...
  const kcode = asRecord(meta.kcode);
  return (
    stringValue(meta.parentToolUseId) ??
    stringValue(meta.parentToolCallId) ??
    stringValue(kcode.parentToolUseId) ??
    stringValue(kcode.parentToolCallId)
  );
}
...
function markStreamedToolInputIfPresent(
  mappedEvent: KCodeSessionEvent,
  toolInputTransmissions: KCodeProtocolToolInputTransmissionState,
): void {
...
function mapProtocolSessionEvent(
...
): KCodeSessionEvent | null {
  if (shouldHideProtocolSessionEvent(record, event)) {
    return null;
  }
  if (!shouldExposeSessionEventToProtocol(event)) {
    const payload = asRecord(event.payload);
    context.logger?.debug("KCode Protocol session event filtered", {
      event: "kcode_protocol.session_event.filtered",
  ...
  // tool_input_delta 等高频模型中间态。KCode Protocol 的 seq 是 UI/replay 的恢复键，
...
): Promise<KCodeSessionEvent[]> {
  const events = await record.eventStore.getEvents(record.app.sessionId as SessionId);
  const visibleEvents: KCodeSessionEvent[] = [];
  const toolInputTransmissions: KCodeProtocolToolInputTransmissionState = {
    streamedToolCallIdsWithInput: new Set(),
  };
...
function logProtocolSessionEventSent(
  ...
  context.logger?.debug("KCode Protocol session event sent", {
    deliveryKind: mappedEvent.deliveryKind,
    event: "kcode_protocol.session_event.sent",
...
function isImportedHistoryMessage(
  ...
  source: NonNullable<KCodeSessionCreateParams["importedHistory"]>["source"],
): boolean {
...
async function removePreviousImportedSessionHistory(params: {
  sessionStore: NonNullable<KCodeProtocolAgentServerContext["deps"]["sessionStore"]>;
  sessionId: SessionId;
  source: NonNullable<KCodeSessionCreateParams["importedHistory"]>["source"];
}): Promise<number> {
...
async function persistImportedSessionHistory(params: {
  ...
  record: KCodeProtocolSessionRecord;
  sessionId: SessionId;
  createParams: KCodeSessionCreateParams;
}): Promise<void> {
  ...
  params.context.logger?.info("KCode Protocol imported history persisted", {
    event: "kcode_protocol.session_import.persisted",
...
async function createSessionWithProjection<T>(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
  trace: KCodeProtocolTrace | undefined,
  project: (
    record: KCodeProtocolSessionRecord,
  ) => Promise<{ value: T; phaseDurationsMs?: SnapshotPhaseDurationsMs; messageCount?: number }>,
): Promise<T> {
  const params = parseParams(kcodeSessionCreateParamsSchema, rawParams);
  ...
  context.logger?.info("KCode Protocol session/create started", {
    event: "kcode_protocol.session_create.started",
  ...
  try {
    await runSessionModelConfigMutation(record.app, async () => {
      ...
      if (supportedInitialThoughtLevel) {
        ...
        context.logger?.warn("KCode Protocol session/create skipped unsupported thought level", {
          event: "kcode_protocol.session_create.thought_level_skipped",
          module: "bootstrap.kcode_protocol",
    ...
    if (params.importedHistory) {
      ...
      // 让导入结果从第一刻起就是可续聊、可切模型的 KCode session。
    ...
    context.logger?.info("KCode Protocol session/create completed", {
      durationMs: Date.now() - startedAt,
      event: "kcode_protocol.session_create.completed",
    ...
    context.logger?.warn("KCode Protocol session/create failed", {
      durationMs: Date.now() - startedAt,
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "kcode_protocol.session_create.failed",
      module: "bootstrap.kcode_protocol",
      persistence: params.persistence,
      recordCreateDurationMs,
      rootTraceId: record.traceContext.traceId,
...
export async function activateSessionForResume(
  context: KCodeProtocolAgentServerContext,
  params: KCodeSessionResumeParams,
  ...
  context.logger?.debug("KCode Protocol session resume started", {
    activeBeforeWait,
    event: "kcode_protocol.session.resume_started",
  ...
  if (!session) {
    ...
    context.logger?.warn("KCode Protocol session resume found no persisted record", {
      activeBeforeWait,
      activeSessionCount: context.sessions.size,
      durationMs: Math.max(0, Date.now() - resumeStartedAt),
      event: "kcode_protocol.session.resume_persisted_missing",
      hasSessionStore: Boolean(context.deps.sessionStore),
      module: "bootstrap.kcode_protocol",
      sessionId: params.sessionId,
    });
    throw new ProtocolRequestError(
      kcodeProtocolErrorCodes.sessionUnavailable,
  ...
  context.logger?.info("KCode Protocol session resume completed", {
    activeSessionCount: context.sessions.size,
    durationMs: Math.max(0, Date.now() - resumeStartedAt),
    event: "kcode_protocol.session.resume_completed",
    module: "bootstrap.kcode_protocol",
...
export async function resumeSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionResumeParamsSchema, rawParams);
...
async function repairLegacyRemoteSessionWorkspaceForResume(
  ...
  if (!repairPersisted) {
    context.logger?.warn("legacy remote session workspace repair was not persisted", {
      event: "kcode_protocol.session_resume.legacy_remote_workspace_repair_rejected",
      legacyWorkspaceDirectory,
  ...
  if (repaired) {
    context.logger?.info("legacy remote session workspace repaired", {
      event: "kcode_protocol.session_resume.legacy_remote_workspace_repaired",
      legacyWorkspaceDirectory,
      legacyWorkspaceIdentity: legacyRemote.workspaceIdentity,
      module: "bootstrap.kcode_protocol",
      sessionId: session.id,
...
export async function listSessions(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionListParamsSchema, rawParams ?? {});
...
export async function listSessionSubagents(
  ...
  const params = parseParams(kcodeSessionSubagentsParamsSchema, rawParams ?? {});
  const store = context.deps.sessionStore;
  ...
  if (!parentSession) {
    ...
    context.logger?.warn("KCode Protocol session subagents has no persisted parent", {
      activeSessionCount: context.sessions.size,
      activeSession: Boolean(liveParent),
      event: "kcode_protocol.session.persisted_missing",
      module: "bootstrap.kcode_protocol",
...
export async function getUsageStats(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeUsageStatsParamsSchema, rawParams ?? {});
...
export async function readSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionReadParamsSchema, rawParams);
...
export async function readMessages(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionMessagesParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function readEvents(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionEventsParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  return {
...
export async function subscribeSession(
  ...
  const params = parseParams(kcodeSessionSubscribeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
export async function sendPrompt(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionSendParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  ...
  if (record.activeAbortController) {
    context.logger?.warn("KCode Protocol session/send rejected: active prompt exists", {
  ...
  context.logger?.info("KCode Protocol session/send accepted", {
...
export async function compactSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionCompactParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
async function runCompactTurnInBackground(
  ...
  context.logger?.info("KCode Protocol background compact started", {
    inputId: params.inputId,
    sessionId: record.app.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  try {
    ...
    context.logger?.info("KCode Protocol background compact completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      sessionId: record.app.sessionId,
      workspacePath: record.workspace.workspacePath,
    ...
    context.logger?.warn("KCode Protocol background compact failed", {
      durationMs: Date.now() - startedAt,
...
export async function goalSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionGoalParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  ...
  if (params.action === "pause") {
    ...
    if (target && activeAbortController) {
      context.logger?.info("KCode Protocol goal pause aborting active turn", {
        inputId: params.inputId,
        sessionId: params.sessionId,
        targetId: target.targetID,
        workspacePath: record.workspace.workspacePath,
      });
      activeAbortController.abort(new Error("KCode Protocol goal paused"));
    }
...
export async function forkSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionForkParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function registerForkedSession(
  context: KCodeProtocolAgentServerContext,
  record: KCodeProtocolSessionRecord,
  fork: Awaited<ReturnType<KCodeProtocolSessionRecord["app"]["forkFromCheckpoint"]>>,
  options: {
    runtimeConfig: {
...
async function inheritForkedSessionTarget(
  context: KCodeProtocolAgentServerContext,
  parentRecord: KCodeProtocolSessionRecord,
  forkRecord: KCodeProtocolSessionRecord,
...
async function runPromptTurnInBackground(
  ...
  const startedAt = Date.now();
  context.logger?.info("KCode Protocol background turn started", {
    inputId: params.inputId,
    queryId: params.queryId,
    sessionId: record.app.sessionId,
    textLength: params.content.length,
  ...
  try {
    ...
    context.logger?.info("KCode Protocol background turn completed", {
      durationMs: Date.now() - startedAt,
      inputId: params.inputId,
      queryId: params.queryId,
      sessionId: record.app.sessionId,
    ...
    context.logger?.warn("KCode Protocol background turn failed", {
      durationMs: Date.now() - startedAt,
      error: error instanceof Error ? error.message : String(error),
      inputId: params.inputId,
      sessionId: record.app.sessionId,
    ...
    if (record.activeAbortController === params.abortController) {
      ...
      record.activeAbortController = undefined;
      context.logger?.info("KCode Protocol background turn cleared active controller", {
        durationMs: Date.now() - startedAt,
        inputId: params.inputId,
        sessionId: record.app.sessionId,
        workspacePath: record.workspace.workspacePath,
...
export async function stopSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionStopParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  const hadActivePrompt = Boolean(record.activeAbortController);
  let pausedTarget: ProtocolGoalTarget | null = null;
  context.logger?.info("KCode Protocol session/stop received", {
    hadActivePrompt,
    sessionId: params.sessionId,
    workspacePath: record.workspace.workspacePath,
  });
  ...
  record.activeAbortController?.abort(new Error("KCode Protocol session stopped"));
  if (pausedTarget) {
...
async function pauseActiveGoalForSessionStop(
  ...
  try {
    ...
    if (pausedTarget) {
      context.logger?.info("KCode Protocol session/stop paused active goal", {
        sessionId: params.sessionId,
    ...
    context.logger?.warn("KCode Protocol session/stop failed to pause active goal", {
      error: error instanceof Error ? error.message : String(error),
      sessionId: params.sessionId,
      targetId: target.targetID,
      workspacePath: record.workspace.workspacePath,
...
export async function cancelBackgroundTask(
  ...
  const params = parseParams(kcodeSessionCancelBackgroundTaskParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  context.logger?.info("KCode Protocol session/cancelBackgroundTask received", {
    sessionId: params.sessionId,
...
export async function setModel(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionSetModelParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
  await runSessionModelConfigMutation(record.app, async () => {
...
export async function setThoughtLevel(
  ...
  const params = parseParams(kcodeSessionSetThoughtLevelParamsSchema, rawParams);
...
export async function setMode(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionSetModeParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
  assertExpectedRevision(record, params.expectedRevision);
...
export async function closeSession(context: KCodeProtocolAgentServerContext, rawParams: unknown) {
  const params = parseParams(kcodeSessionCloseParamsSchema, rawParams);
  const record = requireSession(context, params.sessionId);
...
function shouldCloseSessionForExpectedPersistence(
  currentPersistence: KCodeSessionPersistence,
  expectedPersistence?: KCodeSessionPersistence,
): boolean {
...
export async function generateWorkspaceText(
  ...
  const params = parseParams(kcodeWorkspaceGenerateTextParamsSchema, rawParams);
  ...
  const app =
    active?.app ??
    (await createWorkspaceKCodeApp(context, params.workspace, {
...
function ensureNoActiveTurn(record: KCodeProtocolSessionRecord, message: string): void {
  if (!record.activeAbortController) {
    return;
  }
...
function assertExpectedProviderRevision(
  ...
  _record: KCodeProtocolSessionRecord,
...
async function resolveForkTarget(
  context: KCodeProtocolAgentServerContext,
  record: KCodeProtocolSessionRecord,
  target: KCodeSessionHistoryTarget,
...
export async function getTaskTokenUsage(
  ...
  const params = parseParams(kcodeTaskTokenUsageParamsSchema, rawParams ?? {});
  const usageStore = context.deps.sessionStore as Partial<UsageStorePort> | undefined;
 ...
 */
function reconcileRecordPersistence(record: KCodeProtocolSessionRecord): void {
...
export function onSessionEvent(
  ...
  if (computerUseOperationEvent) {
    try {
      ...
      context.notify({
        method: kcodeProtocolMethods.computerUseOperationEvent,
      ...
      context.logger?.debug("Computer Use operation lifecycle notification sent", {
        event: "kcode_protocol.computer-use.operation-event.sent",
      ...
      context.logger?.warn("Computer Use operation lifecycle notification failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "kcode_protocol.computer-use.operation-event.failed",
        eventId: computerUseOperationEvent.eventId,
        sessionId: computerUseOperationEvent.sessionId,
        turnId:
          "turnId" in computerUseOperationEvent ? computerUseOperationEvent.turnId : undefined,
...
async function requestSessionRuntimePreferences(
  ...
  const startedAt = Date.now();
  // KCodeProtocolTrace.traceId 是协议层的 string；LogContext 需要 branded TraceId，
  // 按本文件既有惯例断言收口，避免多处日志构造重复转换。
  const traceId = trace?.traceId as TraceId | undefined;
  context.logger?.debug("KCode Protocol runtime preferences request started", {
    event: "kcode_protocol.runtime_preferences.request_started",
    module: "bootstrap.kcode_protocol",
    scope,
    sessionId,
    traceId,
  ...
  try {
    const result = await context.requestClient(
      kcodeProtocolMethods.sessionRequestRuntimePreferences,
      { sessionId, scope },
      kcodeSessionRuntimePreferencesResultSchema,
      {
    ...
    context.logger?.debug("KCode Protocol runtime preferences response received", {
      durationMs: Math.max(0, Date.now() - startedAt),
      event: "kcode_protocol.runtime_preferences.response_received",
      module: "bootstrap.kcode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    const diagnostic = {
      ...
      errorCode,
      errorMessage,
      event: "kcode_protocol.runtime_preferences.request_failed",
      module: "bootstrap.kcode_protocol",
      scope,
      sessionId,
      traceId,
    ...
    if (errorCode === -32022) {
      ...
      context.logger?.warn("KCode Protocol runtime preferences request timed out", diagnostic);
    } else if (errorCode !== -32601 && errorCode !== -32020) {
      context.logger?.warn("KCode Protocol runtime preferences request failed", diagnostic);
    } else {
      context.logger?.debug(
        "KCode Protocol runtime preferences compatibility fallback",
        diagnostic,
      );
    }
    if (error instanceof ProtocolRequestError && (error.code === -32601 || error.code === -32020)) {
      ...
      return {
        ...
        memoryEnabled: false,
        modelContextBudgetStrategy: DEFAULT_KCODE_MODEL_CONTEXT_BUDGET_STRATEGY,
...
async function materializeSessionRecord(
  context: KCodeProtocolAgentServerContext,
  params: KCodeSessionRecordParams,
  sessionId: SessionId,
  resume: boolean,
  source: SessionStartupPreferencesSource,
  trace?: KCodeProtocolTrace,
): Promise<KCodeProtocolSessionRecord> {
...
async function createRecord(
  ...
  context.logger?.info("KCode Protocol createRecord MCP config", {
    event: "kcode_protocol.create_record.mcp_config",
  ...
  let ownSessionRecord: KCodeProtocolSessionRecord | undefined;
  const app = await createWorkspaceKCodeApp(context, workspace, {
    env: context.deps.env,
    eventStore,
    resume,
    runtimeConfig: {
      ...
      // desktop-continuous session/create 由 UI 先解析 ~/.kcode/.agents 的 enabled MCP，
      ...
      // KCode Protocol app-server 创建的 session（desktop/web/mobile）没有传，导致
    ...
    // KCode Protocol app-server 以前没有注入可等待的交互 broker，
  ...
  const now = Date.now();
  const record: KCodeProtocolSessionRecord = {
    app,
    createdAt: now,
    eventStore,
...
): KCodeSessionCreateParams["mode"] | undefined {
  for (let index = messages.length - 1; index >= 0; index -= 1) {
    const info = messages[index]?.info;
    if (info?.role === "assistant" && isKCodeSessionMode(info.mode)) return info.mode;
  }
  return undefined;
}

function isKCodeSessionMode(
  value: unknown,
): value is NonNullable<KCodeSessionCreateParams["mode"]> {
  return (
    value === "plan" ||
    value === "build" ||
    value === "edit" ||
...
async function snapshotWithDiagnostics(
  ...
  if (totalDurationMs >= SLOW_SNAPSHOT_LOG_THRESHOLD_MS) {
    ...
    context.logger?.warn("KCode Protocol session snapshot slow", {
      durationMs: totalDurationMs,
      event: "kcode_protocol.session_snapshot.slow",
...
async function readSnapshotTarget(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session goal for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "kcode_protocol.snapshot.target_read_failed",
      sessionId: record.app.sessionId,
    });
    return undefined;
  }
...
async function readSnapshotTodos(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted session todos for protocol snapshot", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "kcode_protocol.snapshot.todos_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
...
async function readSessionTargetCompletionVerificationEvents(
  ...
  try {
    ...
    context.logger?.warn("Failed to read persisted goal verification entries", {
      errorMessage: error instanceof Error ? error.message : String(error),
      event: "kcode_protocol.snapshot.goal_verification_entries_read_failed",
      sessionId: record.app.sessionId,
    });
    return [];
  }
  ...
  return await repairPersistedRemoteSessionPaths(sessionStore, session, {
    onPersistenceFailure: (error) => {
      context.logger?.warn("Session path repair persistence failed; using in-memory repair", {
        errorMessage: error instanceof Error ? error.message : String(error),
        event: "kcode_protocol.session.path_repair.persist_failed",
        sessionId,
      });
    },
  });
...
function emitStateUpdated(
  ...
  const notification: KCodeStateUpdatedNotification = {
    patch,
    reason,
