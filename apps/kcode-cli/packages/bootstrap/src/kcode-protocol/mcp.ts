...
import type { Logger, McpConnectionSnapshot, McpPort, McpServerStatus } from "@kcode/contracts";
import {
  kcodeMcpListParamsSchema,
  kcodeMcpListResultSchema,
  type KCodeMcpListResult,
} from "@kcode/shared";
import {
...
export async function listMcpServers(
  ...
  rawParams: unknown,
): Promise<KCodeMcpListResult> {
  const params = parseParams(kcodeMcpListParamsSchema, rawParams);
  ...
  const startupTimer = new StartupTimer(
    ...
    {
      module: "bootstrap.kcode_protocol.mcp",
  ...
  const configuredMcpServers = {
    ...
    // 设置页的本地 MCP 列表由 desktop main 解析 `.kcode` / `.agents` fallback，
  ...
  if (mcpPort) {
    if (params.mode === "status") {
      ...
      return kcodeMcpListResultSchema.parse({ statuses });
    }
    ...
    if (pendingAuthorizationSnapshot) {
      ...
      return kcodeMcpListResultSchema.parse({
