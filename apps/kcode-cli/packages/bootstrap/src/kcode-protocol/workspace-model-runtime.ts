...
import { createInMemorySessionEventStore } from "@kcode/adapters/storage";
import type { ModelSelection } from "@kcode/contracts";
import {
  kcodeProviderTestModelConnectivityParamsSchema,
  kcodeWorkspaceReadPresentationParamsSchema,
...
export async function readWorkspacePresentation(
  ...
  const params = parseParams(kcodeWorkspaceReadPresentationParamsSchema, rawParams);
  return {
...
export async function testProviderModelConnectivity(
  ...
  const params = parseParams(kcodeProviderTestModelConnectivityParamsSchema, rawParams);
...
export async function createWorkspaceKCodeApp(
  context: KCodeProtocolAgentServerContext,
  ...
  return context.deps.createKCodeApp({
    ...options,
    platform: context.deps.platform,
    providerRuntimeHeadersPort,
    runtimeConfig: {
      ...options.runtimeConfig,
      // createKCodeApp 会把 workingDirectory 规范化为执行 cwd。把协议入口的
...
export function hasSessionModelProvider(
  ...
  record: Pick<KCodeProtocolSessionRecord, "app" | "workspace">,
  providerId: string,
): boolean {
...
export function resolveSessionModelContextWindow(
  _context: KCodeProtocolAgentServerContext,
  record: Pick<KCodeProtocolSessionRecord, "app" | "workspace" | "restoredModelSelection">,
): number | undefined {
