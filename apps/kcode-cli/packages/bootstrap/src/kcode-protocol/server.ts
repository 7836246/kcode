...
import {
  kcodePluginsCancelOperationParamsSchema,
  kcodeProtocolMethods,
  kcodeWorkspaceCancelGenerateTextParamsSchema,
  kcodeWorkspaceHookTrustGrantParamsSchema,
} from "@kcode/shared";
import type { BrowserControlPort } from "@kcode/contracts";
import { InMemoryWorkspaceHookPolicyProvider } from "@kcode/core";
import {
  V4_METHODS,
  V4_NOTIFICATIONS,
...
import type {
  KCodeProtocolError,
  KCodeProtocolMessage,
  KCodeProtocolMethod,
  KCodeProtocolNotification,
  KCodeProtocolRequest,
  KCodeProtocolRequestId,
  KCodeProtocolResponse,
} from "@kcode/shared";
import {
...
} from "../kcode-protocol-v4/interaction-registry.js";
...
import {
  ...
  parseParams,
  toProtocolError,
  type KCodeProtocolClientRequestOptions,
  type KCodeProtocolAgentDependencies,
  type KCodeProtocolAgentServerContext,
  type KCodeProtocolSessionRecord,
} from "./server-types.js";
import { createInMemorySessionEventStore } from "@kcode/contracts";

export type { KCodeProtocolAgentDependencies, KCodeProtocolSessionRecord };

const MAX_CLIENT_REQUEST_REANNOUNCE_INTERVAL_MS = 10_000;

type KCodeProtocolOutboundMessage = KCodeProtocolNotification | KCodeProtocolRequest;
...
async function notifyWorkspaceHookTrustGrantSessions(input: {
  ...
  sessions: Map<string, KCodeProtocolSessionRecord>;
}): Promise<void> {
...
interface KCodeProtocolPostResponseBatch {
  readonly messages: readonly KCodeProtocolOutboundMessage[];
  commit(): boolean;
}
...
export class KCodeProtocolAgentServer {
   ...
   */
  get officialMcpAuthRequestContext(): Pick<KCodeProtocolAgentServerContext, "requestClient"> {
    return this.context;
  }

  private messageSink?: (message: KCodeProtocolOutboundMessage) => void;
  private clientDisconnectError?: Error;
  private readonly context: KCodeProtocolAgentServerContext;
  ...
  private readonly postResponseOutbox = new Map<
    KCodeProtocolRequestId,
    KCodeProtocolPostResponseBatch
  >();
  private nextClientRequestId = 1;

  constructor(deps: KCodeProtocolAgentDependencies) {
    this.runtimeResources = new ProtocolRuntimeResources(deps.createKCodeApp);
    const resolvedDeps = {
      ...deps,
      createKCodeApp: this.runtimeResources.create,
      // 默认 turn 窗口保留策略。
    ...
    this.context = {
      ...
      sessions: new Map<string, KCodeProtocolSessionRecord>(),
  ...
  setNotificationSink(sink: (message: KCodeProtocolOutboundMessage) => void): void {
  ...
  shutdown(): Promise<void> {
    ...
    const error = new Error("KCode Protocol runtime stopping");
    ...
    for (const record of this.context.sessions.values()) {
      ...
      try {
        ...
        this.logger?.warn("Session unsubscribe failed during protocol shutdown", {
          event: "kcode_protocol.session.unsubscribe.failed",
        });
      }
    }
  ...
  takePostResponseMessages(requestId: KCodeProtocolRequestId): KCodeProtocolOutboundMessage[] {
  ...
  takePostResponseBatch(requestId: KCodeProtocolRequestId): KCodeProtocolPostResponseBatch | null {
  ...
  async handleMessage(
    message: KCodeProtocolMessage,
  ): Promise<KCodeProtocolError | KCodeProtocolResponse | undefined> {
    this.runtimeResources.assertServing();
    ...
    if (isNotification(message)) {
      this.logger?.debug("KCode Protocol notification ignored", {
        event: "kcode_protocol.notification.ignored",
  ...
  private async handleRequest(
    request: KCodeProtocolRequest,
  ): Promise<KCodeProtocolError | KCodeProtocolResponse> {
  ...
  private async dispatchRequest(request: KCodeProtocolRequest) {
      ...
      case kcodeProtocolMethods.workspaceHookTrustGrant: {
        ...
        if (grantResult.accepted) {
          await notifyWorkspaceHookTrustGrantSessions({
            ...
            grantedWorkspaceKey: kcodeWorkspaceHookTrustGrantParamsSchema.safeParse(request.params)
              .success
              ? kcodeWorkspaceHookTrustGrantParamsSchema.parse(request.params).workspace
                  .workspaceKey
              : undefined,
  ...
  private cancelPluginOperation(rawParams: unknown) {
    const params = parseParams(kcodePluginsCancelOperationParamsSchema, rawParams);
  ...
  private cancelWorkspaceGenerateText(rawParams: unknown) {
    const params = parseParams(kcodeWorkspaceCancelGenerateTextParamsSchema, rawParams);
  ...
  private ok(id: KCodeProtocolRequestId, result: unknown): KCodeProtocolResponse {
    return { id, result };
  }

  private fail(
    id: KCodeProtocolRequestId,
    code: number,
    message: string,
    data?: unknown,
  ): KCodeProtocolError {
  ...
  private requestClient<T>(
    method: KCodeProtocolMethod,
    params: unknown,
    resultSchema: ParamsSchema<T>,
    options?: KCodeProtocolClientRequestOptions,
  ): Promise<T> {
    if (this.clientDisconnectError) {
      throw this.clientDisconnectError;
    }
    if (!this.messageSink) {
      throw new ProtocolRequestError(-32020, `No KCode Protocol client is attached for ${method}`);
    }
  ...
  private resolveClientRequest(id: KCodeProtocolRequestId, result: unknown): void {
    const key = String(id);
  ...
  private rejectClientRequest(id: KCodeProtocolRequestId, error: Error): void {
    const key = String(id);
    const pending = this.pendingClientRequests.get(key);
    if (!pending) {
      return;
