...
// - 命令执行 = V4CommandExecutor（kcode-protocol-v4/commands/，原生直驱 core）；
...
import {
  ...
  type KCodeSessionContextUsage,
  type KCodeWorkspaceRef,
} from "@kcode/shared";
import { createExternalTurnFaultError } from "@kcode/core";
import {
  V4_NOTIFICATIONS,
...
import { V4CommandExecutor } from "../kcode-protocol-v4/commands/executor.js";
import { V4QueuePromotionLeaseUnavailableError } from "../kcode-protocol-v4/commands/handlers/queue.js";
import { V4CapabilityUnsupportedError } from "../kcode-protocol-v4/commands/handlers/interaction-background.js";
import {
  buildColdFileChangeSummaries,
  readConversationFileChangesFromEvents,
} from "../kcode-protocol-v4/cold-file-change-summaries.js";
import {
  loadPersistedConversationMaterialization,
  mergeColdConversationEvents,
} from "../kcode-protocol-v4/cold-event-merge.js";
import { lookupGlobalCreateSessionCommand } from "../kcode-protocol-v4/create-session-command-fact.js";
import type { V4CommandCoreHost } from "../kcode-protocol-v4/commands/types.js";
import type {
  ConversationRowTargetResolution,
  SessionUsageSeed,
} from "../kcode-protocol-v4/product-projection.js";
import { PersistentCommandIndex } from "../kcode-protocol-v4/persistent-command-index.js";
import { queueItemIdForCommand } from "../kcode-protocol-v4/command-inbox.js";
import { resolveStableForkTargetFromTranscript } from "../kcode-protocol-v4/stable-fork-target.js";
import { shouldAutoDrainV4QueueHead } from "../kcode-protocol-v4/queue-auto-drain.js";
import { persistAssistantFeedback } from "../kcode-protocol-v4/assistant-feedback-persistence.js";
import {
...
} from "../kcode-protocol-v4/persistent-command-facts.js";
import {
  ConversationV4Gateway,
  V4CommandNotImplementedError,
} from "../kcode-protocol-v4/v4-gateway.js";
import {
...
import { HYDRATION_TRACE_ID } from "../kcode-protocol-v4/projection-state.js";
...
function sessionUsageSeedFromRuntimeContextUsage(
  contextUsage: KCodeSessionContextUsage | undefined,
...
function cloneModelSelection(
  selection: ReturnType<KCodeProtocolSessionRecord["app"]["runtime"]["getSessionModelSelection"]>,
): ModelSelection | undefined {
...
async function replayDynamicWorkflowRunEvents(
  ...
  try {
    ...
    context.logger?.warn("v4 hydrate dynamic workflow replay failed", {
      ...
      event: "kcode_protocol.v4.hydrate_workflow_replay_failed",
...
async function resolveConversationBackingRecord(
  context: KCodeProtocolAgentServerContext,
  sessionId: string,
): Promise<KCodeProtocolSessionRecord | undefined> {
...
export function createConversationV4Gateway(
  ...
  const log = createProtocolLogger(context.deps)?.child({
    module: "bootstrap.kcode_protocol_v4_gateway",
  });
  ...
  const autoDrainV4QueueIfReady = async (record: KCodeProtocolSessionRecord): Promise<void> => {
  ...
  const scheduleAutoDrainRetry = (record: KCodeProtocolSessionRecord): void => {
  ...
  const coreHost: V4CommandCoreHost = {
    ...
    ensureModelReady: (record) =>
      ensureSessionModelAvailableForNextTurn(context, record as KCodeProtocolSessionRecord),
    ...
    afterLegacyStateMutation: async (record, reason) => {
      await afterStateMutation(context, record as KCodeProtocolSessionRecord, reason);
      await autoDrainV4QueueIfReady(record as KCodeProtocolSessionRecord);
    },
  ...
  const loadStoredSessionSummaries = async (
    ...
    try {
        ...
        try {
          ...
          if (claimedCount > 0) {
            context.logger?.info("legacy remote sessions claimed by task-index allowlist", {
              claimedCount,
              event: "kcode_protocol.v4.sessions_index_legacy_remote_claimed",
              module: "bootstrap.kcode_protocol",
              workspaceId,
            });
          }
          ...
          context.logger?.warn("legacy remote sessions claim failed; continuing strict load", {
            error: error instanceof Error ? error.message : String(error),
            event: "kcode_protocol.v4.sessions_index_legacy_remote_claim_failed",
            module: "bootstrap.kcode_protocol",
            workspaceId,
          });
        }
      ...
      context.logger?.warn("sessions-index stored summaries failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "kcode_protocol.v4.sessions_index_stored_failed",
        module: "bootstrap.kcode_protocol",
      });
      return [];
    }
  ...
  return new ConversationV4Gateway({
    ...
    resumePersistedSession: async (
      ...
      if (!persisted) {
        context.logger?.warn("KCode Protocol v4 cold resume has no persisted session", {
          activeSessionCount: context.sessions.size,
          event: "kcode_protocol.v4.resume_persisted_missing",
          module: "bootstrap.kcode_protocol",
          sessionId,
        });
    ...
    loadPersistedEvents: async (sessionId, persistedMessages) => {
      ...
      if (!record) {
        ...
        context.logger?.warn("KCode Protocol v4 hydrate has no active runtime", {
          activeSessionCount: context.sessions.size,
          event: "kcode_protocol.v4.hydrate_runtime_missing",
          module: "bootstrap.kcode_protocol",
                      ...
                      sessionEntries: (input) =>
                        store.sessionEntries!(input).catch((error) => {
                          context.logger?.warn("v4 hydrate session entries read failed", {
                            error: error instanceof Error ? error.message : String(error),
                            event: "kcode_protocol.v4.hydrate_session_entries_failed",
                            module: "bootstrap.kcode_protocol",
                          });
                          return [];
                        }),
      ...
      const fileChangeSummariesByMessageId = await buildColdFileChangeSummaries({
        ...
        onArtifactError: (messageId, error) =>
          context.logger?.warn("v4 cold file change artifact read failed", {
            error: error instanceof Error ? error.message : String(error),
            event: "kcode_protocol.v4.hydrate_file_changes_failed",
            messageId,
            module: "bootstrap.kcode_protocol",
            sessionId,
          }),
      ...
      for (const diagnostic of merged.diagnostics) {
        const fields = {
          ...diagnostic,
          event: "kcode_protocol.v4.hydrate_three_source_merge",
          module: "bootstrap.kcode_protocol",
          sessionId,
        };
        if (
    ...
    onError: (scope, error, errorContext) =>
      context.logger?.warn("KCode Protocol v4 gateway error", {
        ...errorContext,
        error: error instanceof Error ? error.message : String(error),
        event: "kcode_protocol.v4.gateway_error",
        module: "bootstrap.kcode_protocol",
        scope,
      }),
  });
