...
import type { McpTelemetryTracker } from "@kcode/adapters";
import type { WorkspaceHookPolicyProvider } from "@kcode/core";
import type { AccountProviderConfigSnapshot } from "@kcode/provider";
import {
  ...
  type KCodeProtocolMessage,
  type KCodeProtocolMethod,
  type KCodeProtocolNotification,
  type KCodeProtocolRequest,
  type KCodeProtocolRequestId,
  type KCodeProtocolTrace,
  type KCodeSessionMode,
  type KCodeSessionPersistence,
  type KCodeWorkspaceRef,
} from "@kcode/shared";
import type { KCodeApp, KCodeAppOptions } from "../app/types.js";
import type { V4InteractionRegistry } from "../kcode-protocol-v4/interaction-registry.js";
import type { ConversationV4Gateway } from "../kcode-protocol-v4/v4-gateway.js";
...
export interface KCodeProtocolAgentDependencies {
  createKCodeApp(options?: Omit<KCodeAppOptions, "providerRegistry">): KCodeApp | Promise<KCodeApp>;
  /**
...
export type KCodeProtocolAgentResolvedDependencies = KCodeProtocolAgentDependencies & {
  createSessionEventStore(sessionId: string): SessionEventStorePort;
  workspaceHookPolicyProvider: WorkspaceHookPolicyProvider;
};

export interface KCodeProtocolEventSequenceState {
  lastSeq: number;
  seqBySourceEventKey: Map<string, number>;
}

export interface KCodeProtocolToolInputTransmissionState {
  streamedToolCallIdsWithInput: Set<string>;
}

export interface KCodeProtocolSessionRecord {
  app: KCodeApp;
  ...
  protocolEventSequences: Map<string, KCodeProtocolEventSequenceState>;
  protocolToolInputTransmissions: Map<string, KCodeProtocolToolInputTransmissionState>;
...
export interface KCodeProtocolAgentServerContext {
  /** 进程退出期间，迟到的异步物化不能重新登记 session。 */
  assertServing?: () => void;
  deps: KCodeProtocolAgentResolvedDependencies;
  logger?: Logger;
  ...
  notify(notification: KCodeProtocolNotification): void;
  requestClient<T>(
...
export function isRequest(message: KCodeProtocolMessage): message is KCodeProtocolRequest {
  return "method" in message && "id" in message;
}

export function isNotification(
  message: KCodeProtocolMessage,
): message is KCodeProtocolNotification {
  return "method" in message && !("id" in message);
}

export function isResponse(
  message: KCodeProtocolMessage,
): message is { id: KCodeProtocolRequestId; result: unknown } {
  return "id" in message && "result" in message;
}

export function isErrorResponse(message: KCodeProtocolMessage): message is {
...
export function protocolTraceFromTraceContext(context: TraceContext): KCodeProtocolTrace {
  return {
...
export function requireSession(
  ...
  sessionId: string,
  options: { deliveryKind?: KCodeDeliveryKind; operation?: string } = {},
  ...
  if (!record) {
    ...
    context.logger?.warn("KCode Protocol session runtime missing", {
      activeSessionCount: context.sessions.size,
      event: "kcode_protocol.session.require_missing",
...
export function createProtocolLogger(deps: KCodeProtocolAgentDependencies): Logger | undefined {
  return deps.loggerFactory?.createLogger("kcode").child({
    module: "bootstrap.kcode_protocol",
  });
}
