import {
  ...
  type KCodeActiveToolCall,
  type KCodeContextUsageBreakdownItem,
  type KCodeDeliveryKind,
  type KCodePendingPermission,
  type KCodeSessionContextUsage,
  type KCodeSessionEvent,
  type KCodeSessionGoal,
  type KCodeSessionGoalVerification,
  type KCodeSessionGoalVerificationTimeline,
  type KCodeSessionGoalStats,
  type KCodeSessionInfo,
  type KCodeSessionKind,
  type KCodeSessionProjection,
  type KCodeSessionRuntimeState,
  type KCodeSessionSettingsState,
  type KCodeSessionStateSnapshot,
  type KCodeSessionTodoGroup,
...
import type { KCodeApp } from "../app/types.js";
...
export async function buildSessionSnapshot(input: {
  ...
  return {
    ...
    protocol: {
      name: KCODE_PROTOCOL_NAME,
      version: KCODE_PROTOCOL_VERSION,
    },
...
async function mapSnapshotMessages(
  app: Pick<KCodeApp, "readToolResultArtifact">,
...
async function hydrateSnapshotFilePartUrl(
  ...
  // 历史图片附件持久化后只剩 kcode-artifact:// 引用，UI/手机端不能直接渲染。
...
function snapshotFilePartArtifactUri(
  ...
  return artifactUri.startsWith("kcode-artifact://") ? artifactUri : undefined;
}
...
): Promise<KCodeSessionSettingsState> {
...
export function mapSessionInfo(input: {
  app?: Pick<KCodeApp, "getMode" | "getModel" | "sessionId" | "traceId">;
  fallbackCreatedAt?: number;
  fallbackUpdatedAt?: number;
  ...
  return {
    ...
    sessionId,
    sessionKind: (input.session?.taskType ?? input.taskType ?? "interactive") as KCodeSessionKind,
...
export function mapSessionEvents(
  ...
  return events
    ...
    .filter((event): event is KCodeSessionEvent => event !== null);
}
...
function mapModelNetworkStatusPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = kcodeApiRetryFromModelNetworkStatusPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
  const kcodeMeta = asRecord(meta.kcode);
  return {
    ...record,
    _meta: {
      ...meta,
...
function mapStreamRecoveryPayload(payload: unknown): Record<string, unknown> {
  const record = asRecord(payload);
  const apiRetry = kcodeApiRetryFromStreamRecoveryPayload(record);
  if (apiRetry === undefined) {
    return record;
  }
  const meta = asRecord(record._meta);
...
function mapSessionProjection(projection: SessionProjection): KCodeSessionProjection {
  return {
...
function mapRuntimeState(input: {
  activeTurn?: ReturnType<KCodeApp["runtime"]["getActiveTurnInfo"]>;
  deliveryKind?: KCodeDeliveryKind;
  eventSeq: number;
  messages: MessageWithParts[];
...
export function resolveSessionContextUsage(input: {
  ...
  persistedContextUsageBreakdownEvents?: readonly SessionEvent[];
  projection: SessionProjection;
}): KCodeSessionContextUsage | undefined {
...
): KCodeSessionGoalVerificationTimeline[] {
...
function contextUsageFromProjection(
  projection: SessionProjection,
  cache: KCodeSessionContextUsage["cache"] | undefined,
): KCodeSessionContextUsage | undefined {
...
function contextCacheUsageFromMessages(
  messages: readonly MessageWithParts[],
): KCodeSessionContextUsage["cache"] | undefined {
  let inputTokens = 0;
...
function mapPendingPermission(permission: PendingPermission): KCodePendingPermission {
  // display / optionsPolicy 刻意不进 legacy v3 输出。
  // 根因不是"扩 schema 只能单向兼容"，而是 strict schema 随 packages/shared 打进每个桌面端
  // 的产物：今天把 kcodePendingPermissionSchema（shared/src/kcode-protocol/index.ts:1139）和
  // kcodePermissionRequestedEventPayloadSchema（同文件:1536）改成可选，也保护不了已经装出去
...
function mapPermissionRequestedPayload(payload: unknown): Record<string, unknown> {
  ...
  // 剔除，否则会直接漏进 strict 的 kcodePermissionRequestedEventPayloadSchema。
...
function mapActiveToolCall(toolCall: ActiveToolCall): KCodeActiveToolCall {
  return {
...
function mapSessionGoal(goal: SessionGoal | null | undefined): KCodeSessionGoal | null | undefined {
...
function buildGoalStats(
  projection: SessionProjection,
  messages: readonly MessageWithParts[],
): KCodeSessionGoalStats | undefined {
  const target = projection.target;
  if (!target) {
    return undefined;
  }
...
function buildTodoGroups(
  ...
  projection: SessionProjection,
): KCodeSessionTodoGroup[] {
  const target = projection.target;
  const timeline = getTargetGoalVerificationTimeline(projection, target);
  const groups = new Map<string, KCodeSessionTodoGroup>();
...
function ensureTodoGroup(
  groups: Map<string, KCodeSessionTodoGroup>,
  input: {
  ...
  const group: KCodeSessionTodoGroup = {
    id: input.groupId,
...
function mapSessionEventType(type: SessionEvent["type"]): KCodeSessionEvent["type"] {
  switch (type) {
