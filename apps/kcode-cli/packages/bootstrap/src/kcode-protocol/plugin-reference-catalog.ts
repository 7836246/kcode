...
import {
  KCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID,
  kcodeProtocolNotifications,
  kcodePluginsReferenceCatalogParamsSchema,
  kcodePluginsResolveSuggestedReferenceParamsSchema,
  type KCodePluginReferenceCatalogEntry,
  type KCodePluginDiagnostic as SharedPluginDiagnostic,
  type KCodePluginsReferenceCatalogResult,
  type KCodePluginsResolveSuggestedReferenceResult,
} from "@kcode/shared";
import type { PluginReferenceCatalogEntry } from "@kcode/contracts";
import { buildPluginReferenceCatalog } from "@kcode/core";
import {
...
export async function getPluginReferenceCatalog(
  ...
  const params = parseParams(kcodePluginsReferenceCatalogParamsSchema, rawParams);
  if (params.sessionId) {
...
const SUGGESTED_PLUGIN_MARKETPLACE = KCODE_OFFICIAL_PLUGIN_MARKETPLACE_ID;
...
export async function resolveSuggestedPluginReference(
  ...
  rawParams: unknown,
  signal?: AbortSignal,
): Promise<KCodePluginsResolveSuggestedReferenceResult> {
  const params = parseParams(kcodePluginsResolveSuggestedReferenceParamsSchema, rawParams);
  ...
  if (
    ...
    return unavailable(
      ...
      "推荐插件不是受信任的官方 kcode-plugins-official 来源",
    );
  }

  const workingDirectory = params.workspace.workspacePath;
  const readState = () => {
    const outcome = resolveKCodePlugins({ logger: context.logger, workingDirectory });
  ...
  context.notify({
    method: kcodeProtocolNotifications.pluginOperationProgress,
  ...
  try {
    const refreshRequest = updateKCodePluginMarketplace({
    ...
    const refreshTimeoutRequest = new Promise<never>((_, reject) => {
      ...
      refreshTimeout = setTimeout(() => {
        ...
        const timeoutError = new Error("刷新 kcode-plugins-official 超时（10000 ms）");
  ...
  const overview = getKCodePluginsOverview({ logger: context.logger, workingDirectory });
...
function resolveReferenceListingDisplayByPluginId(
  ...
  const overview = getKCodePluginsOverview({ workingDirectory: workspacePath });
...
function toReferenceCatalogEntry(
  ...
  includeCategory = false,
): KCodePluginReferenceCatalogEntry {
