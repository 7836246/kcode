import {
  kcodePluginsConfigureParamsSchema,
  kcodePluginsResetConfigParamsSchema,
  kcodePluginsInstallParamsSchema,
  kcodePluginsMarketplaceAddParamsSchema,
  kcodePluginsMarketplaceRemoveParamsSchema,
  kcodePluginsMarketplaceUpdateParamsSchema,
  kcodePluginsOverviewParamsSchema,
  kcodePluginsListParamsSchema,
  kcodePluginsSetEnabledParamsSchema,
  kcodePluginsUninstallParamsSchema,
  kcodePluginsUpdateParamsSchema,
  kcodePluginsValidateParamsSchema,
  kcodePluginsDescribeParamsSchema,
  kcodePluginsRestoreBuiltinParamsSchema,
  type KCodeAvailablePluginSummary,
  type KCodeInstalledPluginSummary,
  type KCodePluginComponentGroup,
  type KCodePluginDiagnostic,
  type KCodePluginInfo,
  type KCodePluginMarketplaceSummary,
  type KCodePluginsConfigureResult,
  type KCodePluginsDescribeResult,
  type KCodePluginsInstallResult,
  type KCodePluginsListResult,
  type KCodePluginsMarketplaceMutationResult,
  type KCodePluginsOverviewResult,
  type KCodePluginsRestoreBuiltinResult,
  type KCodePluginsSetEnabledResult,
  type KCodePluginsUninstallResult,
  type KCodePluginsValidateResult,
} from "@kcode/shared";
import type { PluginDiagnostic, PluginMetadata } from "@kcode/contracts";
import {
  addKCodePluginMarketplace,
  configureKCodePlugin,
  describeKCodePlugin,
  getKCodePluginsOverview,
  installKCodeMarketplacePlugin,
  removeKCodePluginMarketplace,
  resolveKCodePlugins,
  resetKCodePluginConfig,
...
import { listInstalledPluginRecords } from "@kcode/adapters/plugins";
...
import { createConfig, resolvePath, type ConfigResult } from "@kcode/adapters/config";
import { parseParams, type KCodeProtocolAgentServerContext } from "./server-types.js";

// 把 CLI 的 PluginMetadata 投影成协议可序列化的 KCodePluginInfo (只保留 UI 需要的字段)。
function toPluginInfo(plugin: PluginMetadata, configResult?: ConfigResult): KCodePluginInfo {
...
function toPluginDiagnostic(diagnostic: PluginDiagnostic): KCodePluginDiagnostic {
  return {
...
export async function listPlugins(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodePluginsListResult> {
  const params = parseParams(kcodePluginsListParamsSchema, rawParams);
...
): Promise<KCodePluginsSetEnabledResult> {
  const params = parseParams(kcodePluginsSetEnabledParamsSchema, rawParams);
  abortSignal?.throwIfAborted();
  const result = await setKCodePluginEnabled({
...
export async function getPluginsOverview(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodePluginsOverviewResult> {
  const params = parseParams(kcodePluginsOverviewParamsSchema, rawParams);
  const overview = getKCodePluginsOverview({
...
export async function addPluginMarketplace(
  ...
  const params = parseParams(kcodePluginsMarketplaceAddParamsSchema, rawParams);
...
export async function removePluginMarketplace(
  ...
  const params = parseParams(kcodePluginsMarketplaceRemoveParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function updatePluginMarketplace(
  ...
  const params = parseParams(kcodePluginsMarketplaceUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function installPlugin(
  ...
  rawParams: unknown,
  abortSignal?: AbortSignal,
): Promise<KCodePluginsInstallResult> {
  const params = parseParams(kcodePluginsInstallParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    installKCodeMarketplacePlugin({
      abortSignal,
...
export async function uninstallPlugin(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodePluginsUninstallResult> {
  const params = parseParams(kcodePluginsUninstallParamsSchema, rawParams);
  const removed = await uninstallKCodeMarketplacePlugin({
    logger: context.logger,
    marketplace: params.marketplace,
...
export async function updatePlugin(
  ...
  const params = parseParams(kcodePluginsUpdateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  ...
  return withPluginStorageLock(pluginStorageRoot, async () => {
    const installedPlugins: KCodeInstalledPluginSummary[] = [];
    const dependencyClosure: string[] = [];
    // 聚合每条记录重装产生的诊断：installKCodeMarketplacePlugin 失败时不抛错，而是返回
    // CLI 形态的 PluginDiagnostic（见其错误分支的 toMarketplaceInstallDiagnostic），
    // 这里逐条经协议侧 toPluginDiagnostic 投影成 KCodePluginDiagnostic 回传，
    // 让失败的重装显式暴露，而不是静默"成功"。
    const diagnostics: KCodePluginDiagnostic[] = [];
    for (const record of installed) {
      const result = await installKCodeMarketplacePlugin({
        logger: context.logger,
...
export async function restoreBuiltinPlugin(
  ...
  const params = parseParams(kcodePluginsRestoreBuiltinParamsSchema, rawParams);
...
export async function configurePlugin(
  ...
  const params = parseParams(kcodePluginsConfigureParamsSchema, rawParams);
  await configureKCodePlugin({
...
export async function resetPluginConfig(
  ...
  const params = parseParams(kcodePluginsResetConfigParamsSchema, rawParams);
  await resetKCodePluginConfig({
    logger: context.logger,
    pluginId: params.pluginId,
    scope: params.scope,
    workingDirectory: params.workspace.workspacePath,
...
export async function validatePlugin(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodePluginsValidateResult> {
  const params = parseParams(kcodePluginsValidateParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
...
export async function describePlugin(
  context: KCodeProtocolAgentServerContext,
  rawParams: unknown,
): Promise<KCodePluginsDescribeResult> {
  const params = parseParams(kcodePluginsDescribeParamsSchema, rawParams);
  const pluginStorageRoot = resolvePluginStorageRoot(params.workspace.workspacePath);
  const result = await withPluginStorageLock(pluginStorageRoot, async () =>
    describeKCodePlugin({
      logger: context.logger,
  ...
  const components: KCodePluginComponentGroup[] = result.components.map((group) => ({
    kind: group.kind,
    items: group.items.map((item) => ({
      name: item.name,
      ...(item.description ? { description: item.description } : {}),
...
function toMarketplaceSummary(input: {
  ...
  isOfficial?: boolean;
  featured?: string[];
  refreshFailure?: KCodePluginMarketplaceSummary["refreshFailure"];
}): KCodePluginMarketplaceSummary {
  return {
    id: input.id,
    name: input.name,
    source: input.source,
...
function toAvailablePluginSummary(input: {
  ...
  listing?: KCodeAvailablePluginSummary["listing"];
}): KCodeAvailablePluginSummary {
  return {
    id: input.id,
    name: input.name,
...
function toInstalledPluginSummary(input: {
  ...
  installedAt?: string;
  componentTypes?: string[];
  hookDetails?: KCodeInstalledPluginSummary["hookDetails"];
  updateStatus?: "none" | "update-available" | "version-changed";
  latestVersion?: string;
  listing?: KCodeInstalledPluginSummary["listing"];
}): KCodeInstalledPluginSummary {
  return {
    id: input.id,
    name: input.name,
    marketplace: input.marketplace,
