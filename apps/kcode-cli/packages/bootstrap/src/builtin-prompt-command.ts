...
interface ResolveKCodeBuiltinPromptCommandOptions {
  workingDirectory?: string;
}

export function resolveKCodeBuiltinPromptCommand(
  input: string,
  options: ResolveKCodeBuiltinPromptCommandOptions = {},
): string | undefined {
...
function buildInitAgentsPrompt(params: {
  ...
  return [
    "You are running KCode's built-in /init command.",
    "",
    "Your task is to create or update a concise workspace instruction file for future KCode agents.",
    "",
    "Target:",
    `- Workspace directory: ${params.workingDirectory}`,
    `- Instruction file: ${params.targetPath}`,
    `- Existing hidden instruction candidates: ${join(params.workingDirectory, ".kcode", "AGENTS.md")} and ${join(params.workingDirectory, ".agents", "AGENTS.md")}`,
    "- File name must be exactly AGENTS.md.",
    "- This command targets the current workspace only. Do not write ~/.kcode/AGENTS.md.",
    additionalInstructions,
    "",
    "Process:",
    "1. First check whether .kcode/AGENTS.md or .agents/AGENTS.md exists in the workspace. If either exists, tell the user they already have an instructions file, mention the path found, and stop without creating a new AGENTS.md.",
    ...
    "6. Include only project-specific facts future KCode agents would otherwise miss.",
