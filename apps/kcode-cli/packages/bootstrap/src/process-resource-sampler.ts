...
import {
  KCODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS,
  type KCodeProcessResourceSample,
} from "@kcode/shared";

/** 采样周期与 app 侧聚合共用 shared 的同一个常量，避免两侧节拍各自漂移。 */
const KCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS = KCODE_CLI_RESOURCE_SAMPLE_INTERVAL_MS;
...
interface CreateKCodeProcessResourceSamplerOptions {
  /**
   * 第二个参数是本周期完整的内存快照，供进程内本地诊断日志使用；
   * 协议样本自身只带 rss 与 heapUsed 两项内存字段。
   */
  onSample(sample: KCodeProcessResourceSample, memoryUsage: ProcessMemoryUsageSnapshot): void;
  platform?: KCodeProcessResourceSample["platform"];
  arch?: KCodeProcessResourceSample["arch"];
...
export interface KCodeProcessResourceSampler {
  start(): void;
  stop(): void;
}
...
export function createKCodeProcessResourceSampler(
  options: CreateKCodeProcessResourceSamplerOptions,
): KCodeProcessResourceSampler {
  const platform = options.platform ?? (process.platform as KCodeProcessResourceSample["platform"]);
  const arch = options.arch ?? (process.arch as KCodeProcessResourceSample["arch"]);
  ...
  const sample = (): void => {
    try {
      ...
      baseline = nextBaseline;
      const resourceSample: KCodeProcessResourceSample = {
        platform,
        arch,
        logicalCpuCount,
        intervalMs,
  ...
  return {
    start() {
      ...
      try {
        timerHandle = timer.setInterval(sample, KCODE_PROCESS_RESOURCE_SAMPLE_INTERVAL_MS);
      } catch {
