...
export const KCODE_OFFICIAL_PLUGIN_MARKETPLACE = "kcode-plugins-official";
/** 上游 ZCode CDN 仍使用的官方市场 id；解析后必须改写成规范 id。 */
export const LEGACY_KCODE_OFFICIAL_PLUGIN_MARKETPLACE = "zcode-plugins-official";
export const KCODE_INLINE_PLUGIN_MARKETPLACE = "inline";
export const KCODE_PLUGIN_HOST_COMMAND = "__kcode-plugin-host";
/**
 * 隐藏子命令：dynamic workflow 的沙箱子进程入口（`__kcode-dwf-child <entry path>`；argv 末位是
 * harness 写好的入口文件路径，payload 不过命令行）。
 *
 * 与 {@link KCODE_PLUGIN_HOST_COMMAND} 同族、同机制：SEA 单文件二进制不解释 Node CLI 旗标，
 ...
 */
export const KCODE_DWF_CHILD_COMMAND = "__kcode-dwf-child";

export function normalizeOfficialMarketplaceId(id: string): string {
  return id === LEGACY_KCODE_OFFICIAL_PLUGIN_MARKETPLACE
    ? KCODE_OFFICIAL_PLUGIN_MARKETPLACE
    : id;
}

export function isOfficialMarketplaceId(id: string): boolean {
  return normalizeOfficialMarketplaceId(id) === KCODE_OFFICIAL_PLUGIN_MARKETPLACE;
}
...
export interface PluginDiscoverRequest {
  ...
  // 让用户不必先 `kcode plugins enable` 就能用 (例如纯内容型的 skill-creator)。
