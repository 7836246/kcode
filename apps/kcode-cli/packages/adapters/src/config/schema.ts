/* eslint-disable max-lines -- kcode-cli 配置 schema 需要集中维护文件解析和 provider 继承，拆散会让配置语义更难对齐。 */
import { z } from "zod";
import type { RuntimeConfigPatch } from "@kcode/contracts";
...
export const LEGACY_CUA_PLUGIN_ID = "kcode-cua@kcode-plugins-official";
export const CANONICAL_CUA_PLUGIN_ID = "computer-use@kcode-plugins-official";
...
export const KCodeConfigFileSchema = z
  .object({
...
export type KCodeConfigFile = z.infer<typeof KCodeConfigFileSchema>;
...
function normalizeMcpServerConfigInput(value: unknown): unknown {
  ...
  // External MCP configs may carry provider-specific timeout fields. KCode does not migrate
  ...
  if (server.type === "remote") {
    // 外部 Agent 配置常把 HTTP MCP 标记为 remote；KCode 运行态协议类型是 http。
    server.type = "http";
...
export function parseConfigFileToRuntimePatchWithDiagnostics(
  ...
  const parsed = KCodeConfigFileSchema.parse(normalized);
  return {
...
function parsedConfigFileToRuntimePatch(parsed: KCodeConfigFile): RuntimeConfigPatch {
...
function normalizePluginConfig(
  plugins: KCodeConfigFile["plugins"],
