// Env Config Adapter - Parse the intentionally small KCODE_* environment surface.
...
const DEFAULT_PREFIX = "KCODE_";

/**
 * Parse KCODE_* environment variables into config
 */
...
export function getToolConcurrencyConfig(): { maxConcurrency: number } {
  return {
    maxConcurrency: normalizeNumber(process.env.KCODE_MAX_TOOL_CONCURRENCY ?? "10"),
  };
}
