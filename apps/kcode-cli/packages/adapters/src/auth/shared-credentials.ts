...
import { atomicWritePrivateTextFile, backupCorruptFile, withFileLock } from "@kcode/shared/node";
import { createKCodeCredentialCipher, type KCodeCredentialCipher } from "./credential-cipher.js";

const KCODE_DATA_BASE_DIR_ENV_KEY = "KCODE_DATA_BASE_DIR";
...
export const SHARED_KCODE_CREDENTIAL_KEYS = {
  ...
  kcodeJwtToken: "kcodejwttoken",
} as const;

export interface SharedKCodeCredentialStoreOptions {
  baseDir?: string;
  cipher?: KCodeCredentialCipher;
...
export interface SharedKCodeCredentialStore {
...
export function createSharedKCodeCredentialStore(
  options: SharedKCodeCredentialStoreOptions = {},
): SharedKCodeCredentialStore {
  const env = options.env ?? process.env;
  const filePath = resolveSharedKCodeCredentialsPath(options);
  const cipher = options.cipher ?? createKCodeCredentialCipher({ env });
  ...
  return {
    ...
    async clearZaiLoginCredentials(): Promise<void> {
      await mutateRawCredentialRecord(filePath, async (rawCredentials) => {
        const activeProviderRaw = rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.activeProvider];
        const activeProvider = activeProviderRaw ? cipher.decrypt(activeProviderRaw) : null;
        delete rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.zaiAccessToken];
        delete rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.zaiRefreshToken];
        delete rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.zaiUserInfo];
        delete rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.kcodeJwtToken];
        if (activeProvider === ZAI_PROVIDER_ID) {
          delete rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.activeProvider];
        }
      });
    },
    ...
    async saveZaiLoginCredentials(payload: ZaiLoginCredentialPayload): Promise<void> {
      ...
      await mutateRawCredentialRecord(filePath, (rawCredentials) => {
        rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.activeProvider] =
          encryptedCredentials.activeProvider;
        rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.zaiAccessToken] =
          encryptedCredentials.accessToken;
        rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.kcodeJwtToken] = encryptedCredentials.jwtToken;
        rawCredentials[SHARED_KCODE_CREDENTIAL_KEYS.zaiUserInfo] = encryptedCredentials.userInfo;
      });
    },
  };
}

export function loadSharedKCodeCredentialSync(
  key: string,
  options: SharedKCodeCredentialStoreOptions = {},
): string | undefined {
  const filePath = resolveSharedKCodeCredentialsPath(options);
  ...
  try {
    ...
    const cipher = options.cipher ?? createKCodeCredentialCipher({ env: options.env });
...
export function resolveSharedKCodeCredentialsPath(
  options: SharedKCodeCredentialStoreOptions = {},
): string {
  if (options.filePath) {
  ...
  const env = options.env ?? process.env;
  const baseDir = options.baseDir ?? env[KCODE_DATA_BASE_DIR_ENV_KEY] ?? homedir();
  return join(resolveUserPath(baseDir), ".kcode", "v2", "credentials.json");
}
...
async function readRawCredentialRecord(filePath: string): Promise<Record<string, string>> {
  ...
  try {
    ...
    throw new Error(`Unable to read shared KCode credentials: ${filePath}`, { cause: error });
  }
  ...
  try {
    ...
    throw new Error(`Shared KCode credentials are corrupt: ${filePath}.${evidence}`, {
      cause: error,
    });
  }
}
