...
const CREDENTIAL_SECRET_ENV_KEY = "KCODE_CREDENTIAL_SECRET";

export interface KCodeCredentialCipher {
  decrypt(value: string): string;
  encrypt(value: string): string;
}

export interface KCodeCredentialCipherOptions {
  env?: Record<string, string | undefined>;
}

export function createKCodeCredentialCipher(
  options: KCodeCredentialCipherOptions = {},
): KCodeCredentialCipher {
  const key = deriveCipherKey(resolveCredentialSecret(options.env ?? process.env));

  return {
    decrypt(value: string): string {
      if (!isEncryptedKCodeCredentialValue(value)) {
        return value;
      }
...
export function isEncryptedKCodeCredentialValue(value: string): boolean {
