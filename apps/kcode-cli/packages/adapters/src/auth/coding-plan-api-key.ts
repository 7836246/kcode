import type { HttpClientPort, HttpClientRunOptions, TraceContext } from "@kcode/contracts";
import { resolveBigModelApiOrigin } from "@kcode/shared";

const ZAI_API_HOST = "https://api.z.ai";
const JSON_CONTENT_TYPE = "application/json";
const KCODE_API_KEY_NAME = "kcode-api-key";
...
async function resolveBizApiKey(
  ...
  const keyEntry =
    keys.find((item) => item.name === KCODE_API_KEY_NAME) ??
    (await requestRemoteData<RemoteApiKeySummary>(
      input.httpClient,
      {
        body: new TextEncoder().encode(JSON.stringify({ name: KCODE_API_KEY_NAME })),
        headers: createBizAuthHeaders(input.authorization),
        method: "POST",
        trace: input.trace,
        url: listUrl,
