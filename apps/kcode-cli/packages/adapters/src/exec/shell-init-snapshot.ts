...
import { windowsPathToGitBashPath } from "@kcode/contracts";
import type { ExecutionShellDialect } from "@kcode/contracts";
...
function pathExportLines(pathValue: string): string[] {
  return [
    ...
    "cat >> \"$SNAPSHOT_FILE\" << 'PATH_END_KCODE_SHELL_INIT_SNAPSHOT'",
    `export PATH=${shellQuoteAlways(pathValue)}`,
    "PATH_END_KCODE_SHELL_INIT_SNAPSHOT",
  ];
}
