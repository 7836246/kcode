...
export function createCwdCapturePlan(
  ...
  const cwdFilePath = join(cwdCaptureDir, `kcode-${crypto.randomUUID()}-cwd`);
...
function createPosixCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    "__kcode_status=$?",
    `if [ "$__kcode_status" -eq 0 ]; then pwd -P > ${shellQuote(cwdFilePath)}; fi`,
    'exit "$__kcode_status"',
  ].join("\n");
}
...
function createWindowsCmdCwdCaptureCommand(command: string, cwdFilePath: string): string {
  return [
    command,
    'set "__kcode_status=%ERRORLEVEL%"',
    `if "%__kcode_status%"=="0" cd > ${cmdQuote(cwdFilePath)}`,
    "exit /b %__kcode_status%",
  ].join("\r\n");
}
