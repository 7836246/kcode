...
import type { RegistryProviderConfig } from "@kcode/provider";
import { withOpenRouterAttributionHeaders } from "@kcode/shared";
...
/**
 * 模型请求出口：直连 provider，再进入用户 HTTP 代理 fetch。
 * 官方 Coding Plan 网关已下线，不再改写请求目标。
 */
...
export function readProviderBusinessFailureFromBody(body: unknown):
  ...
  const providerCode =
    ...
    // 二次包装后的外层 code 是 KCode 自己的 PROVIDER_BUSINESS_ERROR，
