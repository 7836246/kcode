...
import type { SkillRoot, SkillSource } from "@kcode/contracts";
...
function skillRootsForBase(
  ...
  // 合并而不是 fallback：用户可能同时安装原生 `.kcode` skill 和兼容 `.agents` skill。
  // 同一级别仍保持 `.kcode` 优先，后续同名按 root 顺序解析。
  return [
    root(join(baseDirectory, KCODE_DIR, SKILLS_DIR), scope, "kcode", nextPriority()),
