import {
  ...
  KCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY,
  readKCodeToolEnvPassthroughEnv,
} from "@kcode/shared";
...
const EXPLICIT_CA_SOURCE_KEYS = [KCODE_AGENT_CA_CERT_ENV_KEY] as const;
const EXPLICIT_NO_PROXY_SOURCE_KEYS = [KCODE_NO_PROXY_ENV_KEY] as const;
...
export function applyNetworkEgressEnv(
  ...
  deleteEnvKey(env, KCODE_TOOL_ENV_PASSTHROUGH_ENV_KEY, platform);
...
function applyToolEnvPassthroughEnv(
  ...
  const passthroughEnv = readKCodeToolEnvPassthroughEnv(sourceEnv);
...
function applyProxyEnv(
  ...
  const kcodeProxy = normalizeProxyValue(
    getFirstEnvValue(sourceEnv, [KCODE_HTTP_PROXY_ENV_KEY], platform),
  );
  if (kcodeProxy) {
    for (const key of ALL_PROXY_KEYS) {
      setEnvKey(env, key, kcodeProxy, platform);
    }
    return;
  }
}
