...
import { acquireFileLock } from "@kcode/shared/node";
import { KCODE_FILE_LOCK_TIMEOUT_ERROR_CODE } from "@kcode/shared";
import type { SharedKCodeCredentialStore } from "../auth/shared-credentials.js";
...
export async function tryAcquireAuthorizationLease(input: {
  ...
  try {
    ...
    if (getErrorCode(error) === KCODE_FILE_LOCK_TIMEOUT_ERROR_CODE) return undefined;
