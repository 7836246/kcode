...
import { sanitizeKCodeRuntimeEnv } from "@kcode/shared";
...
export function buildMcpStdioEnv(options: {
  ...
  return prependRunningNodeDirectory(
    applyNetworkEgressEnv(sanitizeKCodeRuntimeEnv(filterStringEnv(sourceEnv)), {
...
function prependRunningNodeDirectory(
  ...
  // remote Agent 由 ~/.kcode/server/node 启动，但登录环境 PATH 不含该目录，
