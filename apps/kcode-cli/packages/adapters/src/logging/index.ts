...
import type { LogContext, LogEntry, Logger, LoggerFactory, LogRedactor } from "@kcode/contracts";
import { LogLevel, LogLevelName } from "@kcode/contracts";
...
export function createNodeLoggerFactory(options: NodeLoggerFactoryOptions = {}): NodeLoggerFactory {
  ...
  const logDir = options.logDir ?? options.env?.KCODE_LOG_DIR ?? getDefaultLogDir();
  const consoleStream =
    typeof options.console === "object"
      ? options.console.stream
      : options.console === true || options.env?.KCODE_LOG_CONSOLE === "1"
        ? process.stderr
        : undefined;
...
export function getDefaultLogDir(): string {
  return join(homedir(), ".kcode", "cli", "log");
}
...
function isDevelopmentMode(env: NodeJS.ProcessEnv): boolean {
  const runtimeEnv = normalizeKCodeRuntimeEnv(env[KCODE_RUNTIME_ENV_KEY]);
...
function getLogFileName(): string {
  return `kcode-${formatLocalLogDate(new Date())}.jsonl`;
}
