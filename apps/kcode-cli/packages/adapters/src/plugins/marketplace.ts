...
import type { PluginDiagnostic, PluginManifest, PluginStoreListing } from "@kcode/contracts";
import {
...
const KCODE_MANIFEST_PATH = join(".kcode-plugin", "plugin.json");
...
export interface PluginMarketplaceManifest {
  ...
  pluginRoot?: string;
  // 商店「公开」分段 Featured 区的策展名单（插件 name，按序）；来自 bundled manifest 顶层 featured。
  featured?: string[];
  raw: Record<string, unknown>;
}
...
function buildMarketplaceGitEnv(
  ...
  const env = sanitizeKCodeRuntimeEnv(sourceEnv);
  // marketplace 安装会启动 Git 子进程，不能只依赖父进程继承的 shell 代理。
  // 这里统一从 KCode 显式网络环境恢复 HTTP(S)/NO_PROXY/CA，避免安装按钮卡到协议超时。
...
export function ensureDefaultPluginMarketplaces(storageRoot: string): KnownMarketplaceRecord[] {
  ...
  for (const marketplace of DEFAULT_PLUGIN_MARKETPLACES) {
    const existingIndex = next.findIndex((record) => record.id === marketplace.id);
    if (existingIndex === -1) {
      next.push({
        id: marketplace.id,
        source: officialMarketplaceSourceForDefault(marketplace),
    ...
    if (existing && shouldRewriteOfficialMarketplaceSource(existing)) {
      const { lastRefreshFailure: _lastRefreshFailure, ...rest } = existing;
      next[existingIndex] = {
        ...rest,
        source: createOfficialBundledMarketplaceSource(),
        description: marketplace.description,
      };
      changed = true;
    }
  ...
  // leftover CDN 分片必须在登记官方 known record 时清掉，否则公开目录仍会混入 Z.ai 清单。
  clearCdnOfficialMarketplacePartitionSync(storageRoot);
  if (changed) {
...
function officialMarketplaceSourceForDefault(
  marketplace: (typeof DEFAULT_PLUGIN_MARKETPLACES)[number],
): MarketplaceSource {
  if (isOfficialMarketplaceId(marketplace.id) || !marketplace.source) {
    return createOfficialBundledMarketplaceSource();
  }
  return defaultMarketplaceSourceFromString(marketplace.source);
}

function shouldRewriteOfficialMarketplaceSource(record: KnownMarketplaceRecord): boolean {
  return isOfficialMarketplaceId(record.id) && record.source.source !== "bundled";
}
...
export async function ensureMarketplaceManifestAvailable(input: {
  ...
  // 官方市场只由内置播种维护，缺失 manifest 时不再按 known source 拉远端。
...
export async function addMarketplace(input: {
  ...
  try {
    ...
    if (
      input.trustedId === KCODE_OFFICIAL_PLUGIN_MARKETPLACE &&
      loaded.manifest.name !== KCODE_OFFICIAL_PLUGIN_MARKETPLACE
    ) {
      throw new Error(
        `Official marketplace source must provide ${KCODE_OFFICIAL_PLUGIN_MARKETPLACE}, received ${loaded.manifest.name}`,
      );
    }
    const persistedManifest =
      loaded.manifest.name === KCODE_OFFICIAL_PLUGIN_MARKETPLACE
        ? parseRequiredMarketplaceManifest(rebuildOfficialMarketplaceSync(input.storageRoot))
        : loaded.manifest;
    ...
    } else if (loaded.manifest.name !== KCODE_OFFICIAL_PLUGIN_MARKETPLACE) {
...
export async function updateMarketplace(input: {
  ...
  for (const record of selected) {
    ...
    // 官方市场不再走远端刷新；个人来源仍按 known source 拉取。
...
export async function uninstallMarketplacePlugin(input: {
  ...
  storageRoot: string;
  removeCache?: boolean;
  /** `kcode plugins uninstall --keep-data`：删安装缓存但保留 data/<plugin-id> 用户数据目录。 */
  keepData?: boolean;
...
async function clonePluginSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "kcode-plugin-src-"));
  const args = ["clone"];
...
async function loadMarketplaceFromSource(
  ...
  switch (source.source) {
    ...
    case "bundled": {
      const parsed = loadMarketplaceManifestSync(storageRoot, KCODE_OFFICIAL_PLUGIN_MARKETPLACE);
      if (!parsed) {
        throw new Error("Official bundled marketplace is not available");
      }
...
async function cloneMarketplaceSource(
  ...
  const dir = await mkdtemp(join(tmpdir(), "kcode-marketplace-src-"));
...
async function execGitCommand(args: string[], signal?: AbortSignal): Promise<void> {
  ...
  try {
    ...
    const gitBinary = process.env.KCODE_GIT_BINARY?.trim() || "git";
...
export function parseMarketplaceManifest(value: unknown): PluginMarketplaceManifest | null {
  ...
  const name = normalizeOfficialMarketplaceId(
...
function findPluginManifestPath(rootPath: string): string | null {
  for (const candidate of [KCODE_MANIFEST_PATH, CLAUDE_MANIFEST_PATH, CODEX_MANIFEST_PATH]) {
  ...
  for (const key of UNSUPPORTED_MANIFEST_FIELDS) {
    if (key in input.manifest) {
      input.diagnostics.push({
        ...
        message: `Plugin component is diagnostic-only in this KCode runtime: ${key}`,
