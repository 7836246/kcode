...
} from "@kcode/kcode-cua/frame-contract";
...
import { preserveOfficialCuaFrameResult } from "@kcode/kcode-cua/frame-contract";
...
function readBrowserScreenshotContentIndices(result: McpToolCallResult): Set<number> {
  const value = result._meta?.[KCODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
