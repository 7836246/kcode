...
import type { ModelCatalogEntry, ModelSelection } from "@kcode/contracts";
import {
  KCODE_MODEL_REASONING_SEPARATOR,
  formatModelPickerValue,
  parseModelPickerValue,
} from "@kcode/shared/model-selection";
...
function splitReasoningLevel(text: string): { reference: string; level?: string } {
  ...
  const index = text.indexOf(KCODE_MODEL_REASONING_SEPARATOR, searchFrom);
