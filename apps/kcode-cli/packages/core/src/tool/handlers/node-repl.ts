...
import type { SessionId } from "@kcode/contracts";
...
/**
 * 每个 kcode session 一个持久 NodeReplSession，跨多次 js 调用保持内存状态（globalThis）。
...
function isBrowserSurfaceSideEffect(command: import("@kcode/contracts").BrowserCommand): boolean {
...
function isAutoScreenshotTriggerCommand(
  command: import("@kcode/contracts").BrowserCommand,
): boolean {
  if (
