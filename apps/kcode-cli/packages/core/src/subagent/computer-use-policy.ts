...
import {
  KCODE_CUA_OFFICIAL_MCP_NAMESPACE_NAME as KCODE_CUA_OFFICIAL_MCP_SERVER_NAME,
  KCODE_CUA_OFFICIAL_PLUGIN_ID,
} from "@kcode/shared";
import type { McpToolDescriptor, PluginReferenceCatalog, SkillMetadata } from "@kcode/contracts";
...
const OFFICIAL_CUA_SERVER_PARTS = new Set([
  "computer-use",
  "computer_use",
  toModelVisibleMcpNamePart(KCODE_CUA_OFFICIAL_MCP_SERVER_NAME),
]);
...
export function createOfficialCuaPolicy(
  ...
  for (const descriptor of descriptors) {
    ...
    if (descriptor.serverName === KCODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  if (officialServerNames.has(KCODE_CUA_OFFICIAL_MCP_SERVER_NAME)) {
  ...
  for (const serverName of officialServerNames) {
    if (serverName === KCODE_CUA_OFFICIAL_MCP_SERVER_NAME) {
  ...
  const officialPlugin = catalog?.plugins.find(
    (plugin) => plugin.pluginId === KCODE_CUA_OFFICIAL_PLUGIN_ID,
  );
