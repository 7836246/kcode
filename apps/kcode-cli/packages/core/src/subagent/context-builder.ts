import type { Model, ModelInputMessage } from "@kcode/contracts";
...
function buildSubagentContextSections(config: SubagentContextBuilderConfig): ContextSection[] {
  ...
  if (agentPrompt) {
    sections.push(
      createSubagentSection({
        ...
        // KCode by design：Subagent agent prompt 自带相对 CLI prefix 的单换行左边界。
        content: `\n${agentPrompt}`,
      }),
    );
  }
  ...
  sections.push(
    createSubagentSection({
      ...
      cacheHint: "stable",
      // KCode by design：后续 Subagent system block 统一自带双换行左边界。
