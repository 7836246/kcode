...
import type { FileSystemPort, Logger, TraceContext } from "@kcode/contracts";
...
function resolvePersistentAgentMemoryRoot(input: {
  ...
  return input.scope === "project"
    ? join(workspace, ".kcode", "agent-memory", agentKey)
    : join(workspace, ".kcode", "agent-memory-local", agentKey);
}
