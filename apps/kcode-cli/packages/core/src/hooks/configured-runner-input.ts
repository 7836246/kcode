...
import { resolveWorkspaceHookTimeoutMs } from "@kcode/shared/workspace-hook-discovery";
import {
  CoreErrorType,
  HookEventName,
  createCoreError,
...
export async function createCompatibleHookStdin(input: HookInput): Promise<{
  ...
  const tempDir = await mkdtemp(join(tmpdir(), "kcode-hook-"));
  ...
  if ("toolName" in input) {
    ...
    // 这里只补无损 alias，继续保留 KCode camelCase 字段作为内部主契约。
...
export function createPluginEnvOverlay(
  ...
  const set: Record<string, string> = {
    ...
    KCODE_PROJECT_DIR: input.cwd || workingDirectory,
    KCODE_SESSION_ID: input.sessionId,
  };
  ...
  return {
    set: {
      ...
      KCODE_PLUGIN_DATA: plugin.dataPath,
      KCODE_PLUGIN_ID: plugin.id,
      KCODE_PLUGIN_NAME: plugin.name,
      KCODE_PLUGIN_ROOT: plugin.rootPath,
    },
  };
}
...
export function expandPluginVariables(
  ...
  if (plugin) {
    ...
    replacements.KCODE_PLUGIN_DATA = plugin.dataPath;
    replacements.KCODE_PLUGIN_ROOT = plugin.rootPath;
  }
  return value.replace(
    /\$\{(CLAUDE_CODE_SESSION_ID|CLAUDE_PLUGIN_DATA|CLAUDE_PLUGIN_ROOT|CLAUDE_PROJECT_DIR|CLAUDE_SESSION_ID|CLAUDE_SKILL_DIR|KCODE_PLUGIN_DATA|KCODE_PLUGIN_ROOT|KCODE_PROJECT_DIR|KCODE_SESSION_ID|KCODE_SKILL_DIR)\}/gu,
    (_match, key: string) => {
      if (key === "CLAUDE_SKILL_DIR" || key === "KCODE_SKILL_DIR") {
