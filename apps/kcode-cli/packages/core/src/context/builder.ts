...
import type { ModelInputMessage } from "@kcode/contracts";
import type {
...
export class ContextBuilder {
  ...
  build(): ContextBuildResult {
    ...
    if (!hasCustomSystemPrompt) {
      if (!isWorkflowActor && this.config.presentationSurface === "kcode_desktop") {
  ...
  private assembleSystemMessages(sections: ContextSection[]): ModelInputMessage[] {
    ...
    if (dynamicSystemContent) {
      messages.push({
        role: "system",
        // KCode by design：Main Agent 的 dynamic system block 自带左边界，所有 provider 保持一致。
