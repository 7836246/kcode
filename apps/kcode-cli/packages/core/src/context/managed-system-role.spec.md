...
- 预设可选：`默认`（ZCode 原版受管角色）、`破限`（本仓库的直接执行角色，不搬 Keysmith 原文）、以及用户自建预设。点选以点中的那一项为准高亮并装入草稿；正文与「默认」相同的自定义项也能切过去。保存才写入 `system-role.md`，并把选中的 `presetId` 记入 `system-role.json`。重开设置时只按这份 id 恢复高亮；正文撞车且没有 id 时不高亮任何一项。开关投影只改 `enabled`，不能清掉 `presetId`。
- 用户可把当前草稿另存为自定义预设，写入 `~/.kcode/system-role-presets.json`。选中自定义项后，可用当前草稿覆盖该项正文；只能删除自定义预设，不能删内置两项。编辑草稿不取消高亮。未保存草稿时再点别的预设或恢复默认，先确认再替换。
...
- 预设列表始终包含内置「默认」和「破限」；点选只替换草稿，不立刻写 `system-role.md`。自定义项即使正文与内置相同，点选也会切到该项。保存后重开仍高亮刚才保存的那一项。
- 添加自定义预设会立刻写入 `~/.kcode/system-role-presets.json`；更新和删除只作用于自定义项。未保存草稿切换预设时必须先确认。
