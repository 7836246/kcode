...
const traceHeaderNames = ["x-kcode-trace-id", "x-trace-id", "traceparent"];
...
export class NetworkCaptureService {
  ...
  getStatus(): NetworkCaptureStatus {
    ...
    const env: Record<string, string> = url
      ? {
          KCODE_HTTP_PROXY: url,
          KCODE_AGENT_CA_CERT: caCertPath,
        }
      : {};
  ...
  private recordForContext(ctx: IContext): NetworkRequestRecord | undefined {
    const id = stringValue(ctx.tags?.kcodeCaptureId) ?? ctx.uuid;
...
export function createNetworkCaptureServiceFromEnv(
  ...
  if (isDisabled(env.KCODE_DEBUG_NETWORK_CAPTURE)) return undefined;
  return new NetworkCaptureService({
    host: env.KCODE_DEBUG_NETWORK_HOST || undefined,
    port: parsePositiveInteger(env.KCODE_DEBUG_NETWORK_PORT) ?? DEFAULT_PORT,
    caDir: env.KCODE_DEBUG_NETWORK_CA_DIR || undefined,
    maxEntries: parsePositiveInteger(env.KCODE_DEBUG_NETWORK_MAX_ENTRIES) ?? DEFAULT_MAX_ENTRIES,
  });
}
