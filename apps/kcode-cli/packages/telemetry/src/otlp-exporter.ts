...
export function createOwnedAgentTelemetryRuntime(
  ...
  const tracer = provider.getTracer("@kcode/cli-agent-telemetry", String(TELEMETRY_SCHEMA_VERSION));
  const meter = meterProvider.getMeter(
    "@kcode/cli-agent-telemetry",
...
function metricViews(): ViewOptions[] {
  return [
    histogramView("kcode.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("kcode.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "kcode.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "kcode.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("kcode.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("kcode.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "kcode.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("kcode.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("kcode.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("kcode.model.attempt.tokens"),
    cardinalityView("kcode.model.attempt.stream_stall.count"),
    cardinalityView("kcode.telemetry.creation_drop.count"),
    cardinalityView("kcode.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "kcode.build.commit_id": resource.buildCommitId,
    "kcode.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "kcode.product.version": resource.productVersion,
    "kcode.runtime.distribution": resource.runtimeDistribution,
    "kcode.runtime.surface": resource.runtimeSurface,
    "kcode.telemetry.schema_owner": "cli",
    "kcode.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
