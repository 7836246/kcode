...
import type {
  ...
  ToolTraceStart,
} from "@kcode/contracts/telemetry";
import {
  activeWriterContext,
  BaseSpanWriter,
  compactAttributes,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.agent_turn.turn_number": integer(input.turnNumber),
            "kcode.agent_turn.input_source": safeEnum(input.inputSource),
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.agent_step.step_id": safeId(input.stepId),
            "kcode.agent_step.step_index": integer(input.stepIndex),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.execution.tool_call_id": safeId(input.toolCallId),
            "kcode.tool_execution.tool_name": toolName,
            ...toolCompatibilityAttributes({
    ...
    return this.safeCreate(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent?.correlation),
            "kcode.context_compaction.trigger": safeEnum(input.trigger),
            "kcode.context_compaction.phase": safeEnum(input.phase),
            "kcode.context_compaction.model_mode": safeEnum(input.modelMode),
            "kcode.context_compaction.outer_attempt": integer(input.outerAttempt),
            "kcode.context_compaction.max_attempts": integer(input.maxAttempts),
            "kcode.context_compaction.triggering_step_index": integer(input.triggeringStepIndex),
            "kcode.context_compaction.policy_context_window_tokens": finiteNonNegative(
              input.policyContextWindowTokens,
            ),
            "kcode.context_compaction.threshold_tokens": finiteNonNegative(input.thresholdTokens),
            "kcode.context_compaction.token_source": safeEnum(input.tokenSource),
            "kcode.context_compaction.recovered_from_logical_call_id": safeId(
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.detached_operation.operation": safeEnum(input.operation),
            "kcode.detached_operation.execution_kind": safeEnum(input.executionKind),
            "kcode.detached_operation.trigger": safeEnum(input.trigger),
            "kcode.detached_operation.target_kind": safeEnum(input.targetKind),
            "kcode.detached_operation.goal_iteration": integer(input.goalIteration),
            "kcode.detached_operation.chunk_index": integer(input.chunkIndex),
            "kcode.detached_operation.chunk_count": integer(input.chunkCount),
          }),
          context: parentContext,
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.execution.logical_call_id": safeId(input.logicalCallId),
            "kcode.model_call.operation": safeEnum(input.operation),
            "kcode.model_call.streaming": input.streaming,
            "kcode.model_call.model_role": safeEnum(input.modelRole),
            "kcode.model_call.requested_provider_id": safeString(input.requested.providerId, 128),
            "kcode.model_call.requested_model": safeString(input.requested.requestedModel, 128),
            "kcode.model_call.reasoning_capability": safeEnum(input.requested.reasoning.capability),
            "kcode.model_call.reasoning_requested_state": safeEnum(
              input.requested.reasoning.requestedState,
            ),
            "kcode.model_call.reasoning_requested_control": safeEnum(
              input.requested.reasoning.requestedControl,
            ),
            "kcode.model_call.reasoning_requested_level": safeString(
              input.requested.reasoning.requestedLevel,
              128,
            ),
            "kcode.model_call.reasoning_requested_budget_tokens": integer(
              input.requested.reasoning.requestedBudgetTokens,
            ),
            "kcode.model_call.call_cause": safeEnum(input.callCause),
            "kcode.model_call.previous_logical_call_id": safeId(input.previousLogicalCallId),
          }),
          context: parentContext,
          correlation: parent?.correlation,
          parent,
    ...
    return this.safeCreate(
      ...
      () => {
        const span = this.startSpan({
          attributes: compactAttributes({
            ...executionProjection(parent.correlation),
            "kcode.execution.tool_call_id": safeId(parent.toolCallId),
            "kcode.command_execution.safe_name": safeString(input.safeName, 128),
            "kcode.command_execution.category": safeEnum(input.category),
            "kcode.command_execution.command_count": integer(input.commandCount),
            "kcode.command_execution.shell_kind": safeEnum(input.shellKind),
            "kcode.command_execution.sandboxed": input.sandboxed,
          }),
      ...
      () => {
        ...
        const span = this.startSpan({
          attributes: compactAttributes({
            ...
            "kcode.execution.tool_call_id": safeId(parent.toolCallId),
            "kcode.execution.logical_call_id": parentWriter.logicalCallId,
            "kcode.execution.model_operation": parentWriter.operation,
            "kcode.execution.model_role": parentWriter.modelRole,
            "kcode.model_attempt.request_id": safeId(input.requestId),
            "kcode.model_attempt.attempt_number": integer(input.attemptNumber),
            "kcode.model_attempt.max_attempts": integer(input.maxAttempts),
            "kcode.model_attempt.attempt_cause": safeEnum(input.attemptCause),
            "kcode.model_attempt.previous_request_id": safeId(input.previousRequestId),
            "kcode.model_attempt.retry_delay_ms": finiteNonNegative(input.retryDelayMs),
            "kcode.model_attempt.provider_id": safeString(target.providerId, 128),
            "kcode.model_attempt.provider_kind": safeEnum(target.providerKind),
            "kcode.model_attempt.provider_origin": safeString(target.providerOrigin),
            "kcode.model_attempt.provider_route": safeString(target.providerRoute),
            "kcode.model_attempt.requested_model": safeString(target.requestedModel, 128),
            "kcode.model_attempt.transport": safeEnum(input.transport),
            "kcode.model_attempt.api_operation": safeEnum(input.apiOperation),
            "kcode.model_attempt.reasoning_capability": safeEnum(target.reasoning.capability),
            "kcode.model_attempt.reasoning_requested_state": safeEnum(
              target.reasoning.requestedState,
            ),
            "kcode.model_attempt.reasoning_requested_control": safeEnum(
              target.reasoning.requestedControl,
            ),
            "kcode.model_attempt.reasoning_requested_level": safeString(
              target.reasoning.requestedLevel,
              128,
            ),
            "kcode.model_attempt.reasoning_requested_budget_tokens": integer(
...
class TurnWriter extends TrackedBaseWriter implements AgentTurnSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() => this.setAttribute("kcode.agent_turn.result_type", resultType));
  }
...
class StepWriter extends TrackedBaseWriter implements AgentStepSpanWriter {
  ...
  finishCompleted(
    ...
    this.finishCompletedIfOpen(() =>
      this.setAttribute("kcode.agent_step.terminal_reason", terminalReason),
    );
  }
...
class ToolWriter extends TrackedBaseWriter implements ToolExecutionSpanWriter {
  ...
  setPermissionDecision(decision: "granted" | "denied" | "not_required"): void {
    this.setAttribute("kcode.tool_execution.permission_decision", decision);
  ...
  setOutputBytes(bytes: number): void {
    this.setAttribute("kcode.tool_execution.output_bytes", finiteNonNegative(bytes));
  }

  setOutputTruncated(truncated: boolean): void {
    this.setAttribute("kcode.tool_execution.output_truncated", truncated);
  }
  ...
  finishDenied(reason: "user_denied" | "policy_denied" | "unavailable" | "unknown"): void {
    this.finishDomainOutcomeIfOpen("denied", () =>
      this.setAttribute("kcode.tool_execution.permission_denial_reason", reason),
    );
  }
...
class CommandWriter extends TrackedBaseWriter implements CommandExecutionSpanWriter {
  ...
  markFirstOutput(): void {
    ...
    this.setAttribute("kcode.command_execution.first_output_ms", elapsed);
  ...
  setExitCode(exitCode: number): void {
    ...
    this.setAttribute("kcode.command_execution.exit_code", normalized);
  ...
  setSignal(signal: string): void {
    ...
    this.setAttribute("kcode.command_execution.signal", normalized);
    this.setAttributes(commandCompatibilityAttributes({ signal: normalized }));
  }

  setOutputBytes(bytes: number): void {
    this.setAttribute("kcode.command_execution.output_bytes", finiteNonNegative(bytes));
  }

  setTimedOut(timedOut: boolean): void {
    this.setAttribute("kcode.command_execution.timed_out", timedOut);
  }
...
class CompactionWriter extends TrackedBaseWriter implements ContextCompactionSpanWriter {
  ...
  setInputTokens(tokens: number): void {
    this.setAttribute("kcode.context_compaction.input_tokens", finiteNonNegative(tokens));
  }

  setOutputTokens(tokens: number): void {
    this.setAttribute("kcode.context_compaction.output_tokens", finiteNonNegative(tokens));
  }
...
class DetachedWriter extends TrackedBaseWriter implements DetachedOperationSpanWriter {
  ...
  setResultType(resultType: "text" | "boolean" | "metadata" | "other"): void {
    this.setAttribute("kcode.detached_operation.result_type", resultType);
  }
...
class ModelAttemptWriter extends TrackedBaseWriter implements ModelAttemptSpanWriter {
  ...
  setProviderRequestId(requestId: string): void {
    this.setAttribute("kcode.model_attempt.provider_request_id", safeString(requestId, 256));
  }

  setResponseModel(model: ResponseModelTelemetryDescriptor): void {
    const value = safeString(model.model, 128);
    this.setAttribute("kcode.model_attempt.response_model", value);
  ...
  setEffectiveReasoningState(state: ModelReasoningState): void {
    this.setAttribute("kcode.model_attempt.reasoning_effective_state", safeEnum(state));
  }

  setEffectiveReasoningControl(control: ModelReasoningControlType): void {
    this.setAttribute("kcode.model_attempt.reasoning_effective_control", safeEnum(control));
  }

  setEffectiveReasoningLevel(level: string): void {
    this.setAttribute("kcode.model_attempt.reasoning_effective_level", safeString(level, 128));
  }

  setEffectiveReasoningBudgetTokens(tokens: number): void {
    this.setAttribute(
      "kcode.model_attempt.reasoning_effective_budget_tokens",
  ...
  setFinishReason(reason: ModelFinishReason): void {
    ...
    this.setAttribute("kcode.model_attempt.finish_reason", value);
    this.setAttributes(modelResponseCompatibilityAttributes({ finishReason: value }));
  }

  setInputTokens(tokens: number): void {
    this.setUsage("inputTokens", "input", tokens, "kcode.model_attempt.input_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ inputTokens: tokens }));
  }

  setOutputTokens(tokens: number): void {
    this.setUsage("outputTokens", "output", tokens, "kcode.model_attempt.output_tokens");
    this.setAttributes(modelResponseCompatibilityAttributes({ outputTokens: tokens }));
  }

  setReasoningTokens(tokens: number): void {
    this.setUsage("reasoningTokens", "reasoning", tokens, "kcode.model_attempt.reasoning_tokens");
  }

  setCacheReadTokens(tokens: number): void {
    this.setUsage("cacheReadTokens", "cache_read", tokens, "kcode.model_attempt.cache_read_tokens");
  }
  ...
  setCacheWriteTokens(tokens: number): void {
    this.setUsage(
      ...
      "cache_write",
      tokens,
      "kcode.model_attempt.cache_write_tokens",
    );
  }

  setStreamOutputCommitted(committed: boolean): void {
    this.setAttribute("kcode.model_attempt.stream_output_committed", committed);
  }

  setHttpStatusCode(statusCode: number): void {
    const normalized = integer(statusCode);
    this.setAttribute("kcode.model_attempt.http_status_code", normalized);
    this.setAttributes(httpResponseCompatibilityAttributes(statusCode));
  }

  setProviderErrorCode(code: string): void {
    this.setAttribute("kcode.model_attempt.provider_error_code", safeString(code, 128));
  }

  setProviderErrorMessage(message: string): void {
    this.setAttribute("kcode.model_attempt.provider_error_message", sanitizeErrorMessage(message));
  }

  setRetryAfterMs(delayMs: number): void {
    this.setAttribute("kcode.model_attempt.retry_after_ms", finiteNonNegative(delayMs));
  }
  ...
  markFirstProviderEvent(): void {
    ...
    this.setAttribute("kcode.model_attempt.time_to_first_provider_event_ms", elapsed);
  ...
  markFirstContent(): void {
    ...
    this.setAttribute("kcode.model_attempt.time_to_first_content_ms", elapsed);
  ...
  markFirstText(): void {
    ...
    this.setAttribute("kcode.model_attempt.time_to_first_text_ms", elapsed);
