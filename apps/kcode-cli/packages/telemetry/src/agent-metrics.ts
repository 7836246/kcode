...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "kcode.agent.turn.duration"),
      agent_step: duration(meter, "kcode.agent.step.duration"),
      tool_execution: duration(meter, "kcode.tool.execution.duration"),
      command_execution: duration(meter, "kcode.command.execution.duration"),
      context_compaction: duration(meter, "kcode.context.compaction.duration"),
      detached_operation: duration(meter, "kcode.detached.operation.duration"),
      model_call: duration(meter, "kcode.model.call.duration"),
      model_attempt: duration(meter, "kcode.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("kcode.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("kcode.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "kcode.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "kcode.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "kcode.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("kcode.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "kcode.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "kcode.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("kcode.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("kcode.telemetry.abandoned.count", {
