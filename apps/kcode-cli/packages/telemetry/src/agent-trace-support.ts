...
const ACTIVE_WRITER_CONTEXT_KEY = createContextKey("@kcode/telemetry/active-writer-v4");
...
export function contextFromCausation(causation: AgentTelemetryCausation): Context {
  ...
  // 为底，否则会把另一个异步任务的 Baggage 或 KCode Writer 私有状态夹带进来。
...
export function executionProjection(
  ...
  return {
    "kcode.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "kcode.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "kcode.execution.turn_id": safeId(correlation.turnId),
    "kcode.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "kcode.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "kcode.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "kcode.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "kcode.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "kcode.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "kcode.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
