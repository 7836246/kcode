...
const cliBundle = resolve(dist, "kcode.cjs");
const seaBlobForTarget = (target) => resolve(dist, `kcode-${target}.sea.blob`);
    ...
    JSON.stringify(
      {
        assets: {
          ...
          "kcode-node-license": nodeLicensePath,
        },
        disableExperimentalSEAWarning: true,
        main: "dist/kcode.cjs",
        output: `dist/kcode-${target}.sea.blob`,
        useCodeCache: false,
        useSnapshot: false,
      },
      null,
...
const smokeTestHostTarget = async (target, binaryPath) => {
  if (isHostTarget(target)) {
    const storageRoot = await mkdtemp(join(tmpdir(), "kcode-sea-smoke-"));
    try {
      const env = {
        ...process.env,
        KCODE_STORAGE_DIR: storageRoot,
      };
      delete env.KCODE_BFS_BINARY;
      delete env.KCODE_RG_BINARY;
      delete env.KCODE_UGREP_BINARY;
...
export const main = async (argv = process.argv.slice(2)) => {
  ...
  if (!existsSync(cliBundle)) {
    throw new Error("Missing dist/kcode.cjs. Run `pnpm build` first.");
  }
