...
// 品牌类型（SessionId）和 createKCodeApp 的联合签名收窄错。
export async function prepareTuiAppRuntime(
  ...
  const createAppFactory = deps.createKCodeApp ?? bootstrapModule?.createKCodeApp;
  if (!createAppFactory) throw new Error("KCode app factory is unavailable.");
  const prepareTelemetry =
    deps.prepareKCodeTelemetryEnv ?? bootstrapModule?.prepareKCodeTelemetryEnv;
  if (prepareTelemetry) {
    state.shutdownTelemetry =
      deps.shutdownKCodeTelemetry ?? bootstrapModule?.shutdownKCodeTelemetry;
  }
