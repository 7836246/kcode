...
import {
  ...
  normalizeKCodeRuntimeEnv,
  sanitizeKCodeRuntimeEnv,
  sanitizeKCodeRuntimeEnvInPlace,
...
export function prepareCliRuntimeEnv(
  ...
  const prepared = {
    ...sanitizeKCodeRuntimeEnv(env),
    ...buildKCodeToolEnvPassthroughEnv(env),
  };
...
export function applyCliRuntimeEnvSanitization(
  ...
  const toolEnvPassthrough = buildKCodeToolEnvPassthroughEnv(env);
  sanitizeKCodeRuntimeEnvInPlace(env);
...
function applyCliRuntimeEnvDefaults(env: CliEnv, argv: readonly string[]): void {
  env[KCODE_RUNTIME_ENV_KEY] = resolveCliRuntimeEnv(env, argv);
  applyBetaStorageDefault(env, argv);
}

function resolveCliRuntimeEnv(env: CliEnv, argv: readonly string[]): KCodeRuntimeEnv {
  const explicit = normalizeKCodeRuntimeEnv(env[KCODE_RUNTIME_ENV_KEY]);
  if (explicit) {
    return explicit;
  }
...
function applyBetaStorageDefault(env: CliEnv, argv: readonly string[]): void {
  if (env.KCODE_STORAGE_DIR?.trim()) return;
  const explicitBeta = env.KCODE_BETA === "1" || env.KCODE_ENV === "beta";
  const invokedAsBeta = argv.some((arg) => /(^|[/\\])kcode-beta(?:$|\.)/u.test(arg));
  if (!explicitBeta && !invokedAsBeta) return;
  env.KCODE_STORAGE_DIR = join(homedir(), ".kcode-beta");
}
