...
import type { SetKCodePluginEnabledResult, KCodePluginInstallData } from "@kcode/bootstrap";
import {
...
const PLUGINS_COMMAND_USAGE = `Usage: kcode plugins <command> [options]
...
Scopes: user (default), project. \`kcode plugin\` is an alias of \`kcode plugins\`.`;
...
async function runPluginsUpdateCommand(
  ...
  ctx.stdout.write(
    version && version === result.previousVersion
      ...
      : `Updated plugin ${pluginId} from ${result.previousVersion || "unknown"} to ${version || "unknown"}. Restart kcode to apply.\n`,
  );
...
async function runPluginsDisableCommand(
  ...
  const entries: Array<{ plugin: PluginListItem; result?: SetKCodePluginEnabledResult; error?: string }> = [];
