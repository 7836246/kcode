...
import { createDenyPermissionBroker } from "@kcode/core";
...
export const createHeadlessPermissionBroker = (): NonNullable<
  KCodeAppOptions["permissionBroker"]
> => {
 ...
 *
 * 刻意**不**进 `kcodeSessionEventTypeSchema`（`shared/src/kcode-protocol/index.ts`）：
 * 那是个闭集 `z.enum` 且喂给 `kcodeSessionEventSchema` 的 discriminated union，加值等于让
 ...
 * `RunDependencies.createKCodeApp` 注入点边界）。
 *
