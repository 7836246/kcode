...
import { formatJson, type PresentationSurface } from "@kcode/core";
import type { RunContext, GlobalOptions } from "@kcode/shared-types";
...
export const runPrompt = async (
  ...
  let app:
    | Awaited<ReturnType<Awaited<ReturnType<typeof loadBootstrapModule>>["createKCodeApp"]>>
    | undefined;
  ...
  try {
    ...
    const bootstrapModule = deps.createKCodeApp ? undefined : await loadBootstrapModule();
    const createApp = deps.createKCodeApp ?? bootstrapModule?.createKCodeApp;
    if (!createApp) throw new Error("KCode app factory is unavailable.");
    ...
    const prepareTelemetry =
      deps.prepareKCodeTelemetryEnv ?? bootstrapModule?.prepareKCodeTelemetryEnv;
    if (prepareTelemetry) {
      shutdownTelemetry = deps.shutdownKCodeTelemetry ?? bootstrapModule?.shutdownKCodeTelemetry;
    }
    const appEnv = prepareTelemetry
      ? await prepareTelemetry(env, {
          cliVersion: version,
          productVersion: env.KCODE_APP_VERSION,
        })
      : env;
    ...
    // 回合。结果是 `kcode -p "/workflow ..."` 在第一个回合后就退出，把在飞的 run 孤儿化
...
async function isResolvableCustomCommand(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  //
  // 探测刻意用「保留名检查 + load」这一对，而不是直接调 resolveKCodeCustomCommandPrompt：
...
async function isReservedSlashCommandName(deps: RunDependencies, name: string): Promise<boolean> {
  ...
  return bootstrap.isReservedKCodeSlashCommandName(name);
}
...
async function listCustomCommandsForPrompt(deps: RunDependencies) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.listKCodeCustomCommands({ env, logger: deps.logger, workingDirectory });
}
...
async function loadCustomCommandForPrompt(deps: RunDependencies, name: string) {
  ...
  const bootstrap = await loadBootstrapModule();
  return await bootstrap.loadKCodeCustomCommand({
    env,
    logger: deps.logger,
    name,
    workingDirectory,
...
function writeHeadlessWorkspaceHookTrustDiagnostic(
  ...
  ctx.stderr.write(
    [
      ...
      `Review with: kcode hooks trust review --workspace ${JSON.stringify(status.workspaceIdentity)}`,
    ].join("\n") + "\n",
  );
}
