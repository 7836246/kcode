...
type CustomCommandListOutcome = Awaited<ReturnType<typeof listKCodeCustomCommands>>;
...
const COMMANDS_COMMAND_USAGE = "Usage: kcode commands [list|inspect <name>]";
...
async function resolveListCustomCommands(deps: CommandsCommandDependencies) {
  ...
  return (await bootstrap()).listKCodeCustomCommands;
}
...
async function resolveInspectCustomCommand(deps: CommandsCommandDependencies) {
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@kcode/bootstrap"));
  return (await bootstrap()).inspectKCodeCustomCommand;
}
...
function formatHumanCommandInspection(
  inspection: KCodeCustomCommandInspection,
  options: GlobalOptions,
): string {
