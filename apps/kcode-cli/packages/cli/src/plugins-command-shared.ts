...
import type { listKCodePlugins } from "@kcode/bootstrap";
...
export type PluginListOutcome = ReturnType<typeof listKCodePlugins>;
 ...
 * 避免 `kcode plugins list` 这类轻命令把整个 bootstrap 图提前拉起来。
 */
export const BOOTSTRAP_EXPORTS = {
  addMarketplace: "addKCodePluginMarketplace",
  getPluginsOverview: "getKCodePluginsOverview",
  installPlugin: "installKCodeMarketplacePlugin",
  listPlugins: "listKCodePlugins",
  removeMarketplace: "removeKCodePluginMarketplace",
  setPluginEnabled: "setKCodePluginEnabled",
  uninstallPlugin: "uninstallKCodeMarketplacePlugin",
  updateMarketplace: "updateKCodePluginMarketplace",
  updatePlugin: "updateKCodeMarketplacePlugin",
  validatePluginPath: "validateKCodePluginPath",
...
/** `kcode plugins` 子命令专属旗标；由 run.ts 的全局解析器收集后原样透传。 */
...
export async function resolveDep<K extends PluginDepName>(
  ...
  const bootstrap = deps.loadBootstrapModule ?? (() => import("@kcode/bootstrap"));
...
export function resolveScope(value: string | undefined): PluginScope | undefined {
  ...
  if (value === "local") {
    throw new PluginsUsageError("Scope 'local' is not supported by kcode. Use: user, project");
  }
