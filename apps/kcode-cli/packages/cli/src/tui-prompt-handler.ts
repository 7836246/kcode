import { getDefaultConfigPath, updateUiLocaleInFileConfig } from "@kcode/adapters/config";
import type { SessionEvent } from "@kcode/contracts";
import type { KCodeAppOptions } from "@kcode/bootstrap";
import { DEFAULT_LOCALE, type SupportedLocale } from "@kcode/i18n";
import type { TuiRequestPermission } from "@kcode/tui";
import type { GlobalOptions } from "@kcode/shared-types";
...
export function createTuiSubmitPrompt(
  ...
  let app: Awaited<ReturnType<NonNullable<RunDependencies["createKCodeApp"]>>> | undefined;
  ...
  const permissionBroker: NonNullable<KCodeAppOptions["permissionBroker"]> = {
  ...
  const replaceApp = async (
    factory: () => Promise<Awaited<ReturnType<NonNullable<RunDependencies["createKCodeApp"]>>>>,
  ...
  const createApp = async (request: CliResumeRequest) => {
    ...
    let createdApp: Awaited<ReturnType<NonNullable<RunDependencies["createKCodeApp"]>>>;
    try {
