...
import {
  KCODE_PROCESS_DIAGNOSTIC_PREFIX,
...
function writeProcessErrorDiagnostic(
  ...
  try {
    const detail = formatProcessError(reason).slice(0, KCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS);
    const diagnostic: KCodeProcessDiagnostic = {
      version: 1,
      errorId: randomUUID(),
      kind,
    ...
    stderr.write(
      `${KCODE_PROCESS_DIAGNOSTIC_PREFIX}${JSON.stringify(diagnostic)}\n[kcode] process error kind=${kind} origin=${origin}\n${detail}\n`,
    );
  } catch {
    // 诊断输出不能再次击穿进程级异常边界。
  }
