...
type BootstrapModule = typeof import("@kcode/bootstrap");
type CliSkillListOutcome = Awaited<ReturnType<typeof listKCodeSkills>>;
...
const SKILLS_COMMAND_USAGE = "Usage: kcode skills [list|inspect <name>]";
...
async function runSkillsListCommand(
  ...
  try {
    ...
    const listSkills = deps.listSkills ?? (await bootstrap()).listKCodeSkills;
...
async function runSkillsInspectCommand(
  ...
  try {
    ...
    const workingDirectory = (deps.cwd ?? process.cwd)();
    const bootstrap = deps.loadBootstrapModule ?? (() => import("@kcode/bootstrap"));
    const inspectSkill = deps.inspectSkill ?? (await bootstrap()).inspectKCodeSkill;
...
function formatHumanSkillInspection(
  inspection: KCodeSkillInspection,
  options: GlobalOptions,
): string {
...
const formatSkillInspectionJson = (inspection: KCodeSkillInspection, cwd: string): string =>
  formatJson({
    cwd,
