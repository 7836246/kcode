...
import { createNodeLoggerFactory } from "@kcode/adapters";
import { getRuntimeInfo, type PresentationSurface } from "@kcode/core";
import { color, formatJson, supportsColor } from "@kcode/core";
import { getKCodeCopy, isUiLocale, type UiLocale } from "@kcode/i18n";
import type { RunContext, GlobalOptions, GlobalOutputFormat } from "@kcode/shared-types";
import {
...
const normalizeLocaleOption = (value: string | undefined): UiLocale | undefined => {
  ...
  throw new Error(getKCodeCopy().cli.errors.localeUnsupported(value));
};
...
const normalizePresentationSurface = (value: string | undefined): PresentationSurface => {
  ...
  if (value.toLowerCase() === "desktop") return "kcode_desktop";
...
const runDoctor = (ctx: RunContext, options: GlobalOptions, workingDirectory: string): number => {
  ...
  ctx.stdout.write(`${color.bold("kcode doctor", colors)}\n`);
...
const runKCodeProtocolCommand = async (
  ctx: RunContext,
  ...
  try {
    ...
    // 直接退出，外层只能看到 KCode agent transport closed。
    ...
    const runProtocolAgent =
      deps.runKCodeProtocolAgent ?? (await loadBootstrapModule()).runKCodeProtocolAgent;
...
export const run = async (ctx: RunContext, deps: RunDependencies = {}): Promise<number> => {
  ...
  const commandDeps: RunDependencies = {
    ...
    logger:
      deps.logger ??
      createNodeLoggerFactory({ env }).createLogger("kcode").child({ module: "cli" }),
  ...
  switch (commandName(parsed.positionals)) {
    ...
    case "agent-server":
    case "app-server":
      return await runKCodeProtocolCommand(
        ctx,
        options,
        commandDeps,
        presentationSurface,
