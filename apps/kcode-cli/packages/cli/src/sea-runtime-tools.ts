...
import { getRuntimeToolRuntime, type RuntimeToolId } from "@kcode/shared/runtime-tool-runtime";
...
const assetPrefix = "kcode-runtime-tools/";
const manifestAssetKey = `${assetPrefix}manifest.json`;
const markerFileName = ".kcode-runtime-tool.json";
...
export async function ensureSeaRuntimeTools(
  ...
  const configuredStorageRoot = options.storageRoot ?? env.KCODE_STORAGE_DIR?.trim();
  const storageRoot = configuredStorageRoot || join(homedir(), ".kcode");
