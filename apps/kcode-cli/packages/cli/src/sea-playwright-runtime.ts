...
import type { PlaywrightChromiumModule } from "@kcode/adapters/browser";
...
const ASSET_PREFIX = "kcode-playwright-runtime/";
...
export async function loadCliPlaywrightChromium(): Promise<PlaywrightChromiumModule> {
  ...
  const require = createRequire(join(runtimeDirectory, "kcode-playwright-loader.cjs"));
...
function cacheBaseDirectory(): string {
  ...
  if (platform() === "darwin" && home) {
    return join(home, "Library", "Caches", "kcode", "sea-assets");
  }
  if (platform() === "win32") {
    return join(
