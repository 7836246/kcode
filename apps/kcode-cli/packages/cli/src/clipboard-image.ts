...
import type { TuiClipboardImage, TuiImageMediaType, TuiReadClipboardImage } from "@kcode/tui";
...
function resolveDefaultClipboardDirectory(processEnv: NodeJS.ProcessEnv = process.env): string {
  const storageRoot = processEnv.KCODE_STORAGE_DIR?.trim() || join(homedir(), ".kcode");
...
async function readMacClipboardImage(options: {
  ...
  const directory = await mkdtemp(join(options.tempDirectory, "kcode-clipboard-"));
