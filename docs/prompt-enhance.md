...
7. 作为用户，我想在点过增强之后、结果落地之前被我改过的草稿不会被结果覆盖（无论我有没有点过取消），这样我的输入不会丢。
...
```
 ...
 ├─ generateWorkspaceText（runId+1 并生成 operationId，记录发起时 scopeKey 与草稿快照）
 │    ├─ 用户再点按钮 → 作废 run + 按 operationId 发取消，toast「已取消」，迟到响应按 runId 丢弃
 │    ├─ scope 变化 → 同样作废并取消，结果不回填、不立还原点
 │    ├─ 组件卸载 → 同样作废并取消：组件已不再渲染，结果既不回填也不再弹提示
 │    ├─ 失败 / 超时 / 空内容 → toast 错误原因，草稿不动
 │    └─ 成功 → 回填闸（当前草稿 = 发起时快照？）
 │         ├─ 不一致 → 丢弃结果，不覆盖、不立还原点，toast 说明（用户在途改过草稿）
 │         └─ 一致 → 回填三连 → 读回比对
 │              ├─ 比对失败 → 写回原文，toast 失败
...
点「还原」
 ...
 └─ 一致 → 写回原文 → 读回比对
      ├─ 未通过 → 保留还原点，toast 失败（可再试一次）
      └─ 通过 → 清还原点，toast 成功
```

回填三连顺序与 `restoreSubmittedDraft` 一致：`updateComposerContent({ text })` → 编辑器句柄 `setText` → `updateText`。读回比对用编辑器 `getMarkdown()`，比对前只做 CRLF 与首尾空白归一化；写入抛异常或读回不一致都按「回填没有完整落地」处理：写回原文，不留半截文本。

**回填闸（在途期间草稿被改）**：编辑器在增强进行中不受 `running` 约束——`runningStartedAt` 只存在于 `PromptEnhanceActions` 内部，从不回传给 composer 去禁用编辑器，因此用户可以在等待期间继续输入。若结果无条件回填，会把他刚落笔的内容一起覆盖，而随后立下的还原点只含发起时的原文，这些新输入没有任何找回路径。因此以**发起时的草稿快照**为基准比对（`evaluatePromptEnhanceFill`，归一化口径与其余比对一致），不一致就整条结果丢弃并提示，与取消、scope 变化同一口径。只在成功分支判定，失败与空内容分支本就不写入。

**卸载**：组件卸载（切标签页 / 关标签 / 切 workspace）后 hook 的 async 闭包仍持有 run tracker，`isCurrent` 会继续返回 true。不作废的话，迟到的成功结果会照常回填并弹出属于已离开会话的 toast，Host 侧请求也会空跑到超时。因此补一条只含 cleanup 的 effect：卸载时作废 run 并按发起时固化的 `operationId` 发出与取消同一条 best-effort 通知。

**「还原」清点顺序**：`restorePointRef` 只在**确认写入已落地**（读回比对通过）之后才清，写失败或读回不一致只记日志并提示（`restoreFailed`）并保留还原点以便重试。若先清点再写，写失败会让用户既拿不回原文、也没有重试路径，却收到成功提示。这与回填路径「先比对再立点」是同一口径：还原点的存续必须反映「原文确实还在编辑器里」这一事实。
...
- 增强进行中：按钮切加载态（Spinner + 「增强中 Ns」，秒数用 `useNowTicker` 跳动），再点 = 取消。「还原」同时禁用：进行中还原会与在途结果互相覆盖。**编辑器本身不禁用**——不打断用户继续写，改过的草稿由回填闸在结果回来时挡住（见「增强流程」）。
...
- UI 层用 `packages/ui` 的 `logger`；发起、成功（带耗时与模型名）、取消、失败各一条 `info`/`warn`，失败详情 `error`。可恢复异常另各留一条 `warn`：档位被回退、取消未送达 / Host 未找到待取消请求、无可用选型、回填未完整落地、还原未完整落地、模型返回空内容、草稿 scope 已变化、草稿在增强期间被修改、组件卸载取消在途增强。
- 不把草稿正文写入日志（用户数据）。
...
- **手动验证**（`pnpm dev:desktop`）：三档各跑一次增强、进行中取消、还原成功、手改后还原被拒、设置持久化（重启后保留）、自动通道跟随模型切换；**在途改草稿**——点增强后立刻在输入框里补一段，结果回来时草稿必须保持用户改后的内容、只给「已在增强期间被修改」提示，不得被结果覆盖；**卸载取消**——增强进行中关掉标签页或切走 workspace，再回来不得看到属于上一个会话的成功提示；闸门口径三条——**加了附件仍可增强**（chip 保留、增强结果正常回填）、草稿里插入行内引用（@文件 或 `/命令`）默认被拒、开着「允许改写草稿里的行内引用」时放行且引用变成纯文本；另需覆盖两条模型契约——独立通道把推理档位设成「默认」仍能成功发起（选择必须带模型支持的档位），模型被删除后点击给出「没有可用的增强模型」而不是模型校验错误；以及在非标准 openai-compatible 网关（非流式响应带自定义信封，如 CPA）下增强仍成功。
